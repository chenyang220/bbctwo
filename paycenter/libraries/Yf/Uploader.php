...
class Yf_Uploader
{
    ...
    private $stateMap = array( //上传状态映射表，国际化用户需考虑此处数据的国际化
        ...
        "INVALID_URL" => "非法 URL",
        "INVALID_IP" => "非法 IP",
        "ERROR_REMOTE_UNKNOWN1" => "远程服务器异常1",
        "ERROR_REMOTE_UNKNOWN2" => "远程服务器异常2",
        "ERROR_REMOTE_UNKNOWN3" => "远程服务器异常3"
    );
    ...
    private function upFile()
    {
        ...
        if (!(move_uploaded_file($file["tmp_name"], $this->filePath) && file_exists($this->filePath))) { //移动失败
            ...
            if (Web_ConfigModel::value('remote_image_status'))
            {
				...
				{
					//
					$this->stateInfo = $this->getStateInfo("ERROR_REMOTE_UNKNOWN1");
					return;
				}
			}
        }
    ...
    private function upBase64()
    {
        ...
        if (!(file_put_contents($this->filePath, $img) && file_exists($this->filePath))) { //移动失败
			...
			if (Web_ConfigModel::value('remote_image_status'))
			{
				...
				{
					//
					$this->stateInfo = $this->getStateInfo("ERROR_REMOTE_UNKNOWN2");
					return;
				}
			}
        }
    ...
    private function saveRemote()
    {
        ...
        if (!(file_put_contents($this->filePath, $img) && file_exists($this->filePath))) { //移动失败
			...
			if (Web_ConfigModel::value('remote_image_status'))
			{
				...
				{
					//
					$this->stateInfo = $this->getStateInfo("ERROR_REMOTE_UNKNOWN3");
					return;
				}
			}
        }
