...
if ($result)
{
	...
	if ($notify_row)
	{
		...
		}elseif ($trade_type == 'plus'){//开通PLUS会员业务处理 @nsy 2019-01-16
            Yf_Log::log(var_export($notify_row,true), Yf_Log::INFO, '__plus_notify');
            //1.修改paycenterPLUS订单状态
            //2.同步信息到shop，处理plus相关业务
            $result = $Union_OrderModel->dealPlusOrder($notify_row['order_id'],'支付宝','alipay');
            Yf_Log::log(var_export($result,true), Yf_Log::INFO, '__plus_notify_result');
            if($result){
                echo "success";
            }else{
                echo "fail1";
		...
		else
		{
			//修改订单表中的各种状态
			$Consume_DepositModel = new Consume_DepositModel();
			Yf_Log::log($notify_row['order_id'],Yf_Log::INFO,'notifyShop_order_id');
