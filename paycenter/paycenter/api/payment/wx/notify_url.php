...
//获取支付渠道code
$union_order_info = $order_model->getOne($data['out_trade_no']);
$payment_channel_code = $union_order_info['payment_channel_code'];
...
//}
$Payment_WxNativeModel = PaymentModel::create($payment_channel_code);
...
if ($verify_result)
{
	...
	if ($notify_row)
	{
        ...
        if($trade_type == 'deposit')
        {
            ...
            $Union_OrderModel->dealPlusOrder($notify_row['order_id'],'微信支付','wx');
        }
        else
        {
            //处理一步回调-通知商城更新订单状态
