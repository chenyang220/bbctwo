	...
	public function edit()
	{
		...
		foreach ($config_type_row as $config_type)
		{
			...
			if ('yunshan' == $config_type) {
                ...
                unset($formvars['admin_account']);
                unset($formvars['rtime']);
                unset($formvars['token']);
		        get_url_with_encrypt($key, sprintf('%s?ctl=Api_Config&met=edit&typ=json', $url), $formvars, 'json', 'POST');
			}
			if ('sms' == $config_type)
			{
