...
class Api_Paycen_PayRecordCtl extends Api_Controller
{
    ...
    function getRecordList() {
        ...
        $payOrders  = trim(request_string('payOrders'));   //订单号
        $trade_type_id = request_int('trade_type_id');
        $page = request_int('page');
        $rows = request_int('rows');
        $cond_row = array();
        $Consume_RecordModel = new Consume_RecordModel();
        $data           = $Consume_RecordModel->getRecordList(null,null,null,$page,$rows,'asc',$username,$trade_type_id,$payorder,$payOrders);
        if ($data)
        {
            $msg    = 'success';
            $status = 200;
