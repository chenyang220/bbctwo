...
/* 		"TRADE_FINISHED" => '未付款', */
	...
	public function notifyShop($order_id = null,$user_id = null)
	{
        ...
        if (($union_order['trade_payment_amount'] == $union_order['union_online_pay_amount'] + $union_order['union_money_pay_amount'] + $union_order['union_cards_pay_amount']) || $bt_flag)
        {
            ...
            $edit_row = array();
            if($union_order['is_presale']==1&&$union_order['order_state_id']==Order_StateModel::ORDER_WAIT_PAY){
            	$edit_row['order_state_id'] = Order_StateModel::ORDER_PRESALE_DEPOSIT;
            }else{
            ...
            if(count($order_id) > 1)
			{
				...
				Yf_Log::log($order_id,Yf_Log::INFO,'notifyShop_inorder');
                $uorder_id_row = array_column($uorder_row,'union_order_id');
                $edit_uorder_row = array();
                if($uorder_row['is_presale']==1&&$uorder_row['order_state_id']==Order_StateModel::ORDER_WAIT_PAY){
            		$edit_uorder_row['order_state_id'] = Order_StateModel::ORDER_PRESALE_DEPOSIT;
	            }else{
				...
				Yf_Log::log($flag,Yf_Log::INFO,'notifyShop_inorder_editUnionOrder');
                check_rs($flag, $rs_row);
            }
            ...
            $user_ResourceModel = new User_ResourceModel();
        	$user_info = $user_ResourceModel->getOne($user_id);
        	$month = $user_info['user_credit_cycle'];
            $timestamp = strtotime("{$value['trade_pay_time']} +".$month." month");
            $yearAndMonth = date('Y-m',$timestamp);
            $day = date('t',$timestamp); //给定月份所应有的天数
            $repayment_time = $yearAndMonth.'-'.$day. " 23:59:59";
            $order_edit_row['repayment_time'] = $repayment_time;
            if($union_order['is_presale']==1&&$union_order['order_state_id']==Order_StateModel::ORDER_WAIT_PAY){
            	 $order_edit_row['order_state_id'] = Order_StateModel::ORDER_PRESALE_DEPOSIT;
            }else{
			...
			Yf_Log::log($flag,Yf_Log::INFO,'notifyShop_editTrade');
            check_rs($flag, $rs_row);
            //修改交易明细中的订单状态
            $Consume_RecordModel = new Consume_RecordModel();
			...
			{
				if($union_order['is_presale']==1&&$union_order['order_state_id']==Order_StateModel::ORDER_WAIT_PAY){
					$edit_consume_record['record_status'] = RecordStatusModel::ORDER_PRESALE_DEPOSIT;
				}else{
			...
			Yf_Log::log($flag,Yf_Log::INFO,'notifyShop_editRecord');
            check_rs($flag, $rs_row);
            if($union_order['union_money_pay_amount'] > 0){
                //修改用户的资源状态
                $User_ResourceModel = new User_ResourceModel();
                //1.用户资源中订单金额冻结(现金)
                if($union_order['is_presale']==1&&$union_order['order_state_id']==1){
                	$union_order['union_money_pay_amount'] = $union_order['presale_deposit'];
                }
                if($union_order['is_presale']==1&&$union_order['order_state_id']==20){
                	$union_order['union_money_pay_amount'] = $union_order['final_price'];
                }
	 ...
	 * @license   仅限本公司授权用户使用。
	 * @version   3.8.1
	 * @param     string          $order_id union_id主键,
	 * @param     string          $user_id  付款者
	 * @return    boolean         修改状态
	 */
	public function notifyShopYl($order_id, $user_id){
		 $rs_row = array();
		 $Union_OrderModel = new Union_OrderModel();
		 //修改合并订单中的订单支付状态
		 $union_order = $Union_OrderModel->getOne($order_id);
        if (true || $union_order['trade_payment_amount'] == $union_order['union_online_pay_amount'] + $union_order['union_money_pay_amount'] + $union_order['union_cards_pay_amount'])
        {
            $Union_OrderModel->sql->startTransactionDb();
            $edit_row = array();
            $edit_row['order_state_id'] = Order_StateModel::ORDER_PAYED;
	 ...
	 * @access public
	 */
	public function webpos_notifyShop($order_id = null,$user_id = null)
	{
        $rs_row = array();
        ...
        if (($union_order['trade_payment_amount'] == $union_order['union_online_pay_amount'] + $union_order['union_money_pay_amount'] + $union_order['union_cards_pay_amount']) || $bt_flag)
        {
            ...
            $Consume_TradeModel = new Consume_TradeModel();
            $flag = $Consume_TradeModel->editTrade($order_id,$order_edit_row);
            Yf_Log::log('9'.encode_json($flag), Yf_Log::ERROR, 'db_error');
            check_rs($flag, $rs_row);
            ...
            if (is_ok($rs_row) && $Union_OrderModel->sql->commitDb())
            {
                ...
                $rs = get_url_with_encrypt($key, sprintf('%s?ctl=Api_Trade_Order&met=webpos_editOrderRowSatus&typ=json', $url), $formvars);
                return $rs;
            }
            else
            {
