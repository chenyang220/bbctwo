...
class Consume_TradeModel extends Consume_Trade
{
	...
	public function returnCredit($user_id,$user_return_credit)
	{
		...
		foreach($rows as $key => $val)
		{
			...
			else
			{
				...
				$edit_row['trade_payment_status'] = 1;
     ...
     * @return mixed
     */
    public function getCurrentMonthRepaymentOrderList($cond_row, $order_row, $page, $rows){

		$offset = $rows * ($page - 1);

    	$sql = "SELECT
						pay_user_id,sum(order_payment_amount - trade_payment_amount) as repay_price
					FROM
						" . TABEL_PREFIX . "consume_trade where payment_channel_id=9 and trade_payment_status = 0".
					 " group by pay_user_id limit {$offset} ,{$rows}";
		$rows = $this->sql->getAll($sql);
