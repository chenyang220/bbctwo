...
<?php
    ...
    $return = Yf_Registry::get('paycenter_api_url') . "?ctl=Info&met=pay_api&uorder="  . $uorder ."&callbackurl=" . $return_url;
...
<script type="text/javascript">
  function setCookie(name,value)
    ...
    if (getCookie("appToken")  == "undefined" || getCookie("appToken") == null  ||  "<?=$appToken?>" != '') {
            ...
            <div class="module-new">
                ...
                { ?>
                    ...
                    <div class="box clearfix">
                        ...
                        <?php  if($bt_type==2){ ?>
                           <div>
                                <input type="checkbox" <?php if($bt_use==0){echo 'disabled'; }?> class="pay_yue" name="choice" value="bt"><?= _('使用白条支付（可用额度：') ?><?= __('不限额度'); ?>） <input type="hidden" name="bt" id="bt" value="<?= $bt_money ?>">
                                <!--  用白条支付的金额  -->
                                <input type="hidden" name="bt_pay" id="bt_pay" value="0"/> <span id="pay_yue_bt_span" style="margin-left:73px;color:red;"><?php if($bt_use==1){echo '(注：白条不能与其他支付方式同时使用)'; }else{echo '(你的白条欠款已逾期，暂不可用，请及时还款已逾期欠款)';}?></span>
                            </div>
                        <!-- 固定额度 -->
                        <?php }else if (isset($bt_money) && $bt_money > $uorder_base['trade_payment_amount']) { ?>
                            <div>
                                <input type="checkbox" <?php if($bt_use==0){echo 'disabled'; }?> class="pay_yue" name="choice" value="bt"><?= _('使用白条支付（可用额度：') ?><?= format_money($bt_money) ?>） <input type="hidden" name="bt" id="bt" value="<?= $bt_money ?>">
                                <!--  用白条支付的金额  -->
                                <input type="hidden" name="bt_pay" id="bt_pay" value="0"/> <span id="pay_yue_bt_span" style="margin-left:73px;color:red;"><?php if($bt_use==1){echo '(注：白条不能与其他支付方式同时使用)'; }else{echo '(你的白条欠款已逾期，暂不可用，请及时还款已逾期欠款)';}?></span>
                            </div>
                        <?php } ?>
                ...
                <?php if ($sub_user_status && !$act) { ?>
                    ...
                    <div class="sub_user_pay box">
                        ...
                        <select id="sub_user">
                            <?php foreach ($sub_user['data']['sub'] as $k => $v) { ?>
                                <option value="<?= ($v['user_id']) ?>"><?= ($v['user_account']) ?></option>
                            <?php } ?>
                        </select>
                    </div>
                <?php } ?>
...
<script>
  ...
  function checkPassword() {
    var bt_type = '<?php echo $bt_type;?>';
    if (bt_type==1 && !checkMoney()) {
          ...
          if (result.status == 200) {
            if (result.data.user_mobile == '') {
              ...
              $.dialog({
                ...
                drag: false,
                ok: function () {
                  window.location.href = UCENTER_URL + '?ctl=User&met=security&op=mobiles';
                },
              });
            }  else {
    ...
    if (sub_user_payway == 1) {
      ...
      $.post(SITE_URL + "?ctl=Pay&met=subpay&typ=json", {trade_id: uorder_id, mark_id: "<?=$_GET['mark_id']?>"}, function (data) {
        if (data.status == 200) {
          ...
          //付款成功页
          if (!return_url) {
              return_url = data.data.return_app_url;
          }
          window.location.href = SITE_URL + "?ctl=Info&met=after_pay&order_id=" + data.data.order_id + '&return_url=' + return_url + "&type=api";
        } else {
  ...
  function paySubmit(e) {
    if (e.hasClass("submit_able")) {
      ...
      var bt_use = '<?=$bt_use?>';
      if (bt_payway && bt_use==0) {
        Public.tips.error('暂不可用，请及时还款已逾期欠款');
        return false;
      }
      //将选用的付款方式保存如数据库 
      ...
      $.post(SITE_URL + "?ctl=Info&met=checkPayWay&typ=json", data, function (data) {
            if (data.status == 200) {
              ...
              //如果选择了在线支付方式
              if (online_payway) {
                window.location.href = SITE_URL + "?ctl=Pay&met=" + online_payway + "&trade_id=" + uorder_id + "&payway=" + online_payway + "&payway_type=" + online_payway + '&return_url=' + return_url + "&type=api";
              } else {
                var payway = '';
                money_payway == true ? payway = 'balance':payway = 'baitiao';
                $.post(SITE_URL + "?ctl=Pay&met=money&typ=json", {trade_id: uorder_id, payway: payway}, function (data) {
                  if (data.status == 200) {
                    // window.location.href = data.data.return_app_url + '?ctl=Buyer_Order&met=<?=$order_g_type?>';
                    //付款成功页
                    window.location.href = SITE_URL + "?ctl=Info&met=after_pay&order_id=" + data.data.order_id + '&return_url=' + return_url + "&type=api";
                  } else {
                    $(".loading-msg").html(data.msg);
                    window.location.reload();
                  }
                }).error(function () {
                  $("#mask_box").hide();
