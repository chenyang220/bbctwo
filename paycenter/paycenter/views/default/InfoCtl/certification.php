            ...
            <div class="account_left fl">
                <div class="account_mes">
                    ...
                    <table class="account_table">
                        <tbody>
                        <tr>
                            <td class="check_name"><?=__('用户名称：')?></td>
                            <td><?=$data['user_nickname']?></td>
                        </tr>
                        <tr>
                            <td class="check_name"><?=__('真实姓名：')?></td>
                            <td><input type="text" class="w168" value="<?=$data['user_realname']?>" name="user_realname" id="real_name" ><div class="form-error"></div></td>
                        </tr>
                        <tr>
                            <td class="check_name"><?=__('证件类型：')?></td>
                            <td>
                                <select name="user_identity_type" id="user_identity_type">
                                    <option value="1"  <?php if($data['user_identity_type']==1){?>selected<?php }?>><?=__('身份证')?></option>
                                    <option value="2"  <?php if($data['user_identity_type']==2){?>selected<?php }?>><?=__('护照')?></option>
                                    <option value="3" <?php if($data['user_identity_type']==3){?>selected<?php }?>><?=__('军官证')?></option>
                                </select>
                                <div class="form-error"></div></td>
                        </tr>
                        <tr>
                            <td class="check_name"><?=__('证件号码：')?></td>
                        ...
                        <tr>
                            ...
                            <td style="display: flex;">
                                <input readonly="readonly"  id="start_time"  name="user_identity_start_time"  class="w100 hasDatepicker" type="text" value="<?php echo $data['user_identity_start_time']>0 ? $data['user_identity_start_time'] : '';?>" /><em><i class="iconfont icon-rili"></i></em>
                                <span></span>&nbsp-
                                <input readonly="readonly" <?php if($data['user_identity_end_time']==1){ echo 'style=
                                "display:none;"'; }?>id="end_time" name="user_identity_end_time"  class="w100 hasDatepicker" type="text" value="<?php echo $data['user_identity_end_time']>1 ? $data['user_identity_end_time'] : '';?>" /><em><i class="iconfont icon-rili"></i></em>
                                <label for="longtime">长期&nbsp</label>
                                <input style="margin-top: 13px" name='longtime' id="longtime" <?php if($data['user_identity_end_time']==1){ echo 'checked'; }?> type="checkbox" value="1" />
                            </td>
                        </tr>
                        <tr>
                            <td class="check_name"><?=__('正面照预览：')?></td>
                            <td>
                        ...
                        <tr>
                            <td class="check_name"><?=__('证件正面照：')?></td>
                            <td>
                     ...
                     <input name="user_identity_face_logo" id="user_identity_face_logo" type="hidden" value="<?=$data['user_identity_face_logo']?>" />
                        ...
                        <tr>
                            <td class="check_name"><?=__('证件背面照：')?></td>
                            <td>
                                <div > <a href="javascript:void(0);"><span>
                     <input name="user_identity_font_logo" id="user_identity_font_logo" type="hidden" value="<?=$data['user_identity_font_logo']?>" />
                      </span>
                                        <p id='user_font_upload' style="float:left;" class="bbc_btns"><i class="iconfont icon-upload-alt"></i><?=__('图片上传')?></p>
            ...
            <div class="account_right fr">
                <div class="account_right_con">
                    <ul class="cert_instructions">
                        <li>
                            <h5><?=__('什么是实名认证？')?></h5>
                            <p><?=__('实名认证，是利用其国家级身份认证平台“身份通实名认证平台”推出的实名身份认证服务。在Pay Center平台进行实名认证无需繁琐步骤，只需如实填写您的姓名和身份证号，并支付5元实名认证费用（国家发改委定价，线上线下均可支付），就能完成实名认证。')?></p>
                        </li>
                        <li>
                            <h5><?=__('为什么要实名认证?')?></h5>
