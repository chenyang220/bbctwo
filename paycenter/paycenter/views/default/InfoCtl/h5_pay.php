...
<?php
 //免登录鉴权
$token = isset($_COOKIE['token']) ? $_COOKIE['token'] : $ucenter_user_info['token']; //令牌
$enterprise_id = isset($_COOKIE['enterId']) ? $_COOKIE['enterId'] : $ucenter_user_info['enterId']; //企业id
...
<div class="main">
    <div class="form">
        <p><?= __('请确认微信支付是否已完成'); ?></p>
        <div class="btn">
            <a href="<?=Yf_Registry::get('paycenter_api_url')?>?ctl=Info&met=wx_paystatus&order_id=<?=$order_id?>&token=<?=$token?>&enterId=<?=$enterprise_id?>"><?= __('点击确认支付'); ?></a>
        </div>
        <div class="btn">
            <a class="s_btn" href="<?=Yf_Registry::get('paycenter_api_url')?>?ctl=Info&met=wx_paystatus&order_id=<?=$order_id?>&token=<?=$token?>&enterId=<?=$enterprise_id?>"><?= __('支付遇到问题，重新支付'); ?></a>
        </div>
    </div>
</div>
<script type="text/javascript" src="<?php echo $this->view->js; ?>/ucsdk.min.js" ></script>
<script type="text/javascript">
        ...
        UC.config = {
            ...
            // // 当前企业ID，不传会默认从地址栏获取
            enterId: "<?php echo $enterprise_id;?>",
            // 必填,从开放平台处获取
