	...
	<div class="pc_user_mes clearfix">
		...
		<div class="pc_user_mes_rt fr clearfix">
			...
			<div class="pc_user_mes_rt_text fl">
				...
				<?php if(Payment_ChannelModel::status('baitiao') == Payment_ChannelModel::ENABLE_YES && Web_ConfigModel::value('yunshan_status') != 1) {?>
				<dl class="clearfix dl-public">
...
<div class="pc_transaction wrap">
	<h4><?=__('最近交易')?><span class="trade_types">
		<?php if(Web_ConfigModel::value('yunshan_status') != 1){?>
		<a target="_blank" href="<?= Yf_Registry::get('url') ?>?ctl=Info&met=recordlist&type=3&typ=e" ><?=__('充值记录')?></a>&nbsp;|&nbsp;
		<a target="_blank" href="<?= Yf_Registry::get('url') ?>?ctl=Info&met=recordlist&type=2&typ=e" ><?=__('转账记录')?></a>&nbsp;|&nbsp;
		<?php } ?>
		<a target="_blank" href="<?= Yf_Registry::get('url') ?>?ctl=Info&met=recordlist&type=4&typ=e" ><?=__('提现记录')?></a></span></h4>
		...
		<div class="pc_trans_det pc_trans_other">
			...
			<p class="wp20">
				<?php if($conval['act'] == 'pay'){ ?>
                    ...
                    <a class="pc_btn btn_active btn-pay-reset" onclick="ckeckOrder('<?=$conval['uorder']?>')"><?=__('付款')?></a>
					<?php }?>
