...
<?php if($this->user_info['user_mobile']){?>
    <form id="form" action="#" method="post" >
        ...
        <div class="withdrawals-content-center content-public clearfix">
        	...
        	<ul style="margin-top:2rem;border:none !important;padding-bottom: 10px;">
            	<input type="radio" name="withdraw_type" value="0" checked="">提现到银行卡
				<input type="radio" name="withdraw_type" value="1">提现到微信零钱
                <input type="radio" name="withdraw_type" value="2">提现到支付宝
            </ul>
        ...
        <div class="withdrawals-content-bottom content-public clearfix">
            ...
            <div class="left clearfix" id="withdraw_wx" style="display: none;">
            	<div class="leftauto">
                    ...
                    <div>
                        <dl class="clearfix">
                            <dt><?=_('提取金额：')?></dt>
                            <dd id="width">
                                <em class="symbol">￥</em><input type="text" AUTOCOMPLETE="off" class="text text-2" maxlength="10" name="withdraw_money" id="withdraw_moneys"  onkeyup="amount(this)" onblur="checkMoney(this)" />
                                <p class="error_msg" id="error_msg_money"></p>
                            </dd>
                        </dl>
                    </div>
                    ...
                    <div>
                        <dl class="clearfix">
                            <dt><?=_('提取说明：')?></dt>
                            <dd>
                                <input type="text" name="con" id="cons" class="text text-5 "> </dd>
                        </dl>
                    </div>
                    <div>
                        <dl class="clearfix ">
                            <dt>手机：</dt>
                            <dd><input type="text" class="text text-6" readonly="readonly" name="mobile" id="mobiles" style="border:none;"  value="<?=substr($this->user_info['user_mobile'],0,3).'***'.substr($this->user_info['user_mobile'],-3,3)?>"/></dd>
                        </dl>
                    </div>
                    ...
                    <!--  <div>
                        ...
                        <dl class="clearfix ">
                            <dt><em class="must" style="color: #f00;">*</em><?=_('图形验证码：')?></dt>
                            <dd>
                                <input type="text"  name="img_yzm" id="img_yzms" maxlength="6" style="width:86px;vertical-align:top;" placeholder="<?=_('请输入验证码')?>" default="<i class=&quot;i-def&quot;></i><?=_('看不清？点击图片更换验证码')?>"  />
                                <img style="vertical-align: middle;" onClick="get_randfunc(this);" title="<?=_('换一换')?>" class="img-code" style="cursor:pointer;" src='./libraries/rand_func.php'/>
                            </dd>
                        </dl>
                    ...
                    <div>
                        <dl class="clearfix">
                            <dt><?=_('确认支付密码:')?></dt>
                            <dd>
                                <input type="password" name="password" id="passwords" class="text text-5 ">
                            </dd>
                            <dd style="line-height: 40px;margin-left:10px">
                            	<a href="#" onclick="checkCer()"><?=__('忘记密码')?></a>
                            </dd>
                        </dl>
                    </div>
                </div>
            ...
            <div class="left clearfix" id="withdraw_zfb" style="display: none;">
                <div class="leftauto">
                    ...
                    <div>
                        <dl class="clearfix">
                            <dt><?=_('提取金额：')?></dt>
                            <dd id="width">
                                <em class="symbol">￥</em><input type="text" AUTOCOMPLETE="off" class="text text-2" maxlength="10" name="withdraw_money" id="withdraw_moneys_z"  onkeyup="amount(this)" onblur="checkMoney(this)" />
                                <p class="error_msg" id="error_msg_money"></p>
                            </dd>
                        </dl>
                    </div>
                    <div>
                        <dl class="clearfix">
                            <dt><?=_('支付宝账号')?></dt>
                            <dd >
                                <input type="text" name="withdraw_identity" id="withdraw_identity" class="text-5"/>
                            </dd>
                        </dl>
                    </div>
                    <div>
                        <dl class="clearfix">
                            <dt><?=_('真实姓名')?></dt>
                            <dd >
                                <input type="text" name="withdraw_name" id="withdraw_name" class="text-5"/>
                            </dd>
                        </dl>
                    </div>
                    <div>
                        <dl class="clearfix">
                            <dt><?=_('提取说明：')?></dt>
                            <dd>
                                <input type="text" name="con" id="cons_z" class="text text-5 "> </dd>
                        </dl>
                    </div>
                    <div>
                        <dl class="clearfix ">
                            <dt>手机：</dt>
                            <dd><input type="text" class="text text-6" readonly="readonly" name="mobile" id="mobiles_z" style="border:none;"  value="<?=substr($this->user_info['user_mobile'],0,3).'***'.substr($this->user_info['user_mobile'],-3,3)?>"/></dd>
                        </dl>
                    </div>
                    <div>
                        <dl class="clearfix">
                            <dt><?=_('确认支付密码:')?></dt>
                            <dd>
                                <input type="password" name="password" id="passwords_z" class="text text-5 ">
                            </dd>
                            <dd style="line-height: 40px;margin-left:10px">
                                <a href="#" onclick="checkCer()"><?=__('忘记密码')?></a>
                            </dd>
    ...
    <script>
        ...
        {
            $.post("./index.php?ctl=Info&met=getUserResourceInfo&typ=json", function(data){
...
//				}
                ...
                checkCardno:function()
                {
                    ...
                    if($('#cardno').val().length<16 ||$('#cardno').val().length >21) {
                        return '银行卡号格式错误';
                    }
            ...
            fields: {
                ...
                'password':'required',
                'withdraw_identity':'required',
                'withdraw_name':'required',
                //'withdraw_moneys': 'required;',
                //'passwords':'required',
            },
            valid:function(form){

            	var withdraw_type = $("input[name='withdraw_type']:checked").val();
            	...
            	if(withdraw_type == 0)
            	{
                    ...
                    var withdraw_money = $("#withdraw_moneys").val();
                    var con       = $("#cons").val();
                    var paypasswd = $("#passwords").val();
                    var mobile = $("#mobiles").val();
                    
		
                }else{
                    var withdraw_money = $("#withdraw_moneys_z").val();
                    var withdraw_identity = $("#withdraw_identity").val();
                    var withdraw_name = $("#withdraw_name").val();
                    var con       = $("#cons_z").val();
                    var paypasswd = $("#passwords_z").val();
                    var mobile = $("#mobiles_z").val();
                }
                ...
                if(withdraw_type == 1)
                {
                	var ajax_url = '<?= Yf_Registry::get('url');?>?ctl=Info&met=addWithdrawWx&typ=json';
                }else if(withdraw_type == 2){
                    var ajax_url = '<?= Yf_Registry::get('url');?>?ctl=Info&met=addWithdrawZfb&typ=json';
                }else{
                ...
                $.ajax({
                    url: ajax_url,
                    async:false,
                    data:{id:id,bank_name:bank_name,cardno:cardno,bank:bank,withdraw_money:withdraw_money,con:con,paypasswd:paypasswd,yzm:yzm,mobile:mobile,val:val,withdraw_identity:withdraw_identity,withdraw_name:withdraw_name},
                    success:function(a){
                        console.info(a);
                        if(a.status == 200)
                        {
    ...
    function checkCer()
    {
    	window.location.href = "https://paycenter.rzjchina.com/index.php?ctl=Info&met=passwd";
    }
