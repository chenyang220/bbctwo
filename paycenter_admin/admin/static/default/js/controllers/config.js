...
$(function ()
{
    ...
    if ($('#ht-setting-form').length)
    {

        $('#ht-setting-form').validator({
            ignore: ':hidden',
            theme: 'yellow_bottom',
            timely: 1,
            stopOnError: true,
            ...
            valid: function (form)
            {
                ...
                parent.$.dialog.confirm('修改立马生效,是否继续？', function ()
                    {
                        Public.ajaxPost(SITE_URL + '?ctl=Config&met=edit&typ=json', $('#ht-setting-form').serialize(), function (data)
                        {
                            if (data.status == 200)
                            {
                                parent.Public.tips({content: '修改操作成功！'});
...
$(function ()
{
    if ($('#yunshan-setting-form').length > 0)
    {
        $('#yunshan-setting-form').validator({
            ignore: ':hidden',
            theme: 'yellow_bottom',
            timely: 1,
            stopOnError: true,
            ...
            {
                parent.$.dialog.confirm('修改立马生效,是否继续？', function ()
                    {   console.log($('#yunshan-setting-form').serialize())
                        Public.ajaxPost(SITE_URL + '?ctl=Config&met=edit&typ=json', $('#yunshan-setting-form').serialize(), function (data)
                        {
                            if (data.status == 200)
                            {
                                parent.Public.tips({content: '修改操作成功！'});
