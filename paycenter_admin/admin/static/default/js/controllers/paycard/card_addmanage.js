...
function initValidator()
{
    $_form.validator({
        ...
        valid: function (form)
        {
            ...
            //判断是否填写卡号、卡片数
            if(!b.card_id || !b.card_sum)
            {
                parent.parent.Public.tips({type: 1, content: "请填写卡号、卡片数！"})
                return false;
            }
            var reg = /^\d{4}$/;
            if(!reg.test(b.card_id))
            {
                parent.parent.Public.tips({type: 1, content: "请填写正确的卡号！"})
                return false;
            }
