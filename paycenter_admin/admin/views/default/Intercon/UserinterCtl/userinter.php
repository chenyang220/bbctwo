...
<div class="wrapper">
  ...
  <div id="para-wrapper">
  <div class="ilodate"><?= __('互联设置'); ?></div>
    <div class="para-item">
      <h3  id="qqdata">QQ<?= __('互联'); ?></h3>
      <ul class="mod-form-rows" id="establish-form">
        <li class="row-item">
          <div class="label-wrap">
            <label for="qq_status"><?= __('是否开启'); ?>:</label>
          </div>
          <div class="QQ ctn-wrap">
				<input type="radio"  name="qq_status" value="1" <?php if($data['qq_status'] == 1){echo 'checked';} ?> >&nbsp;<?= __('是'); ?>&nbsp;
				<input type="radio"  name="qq_status" value="2" <?php if($data['qq_status'] == 2){echo 'checked';} ?>>&nbsp;<?= __('否'); ?>&nbsp;&nbsp;
				[<a href="http://connect.qq.com/"><?= __('申请'); ?>API</a>]
    ...
    <div class="para-item">
      ...
      <ul class="mod-form-rows" id="establish-form">
        <li class="row-item">
          <div class="label-wrap">
            <label for="weibo_status"><?= __('是否开启'); ?>:</label>
          </div>
          <div class="sina ctn-wrap">
            <input type="radio" name="weibo_status" value="1" <?php if($data['weibo_status'] == 1){echo 'checked';} ?> >&nbsp;<?= __('是'); ?>&nbsp;
			<input type="radio" name="weibo_status" value="2" <?php if($data['weibo_status'] == 2){echo 'checked';} ?> >&nbsp;<?= __('否'); ?>&nbsp;
			[<a href="http://open.weibo.com/connect"><?= __('审请'); ?>API</a>]
          </div>
        </li>
        <li class="row-item">
          <div class="label-wrap">
            <label for="companyAddress">APP ID<?= __('：'); ?></label>
          </div>
          <div class="ctn-wrap">
        ...
        <li class="row-item">
          <div class="label-wrap">
            <label for="companyTel">KEY<?= __('：'); ?></label>
          </div>
          <div class="ctn-wrap">
	...
	<div class="para-item">
      ...
      <ul class="mod-form-rows" id="establish-form">
        <li class="row-item">
          <div class="label-wrap">
            <label for="companyName"><?= __('是否开启'); ?></label>
          </div>
          <div class="weixin ctn-wrap">
            <input type="radio" name="weixin_status" value="1" <?php if($data['weixin_status'] == 1){echo 'checked';} ?> >&nbsp;<?= __('是'); ?>&nbsp;
			<input type="radio"  name="weixin_status" value="2" <?php if($data['weixin_status'] == 2){echo 'checked';} ?> >&nbsp;<?= __('否'); ?>&nbsp;&nbsp;
			[<a href="http://open.weixin.qq.com"><?= __('审请'); ?>API</a>]
          </div>
        </li>
        <li class="row-item">
          <div class="label-wrap">
...
<?= __('　　'); ?>	var qq_status = $('.QQ input[name="qq_status"]:checked ').val();
		...
		if(qq_status == 1)
		{
			if(qq_app_id && qq_app_key)
			{
				...
				alert("<?= __('失败'); ?>,<?= __('请填写申请的'); ?>QQ<?= __('的'); ?>APPID<?= __('和'); ?>KEY");
				qq_status == NULL;
		...
		if(weibo_status == 1)
		{
			if(weibo_app_id && weibo_app_key)
			{
				...
				flag1=0;
				alert("<?= __('失败'); ?>,<?= __('请填写申请的新浪微博的'); ?>APPID<?= __('和'); ?>KEY");
				weibo_status == NULL;
			}
		}
		if(weixin_status == 1)
		{
			if(weixin_app_id && weixin_app_key)
			{
				...
				flag1=0;
				alert("<?= __('失败'); ?>,<?= __('请填写申请的微信的'); ?>APPID<?= __('和'); ?>KEY");
				weixin_status == NULL;
			}
		}
