...
<body>
       ...
       <div class="ncap-form-default">
              <dl class="row">
               ...
               <dl class="row">
                   <dt class="tit">
                       <label for="user_nickname"><?= __('收款金额'); ?></label>
                   </dt>
                   <dd class="opt">
...
{
  var consume_record_id = '<?=$consume_record_id;?>';
	$_form.validator({
        ...
        valid: function (form)
        {
			...
			Public.ajaxPost(SITE_URL +"?ctl=Paycen_PayInfo&met=editCreditReturn&typ=json", params, function (e)
			{
				if (200 == e.status)
				{
					parent.parent.Public.tips( {content:"<?= __('还款成功！'); ?>"});
					 var callback = frameElement.api.data.callback;
                                            callback();
				}
				else
				{
					parent.parent.Public.tips({type: 1, content:  "<?= __('还款失败！'); ?>" + e.msg})
				}
			})
        },
        ignore: ":hidden",
