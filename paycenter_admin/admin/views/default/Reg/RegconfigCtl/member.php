...
<div class="wrapper">
  ...
  <div id="para-wrapper">
  <div class="ilodate"><?= __('会员登录设置：'); ?></div>
    <div class="para-item">
      <ul class="mod-form-rows" id="establish-form">
        <li class="row-item">
          <div class="label-wrap">
            <label for="zend_status_type"><?= __('允许新用户注册：'); ?></label>
          </div>
          <div class="mb6 ctn-wrap">
				<input type="radio"  name="zend_status_type" value="1" <?php if($data['zend_status_type'] == 1){echo 'checked';} ?> >&nbsp;<?= __('是'); ?>&nbsp;
				<input type="radio"  name="zend_status_type" value="2" <?php if($data['zend_status_type'] == 2){echo 'checked';} ?>>&nbsp;<?= __('否'); ?>&nbsp;&nbsp;
				<span class="tip"><?= __('设置是否允许游客注册成为会员。'); ?></span>
          </div>
        </li>
        <li class="row-item">
          <div class="label-wrap">
            <label for="zend_closecon"><?= __('关闭注册提示信息：'); ?></label>
          </div>
          <div class="ctn-wrap">
				...
				<span class="tip" style="margin-left:25px;"><?= __('当站点关闭注册时的提示信息'); ?></span>
          </div>
        </li>
        <li class="row-item">
          <div class="label-wrap">
            <label for="zend_censoruser" style="line-height:80px;"><?= __('受保护用户名：'); ?></label>
          </div>
          <div class="ctn-wrap">
			...
			<span class="tip" style="margin-left:25px;"><?= __('用户在注册时无法使用这些用户名。每个用户名一行'); ?></span>
		  </div>
        </li>
		<li class="row-item">
          <div class="label-wrap">
            <label for="zend_pwlength"><?= __('密码最小长度：'); ?></label>
          </div>
          <div class="ctn-wrap">
			...
			<span class="tip" style="margin-left:30px;"><?= __('新用户注册时密码最小长度'); ?>5<?= __('，'); ?>0<?= __('或不填为不限制'); ?></span>
		  </div>
        </li>
		<li class="row-item">
          <div class="label-wrap">
            <label for="zend_pwlength"><?= __('强制密码复杂度：'); ?></label>
          </div>
          <div class="pw ctn-wrap">
				<input  type="checkbox" name="zend_strongpw1" class="zend_strongpw1" value="1" <?php if($data['zend_strongpw'][0] == 1) echo 'checked' ?>  />&nbsp;<?= __('数字'); ?>
				<input  type="checkbox" name="zend_strongpw2" class="zend_strongpw2" value="2" <?php if($data['zend_strongpw'][1] == 2) echo 'checked' ?> />&nbsp;<?= __('小写字母'); ?>
				<input  type="checkbox" name="zend_strongpw3" class="zend_strongpw3" value="3" <?php if($data['zend_strongpw'][2] == 3) echo 'checked' ?>  />&nbsp;<?= __('大写字母'); ?>
				<input  type="checkbox" name="zend_strongpw4" class="zend_strongpw4" value="4" <?php if($data['zend_strongpw'][3] == 4) echo 'checked' ?> />&nbsp;<?= __('符号'); ?>
				<span class="tip" style="margin-left:32px;"><?= __('新用户注册时密码中必须存在所选字符类型，不选则为无限制'); ?></span>     
		  </div>
        </li>
		<li class="row-item">
          <div class="label-wrap">
            <label for="zend_pwlength"><?= __('新用户注册验证：'); ?></label>
          </div>
          <div class="reg ctn-wrap" style="height:50px;">
			<input type="radio" name="zend_user_reg" value="1" <?php if($data['zend_user_reg'] == 1){echo 'checked';} ?> />&nbsp;<?= __('无'); ?>
			<input type="radio" name="zend_user_reg" value="2" <?php if($data['zend_user_reg'] == 2){echo 'checked';} ?> />&nbsp;Email <?= __('验证'); ?>
			<input type="radio" name="zend_user_reg" value="3" <?php if($data['zend_user_reg'] == 3){echo 'checked';} ?> />&nbsp;<?= __('手机验证'); ?>
			<span class="tip" style="width:730px;margin-left:82px;display:block;float:right;"><?= __('选择'); ?>"<?= __('无'); ?>"<?= __('用户可直接注册成功；选择'); ?>"Email <?= __('验证'); ?>"<?= __('将向用户注册'); ?> Email <?= __('发送一封验证邮件以确认邮箱的有效性；选择'); ?>"<?= __('手机'); ?> <?= __('验证'); ?>"<?= __('将向用户注册手机发送一条短信验证码验以确认手机的有效性。'); ?></span>
		  </div>
        </li>
		
		<li class="row-item">
          <div class="label-wrap">
            <label for="zend_pwlength"><?= __('同一'); ?> IP <?= __('注册间隔限制：'); ?></label>
          </div>
          <div class="ctn-wrap">
			...
			<span class="tip" style="margin-left:30px;"><?= __('同一'); ?> IP <?= __('在本时间间隔内将只能注册一个帐号，'); ?>0 <?= __('为不限制'); ?></span>
		  </div>
        </li>
		<li class="row-item">
          <div class="label-wrap">
            <label for="zend_pwlength">24<?= __('小时注册的最大次数：'); ?></label>
          </div>
          <div class="ctn-wrap">
			...
			<span class="tip" style="margin-left:30px;"><?= __('同一'); ?> IP <?= __('地址在'); ?> 24 <?= __('小时内尝试注册的次数限制，建议在'); ?> 30 - 100 <?= __('范围内取值，'); ?>0 <?= __('为不限制'); ?></span>
		  </div>
        </li>
		<li class="row-item">
          <div class="label-wrap">
            <label for="zend_pwlength">IP<?= __('注册间隔限制'); ?>(<?= __('小时'); ?>)<?= __('：'); ?></label>
          </div>
          <div class="ctn-wrap">
		...
		<li class="row-item" style="height:60px;">
          <div class="label-wrap">
            <label for="zend_pwlength" style="line-height:60px;"><?= __('限时注册的'); ?> IP <?= __('列表：'); ?></label>
          </div>
          <div class="ctn-wrap">
			...
			<span class="tip" style=";margin-left:30px;"><?= __('当用户处于本列表中的'); ?> IP <?= __('地址时，在限时注册'); ?>IP<?= __('注册间隔限制内将至多只允许注册一个帐号。每个'); ?> IP <?= __('一行。'); ?></span>
		  </div>
        </li>
		<li class="row-item">
          <div class="label-wrap">
            <label for="zend_pwlength" style="line-height:200px;font-weight:600;"><?= __('注册协议：'); ?></label>
          </div>
          <div class="ctn-wrap">
        ...
        <div class="btn-wrap"> <a name="submit" id="submit" class="ui-btn ui-btn-sp"><?= __('提交'); ?></a> </div>
        <!--</form>-->
	</div>
</div>
