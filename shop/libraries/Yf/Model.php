<?php
    ...
    class Yf_Model extends Yf_Singleton
    {
         ...
         * @return array|int
         */
        public function getJoin(array $fields,array  $join_str,$cond_row=array(),$order_row=array(),$group_row = array(),$limitFlag=false,$page=1,$rows=1000){
            $fileds_arr = array();
            if($fields){
                foreach ($fields as $key=>$val){
                    if(is_string($key) && is_array($val)){
                        $fileds_arr[] = "{$key}.".implode(",{$key}.",$val);
                    }else{
                        $fileds_arr[] = $val;
                    }
                }
            ...
            $fields_str = '*';
            $fileds_arr && $fields_str = implode(",",$fileds_arr);
            $this->sql->setFileds($fields_str);
            $main_table = $this->_tableName;
            $main_table_alias = $this->_tableAliasName?:$this->_tableName;
            foreach ($join_str as $item){
                $this->sql->setJoin($item);
            }
            if ($cond_row) {
            ...
            if($group_row){
                $group_str = implode($group_row,' , ');
                $this->sql->setGroup($group_str);
            }
            if ($order_row) {
                foreach ($order_row as $k => $v) {
                    $this->sql->setOrder($k, $v);
            ...
            $tables = $main_table." ".$main_table_alias;
            $ret = $this->_union_select($tables,$limitFlag);
            $data = array();
            if($limitFlag){
                $total = $this->getFoundRows();//行数
                $data['page'] = $page;
        ...
        public function alias($alias_name){
            $alias_name && $this->_tableAliasName = $alias_name;
            return $this;
        }
         ...
         * 联合查询
         */
        public function join($type,$join,$on){
            $opt_str = '';
            if($type && $join && $on){
                $opt_str = $type." JOIN ";
                $opt_str.= " ".$join." ";
         ...
         * 设置数据表查询字段
         */
        public function fileds(array $fileds_arr){
            $fields_str = '*';
            $fileds_arr && $fields_str = implode(",",$fileds_arr);
            $this->sql->setFileds($fields_str);
            return $this;
        ...
        protected function _union_select($main_table=null,$limitFlag=false)
        {
            $fileds = $this->sql->getFileds();
            !$fileds && $fileds = '*';
            $sql = "SELECT SQL_CALC_FOUND_ROWS {$fileds} FROM " . $main_table." ";
            $sql .= $this->sql->getJoin();
            $sql .= $this->sql->getWhere();
            $sql .= $this->sql->getOrder();
            $limitFlag &&  $sql .= $this->sql->getLimit();
            $rs = $this->sql->getAll($sql);
            !$rs && $rs = array();
            return $rs;
        }
        ...
        public function getNum($cond_row)
        {
            if ($cond_row) {
                foreach ($cond_row as $k => $v) {
                    $k_row = explode(':', $k);
         ...
         * @access public
         */
        protected function add($field_row, $return_insert_id = false, $html = true)
        {
            //防止cache有误，可以考虑清下对应的cache，容错
            $add_flag = $this->_insert($field_row, $html);
         ...
         * @access protected
         */
        protected function _insert(&$a, $html = true)
        {
            $data_row = array();
         ...
         * @access public
         */
        protected function edit($table_primary_key_value = null, $field_row, $flag = null, $html = true)
        {
            $update_flag = false;
            ...
            if ($table_primary_key_value) {
                ...
                $update_flag = $this->_update($field_row, $flag, $html);
         ...
         * @access protected
         */
        protected function _update(&$a, $flag = null, $html = true)
        {
        ...
        /** 重新封装 **/
        public function setCondWhere($cond_row,$pre)
        {
            $cond_str = '';
            if ($cond_row) {
                foreach ($cond_row as $k => $v) {
                    ...
                    if (count($k_row) > 1) {
                        $cond_str .= $this->setWhere($pre.$k_row[0], $v, $k_row[1]);
                    } else {
                        $cond_str .= $this->setWhere($pre.$k, $v);
                    }
                }
            }
        ...
        public function setOrderBy($order_row,$pre)
        {
            $order_str = '';
            if ($order_row) {
                foreach ($order_row as $k => $v) {
                    $order_str = $this->setOrder($pre.$k, $v,$order_str);
                }
            }
        ...
        public function setWhere($key, $val, $symbol = '=', $join = 'AND')
        {
            ...
            switch ($symbol) {
                ...
                case '<>' :
                    $val = untrim($val);
                    $cond_str .= ' ' . $join . ' ' . $key . $symbol . $val;
                    break;
                case 'IN' :
                case 'NOT IN' :
                    if (is_array($val)) {
                        $val = array_map('untrim', $val);
                        $cond_str .= ' ' . $join . ' ' . $key . ' ' . $symbol . ' (' . implode(',', $val) . ')';
                    } else {
                        $val = untrim($val);
                ...
                case 'LIKE' :
                    if (is_array($val)) {
                        foreach ($val as $item) {
                            ...
                            $cond_str .= ' ' . $join . ' ' . $key . ' LIKE ' . $item;
                        }
                    } else {
                        $val = untrim($val);

                        $cond_str .= ' ' . $join . ' ' . $key . ' LIKE ' . $val;
                    }
        ...
        public function setOrder($order = null, $flag = 'ASC',$order_str)
        {
            if (!$order_str)
            {
                $order_str .= ' ORDER BY ' . $order . ' ' . strtoupper($flag);
            }
            else
            {
                $order_str .= ', ' . $order . ' ' . strtoupper($flag);
            }
