...
class Yf_Uploader
{
    ...
    private function upFile()
    {
        ...
        if (!(move_uploaded_file($file["tmp_name"], $this->filePath) && file_exists($this->filePath))) { //移动失败
			...
			//临时放入,最终需要传入
			if (Web_ConfigModel::value('remote_image_status') == 1)
			{
    ...
    private function getFullName()
    {
        ...
        $ext = $this->getFileExt();
		//防止多图片上传造成伪随机数重复 @nsy 2019-09-12
		$ex = explode(".", lcg_value());
		$preg = '/[0]*/';
		$num = preg_replace($preg, '', $ex[1], 1);
        return $format .$num. $ext;
    }
    ...
    public function getFileInfo()
    {
		...
		$url_prefix = $base . '/image.php/'  . APP_DIR_NAME . '/data/upload';

		$url = $this->remoteUrl ? $this->remoteUrl : $url_prefix . $this->fullName;
        if(substr($url, 0,7)  == 'http://' 
            && strpos($base,'://')!==false
            && substr($base, 0,7) != "http://"
        ){
