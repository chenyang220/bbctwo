...
function __autoload($class_name)
{
    ...
    $extends_flag = false;
    if (!class_exists($class_name, false) && !interface_exists($class_name, false)) {
        if(false !== strpos($class_name, 'Extends_')) {
            $extends_flag = true;
            $class_name = str_replace("Extends_", "", $class_name);
        }
        if (false !== strpos($class_name, '_')) {
            $class_file_suffix = '/' . str_replace('_', '/', $class_name) . '.php';
        } elseif (false !== strpos($class_name, '\\')) {
        ...
        if($extends_flag){
            $class_file_path = EXTENDS_PATH ."/models". $class_file_suffix;
            if (is_file($class_file_path)) {
                $int_num++;
                import($class_file_path);
            } else {
                $class_file_path = EXTENDS_PATH ."/controllers". $class_file_suffix;
                if (is_file($class_file_path)) {
                    $int_num++;
                    import($class_file_path);
                } else {
        ...
        if($extends_flag){
            $class_name = "Extends_".$class_name;
        }
        if (!class_exists($class_name, false) && !interface_exists($class_name, false)) {
            //throw new Exception('Class ' . $class_name . ' does not exists : ' . $class_file_suffix);
            error_header(404, 'Page Not Found');
...
//	$data = is_array($data) ? array_map('conversion_array_type', $data) : conversion($data);
    ...
    function download_url($url, $timeout = 300)
    {
        ...
        if (!$url) {
            throw new Exception('http_no_url', __('Invalid URL Provided.'));
        }
        $url_filename = basename(parse_url($url, PHP_URL_PATH));
        $tmpfname = Yf_Utils_File::tempnam($url_filename);
        if (!$tmpfname) {
            throw new Exception('http_no_file', __('Could not create Temporary file.'));
        }
    ...
    {
        if (empty($tips_conf)) {
           $tips_conf = include_once LIB_PATH . "/tips.php";
        }
        return $tips_conf[$code];
    }
