...
if($order_base['order_send_type']==1){
	$content_div = '';
	// 商家自主配送，商品已发出
	$content_div = sprintf('<div class="pc"><div class="p-tit">商家自主配送，商品已发出</div><div class="logistics-cont">');
	$content_div = sprintf('%s<a style="color:red;font-weight:800;"> 联系人： %s </a><div class="ftx-13"> </div>',$content_div,$order_base['contact_name']);

	$content_div = sprintf('%s<a style="color:red;font-weight:800;"> 联系电话： %s </a><div class="ftx-13"> </div>',$content_div,$order_base['contact_mobile']);
	$content_div = sprintf('%s<a style="color:red;font-weight:800;"> 备注： %s </a><div class="ftx-13"> </div>',$content_div,$order_base['contact_remarks']);
	$content_div = sprintf('%s</div></div><div class="p-arrow p-arrow-left" style="top: 242px;"></div>',$content_div);
	if($_GET['typ'] == 'json'){
		exit(json_encode([
			'status'=>1,
			'err'=>1,
			'order_send_type'=>$order_base['order_send_type'] ,
			'contact_name'=> $order_base['contact_name'],
			'contact_mobile'=> $order_base['contact_mobile'],
			'contact_remarks'=> $order_base['contact_remarks'],
		]));
	}else{
		 echo $content_div;die;
	}
}else{
	...
	elseif ('kuaidiniao' == $channel)
	{
		...
		/*
		*顺丰查询快递轨迹时，需传递收件人手机号码后四位
		*@nsy 2019-10-14
		*/
		$receiver_mobile = '';
		if('SF' == $express_code){
			//查询收货人手机号码
		   $receiver =  $ExpressModel->sql->getAll("select order_receiver_contact from yf_order_base where order_id='{$order_id}'");
		   if($receiver){
			   $receiver_mobile = substr($receiver[0]['order_receiver_contact'],-4);
		   }
		} 
		$request_rows = array (
		...
		//传入,收件人手机号码后四位参数
		$receiver_mobile && $request_rows['CustomerName'] = $receiver_mobile;
