...
class Api_ConfigCtl extends Api_Controller
{
	...
	public function __call($method, $args)
	{
		...
		foreach ($config_type_row as $config_type)
		{
			...
			if ('plugin' == $config_type)
			{
                ...
                $file_row = array_diff($file_row, ["Analytics.php"]);
				...
				foreach ($file_row as $file)
				{
					if ('.' != $file && '..' != $file && is_file($plugin_user_dir . '/' . $file))
					{
						...
						if ('Plugin_Perm' == $plugin_name || 'Plugin_Fenxiao' == $plugin_name)
						{
							continue;
						}
						try
	...
	public function themeStyle()
	{
		...
		$data['config'] = $Web_ConfigModel->getConfigValue("theme_page_color");
		$data['config_back'] = $Web_ConfigModel->getConfigValue("theme_page_color_back");
		$this->data->addBody(-140, $data );
	}
	...
	*/
	...
	public function edit()
	{
		$Web_ConfigModel = new Web_ConfigModel();
		$config_type_row = request_row('config_type');
		//plus会员权益红包处理 @nsy 2019-01-24
        //plus会员要处理的key
        $plus_array = $plus_quanyi_set = array();
        $deal_plus_key =array(
            'plus_general_red',//超级会员日红包
            'plus_quota',//plus会员每月赠送红包
            'plus_red_packet',//plus会员每月赠送红包,满xx金额使用
        );
        //初始化plus会员红包数组
        $init_plus_red_packet = array(
            'plus_day' => array(
                'redpacket_t_title'=>'PLUS超级会员日红包',
                'redpacket_t_price'=>0,//面额
                'redpacket_t_desc'=>'PLUS超级会员日红包',
                'redpacket_t_orderlimit'=>0,
            ),
            'plus_mon' =>array(//每月送
                'redpacket_t_title'=>'PLUS会员红包',
                'redpacket_t_price'=>0,//面额
                'redpacket_t_desc'=>'PLUS会员红包',
                'redpacket_t_orderlimit'=>0,
            ),
		...
		foreach ($config_type_row as $config_type)
		{
            ...
            if($config_type == 'plus'){
                //查询是否设置过会员红包
                $data_tmp = $Web_ConfigModel->getPlusConfigStr();
                if($data_tmp){
                    foreach ($data_tmp as $k=>$v){
                        in_array($v['config_key'],$deal_plus_key) && $plus_quanyi_set[$v['config_key']] = $v['config_value'];
                    }
                }
            }
            ...
            foreach ($config_value_row as $config_key => $config_row)
            {
                ...
                {
                    in_array($config_key,$deal_plus_key) && $plus_array[$config_key] = $config_row;//满足条件放入数组
                    $add_row = array();
			...
			//无论上边是新增还是修改，此处统一处理plus权益红包
            if($plus_array){
                if($plus_quanyi_set){//已设置过plus会员权益
                    $plusday_arr = array();
                    $plusday_arr = $init_plus_red_packet;
                    ($plus_quanyi_set['plus_general_red'] !=$plus_array['plus_general_red']) && $plusday_arr['plus_day']['redpacket_t_price'] = $plus_array['plus_general_red'];
                    ($plus_quanyi_set['plus_quota'] !=$plus_array['plus_quota']) &&  $plusday_arr['plus_mon']['redpacket_t_price'] = $plus_array['plus_quota'];
                    ($plus_quanyi_set['plus_red_packet'] !=$plus_array['plus_red_packet']) &&  $plusday_arr['plus_mon']['redpacket_t_orderlimit'] = $plus_array['plus_red_packet'];
                    foreach ($plusday_arr as $key=>$val){
                        //分别处理两种会员权益
                        switch ($key){
                            case "plus_day":
                                if($val['redpacket_t_price']){
                                    //新增
                                    $id = $this->createPlusRedPacket($val);
                                    $insert_row = array(
                                        'config_value' => $id?:0,
                                    );
                                    $Web_ConfigModel->editConfig($key.'_redpacket_tpl_id', $insert_row);
                                }
                                break;
                            case "plus_mon":
                                if($val['redpacket_t_price'] || $val['redpacket_t_orderlimit']){
                                    //新增
                                    $id = $this->createPlusRedPacket($val);
                                    $insert_row = array(
                                        'config_value' => $id?:0,
                    ...
                    //组织参数新增plus会员权益红包模板
                    $plusmon_arr =array();
                    $plusmon_arr = $init_plus_red_packet;
                    $plusmon_arr['plus_day']['redpacket_t_price'] = $plus_array['plus_general_red'];
                    $plusmon_arr['plus_mon']['redpacket_t_price'] = $plus_array['plus_quota'];
                    $plusmon_arr['plus_mon']['redpacket_t_orderlimit'] = $plus_array['plus_red_packet'];
                    foreach ($plusmon_arr as $key=>$item){
                        $id = $this->createPlusRedPacket($item);
                        $insert_row = array(
                            'config_key' => $key.'_redpacket_tpl_id',
                            'config_value' => $id?:0,
                            'config_type' => 'plus',
                        );
                        $Web_ConfigModel->addConfig($insert_row,true);
                    }
                }
            }
            ...
            $arr = $this->redPacketTempModel->getOneByWhere(array("redpacket_t_title"=>"PLUS会员红包"));

            $Web_ConfigModeld = new Web_ConfigModel();
            $data_tmpd = $Web_ConfigModeld->getOneByWhere(array("config_key"=>'plus_quota'));
            $data_packetd = $Web_ConfigModeld->getOneByWhere(array("config_key"=>'plus_red_packet'));
            $new_arrd['redpacket_t_price'] = $data_tmpd['config_value'];
            $new_arrd['redpacket_t_orderlimit'] =$data_packetd['config_value'];
            $this->redPacketTempModel = new RedPacket_TempModel();
            $flags = $this->redPacketTempModel->editRedPacketTemplate($arr['redpacket_t_id'], $new_arrd);
...
//$mail->AddAttachment("f:/test.png"); //可以添加附件
	...
	public function editAnalyticsApi()
	{
        ...
        $data['analytics_push_time'] = $analytics_api_row['analytics_push_time'];
    ...
    {
        $analytics_statu = Web_ConfigModel::value('analytics_statu');
        if($analytics_statu){
            $status = 200;
        }else{
            $status = 250;
     ...
     * 创建红包
     */
    public function createPlusRedPacket($parms=array()){
        $RedPacket_TempModel = new RedPacket_TempModel();
        //PLUS 会员权益红包，默认10年有效期
        $now_datetime = time();
        $fmt_start_date = date("Y-m-d H:i:s",$now_datetime);
        $fmt_end_date = date("Y-m-d H:i:s",strtotime("+10 year -1 day",$now_datetime));
        $field_row = array();
        $field_row['redpacket_t_title'] = $parms['redpacket_t_title'];//红包名称
        $field_row['redpacket_t_type'] = RedPacket_TempModel::COMMONRPT;//红包类型，普通红包;
        $field_row['redpacket_t_start_date'] = $fmt_start_date;//红包有效期起始时间
        $field_row['redpacket_t_end_date'] = $fmt_end_date;//有效期结束时间
        $field_row['redpacket_t_price'] = $parms['redpacket_t_price'];//红包面额
        $field_row['redpacket_t_orderlimit'] = $parms['redpacket_t_orderlimit'];//红包使用订单限额
        $field_row['redpacket_t_total'] = $parms['redpacket_t_total']?:100000; //默认发放总数100000张
        $field_row['redpacket_t_add_date'] = get_date_time();//发布时间
        $field_row['redpacket_t_update_date'] = get_date_time();//最后编辑时间
        $field_row['redpacket_t_eachlimit'] = $parms['redpacket_t_eachlimit']?:1; //每人限领张数
        $field_row['redpacket_t_user_grade_limit'] =$parms['redpacket_t_user_grade_limit']?:5; //用户领取等级限制
        $field_row['redpacket_t_img'] = ''; //优惠券图片
        $field_row['redpacket_t_access_method'] = RedPacket_TempModel::GETSYSTEMGIVE;//领取方式，系统发放
        $field_row['redpacket_t_recommend'] = RedPacket_TempModel::UNRECOMMEND;//是否推荐，不推荐
        $field_row['redpacket_t_desc'] = $parms['redpacket_t_desc']?:'PLUS会员权益红包';//红包描述
        return  $RedPacket_TempModel -> addRedPacketTemp($field_row, true);
    }

    /**
     * 我的二维码展示数据
     */
    public function mycode_set(){
        $data = array();
        $Web_ConfigModel = new Web_ConfigModel();
        //描述
        $data_tmp = $Web_ConfigModel->getByWhere(array('config_type' => 'mycodeDepict'));
        $data     = $data + $data_tmp;
        //背景图片
        $data_tmp = $Web_ConfigModel->getByWhere(array('config_type' => 'mycodeBgimg'));
        $data     = $data + $data_tmp;
        $this->data->addBody(-140, $data);
    }
