...
class Api_Goods_CatCtl extends Api_Controller
{
	...
	public function removeCat()
	{
		...
		if ($cat_id_row)
		{
            ...
            $goods_list_info = $Goods_CommonModel->getOneByWhere($cond_row);
			$shop_class_bind_rows = $shopClassBindModel->getOneByWhere(['product_class_id:IN'=> $cat_id]);
                ...
                if(!empty($parent_info)){
                    ...
                    if(!empty($goods_cat)){
                        foreach ($goods_cat as $key => $val) {
                            $goods_cat_nav_recommend_up_display = $val['goods_cat_nav_recommend_up_display'];
                            if (!empty($goods_cat_nav_recommend_display)) {
                                $goods_cat_nav_recommend_up_display = $this->array_recursion($goods_cat_nav_recommend_up_display, $cat_id);
	...
	public function addCat()
	{
		...
		if ($cat_id)
		{
            ...
            // if (Web_ConfigModel::value('Plugin_Directseller')) { //分销
            //     Fenxiao::getInstance()->updateCatCommission([
            //         'cat_id' => $cat_id,
            //         'c_first' => request_string('c_first'),
            //         'c_second' => request_string('c_second'),
            //         'a_first' => request_string('a_first'),
            //         'a_second' => request_string('a_second')
            //     ]);
            // }
	...
	public function editGoodsCat()
	{
        ...
        $Cache->remove($index_key);
 		//清除cat列表缓存
   		$cat_cache_key = md5(Yf_Registry::get('url').'Api_Goods_CatCtlcatkey');
    	$Cache->remove($cat_cache_key);
		$this->data->addBody(-140, $data, $msg, $status);
	}
