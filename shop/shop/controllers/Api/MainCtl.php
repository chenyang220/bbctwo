...
class Api_MainCtl extends Api_Controller
{
	...
	public function getMainInfo()
	{
        ...
        $main_info_key_time = md5(Yf_Registry::get('url').'getMainInfoKey'); //固定值,判断是否需要重新缓存数据
        $main_info_key_time_value = $Cache->get($main_info_key_time);
        $main_info_value = md5(Yf_Registry::get('url').'getMainInfoValue');  //缓存的key
        if($main_info_key_time_value != date('Y-m-d')){
            $data = array();
            ...
            //缓存
            $Cache->save(date('Y-m-d'), $main_info_key_time);
            $Cache->save(json_encode($data), $main_info_value);

        } else {
            $res = $Cache->get($main_info_value,$cache_group);
