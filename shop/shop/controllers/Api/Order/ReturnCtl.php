...
class Api_Order_ReturnCtl extends Api_Controller
{
    ...
    public function index()
    {
        ...
        if ($act == "detail") {
            ...
            $data = $this -> add();
            if(!is_array($data)){
              return   $this -> data -> addBody(-140, $data, __('退货/退款 失败'), 250);
            }
        } else {
            $Yf_Page = new Yf_Page();
            $Yf_Page -> listRows = 10;
            ...
            $cond_row['buyer_user_id'] = $user_id;         //店铺ID
            if ($start_time) {
       ...
       if (is_array($data)) {
                if (empty($data['goods_list'])) {
                    $data['goods_list'] = array_values($data['goods']);
                } else {
                    $data['goods_list'] = '';
                }
                if (isset($data['reason'])) {
                    $data['reason'] = array_values($data['reason']);
                } else {
                    $data['reason'] = '';
                }
                $this -> data -> addBody(-140, $data, __('成功'), 200);
            } else {
                 $data = array();
                $this -> data -> addBody(-140, $data, __('失败'), 250);
            }
    }
    public function detail()
    {
        ...
        if ($order_base['buyer_user_id'] == $user_id || ($order_base['order_sub_pay'] == Order_StateModel::SUB_USER_PAY && $order_base['order_sub_user'] == $user_id)) {
    ...
    public function addReturn()
    {
        ...
        $return_id = sprintf('%s-%s-%s-%s', 'TD', $user_id, 0, $return_number);
        ...
        if (empty($return) && ($return_cash > 0) && $flag2) {
            ...
            if ($order['buyer_user_id'] == $user_id && !strstr($order['payment_name'], '白条支付')) {
                $rs_row = array();
                ...
                if (!empty($dist_order)) {
                    foreach ($dist_order as $key => $value) {
                        ...
                        $key = $this -> addDistReturn($value['order_id'], $field['return_reason_id'], $field['return_message'], $goods_id, $add_flag,$user_id);
                        check_rs($key, $rs_row);
                    }
                }
    ...
    public function addDistReturn($order_id, $return_reason_id, $return_message, $goods_parent_id, $order_return_id,$user_id)
    {
        ...
        $prefix = sprintf('%s-%s-', Yf_Registry::get('shop_app_id'), date('YmdHis'));
        $return_number = $Number_SeqModel -> createSeq($prefix);
        $return_id = sprintf('%s-%s-%s-%s', 'SPTD', $user_id, 0, $return_number);
