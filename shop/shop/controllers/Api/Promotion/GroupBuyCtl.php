<?php if (!defined('ROOT_PATH')) {
    ...
    class Api_Promotion_GroupBuyCtl extends Api_Controller
    {
        ...
        {
            $data = array();
            $groupbuy_allow = Web_ConfigModel::value("groupbuy_allow");
            if ($groupbuy_allow == 1) {
        ...
        //wap活动版块设置
        public function getTPLGroupBuyGoodsList()
        {
            ...
            if ($groupbuy_allow == 1) {
                ...
                $data = $GroupBuy_BaseModel->getGroupBuyGoodsList($cond_row, array('groupbuy_id' => 'DESC'), $page, $rows, 0, true);
                $data['goods_base'] = $goods_base;
                if ($data['items']) {
                    $msg = 'success';
                    $status = 200;
                } else {
                    $msg = '请在后台促销设置中打开团购功能并发布团购商品';
                    $status = 250;
                }
            } else {
                $msg = '请在后台促销设置中打开团购功能并发布团购商品';
