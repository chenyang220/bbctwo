...
class Api_Promotion_PointsCtl extends Api_Controller
{
	...
	public function cancelPointsOrder()
	{
		...
		if ($points_order_id)
		{
			...
			$flag = $this->Points_OrderModel->editPointsOrder($points_order_id, $field_row);
			...
			if($flag)
			{
				...
				$user_points['user_points'] = $data['points_allpoints'];
				$res_flag = $User_ResourceModel->editResource($data['points_buyerid'],$user_points,true);

				//添加积分明细
				$Points_LogModel = new Points_LogModel();
				$points_row['points_log_type'] = 1;//获取积分
				$points_row['class_id'] = Points_LogModel::RETURNBACK;
				$points_row['user_id'] = $data['points_buyerid'];
				$points_row['user_name'] = $data['points_buyername'];
				$points_row['points_log_points'] = $data['points_allpoints'];
				$points_row['points_log_time'] = date('Y-m-d H:i:s',time());
				$points_row['points_log_desc'] = '取消订单，退还积分';
				$flag = $Points_LogModel->addLog($points_row);
			}
		}
		$this->data->addBody(-140, $data);
	}
