...
//        $field_row['redpacket_t_eachlimit']     = request_int('redpacket_t_eachlimit');          //每人限领张数
        $field_row['redpacket_t_eachlimit']     = 1;          //每人限领1张
   ...
   /* 删除优惠券模板*/
    public function removeRedPacketTemp()
    {
        ...
        if ($flag)
        {
            $msg    = __('success');
            $status = 200;
            //拦截点，处理会员权益模板id @nsy 2019-01-25
            $this->dealPlusRedpaketTplID($redpacket_t_id);
        }
        else
        {
            $msg    = __('failure');
     ...
     * @nsy 2019-01-25
     */
    public function dealPlusRedpaketTplID($id){
        $Web_ConfigModel = new Web_ConfigModel();
        $data_tmp = $Web_ConfigModel->getPlusConfigStr();
        !$data_tmp && $data_tmp=array();
        $deal_array = array();
        foreach ($data_tmp as $k=>$v){
            if('plus_day_redpacket_tpl_id'== $v['config_key'] && $v['config_value'] == $id ){
                $deal_array[]=array(
                    'plus_day_redpacket_tpl_id'=>0,
                    'plus_general_red'=>''
                );
            }
            if('plus_mon_redpacket_tpl_id'== $v['config_key'] && $v['config_value'] == $id ){
                $deal_array[]=array(
                    'plus_mon_redpacket_tpl_id'=>0,
                    'plus_quota'=>'',
                    'plus_red_packet'=>''
                );
            }
        }
        foreach ($deal_array as $k=>$v){
            foreach ($v as $kk=>$item){
                $insert_row = array(
                    'config_value' => $item,
                );
                $Web_ConfigModel->editConfig($kk, $insert_row);
            }
        }
        return true;
    }
