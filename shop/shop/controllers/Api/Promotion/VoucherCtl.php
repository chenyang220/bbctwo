...
class Api_Promotion_VoucherCtl extends Api_Controller
{
	...
	{
		$data = array();
        $voucher_allow = Web_ConfigModel::value("voucher_allow");
        if ($voucher_allow == 1) {
			...
			{
				$cond_row['shop_name:LIKE'] = '%' .$voucher_t_shop_name . '%';
			}

			$data = $this->Voucher_TempModel->getVoucherTempList($cond_row, $order_row, $page, $rows);
			$data['voucher_t_shop_name'] = $voucher_t_shop_name;
			if($data) {
                $msg = 'success';
                $status = 200;
            } else {
    ...
    {
        $voucher_status = Web_ConfigModel::value('voucher_allow');
        if($voucher_status){
            $status = 200;
        }else{
            $status = 250;
