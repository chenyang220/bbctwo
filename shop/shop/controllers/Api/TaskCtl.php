...
class Api_TaskCtl extends Api_Controller
{
    ...
    public function transport(){ 
        ...
        if(SHOP_VERSION >= '3.1.3'){
            return $this->data->addBody(-140, array(), __('3.1.3以上版本不需要初始化'), 250);
        }
        //检查是否可以初始化
        $area_model = new Transport_AreaModel();
        $checkInstall= $area_model->getCount(array());
        if($checkInstall !== '0'){
            return $this->data->addBody(-140, array(), __('初始化失败，目标数据表不为空'), 250);
        }
        ...
        return $this->data->addBody(-140, array(), __('初始化完成，'.$s_record.'条成功记录，'.$f_record.'条失败记录'), $status);
