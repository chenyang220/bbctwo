<?php if (!defined('ROOT_PATH')) {
    ...
    class Api_Trade_ReturnCtl extends Api_Controller
    {
        ...
        public function detail()
        {
            ...
            $id = request_int('id');
            if(!$id) {
                return $this -> data -> addBody(-140, array(), __('缺少id'), 250);
            }
            ...
            if ($return['return_state'] < Order_ReturnModel::RETURN_PLAT_PASS) {
                ...
                if ($return['return_shop_handle'] == Order_ReturnModel::RETURN_SELLER_UNPASS) {
                    ...
                    if ($this->yunshanstatus == 1) {
                        ...
                        $order_base = $this->Order_BaseModel->getOne($order_id);
                        $returnfalg = "1" ; // 默认不成功
        ...
        public function getReturnInfo(){
            ...
            $return_order_id = request_int('return_order_id');

            $order_ReturnModel = new Order_ReturnModel();
            $data = $order_ReturnModel->getByWhere(array('order_number'=>$order_id,'order_return_id'=> $return_order_id));
            if(empty($data)){
