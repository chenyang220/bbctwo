 ...
 */
class App_Seller_GoodsCtl extends Seller_Controller
{
...
//            $this ->goodsCommonModel->getShopContract($common_data);
        ...
        /* 只有可发布虚拟商品才会显示 S */
        $is_gv = request_string('is_gv');
        $common_data['common_is_virtual'] = $is_gv;  //虚拟商品
        if ($is_gv == 1) {
            $common_data['common_virtual_date'] = request_string('common_virtual_date');    //虚拟商品有效期
            $common_data['common_virtual_refund'] = request_string('common_virtual_refund');  //支持过期退款
        }
        ...
        if ($action == 'edit') {
            ...
            $common_id = $this->goodsCommonModel->addCommon($common_data, true);
              ...
              $ammon_arr['common_id'] = $common_id;
              $ammon_arr['images_image'] = request_string('imagePath'); 
              $ammon_arr['images_is_default'] = 1; 
              $ammon_arr['shop_id'] = $shop_base['shop_id'];                        //店铺id
              $flag = $Goods_ImagesModel->addImages($ammon_arr,true);
...
//            $spec_cond['spec_readonly'] = 1;
    ...
    public function saveGoodsImage()
    {
        ...
        $user_id = request_string('user_id');
        $image_list[] = request_string('image1');
        $image_list[] = request_string('image2');
        $image_list[] = request_string('image3');
        $image_list[] = request_string('image4');
        $common_id = request_string('common_id');
        $is_color = request_string('is_color');
         
         //第一次添加附图

         $goodsImagesModel = new Goods_ImagesModel();
         $common_idt = $goodsImagesModel->getByWhere(array('common_id' => $common_id));
         $common_idt = array_values($common_idt);


         if(count($common_idt) <5)
         {
            $num = 0;
            foreach ($image_list as $key => $value) {
                $num++;
                $image_arr['common_id'] = $common_id;
                $image_arr['images_image'] = $value;
                $image_arr['images_is_default'] = 0;
                $flag = $goodsImagesModel->addImages($image_arr, true); 
            }   
         }
        ...
        if (!empty($image_list)) {
            ...
            $goodsImagesModel = new Goods_ImagesModel();
            $Shop_BaseModel = new Shop_BaseModel();
            $resule = $Shop_BaseModel->getOneByWhere(array("user_id"=>$user_id));
            $images = $goodsImagesModel->getByWhere(array('common_id' => $common_id));
            $imagest = array_values($images);
            foreach($imagest as $k=>$v)
            {
                //副图
                if($v['images_is_default'] == 0)
                {
                    $new_rowds[]= $v['images_id'];
                    foreach($new_rowds as $ks=>$sv)
                    {
                    $flag = $goodsImagesModel->editImages($v['images_id'],array('images_image' => $image_list[$ks]));
                    }
                }else
                {
                    //主图
                    $new_rowzd[]= $v['images_id'];
                    foreach($new_rowzd as $ks=>$sv)
                    {
                    $flag = $goodsImagesModel->editImages($v['images_id'],array('images_image' => $image[$ks]));
                    }
...
//           if($image)
//           {
//               $Goods_CommonModel->editCommon($common_id, array('common_image' => $image));
//           }
