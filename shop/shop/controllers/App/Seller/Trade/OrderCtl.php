 ...
 * @author     windfnn
 */
class App_Seller_Trade_OrderCtl extends Controller
{
    /**
     * Constructor
     *
    ...
    public function orderList()
    {
        ...
        $buyer_user_id = request_string('user_id');
...
//            return $this->data->addBody(-140, [$condition], "11", 250);
            ...
            if ($order_id)//如果$search_id存在，说明是按照订单号搜索
            {
               ...
               // $data = $Order_BaseModel->getBaseList(['buyer_user_name:IN' => $buyer_user_name, 'shop_id' => $shop_info['shop_id']], $order_row, $page, $rows);
                $condition['shop_id'] =$shop_info['shop_id']; 
                $condition['order_is_virtual'] =$order_is_virtual; 
                $data = $Order_BaseModel->getBaseList($condition, $order_row, $page, $rows);

            //  $data = $Order_BaseModel->getBaseList(['shop_id' => $shop_info['shop_id'],'order_is_virtual'=>$order_is_virtual,'chain_id:>'=>$chain_id], $order_row, $page, $rows);
            } else {
                $data = $Order_BaseModel->getBaseList($condition, $order_row, $page, $rows);
            }
        }
        foreach ($data['items'] as $key => $val) {
            ...
            if ($type == 4) {
                ...
                $data['items'][$key]['goods_list'] =array_values( $Order_GoodsModel->getByWhere(['order_goods_id' => $val['order_goods_id']]));
            } else {
...
//            $check_send = $this->checkSend($order_base['seller_user_id'],$order_base['shop_id']);
    ...
    public function addAddress()
    {
        ...
        $field_row['shipping_address_contact'] = request_string('seller_name');    //联系人
        $field_row['shipping_address_phone'] = request_string('seller_phone');        //联系方式
        $field_row['shipping_address_address'] = request_string('seller_detail');            //详细地址
        $field_row['shipping_address_province_id'] = request_string('province_id');                //省份ID
        $field_row['shipping_address_city_id'] = request_string('city_id');                        //城市ID
        $field_row['shipping_address_area_id'] = request_string('area_id');                        //地区ID
        $field_row['shipping_address_area'] = request_string('seller_address');    //地址信息
        ...
        $field_row['shipping_address_default'] = request_string('seller_default');          //是否设为默认地址
        ...
        if ($shop_base_info && $shop_base_info['shop_status'] == 3) {
            ...
            if (request_string('seller_default') == 1) {
    ...
    public function editCost()
    {
        ...
        $order_id = request_string('order_id');
        $product_row = request_string('product_id');
        $product_price_row = request_string('product_price');
        $shipping = request_string('shipping');
        $product_id = explode(',', $product_row);
        $product_price = explode(',', $product_price_row);
        $product_row = array_combine($product_id,$product_price);
