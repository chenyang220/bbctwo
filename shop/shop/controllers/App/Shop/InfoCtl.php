 ...
 * @author
 */
class App_Shop_InfoCtl extends Controller
{
//
    ...
    public function getShopInfoByUserId()
    {
        ...
        if ($shop_info) {
            ...
            if($shop_info['wap_shop_logo']){
                $data['shop_logo'] = $shop_info['wap_shop_logo'];
            } else {
                $wap_shop_logo = Web_ConfigModel::value('photo_shop_head_logo_wap');
                $data['shop_logo'] = $wap_shop_logo;
            }
        ...
        $this->data->addBody(-140, $data, $wap_shop_logo, $status);
    }
    ...
    public function getUserShopCompany()
    {
        ...
        $seller_rows = $Seller_BaseModel->getByWhere(['user_id' => $user_id]);
        $data = [];
        if ($seller_rows) {
    ...
    public function editShopCompany()
    {
        ...
        $flag1 = true;
        if (!$shop_company['business_id']) {
            $flag1 = $Shop_BaseModel->editBase($shop_id, ['shop_status' => 9]);
        }
        $rs_rows = [];
    ...
    public function addShopCompany()
    {
        ...
        if ($shop_company_data['status'] == 250) {
            return $this->data->addBody(-140, [], $shop_company_data['msg'], 250);
        }
        ...
        $check_district = $Base_DistrictModel->getDistrictList(['district_parent_id' => $district_id]);
        //开启事物
    ...
    public function getShopRequest($apply)
    {
        ...
        if (!preg_match('/^1[3-9]\d{9}$/', $contacts_phone)) {
            $data['msg'] = '手机号码有误';
            $data['status'] = 250;
    ...
    public function getSellledInfo()
    {
        ...
        $shop_company = $this->shopInfoModel->getbaseCompanyList($shop_info['shop_id']);
        $data = [];
        $data['shop_class'] = empty($shop_class) ? []:array_values($shop_class);
        $data['shop_grade'] = empty($shop_grade) ? []:array_values($shop_grade);
        $data['shop_info'] = empty($shop_info) ? []:$shop_info;
        $data['shop_company'] = empty($shop_company) ? []:$shop_company;
        $this->data->addBody(-140, $data);
    }
    ...
    public function editShopBase()
    {
        ...
        $shopClassBind = $this->shopClassBindModel->listByWhere(['shop_id' => $shop_id]);
    ...
    public function shopPaystatus()
    {
        ...
        $shop_domain = $Web_ConfigModel->getByWhere(['config_type' => 'domain']);
        $domain = [];
        ...
        //添加卖家
        $check_seller = $SellerBaseModel->getByWhere(['shop_id' => $shop_id]);
        if (!$check_seller) {
    ...
    public function shopXiYi()
    {
        ...
        $shop_xieyi = $ShopHelpModel->getByWhere(['page_show' => 1], ['help_sort' => "asc"]);
