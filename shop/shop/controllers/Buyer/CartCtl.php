...
class Buyer_CartCtl extends Controller
{
            ...
            if (!empty($cart_list)) {
                foreach ($cart_list as $key => $val) {
                    foreach ($val['goods'] as $k => $v) {
                        ...
                        $plusPrice += $v['goods_num'] * $v['plus_price'];
                        $number += $v['goods_num'];
    ...
    public function newconfirm()
    {
        ...
        $goods_rows = array_column($cart_list,'goods_id', 'cart_id');
        ...
        if ($cart_list) {
            foreach ($cart_list as $k1 => $v1) {
                ...
                $Goods_Base = $Goods_BaseModel->getOneByWhere(array('goods_id'=>$v1['goods_id']));
                $Goods_Common = $Goods_CommonModel->getOneByWhere(array("common_id"=>$Goods_Base['common_id']));
            }
        }
        ...
        //礼包商品只能单独购买
        $result3 = $Goods_BaseModel->getGiftInfo($cart_list);
        if ($result3['state']) {
            return $this->data->addBody(-140, array(), $result3['msg'], 250);
        }
...
file_put_contents(dirname(__FILE__).DIRECTORY_SEPARATOR.'abs.php',print_r($data['glist'] ,true),FILE_APPEND);
        ...
        if ($address_id) {
           ...
           if($address_row['user_address_area_id']){
               $city_id = $address_row['user_id'] != $user_id ? 0 : $address_row['user_address_area_id'];
            }else{
            ...
            if($address_row['user_address_area_id']){
                $city_id = $address_row['user_address_area_id'] ? $address_row['user_address_area_id'] : 0;
            }else{
        ...
        foreach($data['glist'] as $key=>$value)
        {
            if($value['goods'])
            {
                foreach($value['goods'] as $k=>$v)
                {
                    ...
                    if($v['goods_base']['promotion_type']=='presale'){
                        $data['promotion_type'] = 'presale'; 
                    }
                }
            }
        }
        ...
        $data['cost'] = !$transport_cost ? array() : array_values($transport_cost);
        ...
        foreach ($data_t as $key => $value) {
            ...
            unset($data['glist'][$key]['goods']);                      
            foreach ($value['goods'] as $tkey => $gvalue) {
                $transport_template_id_arr[$gvalue['common_base']['transport_template_id']]['goods_info'][$gvalue['goods_base']['goods_id']] = $gvalue;      
            }          
            $data['glist'][$key]['transport_template_id'] = $transport_template_id_arr;
            
        }
        $data['is_discounts'] = Web_ConfigModel::value('cash_on_delivery_status');//小程序 货到付款

        foreach ($data['glist'] as $key => $value) {
            // $sprice = 0;
            // $cost_sum = array_sum(array_column($data['cost'][$key]['goods'], 'cost','cost'));
            // $con_sum = array_sum(array_column($data['cost'][$key]['goods'], 'con','con'));
            // if ($con_sum) {
            //     $sprice = $con_sum; 
            // } else {
            //     $sprice = $cost_sum; 
            // }
            foreach ($value['transport_template_id'] as $transport_template_id => $t_value) {
                $sumprice_sum = array_sum(array_column($t_value['goods_info'], 'sumprice','goods_id'));
                $sprice = $sprice + $sumprice_sum;
                $data['glist'][$key]['transport_template_id'][$transport_template_id]['goods_money_total'] = $sumprice_sum;   
            }
            // $data['glist'][$key]['sprice'] = $sprice;
        }

        //单独购买礼包商品时判断是否使用活动折扣
        $isUseDiscount = $this->getUseDiscount($data, request_row('product_id'));
        $data['isUseDiscount'] = $isUseDiscount['flag'];
        $data['is_mansong'] = $isUseDiscount['is_mansong'];
        $data['is_gift'] = $isUseDiscount['is_gift']? $isUseDiscount['is_gift']:0;
    ...
    public function getUseDiscount($data, $cart_id)
    {
        $row = array();
        $flag = false;
        $is_mansong = false;
        //单独购买
        if (!is_array($cart_id) || count($cart_id) == 1) {
            //购物车商品信息
            $CartModel = new CartModel();
            $cart_id = is_array($cart_id) ? implode('', $cart_id) : $cart_id;
            $cart_info = $CartModel->getOne($cart_id);
            $Goods_BaseModel = new Goods_BaseModel();
            $goods_info = $Goods_BaseModel->getOne($cart_info['goods_id']);

            //判断礼包商品
            if ($cart_info['goods_num'] == 1 && $goods_info['cat_id'] == 9002) {
                //订单商品信息
                $order_goods = current($data['glist']);
                if ($data['is_discount'] == 1) $flag1 = true; //使用会员折扣
                if ($data['rpt_info']) $flag2 = true; //使用平台红包
                if ($order_goods['best_voucher']) $flag3 = true; //使用代金券
                if ($order_goods['mansong_info']) {
                    $flag4 = true; //使用满送
                    $is_mansong = true;
                }
    ...
    public function confirmVirtual()
    {
        ...
        //单独处理rpt信息 @nsy 2019-02-13
        if($data['goods_base']['is_plus']){
            $data['rpt_list'] = array();
            $data['rpt_info'] = array();
        }
        $data['is_plus'] = $data['goods_base']['is_plus'];//plus会员，plus商品标识
...
//        $goods_base = $Goods_BaseModel->getOne($goods_id);
    ...
    public function buyVirtual()
    {
        ...
        //处理plus商品价格 @nsy 2019-02-13
        $data['is_plus'] = 0;
        $openFlag = Web_ConfigModel::value('plus_switch');
        $plus =  Perm::$plus;
        if($openFlag && $plus && $plus['user_status']!=3 && $plus['end_date']>time()){
                $Plus_GoodsModel =  new Plus_GoodsModel();
                //plus价格
                $result= $Plus_GoodsModel->reformPlusGoods(array(array('common_id'=>$data['goods_base']['common_id'],'common_price'=>$data['goods_base']['goods_price'])));
                $isPlus = $result[0]['plus_status']?:0;
                $plus_price = $result[0]['plus_price']?:0;
                if($isPlus && $plus_price){
                    $data['is_plus'] = 1;
                    $data['goods_base']['now_price']  = $plus_price;
                    $data['goods_base']['sumprice'] = number_format($plus_price * $goods_num, 2, '.', '');
                }
        }
    ...
    public function addCart()
    {
        ...
        // if($order_base['order_status']>=1&&$order_base['order_status']<=6 ){
        //     return $this->data->setError("秒杀商品每人只能购买1件！");
        // }
        ...
        if (Perm::$shopId && $goods_shop_base['shop_type'] == 2) {
            ...
            $shopDistributorBase = $shopDistributorModel->getOneByWhere(array('distributor_id' => Perm::$shopId, 'shop_id' => $common_base['shop_id']),['shop_distributor_time'=>'desc']);
...
//        $this->data->addBody(-140, $data);
    ...
    public function confirmChain()
    {
        ...
        if ($this->typ == 'json') {
            $data['rate_service_status'] = Web_ConfigModel::value('rate_service_status');
            if(Web_ConfigModel::value('Plugin_Delivery') ==1){
                $data['delivery_price'] = Web_ConfigModel::value('delivery');
            }else{
    ...
    public function confirmGoods()
    {
        ...
        //货到付款
        $goods_info['cash_on_delivery_status'] = Web_ConfigModel::value('cash_on_delivery_status');
        if ($this->typ == 'json') {
    ...
    private function computeFreight(&$data)
    {
        foreach ($data['glist'] as $k => $shop_data) {
            ...
            $freight = array_sum(array_column($data['cost'][$k]['goods'], 'cost', 'cost'));
            $data['glist'][$k]['freight'] = $freight;
            $data['glist'][$k]['sgprice'] = $data['glist'][$k]['sprice'];
            if($shop_data['goods'][0]['goods_base']['promotion_type']=='presale'){
                //如果是预售商品，不加运费，运费放在尾款支付
                $data['order_price'] += 0;
            }else{
    ...
    public function getUserRedpacket()
    {
        ...
        $shop_plus_goods_flag =  request_string('shop_plus_goods_flag');
        $data = array();
        if($shop_plus_goods_flag){
            $data['rpt_list'] = array();
            $data['rpt_info'] = array();
