...
class Buyer_MessageCtl extends Buyer_Controller
{
    ...
    public function getMessageCount()
    {
        ...
        $data = $MessageModel->getByWhere($cond_row);
        if(!empty($data)){
            $msg = 'success';
            $status = 200;
        }else{
    ...
    public function editMessage()
    {
        ...
        if ($data) {
            $message_id = array_column($data, 'message_id');
            $flag = $MessageModel->editMessage($message_id, array('message_islook' => 1));
        } else {
            $flag = true;
        }
        if ($flag !== false) {
        ...
        $this->data->addBody(-140, array(), 'success', $status);
    }
}
