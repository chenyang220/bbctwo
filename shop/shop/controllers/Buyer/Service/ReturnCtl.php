<?php if (!defined('ROOT_PATH')) {
    ...
    class Buyer_Service_ReturnCtl extends Buyer_Controller
    {
        ...
        public function index()
        {
            ...
            if ($act == "detail") {
                ...
                if($_COOKIE['SHOP_ID']){
                    ...
                    $shop_base = $Shop_BaseModel->getOne($_COOKIE['SHOP_ID']);
                    $cond_row['seller_user_id'] = $shop_base['user_id'];
                }
                if(request_int('shop_id_wap')){
                    $Shop_BaseModel = new Shop_BaseModel();
                    $shop_base = $Shop_BaseModel->getOne(request_int('shop_id_wap'));
                    $cond_row['seller_user_id'] = $shop_base['user_id'];
                }
        ...
        public function addReturn()
        {
            ...
            $field['district_id'] = $order['district_id'];     //该笔订单区分分站
            //如果传递过来的订单号和根据商品id查到的订单号不符，报错
            if ($goods['order_id'] != $order_id) {
                $flag2 = false;
            }
