...
class Chain_GoodsCtl extends Chain_Controller
{
    ...
    public function goods()
    {
        if (Perm::checkUserPerm())
        {
            ...
            if($chain_id){
                ...
                $Chain_Base = $this->Chain_BaseModel->getOneByWhere(array('chain_id'=>$chain_id));
                $recommend_goods_arr = array();
                if ($Chain_Base['recommend_goods']) {
                    $recommend_goods_arr = explode(",",$Chain_Base['recommend_goods']);
                }
                $i=0;
                foreach($goods_common['items'] as $value){
                    foreach($goods_base as $v){
                        if($value['common_id']==$v['common_id']){
                            ...
                            $goods[$i]['recommend_goods']= in_array($v['goods_id'], $recommend_goods_arr) ? 1:2 ;
     ...
     * @access public
     */
    public function setRecommendGoods()
    {
        ...
        if ($Chain_Base['recommend_goods']) {
            ...
            if (count($recommend_goods_arr)>6 && $type == 2) {
                throw new Exception('温馨提示：最多推荐6个商品！');
            }
            if (in_array($goods_id, $recommend_goods_arr)) {
                foreach ($recommend_goods_arr as $key => $gid) {
                    if ($gid == $goods_id) {
                        unset($recommend_goods_arr[$key]);
                    }
                }
                $recommend_goods = implode(",",$recommend_goods_arr);
            } else {
  
                $recommend_goods = $Chain_Base['recommend_goods']  . $goods_id . ',';

            }
            $editChainBase = $this->Chain_BaseModel->editBase($chain_id,array('recommend_goods'=>$recommend_goods));
        } else {
            $recommend_goods = $goods_id . ',';
            $editChainBase = $this->Chain_BaseModel->editBase($chain_id,array('recommend_goods'=>$recommend_goods));
        }
        if ($editChainBase == true)
        {      
            if ($type == 2) {
                $msg = __('推荐成功');
            } else {
        ...
        $this->data->addBody(-140, array('goods_id'=>$goods_id), $msg, $status);
    }
