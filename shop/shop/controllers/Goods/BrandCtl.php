...
//		$this->description       = str_replace("{name}", "心悦品牌", $this->description);
        ...
        //推荐品牌
        $cond = array();
        $cond['brand_recommend'] = 1;
        $cond['brand_enable'] = 1;
        $recomend_list = $Goods_BrandModel->getByWhere($cond);

        //品牌列表
        $brand_list = $Goods_BrandModel->getAllInfo();

        $this->view->setMet('brandIndex');
		include $this->view->getView();
	}
	...
	public function getBrandListInfo()
    {
        //品牌列表
        $Goods_BrandModel = new Goods_BrandModel();
        $brand_initial = request_string('brand_initial');
        $brand_name = request_string('brand_name');
        $data = $Goods_BrandModel->getInitialInfo($brand_initial, $brand_name);
        if($data){
            $status = 200;
            $msg = 'success';
        }else{
	...
	function getFirstCharter($str)
	{
        ...
        if ($status) {
            ...
            $pinyin = new PinYin();
            //取首字母
            $data = $pinyin->pinyin($str, 'first');
            $data = ucfirst(substr($data, 0, 1));
        }
