...
class GroupBuyCtl extends Controller
{
	...
	public function index()
	{
		...
		$groupbuy_id = request_string('groupbuy_id');
		...
		if($groupbuy_id) {
            $groupbuy_id = explode(',',$groupbuy_id);
			$cond_d['groupbuy_id:IN'] = $groupbuy_id;
			$cond_row_phy['groupbuy_id:NOT IN'] = $groupbuy_id;

			$top = $this->groupBuyBaseModel->getGroupBuyGoodsList($cond_d);
			if($top) {
				$top = $top['items'];
			}
		}

		$recommend = $this->groupBuyBaseModel->getGroupBuyGoodsList($cond_row_phy, $order_row, 1, 10);
		$recommend['items'] = array_merge($top,$recommend['items']);

		$data['goods']['physical']['recommend'] = $recommend;
		...
		$data['goods']['virtual']['recommend']  = $this->groupBuyBaseModel->getGroupBuyGoodsList($cond_row_vir, $order_row, 1, 10);
		...
		else
		{
            ...
            if (image_thumb(Web_ConfigModel::value('slider1_image'.$sub_suffix),1043,396) || Web_ConfigModel::value('live_link1'.$sub_suffix)) {
            ...
            if (image_thumb(Web_ConfigModel::value('slider2_image'.$sub_suffix),1043,396) || Web_ConfigModel::value('live_link2'.$sub_suffix)) {
            ...
            if (image_thumb(Web_ConfigModel::value('slider3_image'.$sub_suffix),1043,396) || Web_ConfigModel::value('live_link3'.$sub_suffix)) {
	            $data['banner']['slider3']['slider_image'] = image_thumb(Web_ConfigModel::value('slider4_image'.$sub_suffix),1043,396);
	            $data['banner']['slider3']['live_link'] = Web_ConfigModel::value('live_link4'.$sub_suffix);
            }
            if (image_thumb(Web_ConfigModel::value('slider4_image'.$sub_suffix),1043,396) || Web_ConfigModel::value('live_link4'.$sub_suffix)) {
    ...
    public function groupBuyViews()
    {
        $id = request_int('gid');
        $rs = array();
        if($id){
            $sql = 'update yf_groupbuy_base set groupbuy_views = groupbuy_views +1 where goods_id = '."$id".' and groupbuy_state = 2';
            $db = new YFSQL;
            $rs = $db->find($sql);
        }
        return array($rs);
...
//        exit; //该页面在v3.1.3以后已经关闭
		...
		if ($data['groupbuy_detail'] && $goods_detail)
		{
			...
			$data['hot_groupbuy'] = $this->groupBuyBaseModel->getGroupBuyGoodsList($cond_row_hot, array('groupbuy_buy_quantity'=>'DESC'), 1, 5);;
