...
class Plugin_Analytics implements Yf_Plugin_Interface
{
	function __construct()
	{
        ...
        //注册更新
        Yf_Plugin_Manager::getInstance()->register('analyticsMemberUpdate', $this, 'memberUpdate');
        //入驻商家
        ...
        //计划任务批量更新订单
        Yf_Plugin_Manager::getInstance()->register('analyticsUpdateOrdersStatus', $this, 'updateOrdersStatus');
        //添加商品
     ...
     * @return boolean
     */
    public function memberUpdate($formvars = array())
    {
        if (!$formvars) {
            return false;
        }
        $formvars['app_id'] = Yf_Registry::get('analytics_app_id');
        $init_rs = $this->_sendData(Yf_Registry::get('analytics_api_key'), sprintf('%s?ctl=Api_Shop_Statistics&met=memberUpdate&typ=json', Yf_Registry::get('analytics_api_url')), $formvars);
        return $init_rs;
    }
...
//        echo '<pre>';print_r($formvars);exit;
     ...
     *  计划任务批量更新订单状态
     */
    public function updateOrdersStatus($formvars = array())
    {
        if (!$formvars) {
            return false;
        }
        $formvars['app_id'] = Yf_Registry::get('analytics_app_id');
        $init_rs = $this->_sendData(Yf_Registry::get('analytics_api_key'), sprintf('%s?ctl=Api_Shop_Statistics&met=updateOrdersStatus&typ=json', Yf_Registry::get('analytics_api_url')), $formvars);
        return $init_rs;
    }
