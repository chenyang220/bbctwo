...
class PointsCtl extends Controller
{
	...
	public function index()
	{
		if (Perm::checkUserPerm())
		{
			...
			$pointsornum = $this->pointsOrderModel->getPointsOrderListByWhere(array('points_buyerid' => Perm::$userId,'points_orderstate:<'=>4));   //已兑换礼品数量
			foreach ($pointsornum['items'] as $key => $val) {
				foreach ($val['points_ordergoods_list'] as $k => $v) {
					$odernum[] = $v['points_goodsnum'];
				}
			}
			$data['points_order_num'] = array_sum($odernum);//已兑换礼品数量
			if (!$data['points_order_num']) {
				$data['points_order_num'] = 0;
			}
		...
		if (Perm::checkUserPerm())    //是否登录
		{
			...
			if ($row)
			{
				...
				{						
					...
					//积分兑换商品是否下架
					$points_goods_info = $Points_GoodsModel->getPointsGoodsDetailByWhere(array('points_goods_id'=>$value['points_goods_id']));
					$row[$key]['points_goods_shelves'] =(int)$points_goods_info['points_goods_shelves'];
					if ((int)$points_goods_info['points_goods_shelves'] == Points_goodsModel::OFFSHELVES){	
						$total_points=$total_points-$value['total_points'];
					}
				}
				$data['items']        = $row;
					...
					if ($points_cart_row)  //修改购物车中积分商品数量
					{
						...
						if ($stock > $points_goods_row['points_goods_limitnum'])
						{
							if(($points_cart_row['points_goods_choosenum'] + $quantity < $stock && $points_cart_row['points_goods_choosenum'] + $quantity > $points_goods_row['points_goods_limitnum']) || ($points_cart_row['points_goods_choosenum'] + $quantity > $stock && $points_cart_row['points_goods_choosenum'] + $quantity > $points_goods_row['points_goods_limitnum']))
							{
								...
								{
									$field_row_edit['points_goods_choosenum'] = $quantity;//$points_goods_row['points_goods_limitnum'];
								}
							}else{
                                $field_row_edit['points_goods_choosenum'] =$quantity;
                            }
...
//					}
						...
						$flag = $this->pointsCartModel->editPointsCart($points_cart_row['points_cart_id'], $field_row_edit,true);
					}
					else
					{
						//如果库存大于限购数量
	...
	{
        return $this->checkLimitNumlogic();
	}
     ...
     * @return string
     */
    public function checkLimitNumlogic(){
     ...
     * @return string
     */
    public function appCheckLimitNum(){
        if(!Perm::checkUserPerm()){
            if('json' == $this->typ)
            {
	...
	public function appAddPointsOrder()
	{
		...
		if (Perm::checkUserPerm())
		{
			...
			if ($total_points <= $user_resource['user_points'])
			{
                ...
                $field_deliver_row['area_code'] = request_string('area_code');//国际区号
