<?php if (!defined('ROOT_PATH')) {
    ...
    class RedPacketCtl extends Controller
    {
        ...
        public function redPacket()
        {
            ...
            $cond_row['redpacket_t_start_date:<='] = get_date_time();
            $cond_row['redpacket_t_end_date:>='] = get_date_time();
            $cond_row['redpacket_t_state'] = RedPacket_TempModel::VALID;
            //过滤plus会员权益红包 @nsy 2019-02-15
            $cond_row['redpacket_t_access_method:!='] = RedPacket_TempModel::GETSYSTEMGIVE;
            ...
            if (Perm::checkUserPerm()) {
                ...
                $pointsornum = $this->pointsOrderModel->getPointsOrderListByWhere(array('points_buyerid' => Perm::$userId));   //已兑换礼品数量
...
//                                        $msg = __('你已经达到领取数量限制');
        ...
        //首页是否显示领取红包图片
        public function redpacketPic()
        {
            //判断首页图片是否显示
            /*1.用户未登陆的情况下只弹一次*/
            $redstatus = 0;
            if(Web_ConfigModel::value('redpacketset'))
            {
                if(!Perm::checkUserPerm())  //用户未登陆情况下只弹框一次
                {
                    if(!isset($_COOKIE['redpacketset_yf']))
                    {
                        $redstatus = 1;
                    }
                }
                ...
                {
                    if(!isset($_COOKIE['redpacketset_yf'.Perm::$userId])) {
                        $redstatus = 1;
                        setcookie('redpacketset_yf'.Perm::$userId, 1, 0, '/', TOP_DOMAIN);
                    }
                }
            }
            ...
            if($redstatus)
            {
                setcookie('redpacketset_yf', 1, 0, '/', TOP_DOMAIN);
            }
            else
            {
                setcookie('redpacketset_yf', 0, 0, '/', TOP_DOMAIN);
            }

            $data = array('status'=>$redstatus);
            $this->data->addBody(-140, $data);
        }
