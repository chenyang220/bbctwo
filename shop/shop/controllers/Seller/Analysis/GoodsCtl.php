...
class Seller_Analysis_GoodsCtl extends Seller_Controller
{
    ...
    public function detail()
    {
        ...
        $sql = "SELECT 
                        ...
                        FROM
                          `yf_order_goods` 
                        WHERE order_goods_status = 6 and shop_id = ".Perm::$shopId."
                        GROUP BY goods_id 
                        ORDER BY nums DESC ";

        $data_productbase = $Order_GoodsModel->sql->getAll($sql);
        $shop_id = Perm::$shopId;
        include $this->view->getView();
    }
...
//        $result = $result['data'];
        ...
        $result = $analytics->goodsAnalysisInfo($cond_row);

        $time = $result['time'];
        $categories = $result['categories'];
        $data_order = $result['data_order'];
        $data_sales = $result['data_sales'];
        $data_followr = $result['data_followr'];
        $data_conversion = $result['data_conversion'];
        $data_pv_num = $result['data_pv_num'];
        $data_score = $result['data_score'];
        $starttime = $result['starttime'];

        $x_data = $result['x_data'];
        $y_data_order = $result['y_data_order'];
        $y_data_sales = $result['y_data_sales'];
        $y_data_followr = $result['y_data_followr'];
        $y_data_conversion = $result['y_data_conversion'];
        $y_data_pv_num = $result['y_data_pv_num'];
        $y_data_score = $result['y_data_score'];
