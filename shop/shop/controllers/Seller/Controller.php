...
class Seller_Controller extends Yf_AppController
{
    ...
    {      //共用数据
        ...
        if ($this->ctl != 'Seller_Shop_SettledCtl' && $this->ctl != 'Seller_Supplier_SettledCtl') {
            if (Perm::checkUserPerm()) {
                ...
                if (Perm::$shopId) {
                    $shop_id = Perm::$shopId;
                    $this->shopBase = $shopBaseModel->getByWhere(array("shop_id" => $shop_id));
                    $shop_base_info = current($this->shopBase);
                } else {
                    $user_id = Perm::$userId;
                    $shopBase = $shopBaseModel->getByWhere(array("user_id" => $user_id));
                    $shop_base_info = current($shopBase);
                }
                $shop_id = $shop_base_info['shop_id'];
            ...
            $cond_row = array();
            $cond_row['shop_id'] = $shop_base_info['shop_id'];
            // $cond_row['user_id'] = $shop_base_info['user_id'];
            $cond_row['user_id'] = $this->user_info['user_id'];
            ...
            $limits = '';
            if ($seller_info['seller_is_admin'] !== 1) {
            ...
            if($shop_base_info['shop_type'] == 2){
                //供应商用户去掉促销
                unset($seller_menu['promotion']);
            }
            self::$menu = $seller_menu['seller_menu'];

            //隐藏账号入口
            unset(self::$menu['account']);
    ...
    public function webConfig()
    {
        ...
        $web['supplier_logo'] = Web_ConfigModel::value("setting_supplier_logo");//卖家中心logo
        ...
        $web['shop_web_logo'] = Web_ConfigModel::value("photo_shop_head_logo_wap");//web端商家店铺默认头像
        //积分获取的默认设置
    ...
    private function getMenuList()
    {
        $menu_list = array(
            'goods' => array('name' => '商品', 'sub' => array(
                ...
                array('name' => '商品导入', 'ctl' => 'Seller_Goods_TBImport', 'met' => 'importFile'),
                array('name' => '商品设置', 'ctl' => 'Seller_Goods_Setting', 'met' => 'index'),
                array('name' => '商品推广海报', 'ctl' => 'Seller_Bill', 'met' => 'index'),
                array('name' => 'PLUS会员商品', 'ctl' => 'Seller_Goods_PlusGoods', 'met' => 'index'),
                array('name' => '商品推广海报', 'ctl' => 'Seller_Bill', 'met' => 'index'),
            )),
            'order' => array('name' => '订单', 'sub' => array(
                ...
                array('name' => '售卖区域', 'ctl' => 'Seller_Transport', 'met' => 'tplarea'),
            )),
            'promotion' => array('name' => '促销', 'sub' => array(
                ...
                array('name' => '资讯管理', 'ctl' => 'Seller_Promotion_InformationNews', 'met' => 'index'),
                array('name' => '秒杀管理', 'ctl' => 'Seller_Promotion_Seckill', 'met' => 'index'),
                array('name' => '砍价管理', 'ctl' => 'Seller_Promotion_Bargain', 'met' => 'index'),
                array('name' => '分销设置', 'ctl' => 'Distribution_NewSeller_Setting', 'met' => 'directsellerGoods'),
                array('name' => '分佣商品设置', 'ctl' => 'Distribution_Seller_Setting', 'met' => 'getShopGoods'),
                array('name' => '分佣业绩', 'ctl' => 'Distribution_Seller_Setting', 'met' => 'directsellerDetail'),
            ...
            'shop' => array('name' => '店铺', 'sub' => array(
                ...
                array('name' => '自销产品供应商', 'ctl' => 'Seller_Shop_Supplier', 'met' => 'supplier'),
                ...
                array('name' => '批发市场', 'ctl' => 'Supplier_Index', 'met' => 'index'),
            )),
            'consult' => array('name' => '售后', 'sub' => array(
            ...
            'statistics' => array('name' => '报表', 'sub' => array(
            ...
            'message' => array('name' => '客服', 'sub' => array(
...
//                array('name' => '账号日志', 'ctl' => 'Seller_Seller_Log', 'met' => 'logList')
            ...
            'selleruser' => array('name' => '会员', 'sub' => array(
                array('name' => '会员管理', 'ctl' => 'Seller_Seller_SellerUser', 'met' => 'user'),
                array('name' => '积分管理', 'ctl' => 'Seller_Seller_SellerUser', 'met' => 'points'),
                array('name' => '会员标签', 'ctl' => 'Seller_Seller_SellerUser', 'met' => 'label'),
                
            )),
            'sellerwx' => array('name' => '公众号', 'sub' => array(
                array('name' => '公众号设置', 'ctl' => 'Seller_Seller_SellerWx', 'met' => 'index'),
                array('name' => '菜单设置', 'ctl' => 'Seller_Seller_SellerWx', 'met' => 'wxMenu'),
                array('name' => '自动回复', 'ctl' => 'Seller_Seller_SellerWx', 'met' => 'wxCallback'),
                array('name' => '行业设置', 'ctl' => 'Seller_Seller_SellerWx', 'met' => 'wxIndustrySetting'),
            ))
        );

        $shop_base = current($this->shopBase);
        $Shop_DomainModel = new Shop_DomainModel();
        $shop_domain = $Shop_DomainModel->getOne(Perm::$shopId);
        if(!$shop_domain['shop_sub_domain']){
            unset($menu_list['selleruser']);
        }
        if (!Web_ConfigModel::value('Plugin_Directseller') || @$shop_base['shop_type'] == 2) {
