 <?php if (!defined('ROOT_PATH')) {
    ...
    class Seller_GoodsCtl extends Seller_Controller
    {
        ...
        public function __construct(&$ctl, $met, $typ)
        {
            ...
            $this->shopBaseInfo = $this->getShopBase();
        }
        ...
        public function add()
        {
            ...
            $dataes = $shop_ContractTypeModel->getContractTypeList(array('contract_type_state'=>1));
            ...
            if ($cat_id) {
                if (empty($common_id)) {
                    ...
                    $bind_row['shop_id'] = $shop_id;
                    $bind_row['product_class_id:LIKE'] = "%" . $cat_id . "%";
                ...
                if ($shop_info['shop_self_support'] == 'false'  || $shop_info['shop_type'] == 2) {
        ...
        public function editGoods()
        {
            ...
            if (empty($cat_base)) {
                ...
                if ($common_data['supply_shop_id']) {
                    ...
                    $common_data['transport_template_id'] = $common_data['product_is_behalf_delivery'] == 1 && $common_data['common_parent_id'] ? $supplier_common_info['transport_template_id'] : $common_data['transport_template_id'];
                }
                ...
                $a = current($goods_base_data);
                $contract_type_arr = explode(",", trim($a['contract_type_id'],",")) ;
                /**********构造spec数据S**********/
                /**
                 *  [
                 *      [
                ...
                //获取运费模板信息
                $template_model = new Transport_TemplateModel();
                $transport_template = $template_model -> getOne($common_data['transport_template_id']);
                ...
                $shop_model = new Shop_BaseModel();
                $shop_info = $shop_model -> getOne($shop_id);
                //$data = $this->transport_default('', Perm::$shopId);
        ...
        public function overdue()
        {
            ...
            $goods_rows = $Goods_CommonModel -> overate($cront_row, array('common_id' => 'DESC'), $page, $row);
...
//            $spec_cond['spec_readonly'] = 1;
            $readonly_data = $this->goodsSpecModel->getByWhere($spec_cond);
        ...
        public function addOrEditShopGoods()
        {
            ...
            //是否同城配送
            $common_data['common_is_delivery'] = request_string('common_is_delivery')? request_string('common_is_delivery'):0;

            //处理字数限制 @nsy 2019-09-12
            if (request_string('service')) {
                $common_data['common_service'] = request_string('service');                    //售后服务
				//替换空格，清除HTML标签
				$prg = preg_replace("/(\&nbsp\;)/", " ", strip_tags($common_data['common_service']));
            ...
            if (!$shop_base['shop_self_support']) {
                $Goods_CatModel = new Goods_CatModel();
                $cat_id = request_int('cat_id');
                 
                $Goods_Parent_Cat = $Goods_CatModel->getCatParent($cat_id);
                $Goods_Parent = array_column($Goods_Parent_Cat, 'cat_id','cat_id');
                if (in_array(9000, $Goods_Parent)) {
                    throw new Exception('非自营店铺不能添加礼包商品');
                }
               
            }
			if(!$common_data['common_service']){
            ...
            $arr_cat = explode(",", trim($shop_cat_id,","));
            if ($shop_base['shop_type'] == 2) {
                foreach ($arr_cat as $k=>$value)
                {
                    if($value==0)
                    {
                        return $this -> data -> addBody(-140, array(), __('请设置本店分类'), 250);
                    }
                }
            }
            ...
            $checkbox = '';
            /*$shop_goods_cat_base = $this->shopGoodsCat->getByWhere(array('shop_id' => $shop_base['shop_id']));*/
            $sgcate = request_row('contract_type_id');
            if (empty($sgcate)) {
                $checkbox .= ',';
            } else {
                foreach ($sgcate as $key => $val) {
                    $checkbox .= $val . ',';
                }
            }
            $common_data['label_id'] = $label_id;  
            $common_data['contract_type_id'] = $checkbox;               //消费者保障
            ...
            $common_data['is_commodity'] = request_string('is_commodity');
            ...
            $shop_area  = Perm::$shopId;
            if ($common_id) {
                $Goods_Common_area = $Goods_CommonModel->getOneByWhere(array("common_id"=>$common_id));
                //一件发货
                if ($Goods_Common_area['product_is_behalf_delivery'] == 1 && $Goods_Common_area['common_parent_id']) {
                    $Goods_Common_parent_area = $Goods_CommonModel->getOneByWhere(array("common_id"=>$Goods_Common_area['common_parent_id']));
                        $shop_area  = $Goods_Common_area['supply_shop_id'];
                }  
            }
            if ($transport_area_id) {
                $area_model = new Transport_AreaModel();
                $check_area = $area_model -> checkArea($transport_area_id, $shop_area);
                if (!$check_area) {     
            ...
            //获取运费模板信息 同城配送没有配送模板
            if(request_string('common_is_delivery') != 1){
                $transport_template_id = request_string('transport_template_id');
                $shop_id = Perm::$shopId;
                $template_model = new Transport_TemplateModel();
                $transport_template = $template_model -> getOneByWhere(array("id"=>$transport_template_id));
                if (!$transport_template) {
                    return $this -> data -> addBody(-140, array(), __('请设置运费模板'), 250);
                }
                $common_data['transport_template_id'] = $transport_template_id;
            }
            ...
            if ($common_id && $flag && $this -> goodsCommonModel -> sql -> commitDb()) {
                ...
                $goods_data['goods_name'] = $common_data['common_name'];                //商品名称
                                     //标签
                $goods_data['contract_type_id'] = $common_data['contract_type_id'];                   //消费者保障
         ...
         * 上传商品图片 -- 第三步
         * */
        public function addOrEditShopGoods_base()
        {
            $common_id = request_int('common_id'); //区分是修改商品，还是添加商品
            $spec = request_row('spec');
            $price = request_int('price');
        ...
        public function saveGoodsImage()
        {
            ...
            if (!empty($image_list)) {
                ...
                $num = 0;
                $sql = "SELECT color_id FROM ".TABEL_PREFIX."goods_base
                WHERE common_id=" . $common_id;
                $images_color_id = $goodsBaseModel->sql->getAll($sql);
                foreach ($image_list as $key => $val) {
                    ...
                    foreach ($val as $k => $v) {
                        if (!empty($is_color)) {
                            ...
                            $image_data['images_color_id'] = $images_color_id[0]['color_id'];
                        }
            ...
            foreach ($goods_common_id_rows as $key => $value) {
                ...
                if ($data_goods['shop_id'] == $shop_id) {
                    if (request_string('act') == 'down') {
                        ...
                        $flag1 = $goodsBaseModel -> editBase($goods_ids, array("goods_is_shelves" => Goods_BaseModel::GOODS_DOWN), false);
...
//            $data_re['id'] = $id;
        ...
        public function deleteGoodsCommonRows()
        {
                ...
                foreach ($id as $key => $value) {
                    ...
                    if (!empty($all_dist_common)) {
                        foreach ($all_dist_common as $k => $v) {
                            if ($data['common_parent_id'] > 0 && request_string('act') == 'del') {
                                //2、是分销别人的商品就删除这个商品，
                                $flag1 = $this -> goodsCommonModel -> removeCommon($v['common_id']);
                            } elseif (request_string('act') == 'update') {
                                //3、是自己分销的商品修改商品的分销状态设置为0，分理、佣金设置为0
                                //修改数据
                                $common_data = array(
                                ...
                                $flag1 = $this -> goodsCommonModel -> editCommon($v['common_id'], $common_data);
                            } elseif ($v['common_parent_id'] > 0 && $v['supply_shop_id']>0 && $v['product_is_behalf_delivery'] == 1) {
                                //修改数据
                                $flag1 = $this -> goodsCommonModel -> removeCommon($v['common_id']);
                            }
                        }
            ...
            //删除商品，相应砍价活动结束
            $goodsBaseModel = new Goods_BaseModel();
            $goods_item = $goodsBaseModel->getByWhere(array("common_id:IN" => $common_ids));
            $goods_ids = array_column($goods_item, 'goods_id');
            //砍价活动的状态
            $Bargain_BaseModel = new Bargain_BaseModel();
            $bargain_flag = $Bargain_BaseModel->editBargainStatus(array_unique($goods_ids));
...
//              }
        ...
        {
            $goodsid = explode(",", request_string('check_value'));

            $limits = count($goodsid);
            ...
            $this->exportdata($goodsid, $limits, $url);
        ...
        public function exportdata($arr,$i="",$url=""){
            $goods_item = $this->goodsCommonModel->getCommonList(array("common_id:IN" => $arr));

            foreach ($goods_item['items'] as $key => $val) {
                $goodsid[] = intval($val['goods_id'][0]['goods_id']);
            }
    
            $data = $this->goodsBaseModel->getBaseList(array('goods_id:IN' => $goodsid));

            foreach ($goods_item['items'] as $key => $val) {
                foreach ($data['items'] as $k => $v) {
                    if ($val['goods_id'][0]['goods_id'] == $v['goods_id']) {
                        $data['items'][$k]['common_freight'] = $val['common_freight']; //运费做运费承担
                        $data['items'][$k]['transport_type_id'] = $val['transport_type_id']; // 运费类型做邮费模版ID
                        $data['items'][$k]['transport_type_name'] = $val['transport_type_name']; //运费模板名称common_is_virtual
                        $data['items'][$k]['common_is_virtual'] = $val['common_is_virtual']; //是否虚拟商品
                        $data['items'][$k]['transport_area_id'] = $val['transport_area_id']; //售卖区域
                        $data['items'][$k]['common_promotion_tips'] = $val['common_promotion_tips']; //商品广告词做宝贝描述
                        $data['items'][$k]['common_spec_name'] = implode(",", $val['common_spec_name']); //
                    }
                }
            ...
            foreach ($data['items'] as $key => $val) {
                $data['items'][$key]['shop_cat']  = 0; //店铺类目
                $data['items'][$key]['sale_mode'] = '线上销售'; //出售方式
                $data['items'][$key]['increase_price'] = $val['goods_recommended_min_price']."~".$val['goods_recommended_max_price']; //加价幅度
                $data['items'][$key]['time_validity'] = 7; //有效期
                $data['items'][$key]['inwarehouse'] = 1; //放入仓库
                $data['items'][$key]['create_time'] = get_date_time(); //放入仓库
                $data['items'][$key]['upload_status'] = 200; //放入仓库
            }
          ...
          foreach ($con as $k => $v) {
            ...
            exportExcel( $tit,$newwarr);
            exit;

        }
        public function export($arr,$i="",$url="")
        {
            $goods_item = $this->goodsCommonModel->getCommonList(array("common_id:IN" => $arr));
            ...
            $this->download_excel("商品信息".$i, $tit, $con, $key,$url);
