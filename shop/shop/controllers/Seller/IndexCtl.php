...
class Seller_IndexCtl extends Seller_Controller
{
	...
	public function index()
	{
		...
		if (Perm::checkUserPerm())
		{
            ...
            $is_from = request_string('is_from');
            if ($shop['shop_id'])
            {
                $shop_base = $this->shopBaseModel->getBaseOneList($shop);
                if (($shop_base['shop_type'] == 2 && $is_from == 'Supplier') || $shop_base['shop_type'] == 1) {
                    $user_id = Perm::$userId;
...
//				$total               = $data['data'];
                    ...
                    } else  //非自营店铺
                    {
                        ...
                        if (Web_ConfigModel::value('promotion_allow')) {
                            ...
                            //后台限时折扣设置为0，默认开通
                            if (Web_ConfigModel::value('promotion_discount_price') == 0) {
                                $data['promotion_items']['promotion_discount_combo_flag'] = true;
                            } else {
...
//				$total['month']['cashes'] = $analysis_data_row['order_cash'];
                    ...
                    if ($shop_top_rows) {
                        foreach ($shop_top_rows as $k => $v) {
                            $goods_info = current($Goods_BaseModel->getGoodsListByGoodId($v['goods_id']));
                            if ($goods_info) {
                                $shop_top_rows[$k]['goods_image'] = $goods_info['goods_image'];
                            }
                        }
                    }
