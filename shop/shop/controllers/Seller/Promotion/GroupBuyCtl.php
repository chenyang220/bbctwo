...
//			echo '<pre>';print_r($data);exit;
        ...
        public function addGroupBuy()
        {
            ...
            if (!$this->comboFlag) {
                ...
                $field_row['goods_price'] = request_float('goods_price');
                $field_row['groupbuy_rebate'] = request_float('groupbuy_price') / request_float('goods_price');
...
//				echo '<pre>';print_r($status_flag);exit;
        ...
        public function addVrGroupBuy()
        {
            if (!$this->comboFlag) {
                ...
                $field_row['goods_id'] = request_int('goods_id');
                $field_row['common_id'] = request_int('common_id');
                $field_row['goods_name'] = request_string('voucher_goods_name');
        ...
        public function delGroupBuy()
        {
            ...
            $GroupBuy_Base = $GroupBuy_BaseModel->editGroupBuy($groupbuy_id,array("is_del"=>1));


            file_put_contents(dirname(__FILE__).DIRECTORY_SEPARATOR.'abs.php',print_r($GroupBuy_Base,true),FILE_APPEND);
            if ($GroupBuy_Base) {
                $msg = __('删除成功！');
                $status = 200;
            } else {
        ...
        *购买套餐、套餐续费
        ...
        public function getShopGoods()
        {
            ...
            if ($supplier_end_common_id || $gift_common_id) {
                $s_g_common_id = array_merge($supplier_end_common_id, $gift_common_id);
                $cond_row['common_id:not IN'] = $s_g_common_id;
            }
...
//            }
            ...
            $bargain_common_ids = $Bargain_BaseModel->getBargainCommonIds();
            ...
            $active_ids = array_merge($check_common_ids,$activity_common_id,$active_common_id, $bargain_common_ids);
            foreach ($data as $key => $val) {
                ...
                if ($val['common_is_virtual']==1 && $val['common_virtual_date'] < date('Y-m-d')) {
                   unset($data[$key]);
                }
            }
