...
class Seller_Shop_ChainCtl extends Seller_Controller
{
    ...
    {
        $id = request_int('chain_id');
        #先检查该门店是否还有未完成订单
        $orderModel = new Order_BaseModel;
        $orderList = $orderModel->getByWhere([
            'chain_id'=> $id,
            'order_status'=> Order_StateModel::ORDER_SELF_PICKUP
        ]);

        if (!empty($orderList)) {
            throw new Exception('该门店有待自提订单');
        }

        $flag = $this->chainBaseModel->editBase($id, [
            'status'=> 0 //关闭
        ]);
    ...
    public function chain()
    {
        ...
        $reg_config = get_url_with_encrypt($key, $url, $formvars);
        $reg_row = $reg_config['data']['register_config'];
        if ($act == 'edit') {
    ...
    public function addChain()
    {
        ...
        $formvars['user_mobile'] = request_string('chain_phone');

        $data['chain_name'] = request_string('chain_name'); // 门店名称

        $data['longitude'] = request_string('longitude');
        $data['latitude'] = request_string('latitude');
        ...
        $data['area_code'] = request_string('area_code');//国际区号
...
//        $data['chain_contacter']        = request_string('chain_contacter'); // 联系人
        ...
        if ($init_rs['status'] == 200) {
            if ($init_rs['data']['k']) {
                ...
                if (!$user_id) {
                    ...
                    $user_info_row['area_code'] = request_string('area_code');
    ...
    public function editChain()
    {
        ...
        if ($resetPasswd['status'] == 200)
        {
            $data['chain_name']   = request_string('chain_name'); // 门店名称
            $data['chain_mobile'] = request_string('chain_phone'); // 手机号码
            $data['area_code'] = request_string('area_code'); // 手机号码
            //        $data['chain_telephone']        = request_string('chain_telephone'); // 联系电话
            //        $data['chain_contacter']        = request_string('chain_contacter'); // 联系人
            $chain_area                  = explode(' ', request_string('address_area'));
            $data['chain_province_id']   = request_string('province_id'); // 省id
            ...
            $data['chain_traffic_line']  = request_string('chain_traffic_line'); // 交通路线
            $data['chain_img']           = request_string('chainimagePath'); // 门店图片
            $data['longitude']           = request_string('longitude'); // 经度
            $data['latitude']            = request_string('latitude'); // 纬度
            $chain_id                    = request_int('chain_id');
            $data_rs                     = $data;
