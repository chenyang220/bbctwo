...
class Seller_Shop_DecorationCtl extends Seller_Controller
{
    ...
    public function decoration()
    {
        ...
        if (empty($act)) {
            // 进入这个页面创建店铺装潢
            $shop_decoration_info = $this->shopDecorationModel->getOneByWhere(['shop_id' => $shop_id]);
    ...
    public function decorationBlockSort()
    {
        ...
        foreach ($sort_array as $value) {
            $this->shopDecorationBlockModel->editDecorationBlock($value, ['block_sort' => $sort]);
            $sort = $sort + 1;
        }
        $data = [];
        $data['message'] = '保存成功';
    ...
    public function goodsSearch()
    {
        ...
        $common_list = $GoodsCommonModel->getGoodsList($condition, [], 1, 10);
    ...
    public function decorationBlockSave()
    {
        ...
        if ($result !== false) {
            $data['message'] = '保存成功';
            $decoration_detail['block_content'] = $_POST['content'];
            ob_start();
