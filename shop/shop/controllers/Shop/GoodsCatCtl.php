...
class Shop_GoodsCatCtl extends Yf_AppController
{
	...
	public function tree()
	{
		...
		if (request_int('shop_id')) {
			$shop_id = request_int('shop_id'); 
		} else {
			$shop_id = request_int('shop_id_wap'); 
		}
		$shop_cat = $Shop_GoodsCatModel->getByWhere(array('parent_id'=>0,'shop_id'=>$shop_id));

		foreach ($shop_cat as $k=>$v)
        {
        	$child = $Shop_GoodsCatModel->getByWhere(array('parent_id'=>$v['shop_goods_cat_id']));

        	$child = array_merge($child);
        	$shop_cat[$k]['child'] = $child;
        }
        $data['items'] = $shop_cat;
