...
class Supplier_GoodsCtl extends Controller
{
	...
	//批量上架
	public function addto_distributorrow()
    {
        $shop_id = Perm::$shopId;
        $Goods_CommonModel = new Goods_CommonModel();
        $Shop_BaseModel = new Shop_BaseModel();
        ...
        foreach ($goods_common_id_rows as $key => $value) {
            $old_common_id = $value;
            $old_common   = $Goods_CommonModel->getOne($value);
            //查看是否已经有同步数据
            $check_common = $Goods_CommonModel->getOneByWhere(array('shop_id'=>$shop_id,'common_parent_id'=>$value,'product_is_behalf_delivery' => 1));
            //分销商申请是否通过
            $shopDistributorModel = new Distribution_ShopDistributorModel();
            /**
             * 这有一个潜在bug
...
//
            ...
            if(empty($check_common) && $shopDistributorBase['distributor_enable'] == 1 && (array_intersect($old_shop_cat_id, $allow_shop_cat) || empty($old_shop_cat_id)))
            {
                ...
                $shop_info  = $Shop_BaseModel ->getOne($shop_id); //店铺信息
                //同步商品common,获取common_id
                $new_common_id = $Goods_CommonModel->SynchronousCommon($value,$shop_info);
                //同步商品goods_base，获取规格id
                $new_goods_ids = $Goods_CommonModel->SynchronousGoods($value,$new_common_id,$shop_info);

                //同步商品的图片
                $Goods_CommonModel->SynchronousGoodsImage($value,$new_common_id,$shop_info);
		...
		if($goods_data){
			...
			if($service['items']){
				foreach($service['items'] as $key => $val)
				{
					...
					if($val['tool'] == 1)
					{
						$service[$key]["tool"] = "<a target='_blank' href='https://wpa.qq.com/msgrd?v=3&uin=".$val['number']."&site=qq&menu=yes'><img border='0' src='https://pub.idqqimg.com/qconn/wpa/button/button_121.gif' alt='点击这里'></a>";
					}
					//旺旺
					if($val['tool'] == 2)
					{
						$service[$key]["tool"] = "<a target='_blank' href='https://www.taobao.com/webww/ww.php?ver=3&amp;touid=".$val['number']."&amp;siteid=cntaobao&amp;status=1&amp;charset=utf-8' ><img border='0' src='https://amos.alicdn.com/online.aw?v=2&amp;uid=".$val['number']."&amp;site=cntaobao&s=1&amp;charset=utf-8' alt='点击这里' /></a>";
					}
					//IM
					if($val['tool'] ==3)
					{
