...
class UploadCtl extends Yf_AppController
{
    ...
    public function uploadGoodsExcels()
    {
        ...
        $info = $up->getFileInfo();
        $path = $_SERVER['DOCUMENT_ROOT'].'/shop/data/upload'.$info['url_path'];
        ...
        $Shop_ExpressModel = new ExpressModel();
        $express_arr = $Shop_ExpressModel->getExpressList();


        $new_ordersd =array();
        foreach ($new_orders as $k=>$v)
        {
            $new_ordersd[$k]['id'] =$v[0];
            $new_ordersd[$k]['order_id'] =$v[1];
            $new_ordersd[$k]['order_from'] =$v[2];
            $new_ordersd[$k]['order_create_time'] =$v[3];
            $new_ordersd[$k]['order_payment_amount'] =$v[4];
            $new_ordersd[$k]['order_status_text'] =$v[4];
            $new_ordersd[$k]['payment_number'] =$v[5];
            $new_ordersd[$k]['payment_name'] =$v[6];
            $new_ordersd[$k]['payment_time'] =$v[7];
            $new_ordersd[$k]['express_name'] =$v[8];
            $new_ordersd[$k]['order_refund_amount'] =$v[9];
            $new_ordersd[$k]['order_finished_time'] =$v[10];
            $new_ordersd[$k]['order_buyer_evaluation_status'] =$v[11];
            $new_ordersd[$k]['shop_id'] =$v[12];
            $new_ordersd[$k]['shop_name'] =$v[13];
            $new_ordersd[$k]['buyer_user_name'] =$v[14];
            $new_ordersd[$k]['buyer_user_mobile'] =$v[15];
            $new_ordersd[$k]['order_shipping_code'] =$v[16];

        }
        $new_ordersd = array_values($new_ordersd);
        $Order_BaseModeld = new Order_BaseModel();
        $Order_GoodsModel = new Order_GoodsModel();
        $Order_StateModel = new Order_StateModel();
        $new_ordersd = array_values($new_ordersd);
        foreach ($new_ordersd as $k=>$v)
        {
        	if(isset($v['order_refund_amount']) && isset($v['express_name']))
        	{
	            $arr= $Order_GoodsModel->getOneByWhere(array("order_id"=>$v['order_id']));
		        $key      = Yf_Registry::get('shop_api_key');
		        $url         = Yf_Registry::get('shop_api_url');
		        $shop_app_id = Yf_Registry::get('shop_app_id');
		        $formvars = array();
		        $formvars['app_id']      = $shop_app_id;
		        $formvars['order_id']     = $v['order_id'];
		        $formvars['order_shipping_code']     = $v['express_name'];
		        $formvars['order_shipping_message']     = "";
		        $formvars['order_seller_message']     = "";
		        $formvars['order_send_type']     = 0;
		        $formvars['user_name']     = $v['buyer_user_name'];
		        $formvars['user_mobile']     = $v['buyer_user_mobile']; 
		        $formvars['remarks']     = "";  
		        $formvars['selectOne']     = $arr['order_goods_id']; 
		        $formvars['user_id']  = Perm::$userId;
		         foreach ($express_arr['items'] as $key => $value) {
	            	if($value['express_name']== $v['order_refund_amount'] ||$value['express_abbreviation']== $v['order_refund_amount'])
	            	{
	            		 $formvars['order_shipping_express_id'] = $value['express_id'];
	            	}

	            }
		        $info = get_url_with_encrypt($key, sprintf('%s?ctl=Seller_Trade_Order&met=send&typ=json',$url), $formvars);
     ...
     */
     public function snsFile(){
        ...
        $suffix = strtolower(strrchr($file_name, '.'));
        //暂时不判断文件类型
        if (false) {
        	$state = '文件类型不允许';
	        ...
	        $streams =file_get_contents($_FILES['upfile']['tmp_name']);
	        if (empty($streams)){
	        	$state = '文件不存在';
	        } 
	        $perfix = uniqid('sns_') . date('YmdHis');
	        $file = $perfix . $suffix;
	        $img_file = $perfix . '.jpg';
	        $file_new_name = DATA_PATH.'/upload/sns/' . $file;
	        $len = file_put_contents($file_new_name, $streams);
        	if ($len > 0 && file_exists($file_new_name)) {
        		$state = 'SUCCESS';
        	}else{
        		$state = '服务器权限或者内存不足';
        	}
        ...
        if(in_array($suffix,$video_type)){
            $img = dirname($file_new_name).'/' .$img_file;
            //第一帧图片
            shell_exec("/usr/bin/ffmpeg -y -ss 00:00:01.00 -i $file_new_name -vframes 1 -f image2 $img");
            $poster_image = Yf_Registry::get('base_url') . '/'  . APP_DIR_NAME . '/data/upload/sns/' .$img_file;
        }else{
            $poster_image= "";
            $ts_url = "";
        } 
        $info = array(
	        'state' => $state,
		    'url' => Yf_Registry::get('base_url') . '/'  . APP_DIR_NAME . '/data/upload/sns/' .$file,
		    'title' => '',
		    'original' => $file_name,
		    'type' => $suffix,
		    'size' => $size,
	...
	public function uploadFile($field_name = null, $config = array(), $base64 = "upload")
	{
		...
		{
            if(in_array($info['type'],$this->config['videoAllowFiles'])){
                $info['url'] = Yf_Registry::get('base_url') . '/'  . APP_DIR_NAME . '/data/upload'.$info['url_path'];
            }
			//使用云存储
...
//        $movie = new ffmpeg_movie($info['url']);
//        $ff_frame = $movie->getFrame(0.1);
//        $gd_image = $ff_frame->toGDImage();
//        $img_index = strpos($info['url_path'],'.');
//        $img_urls = substr($info['url_path'] , 0 , $img_index);
//      //  echo $img_urls;die;
//        $img= 'shop/data/upload'.$img_urls.".jpg";
//        imagejpeg($gd_image, $img);
//        imagedestroy($gd_image);
//        $info['poster_image'] = Yf_Registry::get('base_url') . '/'  . APP_DIR_NAME . '/data/upload'.$img;
        //截取视频第一帧当封面
        $video_type = array('.mp4','.mov');
        if(in_array($info['type'],$video_type)){
            // $movie = new ffmpeg_movie($info['url']);
            // $frame = $movie->getFrame(1);
            // $gd = $frame->toGDImage();
            // if ($orientation = $this->get_video_orientation($info['url'])) {
            ...
            //imagejpeg($gd, $img);
            $imgurl = $info['url'];
            shell_exec("/usr/bin/ffmpeg -y -ss 00:00:01.00 -i $imgurl -vframes 1 -f image2 $img");
           // imagedestroy($gd_image);

            $info['poster_image'] = Yf_Registry::get('base_url') . '/'.$img;
        }else{
            $info['poster_image']= "";
        } 

//        $rel = $this->get_video_orientation($info['url']);
