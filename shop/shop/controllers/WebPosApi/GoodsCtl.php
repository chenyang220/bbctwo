...
class WebPosApi_GoodsCtl extends WebPosApi_Controller
{
    ...
    public function getShopGoodsByUserId()
    {
        ...
        if($shop_info)
        {
            ...
            $cond_row['shop_id'] = $shop_info['shop_id'];
            $datas = $Goods_CommonModel->getByWhere($cond_row);
            
            $common_id = [];
            if($datas){
                foreach($datas as $k=>$v){
                    $common_id[] = $v['common_id'];
                }
            }

            $cond_rows['shop_id'] = $shop_info['shop_id'];
            $cond_rows['goods_is_shelves'] = 1;
            if(!empty($common_id)){
                $cond_rows['common_id:IN'] = $common_id;
            }

            $data = $this->goodsBaseModel->listByWhere($cond_rows,array(),$page,$rows);
            if($data['items'])
            {
