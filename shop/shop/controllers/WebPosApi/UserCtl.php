...
class WebPosApi_UserCtl extends WebPosApi_Controller
{
    ...
    public function getUserInfos(){
       ...
       $rows =100;
       $start = 0;
       while ($user_list = $this->userInfoModel->sql->getAll('SELECT user_id, user_sex,user_mobile,user_email,user_birthday,user_realname,user_name,user_regtime,user_grade FROM yf_user_info LIMIT '.$start*$rows.','.$rows)){

           if(empty($user_list)){
               break;
           }else{
               $data[] = array_push($data,$user_list);
               $start += 1;
           }
       }
...
//$data[111]=$data;
    ...
    */   
    public function getVoucher(){
        $userId = request_int('user_id');
        $shop_userid = request_int('shop_userid');

        $shop_info = $this->shopInfoModel->getOneByWhere(['user_id'=>$shop_userid]);
        $data = $this->voucherBaseModel->getByWhere(['voucher_owner_id'=>$userId,'voucher_shop_id'=>$shop_info['shop_id']]);
        foreach ($data as $k => $v) {
