...
class Base_DistrictModel extends Base_District
{
	...
	//获取所有地区 三级
    public function getAllArea($search_province, $search_province_ids)
    {
		$cond_row = array();
		if(!empty($search_province))  {
			$cond_row['district_first_py'] = strtoupper($search_province);
 		} 
		if (!empty($search_province_ids)) {
			$cond_row['district_id:IN'] = $search_province_ids;
    	}

   		if ($cond_row) {
			$province = $this->getByWhere($cond_row);
   		} else {
   			$province = $this->getDistrictTreeData('0', false);
   		}
        $p_id = array_column($province, 'district_id');
   ...
   public function all(){
		...
		$p_id = array_column($city, 'district_id');
		$county = $this->getDistrictTreeData($p_id, false); //区县
    ...
    /**
     * 获取所有父类id
     * @param int $district_parent_id
     * @param bool $recursive
     * @param int $level
     * @return mixed
     */
    public function getDistrictParentData($district_parent_id = 0, $recursive = true, $level = 0){
        $data_rows = $this->getDistrictParent($district_parent_id, $recursive, $level);
