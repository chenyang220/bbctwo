...
class CartModel extends Cart
{
	...
	public function getVirtualCart($goods_id = null, $num = null)
	{
        ...
        $plus =  Perm::$plus;
        $goods_base['goods_base']['is_plus']  = 0;
        if($openFlag && $plus && $plus['user_status']!=3 && $plus['end_date']>time()){
            $Plus_GoodsModel =  new Plus_GoodsModel();
            //plus价格
            $result= $Plus_GoodsModel->reformPlusGoods(array(array('common_id'=>$goods_base['goods_base']['common_id'],'common_price'=>$goods_base['goods_base']['goods_price'])));
            $isPlus = $result[0]['plus_status']?:0;
            $plus_price = $result[0]['plus_price']?:0;
            if($isPlus && $plus_price){
                $goods_base['goods_base']['now_price']  = $plus_price;
                $goods_base['goods_base']['sumprice'] = number_format($plus_price * $num, 2, '.', '');
                $goods_base['goods_base']['is_plus']  = 1;
            }
        }
		//商品的交易佣金
    ...
    public function getChainGoods($goods_id, $num, $chain_id)
    {
        ...
        //商品总价格
        $sumprice = $goods_base['goods_base']['now_price'] * $num;
        //同城配送商品 要加上配送费用
//        if(Web_ConfigModel::value('Plugin_Delivery') == 1){
//            $sumprice += Web_ConfigModel::value('delivery');
//        }
        $goods_base['goods_base']['sumprice'] = number_format($sumprice, 2, '.', '');
        //商品的交易佣金
        $Goods_CatModel = new Goods_CatModel();
        $Shop_ClassBindModel = new Shop_ClassBindModel();
        $goods_cat = $Shop_ClassBindModel->getByWhere(array('shop_id' => $goods_base['goods_base']['shop_id'], 'product_class_id' => $goods_base['goods_base']['cat_id']));
	...
	public function getCardList($cond_row = array(), $order_row = array(),$is_discount=false,$chain_id=0)
	{
		...
		foreach ($cart_row as $key => $val)
		{
			...
			//商品体积
			$val['volume'] = $goods_base['common_base']['common_length']*$goods_base['common_base']['common_width']*$goods_base['common_base']['common_height'];
            ...
            if($openFlag && $plus){
                //判断会员状态，有效日期
                if($plus['user_status']!=3 && $plus['end_date']>time()){
                    $Plus_GoodsModel =  new Plus_GoodsModel();
                    //plus价格
                    $result= $Plus_GoodsModel->reformPlusGoods(array(array('common_id'=>$goods_base['common_base']['common_id'],'common_price'=>$goods_base['common_base']['common_price'])));
                    $val['isPlus'] = $result[0]['plus_status']?:0;
                    $val['plus_price'] = $result[0]['plus_price']?:0;
                    //商品总价格
                    $val['isPlus'] && $val['sumprice'] = number_format($val['plus_price'] * $val['goods_num'], 2, '.', '');
                }
            }
			//分佣开启，并且参与分佣
			...
			if(Web_ConfigModel::value('Plugin_Directseller')&&$goods_base['common_base']['common_is_directseller'])
			{
				...
				if($user_parent['user_parent_0'])
				{
					...
					// }
					if($user_parent['user_parent_type_0']==1){
						$val['directseller_commission_0'] =  number_format(($val['sumprice']*$goods_base['common_base']['common_a_first']/100), 2, '.', '');
						$val['directseller_flag_0'] = 1;
						$val['directseller_flag'] = $goods_base['common_base']['common_is_directseller'];
					}else{
						$val['directseller_commission_0'] =  number_format(($val['sumprice']*$goods_base['common_base']['common_c_first']/100), 2, '.', '');
						$val['directseller_flag_0'] = 1;
						$val['directseller_flag'] = $goods_base['common_base']['common_is_directseller'];
					}
				...
				if($user_parent['user_parent_1'])
				{
					...
					// 	$val['directseller_flag'] = $goods_base['common_base']['common_is_directseller'];
					// }
					if($user_parent['user_parent_type_1']==1){
						$val['directseller_commission_1'] =  number_format(($val['sumprice']*$goods_base['common_base']['common_a_second']/100), 2, '.', '');
						$val['directseller_flag_1'] = 1;
						$val['directseller_flag'] = $goods_base['common_base']['common_is_directseller'];
					}else{
						$val['directseller_commission_1'] =  number_format(($val['sumprice']*$goods_base['common_base']['common_c_second']/100), 2, '.', '');
						$val['directseller_flag_1'] = 1;
						$val['directseller_flag'] = $goods_base['common_base']['common_is_directseller'];
					}
				}
				...
				$directseller_commission += $val['directseller_commission_0'] + $val['directseller_commission_1'];
			}
		...
		foreach ($data as $key => $val)
		{
			...
			if(!$chain_id)
			{
                ...
                if($openFlag && $plus){
                    ...
                    if($plus['user_status']!=3 && $plus['end_date']>time()){
                        $i = 0;
                        foreach ($data[$val['shop_id']]['goods'] as $items){
                            $items['isPlus'] && ++$i;
                        }
                        $i && $mansong_info = array();
                    }
                }
	...
	public function getCartGoodPrice($cart_id = null)
	{
		...
		else
		{
			...
			else
			{
                ...
                if($plus_switch){
                    $Plus_GoodsModel =  new Plus_GoodsModel();
                    $result= $Plus_GoodsModel->reformPlusGoods(array(array('common_id'=>$goods_base['common_id'],'common_price'=>$goods_base['goods_price'])));
                    $result[0]['plus_status'] &&  $price = $cart_base['goods_num'] * $result[0]['plus_price'];
                }
                !$price && $price= $cart_base['goods_num'] * $goods_base['goods_price'];//plus 会员价格
				//计算商品的活动价格
			}
		}
