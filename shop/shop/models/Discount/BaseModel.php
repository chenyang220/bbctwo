...
class Discount_BaseModel extends Discount_Base
{
	...
	//正在、即将参加限时折扣的商品
    public function getDiscountGoodsIds()
    {
        $sql = "SELECT discount_goods.goods_id FROM ";
        $sql .= TABEL_PREFIX . "discount_base AS discount_base JOIN ";
        $sql .= TABEL_PREFIX . "discount_goods AS discount_goods ON ";
        $sql .= "discount_base.discount_id = discount_goods.discount_id ";
        $sql .= "where 1";
        $sql .= " AND discount_base.discount_state = 1 AND discount_base.discount_start_time > '" . date('Y-m-d H:i:s', time()) . "'";
        $sql .= " AND discount_base.shop_id = " . Perm::$shopId;
