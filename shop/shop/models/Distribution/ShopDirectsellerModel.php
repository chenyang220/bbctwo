...
class Distribution_ShopDirectsellerModel extends Distribution_ShopDirectseller
{
     ...
     *  获取用户的下级
     */
    public function getInvitors($cond_row = array(), $order_row = array(), $page = 1, $rows = 100,$level = 0)
    {
        ...
        if ($data['items'])
        {
            foreach ($data['items'] as $k => $v)
            {
                ...
                //带来佣金
                $data['items'][$k]['commission'] = $this->getCommission($v['user_id'], $level);
            }
        }
    ...
    public function getCommission($user_id, $level)
    {
        ...
        $cond_row['directseller_is_settlement'] = 1;//佣金是否结算
        $cond_row['goods_refund_status']        = 0;
        $cond_row['goods_return_status']        = 0;
    ...
    public function getDirectseller($cond_row = array(), $order_row = array(), $page = 1, $rows = 100)
    {
        ...
        if ($data['items'])
        {
            foreach ($data['items'] as $k => $v)
            {
                ...
                //获取上级用户名
                $user_parent  = $User_InfoModel->getOne($data['items'][$k]['info']['user_parent_id']);
                $data['items'][$k]['info']['user_parent'] = $user_parent['user_name'];
            }
        }
        return $data;
    }
