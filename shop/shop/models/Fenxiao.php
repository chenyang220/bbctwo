...
class Fenxiao
{
    ...
    //新分销分类表
    const NEW_CAT_TABLE = 'new_fenxiao_cat';
    ...
    public function __construct()
    {
        ...
        $fenxiao_first = Web_ConfigModel::value('fenxiao_first') ? Web_ConfigModel::value('fenxiao_first') : 0;
        $fenxiao_second = Web_ConfigModel::value('fenxiao_second') ? Web_ConfigModel::value('fenxiao_second') : 0;
        $fenxiao_third = Web_ConfigModel::value('fenxiao_third') ? Web_ConfigModel::value('fenxiao_third') : 0;
        $this->values = [
            $fenxiao_first,
            $fenxiao_second,
            $fenxiao_third
...
EOF;
     ...
     *更新分类分佣
     **/      
    public function updateCatCommission($data){
        if (!array_key_exists('cat_id', $data)) {
            throw new Exception('缺少参数：cat_id');
        }
        $cat_id = $data['cat_id'];
        ...
        $a_first = $data['a_first'];
        $a_second = $data['a_second'];
        //先查询，如果存在则修改，不存在则新增
        $cat_table = TABEL_PREFIX.static::NEW_CAT_TABLE;
 
        $select_sql = <<<EOF
SELECT * FROM {$cat_table} WHERE cat_id={$cat_id}
EOF;
        $cat_rows = $this->db->sql->getAll($select_sql);
        $res_arr = [];
        if(empty($cat_rows)){
        $insert_sql_tpl = <<<EOF
INSERT INTO `$cat_table`
(`cat_id`, `c_first`, `c_second`, `a_first`, `a_second`)
VALUES
("$cat_id", "$c_first", "$c_second", "$a_first", "$a_second")
EOF;
            $flag = $this->db->sql->getAll($insert_sql_tpl);
            array_push($res_arr, ($flag === false ? false : true));
        }else{       
        $update_sql_tpl = <<<EOF
UPDATE `$cat_table` SET `c_first`="$c_first", `c_second`="$c_second", `a_first`="$a_first", `a_second`="$a_second" WHERE cat_id="$cat_id"
EOF;
           $flag = $this->db->sql->getAll($update_sql_tpl); 
           array_push($res_arr, ($flag === false ? false : true));
        }
        if (count($res_arr) !== count(array_filter($res_arr))) {
            throw new Exception('更新分佣比例失败');
...
EOF;
    ...
    public function confirmOrder($order_id)
    {
        ...
        //Yf_Log::log($order_goods_rows, Yf_Log::INFO, 'fenxiao_success');
        $order_goods_ids = implode(',', array_keys($order_goods_rows));
       //Yf_Log::log($order_goods_ids, Yf_Log::INFO, 'fenxiao_success');
...
EOF;
        ...
        if($fenxiao_order_goods_rows) {
           // Yf_Log::log($fenxiao_order_goods_rows, Yf_Log::INFO, 'fenxiao_success');
            $fenxiao_order_goods_ids = implode(',', array_column($fenxiao_order_goods_rows, 'id'));
           // Yf_Log::log($fenxiao_order_goods_rows, Yf_Log::INFO, 'fenxiao_success');
            $date = date('Y-m-d H:i:s');
            $update_commion_sql = <<<EOF
UPDATE `$commission_table` SET `end`="$date" WHERE `order_goods_id` IN ($fenxiao_order_goods_ids) AND  UNIX_TIMESTAMP(`end`) IS NULL
EOF;

            Yf_Log::log($update_commion_sql, Yf_Log::INFO, 'fenxiao_success');
            $flag = $this->db->sql->getAll($update_commion_sql);
            if ($flag === false) {
                Yf_Log::log($order_id, Yf_Log::INFO, 'fenxiao_error');
                Yf_Log::log($fenxiao_order_goods_rows, Yf_Log::INFO, 'fenxiao_error');
     ...
     * 查询单笔订单分销佣金
     */
    public function getCycleinfo($order_id){

      //  $order_id = request_int('order_id');
       // $order_id = "DD-10002-1-102-20191113103813-0001";
...
EOF;
        ...
        if($fenxiao_order_goods_rows) {
            ...
            $date = date('Y-m-d H:i:s');
            $update_commion_sql = <<<EOF
SELECT * FROM `$commission_table` WHERE `order_goods_id` IN ($fenxiao_order_goods_ids)
EOF;
...
EOF;
    ...
    public function getFenxiaoList($user_id)
    {
        ...
        $sql .=  ' order by a.create desc,a.user_id desc';
		$db = new YFSQL;
		$rs = $db->pager([
			'sql' => $sql,
            'sql_count' => $sql_count,
...
//        $user_id_str = $user_id_str ? $user_id_str : "";
    ...
    public function getFenxiaoOrderGoodsByOrderId($order_id)
    {
        ...
        if(is_array($order_id)){
            ...
            $sql = 'select * from '.TABEL_PREFIX.static::ORDER_GOODS_TABLE.' a left join '.TABEL_PREFIX.static::YF_ORDER_GOODS_TABLE.' b on a.order_goods_id=b.order_goods_id where a.order_id ="'.$order_id.'"';
