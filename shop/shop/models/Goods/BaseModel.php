...
class Goods_BaseModel extends Goods_Base
{
	...
	public function getGoodsListByGoodId($goods_id,$is_del_flag = false)
	{
        ...
        //下架商品不显示
        $cond_row['goods_is_shelves'] = self::GOODS_UP;
	...
	public function returnGoodsStock($order_goods_id,$return_num=0,$behalf_deliver=0)
	{
		...
		if (is_array($order_goods_id))
		{
			foreach ($order_goods_id as $key => $val)
			{
                ...
                //如果是秒杀商品，退还秒杀库存和rides
                if($order_goods_base['seckill_goods_id']){
                	$Seckill_GoodsModel = new Seckill_GoodsModel();
                	$seckill_goods_row = $Seckill_GoodsModel->getOne($order_goods_base['seckill_goods_id']);
                	$edit_seckill_row['seckill_stock'] = $seckill_goods_row['seckill_stock'] + $num;
                	$edit_seckill_row['goods_stock'] = $seckill_goods_row['goods_stock'] + $num;
                	$edit_seckill_row['seckill_sold'] = $seckill_goods_row['seckill_sold'] - $num;
                	$Seckill_GoodsModel->editSeckillGoods($order_goods_base['seckill_goods_id'],$edit_seckill_row);
                	//redis进队
                	$redis = new Redis();
			        ...
			        for ($i=1; $i <=$num ; $i++) { 
			        	$redis->rpush("seckill".$order_goods_base['seckill_goods_id'],$i);
			        }
                }
			}
		}
		else
		{
            ...
            if($order_goods_base['seckill_goods_id']){
            	$Seckill_GoodsModel = new Seckill_GoodsModel();
            	$seckill_goods_row = $Seckill_GoodsModel->getOne($order_goods_base['seckill_goods_id']);
            	$edit_seckill_row['seckill_stock'] = $seckill_goods_row['seckill_stcok'] + $num;
            	$edit_seckill_row['goods_stock'] = $seckill_goods_row['goods_stcok'] + $num;
            	$edit_seckill_row['seckill_sold'] = $seckill_goods_row['seckill_sold'] - $num;
            	$Seckill_GoodsModel->editSeckillGoods($order_goods_base['seckill_goods_id'],$edit_seckill_row);
            	//redis进队
            	$redis = new Redis();
	...
	public function getGoodsInfo($goods_id)
	{
		...
		//秒杀
		if (!empty($goods_base['seckill_info']) && $goods_base['seckill_info']['seckill_price'] < $goods_base['goods_price'])
		{
			if ($goods_base['goods_price'] > $goods_base['seckill_info']['seckill_price'])
			{
				$goods_base['promotion_type']  = 'seckill';
				$goods_base['title']           = $goods_base['seckill_info']['seckill_name'];
				$goods_base['promotion_price'] = $goods_base['seckill_info']['seckill_price'];
				$goods_base['down_price']      = $goods_base['goods_price'] - $goods_base['seckill_info']['seckill_price'];
				if($goods_base['seckill_info']['is_limit']==1){
					$goods_base['lower_limit']     = $goods_base['seckill_info']['goods_lower_limit'];
				}
                $goods_base['groupbuy_starttime'] = $goods_base['seckill_info']['goods_start_time'].' '.$goods_base['seckill_info']['day_start_time'].':00:00';
                $goods_base['groupbuy_endtime'] = $goods_base['seckill_info']['goods_end_time'].' '.$goods_base['seckill_info']['day_end_time'].':00:00';
			}

			unset($goods_base['seckill_info']);
		}

			//预售
		if (!empty($goods_base['presale_info']))
		{
			
			$goods_base['promotion_type']  = 'presale';
			$goods_base['title']           = $goods_base['presale_info']['presale_name'];
			$goods_base['presale_deposit'] = $goods_base['presale_info']['presale_deposit'];
			$goods_base['presale_price']   = $goods_base['presale_info']['presale_price'];
			$goods_base['final_price']     = ($goods_base['presale_info']['presale_price']*100 - $goods_base['presale_info']['presale_deposit']*100)/100;
			$goods_base['down_price']      = $goods_base['goods_price'] - $goods_base['presale_info']['presale_price'];
			$goods_base['lower_limit']     = $goods_base['presale_info']['goods_lower_limit'];
            $goods_base['groupbuy_starttime'] = $goods_base['presale_info']['goods_start_time'];
            $goods_base['groupbuy_endtime']   = $goods_base['presale_info']['goods_end_time'];
            $goods_base['presale_final_time'] = $goods_base['presale_info']['goods_final_time'];
            $goods_base['presale_final_time_end'] = date("Y-m-d H:i:s",strtotime($goods_base['presale_info']['goods_final_time']."+3 day")); 
			$goods_base['promotion_price'] = $goods_base['presale_info']['presale_deposit'];
			$goods_base['end_date'] = date('m',strtotime($goods_base['presale_info']['goods_end_time'])).'月'. date('d',strtotime($goods_base['presale_info']['goods_end_time'])).'日';
			$goods_base['end_h'] = date('H:i',strtotime($goods_base['presale_info']['goods_end_time']));
			unset($goods_base['presale_info']);
		}
...
//                $order_goods_num = $Order_GoodsModel->getOrderGoodsNum($cond_row);
		...
		if (!empty($goods_base['seckill_info']) && $goods_base['seckill_info']['seckill_price'] < $goods_base['goods_price'])
		{
			if ($goods_base['goods_price'] > $goods_base['seckill_info']['seckill_price'])
			{
				...
				if($goods_base['seckill_info']['is_limit']==1){
					$goods_base['lower_limit']     = $goods_base['seckill_info']['goods_lower_limit'];
                	$goods_base['seckill_lower_limit']     = $goods_base['seckill_info']['goods_lower_limit'];
				}
                if(strlen($goods_base['seckill_info']['day_start_time'])==1){
                	$goods_base['seckill_info']['day_start_time'] = '0'.$goods_base['seckill_info']['day_start_time'];
                }
                if(strlen($goods_base['seckill_info']['day_end_time'])==1){
                	$goods_base['seckill_info']['day_end_time'] = '0'.$goods_base['seckill_info']['day_end_time'];
                }
                $goods_base['groupbuy_starttime'] = date('Y-m-d '.$goods_base['seckill_info']['day_start_time'].':00:00');
                $goods_base['groupbuy_endtime'] = date('Y-m-d '.$goods_base['seckill_info']['day_end_time'].':00:00');
                $goods_base['groupbuy_starttimes'] = strtotime($goods_base['seckill_info']['goods_start_time']);
                $goods_base['groupbuy_endtimes'] = strtotime($goods_base['seckill_info']['goods_end_time']);
                $goods_base['promotion_is_start'] = $goods_base['groupbuy_starttime'] <= date('Y-m-d H:i:s')&&$goods_base['groupbuy_endtime']>date('Y-m-d H:i:s') ? 1 : 0;
                $goods_base['promotion_is_end'] = $goods_base['groupbuy_endtime'] <= date('Y-m-d H:i:s') ? 1 : 0;
                $goods_base['promotion_stock'] = $goods_base['seckill_info']['seckill_stock'];   
		...
		if (!empty($goods_base['presale_info']) )
		{
				...
				$goods_base['final_price']                = ($goods_base['presale_info']['presale_price']*100 - $goods_base['presale_info']['presale_deposit']*100)/100;
				$goods_base['lower_limit']                = $goods_base['presale_info']['goods_lower_limit'];
                $goods_base['presale_lower_limit']        = $goods_base['presale_info']['goods_lower_limit'];
                $goods_base['groupbuy_starttime']         = $goods_base['presale_info']['goods_start_time'];
                $goods_base['groupbuy_endtime']           = $goods_base['presale_info']['goods_end_time'];
                $goods_base['groupbuy_starttimes']        = strtotime($goods_base['presale_info']['goods_start_time']);
                $goods_base['groupbuy_endtimes']          = strtotime($goods_base['presale_info']['goods_end_time']);
                $goods_base['presale_deposit']            = $goods_base['presale_info']['presale_deposit'];
                $goods_base['promotion_is_start']         = $goods_base['groupbuy_starttime'] > date('Y-m-d H:i:s') ? 0 : 1;
                $goods_base['promotion_is_end']           = $goods_base['groupbuy_endtime'] < date('Y-m-d H:i:s') ? 0 : 1;
                $goods_base['presale_final_time']         = $goods_base['presale_info']['goods_final_time'];
                $goods_base['presale_final_time_end']     = date("Y-m-d H:i:s",strtotime($goods_base['presale_info']['goods_final_time']."+3 day"));
                $goods_base['presale_price']              = $goods_base['presale_info']['presale_price'];
                $goods_base['end_date'] = date('m',strtotime($goods_base['presale_info']['goods_end_time'])).'月'. date('d',strtotime($goods_base['presale_info']['goods_end_time'])).'日';
	...
	public function getGoodsInfoAndPromotionById($goods_id = null)
	{
		...
		//秒杀
		$goods_info['seckill_info'] = $Promotion->getSeckillGoodsInfoByGoodsID($goods_info['goods_id']);

		//预售
		$goods_info['presale_info'] = $Promotion->getPresaleGoodsInfoByGoodsID($goods_info['goods_id']);
	...
	public function getTransportInfo ($area_id , $common_id)
	{
        ...
        $transport = $Transport_TemplateModel->getOpenRuleInfo($parentArea,$common_base['shop_id'],$common_base['transport_template_id']);
        $rule_info = array();
        if($transport['id']){
...
//        dump($data);
    ...
    //商品是否包含礼包商品
    public function getGiftInfo($cart_list)
    {
        $goods_ids = array_column($cart_list, 'goods_id');
        $cart_info = array_column($cart_list, 'goods_num', 'goods_id');

        $sql = "select goods_id,cat_id from yf_goods_base where goods_id in (" . implode(",", $goods_ids) . ")";
        $res = $this->sql->getAll($sql);
        $cat_ids = array_column($res, 'cat_id', 'goods_id');

        $flag = false;
        $msg = "";
        if (in_array(9002, $cat_ids)) {
            if (count($cat_ids) > 1) {
                ...
                foreach ($cart_info as $k => $v) {
                    if ($cat_ids[$k] == 9002 && $cart_info[$k] > 1) {
                        $flag = true;
                        $msg = '礼包商品只能单独购买';
                        break;
                    }
