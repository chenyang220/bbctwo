...
class Goods_BrandModel extends Goods_Brand
{
	...
	{
		$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM " . TABEL_PREFIX . "goods_brand WHERE brand_enable=1 ORDER BY brand_initial ASC";
    ...
    {
        $list = $this->getBrandAll();
        $data = array();
        if ($list) {
            foreach ($list as $k => $v) {
                $data[$v['brand_initial']][] = $v;
            }
        }
        return $data;
    }
    ...
    //首字母筛选
    public function getInitialInfo($brand_initial, $brand_name)
    {
        $sql = "SELECT SQL_CALC_FOUND_ROWS * FROM " . TABEL_PREFIX . "goods_brand WHERE brand_enable=1";
        if($brand_initial !== ''){
            $sql .= " AND brand_initial='" . $brand_initial . "'";
        }
        if ($brand_name !== '') {
            $sql .= " AND brand_name LIKE '%" . $brand_name . "%'";
        }
        $sql .= " ORDER BY brand_initial ASC";
