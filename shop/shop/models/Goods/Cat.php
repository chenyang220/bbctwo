...
class Goods_Cat extends Yf_Model
{
    ...
    public function getTypeInfo()
    {
        ...
        $data = $Goods_TypeModel->getType();
        $row = array();
        foreach ($data as $k => $v) {
            $row[$v['type_id']] = $v['type_name'];
        }
        return $row;
    }
	...
	public function getCatTreeData($cat_parent_id = 0, $recursive = true, $level = 0, $filter = false)
	{
		...
		foreach ($cat_rows as $key => $cat_row)
		{
			...
			if($cat_row['type_id'] && $type_list[$cat_row['type_id']]) {
                $cat_row['type_name'] = $type_list[$cat_row['type_id']];
            } else{
                $cat_row['type_name'] = '无';
            }
	...
	public function filterCatTreeData(&$cat_rows)
	{
		...
		else
		{
			...
			{
				//获取所有父类id
				foreach ($product_class_ids as $m=>$cat_id)
				{

					$cat_id_arrs = explode(",",$cat_id);

					foreach ($cat_id_arrs as $key => $cat_id_val) {
						$father_cat_ids = $this->getCatParent($cat_id_val);
						...
						$cat_child_rows = $this->getCatChildId($cat_id_val);
					...
					$product_class_ids = array_merge($product_class_ids, $cat_id_arrs);
				}
	 ...
	 * @access public
	 */
	public function addCat($field_row, $return_insert_id = true, $html = true)
	{
		$add_flag = $this->add($field_row, $return_insert_id, $html);
	 ...
	 * @access public
	 */
	public function editCat($cat_id = null, $field_row, $html = true)
	{
		$update_flag = $this->edit($cat_id, $field_row, $html);
            ...
            if ($cat_parent_id){
                ...
                if ($cat_parent_row) {
					...
					if ($recursive) {
                        ...
                        ksort($cat_level_row);
                        $cat_level_row = array_values($cat_level_row);
					}

				}
			} else {
                $cat_level_row = array();
                $this_cat_id = $cat_row['cat_id'];
                $cat_level_row[] = $this->getOne($this_cat_id);
            }
            $cat_row = array();
            foreach($cat_level_row as $k=>$v) {
                $cat_row[$v['cat_id']] = $v;
            }
			return array_values($cat_row);
		} else {
			return "";
		}
	}
	 ...
	 * @access public
	 */
	public function sellerGetCatTreeData($cat_parent_id = 0, $recursive = true, $level = 0, $filter = false, $shopId)
	{
		$cat_data = array();
		...
		{
			$this->sql->noLimit = true;
			$cat_rows = $this->getByWhere($cond_row, array('cat_displayorder' => 'DESC'));
			$Cache->save($cat_rows, $cache_key);
		}

		if ($filter)
		{
			$this->sellerFilterCatTreeData($cat_rows,$shopId);
		}
     ...
     * @return array $cat_rows 过滤后的商品分类数据
     */
	public function sellerFilterCatTreeData(&$cat_rows,$shopId)
	{
		//过滤数据
		$shop_id        = $shopId;
		$Shop_BaseModel = new Shop_BaseModel();
