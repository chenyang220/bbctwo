	...
	public function getCommonStateNum($shop_id, $state = Goods_CommonModel::GOODS_STATE_NORMAL,$verfiy = Goods_CommonModel::GOODS_VERIFY_ALLOW)
	{
		...
		*/
		...
		if(-1 != $verfiy)
		{
         ...
         $row['is_del'] = Goods_CommonModel::IS_DEL_NO;
		//过滤过期的虚拟商品
		 $common_is_virtual = array(0,1);
         foreach ($common_is_virtual as $val) {
         	switch ($val) {
         		case 0:
         			$row['common_is_virtual'] = 0;
         			$num1 = $this->getNum($row);
         			break;
         		case 1:
         			$row['common_is_virtual'] = 1;
         			$row['common_virtual_date:>='] = date("Y-m-d");
         			$num2 = $this->getNum($row);
         			break;
         		default:
         			break;
         	}
