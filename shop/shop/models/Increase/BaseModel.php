...
class Increase_BaseModel extends Increase_Base
{
	...
	*/
	...
	//当前店铺参加加价购、换购商品商品
    public function getIncreaseGoodsIds()
    {
        $sql = "SELECT increase_goods.goods_id AS incerase_goods_id,increase_redemp_goods.goods_id AS redemp_goods_id FROM ";
        $sql .= TABEL_PREFIX . "increase_base AS increase_base JOIN ";
        $sql .= TABEL_PREFIX . "increase_goods AS increase_goods ON ";
        $sql .= "increase_base.increase_id = increase_goods.increase_goods_id ";
        $sql .= " JOIN " . TABEL_PREFIX . "increase_redemp_goods AS increase_redemp_goods ON ";
        $sql .= "increase_base.increase_id = increase_redemp_goods.increase_id ";
        $sql .= "where 1";
        $sql .= " AND increase_base.increase_state = 1 AND increase_base.increase_end_time > '" . date('Y-m-d H:i:s', time()) . "'";
        $sql .= " AND increase_base.shop_id = " . Perm::$shopId;

        $result = $this->sql->getAll($sql);
        $increase_goods_id = array_column($result, 'increase_goods_id');
        $redemp_goods_id = array_column($result, 'redemp_goods_id');
        $goods_ids = array_merge($increase_goods_id, $redemp_goods_id);
