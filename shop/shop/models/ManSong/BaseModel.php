...
class ManSong_BaseModel extends ManSong_Base
{
	...
	//正在、即将参见满送的商品id
    public function getManSongGoodsIds()
    {
        $sql = "SELECT mansong_rule.goods_id FROM ";
        $sql .= TABEL_PREFIX . "mansong_base AS mansong_base JOIN ";
        $sql .= TABEL_PREFIX . "mansong_rule AS mansong_rule ON ";
        $sql .= "mansong_base.mansong_id = mansong_rule.mansong_id ";
        $sql .= "where 1";
        $sql .= " AND mansong_base.mansong_state = 1 AND mansong_base.mansong_end_time > '" . date('Y-m-d H:i:s', time()) . "'";
        $sql .= " AND mansong_base.shop_id = " . Perm::$shopId;
