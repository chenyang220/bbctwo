...
class Order_ReturnModel extends Order_Return
{
	...
	public function settleReturn($cond_row = array())
	{
        ...
        if($data){
            $order_ids = array_column($data,'order_number');
...
//                $order_user = array_combine($order_ids,$user_ids);
		...
		$res = array(
			...
			'fenxiao_return_amount' => $commit_coun_price
		);
		return $res;
	}
	...
	//获取退款/退货状态值
	public function getReturnState($order_goods_id,$type)
    {
        $cond_row = array();
        $cond_row['order_goods_id'] = $order_goods_id;
        $cond_row['return_type'] = $type;

        $data = $this->getReturn($cond_row);

        return $data['return_state_text'];
    }

    //商家处理结果
    public function getReturnShopHandel($order_goods_id,$type)
    {
        $cond_row = array();
        $cond_row['order_goods_id'] = $order_goods_id;
        $cond_row['return_type'] = $type;
        ...
        return $data['return_shop_handle'];
    }
