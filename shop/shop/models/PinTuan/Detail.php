...
class PinTuan_Detail extends Yf_Model
{
    ...
    //根据pintuan_id获取对应商品信息
    public function getGoodsIdById($id)
    {
        $detail =  $this->getByWhere(array('pintuan_id:IN'=>$id));
        $detail_info = current($detail);
        $Goods_BaseModel = new Goods_BaseModel();
        $goods_info = $Goods_BaseModel->getBase($detail_info['goods_id']);
        $data['detail_info'] = $detail_info;
        $data['goods_info'] = current($goods_info);
        return $data;
    }
