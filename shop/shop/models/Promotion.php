<?php
    ...
    class Promotion extends Yf_Model
    {
        ...
        */
        ...
        //获取秒杀活动信息
        public function getSeckillGoodsInfoByGoodsID($goods_id)
        {
            $renew_row = array();
            
            if (Web_ConfigModel::value('seckill_allow')){//秒杀开启
                $cond_row['goods_id'] = $goods_id;
                $cond_row['seckill_goods_state'] = Seckill_GoodsModel::NORMAL;
                $renew_row = $this->Seckill_GoodsModel->getSeckillGoodsDetailByWheres($cond_row);
        ...
        //获取预售活动信息
        public function getPresaleGoodsInfoByGoodsID($goods_id)
        {
            $renew_row = array();
            $cond_row['goods_id'] = $goods_id;
            $cond_row['presale_goods_state'] = Presale_GoodsModel::NORMAL;
            $renew_row = $this->Presale_GoodsModel->getPresaleGoodsDetailByWheres($cond_row);
            return $renew_row;
        }
            ...
            foreach ($increase_price_info as $key => $val) {
                ...
                if (Web_ConfigModel::value('Plugin_Directseller')) {
                    ...
                    if ($common_base['common_is_directseller']) {
                        //产品佣金
                        $val['directseller_commission_0'] = $val['redemp_price'] * $val['goods_num'] * $common_base['common_cps_rate'] / 100;
                        $val['directseller_commission_1'] = $val['redemp_price'] * $val['goods_num'] * $common_base['common_second_cps_rate'] / 100;
                        $val['directseller_commission_2'] = $val['redemp_price'] * $val['goods_num'] * $common_base['common_third_cps_rate'] / 100;
                    }
                }
