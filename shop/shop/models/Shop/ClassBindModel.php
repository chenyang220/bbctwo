...
class Shop_ClassBindModel extends Shop_ClassBind
{
	...
	/**
	 * 根据店铺id 获取所有的经营类目名称以及分佣比例(多个比例)
	 *
	 * @param  array $cond_row
	 * @return array $rows 信息
	 * @access public
	 */
	public function getPluralClassBindlist($shop_id = array(), $order_row = array(), $page = 1, $rows = 100)
	{
		...
		foreach ($data['items'] as $key => $value)
		{
			...
			$commission_rate_arr = explode(",",$value['commission_rate']);
			$product_class_id_arr = explode(",",$value['product_class_id']);
			foreach ($product_class_id_arr as $cat_key => $product_class_id) {
				$product_class["product_parent_name"][$key][$cat_key]  = $CatModel->getCatParent($product_class_id);
				$product_class["commission_rate"][$key][$cat_key]           = $commission_rate_arr[$cat_key];
				$product_name[$key][$cat_key]                               = $CatModel->getOne($product_class_id);
			}
		...
		if (!empty($product_class["product_parent_name"]))
		{
			foreach ($product_class["product_parent_name"] as $key => $value)
			{
				foreach ($value as $cat_key => $cat_value) {
					if(!empty($cat_value)){
                    	$product_class["product_parent_name"][$key][$cat_key][] = $product_name[$key][$cat_key];
	                }else{
	                    unset($product_class["product_parent_name"][$key][$cat_key]);
	                }
				}
		...
		if (!empty($product_class["product_parent_name"]))
		{
			foreach ($product_class["product_parent_name"] as $key => $value)
			{
                if(!empty($value)){
                    ...
                    unset($product_class["product_parent_name"][$key]);
                }
