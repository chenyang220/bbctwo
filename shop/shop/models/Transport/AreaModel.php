...
class Transport_AreaModel extends Transport_Area
{
    ...
    public function isSale($id,$area_id){
		...
		*
		*为防止地址最后一级level是3,4
		*从而产生无货的错误，在此处查找上级id进行传递
		*@nsy 2019-07-16
		*/
		$sql = "select * from yf_base_district where 1 and district_id = '{$area_id}'";
		$result = $this->sql->getAll($sql);
		$level =  intval($result[0]['district_is_level']);
		if($level>2){
			$sql = "select district_id from yf_base_district where 1 and district_id = '{$level}'";
			$district = $this->sql->getAll($sql);
			$district[0]['district_id'] && $area_id = $district[0]['district_id'];
		}	
