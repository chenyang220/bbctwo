		...
		if ($de['items'])
		{
			...
			if($shops_id){
				...
				foreach ($data['items'] as $key => $val)
				{
					...
					//查找店铺地址
					$district_name = $Base_DistrictModel->getAllName($re[$val['shop_id']]['district_id']);
					$data['items'][$key]['shop']['address'] = $district_name;

					//如果不存在店铺logo获取店铺默认logo
					if(!$data['items'][$key]['shop']['shop_logo'])
					{
						$data['items'][$key]['shop']['shop_logo'] = Web_ConfigModel::value("photo_shop_logo");
						$data['items'][$key]['shop_logo'] = Web_ConfigModel::value("photo_shop_logo");
					}
					else
                    {
                        $data['items'][$key]['shop_logo'] = $data['items'][$key]['shop']['shop_logo'];
                    }

					if(!$data['items'][$key]['shop']['wap_shop_logo']){
                        $data['items'][$key]['shop']['wap_shop_logo'] = Web_ConfigModel::value("photo_shop_head_logo_wap");
                        $data['items'][$key]['wap_shop_logo'] = Web_ConfigModel::value("photo_shop_head_logo_wap");
                    }
                    else
                    {
                        $data['items'][$key]['wap_shop_logo'] = $data['items'][$key]['shop']['wap_shop_logo'];
                    }
				}
			}
