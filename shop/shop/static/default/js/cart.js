...
$(document).ready(function(){
    ...
    //礼包商品优惠券使用提醒
    $("#pay_sure_btn").click(function () {
        $.dialog({
            title: '礼包商品提醒',
            content: '礼包商品使用优惠券，实付金额不满足升级掌柜提醒',
            height: 100,
            width: 410,
            lock: true,
            drag: false,
	...
	$("#pay_btn").click(function(){
		...
		if(!address_contact)
		{
            var address_contact_list = $("#address_list").find("h5").html();
            if(address_contact_list){
                Public.tips.error('请勾选收获地址！');
                return false;
			}else{
                $('.add_address').click();
		...
		$(".select_increase").each(function(){
			if($(this).is(':checked'))
			{
				...
				var goods_limit = $(this).attr('goods_limit');
                increase_max_num[rule_id] = goods_limit;
		...
		//可以购买多个规则下商品，限制规则下的商品数量
		// var choose_id = increase_rule_id[0].split(',');
        var choose_id = [];
		if (increase_rule_id.length > 0) {
            choose_id = increase_rule_id[0].split(',');
        }
        var map = [];
        ...
        for (let m in increase_max_num) {
        	//判断同意规则下 购买商品数量
			if (increase_max_num[m] < map[m]){
                Public.tips.error('加价购商品数量超出限制');
                return false;
			}
        }
		...
		$.ajax({
			...
			success:function(a){
				if(a.status == 200)
				{
					if(pay_way_id == 1)
					{
						var shop_id_cookie = $('#shop_id_cookie').val();
						window.location.href = PAYCENTER_URL + "?ctl=Info&met=pay&uorder=" + a.data.uorder+'&order_g_type=physical&shop_id_cookie='+shop_id_cookie+'&shop_id_url='+SITE_URL;
						return false;
					}
					else
					{
