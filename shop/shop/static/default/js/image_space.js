...
$(function () {
        ...
        $.post(SITE_URL + '?ctl=Seller_Album&met=listImage&typ=json&firstRow=' + _page.firstRow + '&totalRows=' + _page.totalRows, {
            ...
            if (data.status == 200) {
                ...
                if (itmes.length > 0) {
                    ...
                    _page.$imageList.append('<div class="no_account"><img src="' + BASE_URL + '/shop/static/default/images/ico_none.png"><p>'+__("暂无符合条件的数据记录")+'</p></div>');
                }
            } else {
                alert(__('读取数据失败'));
            }
    ...
    function loadingImageList (itmes) {
        ...
        for (var i = 0; i < itmes.length; i++) {
            ...
            $rename = $('<a href="javascript:;">'+__('改名')+'</a>');
            $link = $('<a href="javascript:;" name="imageUrl" data-imageurl="' + itmes[i].upload_path + '">'+__('链接')+'</a>');
            $assignGroup = $('<a href="javascript:;">'+__('分组')+'</a>');
            $delete = $('<a href="javascript:;">'+__('删除')+'</a>');
            ...
            $rename.on('click', function() {
                $.dialog({
                    ...
                    '<div style="margin-bottom: 6px;">'+__("修改名称(限20个字符)")+'</div>' +
                    '<div style="margin-bottom: 6px;">' +
                    '<input class="js-name-input" maxlength="20" type="text" value="' + $(this).parent().prev().find('span').html() + '" placeholder="" style=" width: 166px;">' +
                    '</div>' +
                    '</div>',
                    title: __('改名'),
                    width: 250,
    ...
    function initEvent() {
        ...
        _page.$addAlbum.on('click', function (e) {
            $.dialog({
                title: __('新增分组'),
                content: '<div class="ui-popover-inner" style="min-width: auto; width: 180px;">' +
                '<div style="margin-bottom: 6px;">'+__("新增分组<span>(限20个字符)</span>")+'</div>' +
                '<div style="margin-bottom: 6px;">' +
                '<input class="js-name-input" maxlength="20" type="text" value="" placeholder="" style=" width: 166px;">' +
                '</div>' +
                '</div>',
                width: 250,
                cancel: true,
                ...
                ok: function () {
                    ...
                    if (!reg.test(group_name)) {
                        Public.tips({ content: __('请输入合适的分组名称'), type: 1});
                        return false;
                    } else {
        ...
        _page.$categoryList.children().on('click', function () {
            ...
            if (album_id != 0) {
                if ( !$('#rename_album').get(0) ) {
                    _page.$uploadImage.before('<div class="ac_btns"><a href="javascript:;" class="bbc_seller_btns" data-album_id="'+ album_id +'" id="rename_album">'+__("重命名")+'</a><a href="javascript:;" class="bbc_seller_btns" data-album_id="'+ album_id +'" id="remove_album">'+__("删除分组")+'</a></div>');
                }
            } else {
        ...
        $('.content:eq(1)').on('click', '#rename_album, #remove_album', function (){
            ...
            if (this.id == 'rename_album') {
                ...
                $.dialog({
                    title:__('编辑名称'),
                    content: '<div class="ui-popover-inner" style="min-width: auto; width: 180px;">' +
                    '<div style="margin-bottom: 6px;">'+__("编辑分组<span>(限20个字符)</span>")+'</div>' +
                    '<div style="margin-bottom: 6px;">' +
                    '<input class="js-name-input" maxlength="20" type="text" value="" placeholder="" style=" width: 166px;">' +
                    '</div>' +
                    '</div>',
                ...
                url += 'removeAlbum';
                $.dialog.confirm(__('仅删除分组，不删除图片，组内图片将自动归入未分组'), function () {
                    $.post(url, param, function ( data ) {
                        if ( data.status == 200 ) {
                            Public.tips({content: data.msg, type: 3}) , window.location.reload();
                        } else {
        ...
        function getImageList(imageList, album_id) {
            ...
            if ( typeof imageList != 'undefined' && imageList.length > 0 ) {
                ...
                $.each(imageList, function(i, e){
                    div_img.push('<div class="image-item" data-album_id="0" data-upload_id="'+ e.upload_id +'"><div class="image-box" style="background-image:url('+ e.src +');"></div><div class="image-title"><label><input name="image-box" type="checkbox"><span>3631203_111238646000_2</span></label></div><div class="image-opt"><a href="javascript:;">'+__("改名")+'</a><a href="javascript:;" name="imageUrl" data-imageurl="'+ e.src +'">'+__("链接")+'</a><a href="javascript:;">'+__("分组")+'</a><a href="javascript:;">'+__("删除")+'</a></div></div>');
                });
    ...
    function deleteImage (data) {
        ...
        $.dialog({
            title: __('删除图片'),
            content: '<div class="ui-popover-inner clearfix" style="min-width: auto;width: 250px;"><div>'+__("确定删除该图片？")+'</div>' +
            '<div style="margin: 6px 0 12px 0; color: #999;">'+__("若删除，不会对目前已使用该图片的相关业务造成影响。")+'</div>' +
            '</div>',
            cancel: true,
            data: { curAlbumId: curAlbumId, upload_id: data.upload_id, thisImageItem: data.thisImageItem },
            ok: function () {
    ...
    $('#image-list').on('click', '[name="imageUrl"]', function () {
        ...
        Public.tips({type:3, content: __('已成功复制到剪切板')});
    });
