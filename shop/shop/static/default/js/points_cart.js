$(document).ready(function(){
	...
	function isChoseAddress()
	{
		...
		if($("#address_list li").length <= 0) {
           ...
           $.dialog({
            ...
            // return $.dialog.alert("确认收货成功"), alert("112");
	...
	$("#pay_btn").click(function(){
        ...
        if($("#address_list li").length <= 0) {
            ...
            var $address_list = $("#address_list");
            if (!$address_list.find("li.add_choose").get(0)) {
                $.dialog({
                    title: '警告',
                    content: '请选择收货地址',
                    height: 80,
		...
		//1.获取收货地址
        	...
        	area_code = $(".add_choose").find("#area_code").val();
		//3.获取商品信息（商品id，商品备注）
		...
		$.ajax({
			...
			data:{receiver_name:address_contact,receiver_address:address_address,receiver_phone:address_phone,area_code:area_code,point_cart_id:point_cart_id,remark:remark},
			dataType: "json",
