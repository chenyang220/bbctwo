...
$(document).ready(function(){
	...
	//新增单位发票
	window.add_shop_invoice = function(e)
	{
		var result = "";
		$.ajax({
			url: SITE_URL+"?ctl=Seller_Shop_Invoice&met=addInvoice&typ=json",
			data:e,
			dataType: "json",
			contentType: "application/json;charset=utf-8",
			async:false,
	...
	window.save_Invoice = function(e)
	{
		...
		con    = "明细";
		name   = $("#generalInvoice_invoice_name").val();
		in_id     = $("#invoice_id").val();
		is_use = $("input[name='is_use']:checked").val();

		state_num = index1.find(".tab-item-selected").val();
        var ck_name = check_InvoiceName();
        if(!ck_name){
        	return false;
		}
        if (state_num == 2) //电子发票
        {
            ...
            //将电子发票的信息保存到数据库
            phone = index.find("#re_user_mobile").val();
            area_code = index.find("#area_code").val();
        ...
        if (state_num == 3)	 //增值税发票
        {
            ...
            area_code = index.find("#area_code2").val();
            ...
            var cphone_reg = /^1[3-9]\d{9}$/;
            if (!cphone_reg.test(cphone) && area_code == 86) {
    ...
    //验证模板名称
    window.check_InvoiceName = function(e)
    {
        var value = $("#generalInvoice_invoice_name").val();
        var errorFlag = false;
        var errorMessage = "";
		if (value == '') {
			errorFlag = true;
			errorMessage = "请填写发票模板名称";
		}
        if (errorFlag) {
            $("#generalInvoice_invoice_name_error").html(errorMessage);
            $("#generalInvoice_invoice_name_error").addClass("error-msg");
            return false;
        } else {
            $("#generalInvoice_invoice_name_error").removeClass("error-msg");
            $("#generalInvoice_invoice_name_error").html("");
            return true;
        }
	...
	{
		var value = $("#re_user_mobile").val();
		var errorFlag = false;
		var errorMessage = "";
		var reg=/^1[3-9]\d{9}$/;
		var area_code = $('#area_code').val();
		if (value != '') {
			if (!reg.test(value) && area_code==86) {
				errorFlag = true;
	 ...
	 */
	 window.check_InvoiceConsignee = function(divId,isGeneral) {
		...
		// 验证地区是否完整
			...
			// 验证地区是否正确
			// if (!((provinceId && cityId && countyId) || ($(".dress_box").html()) ) )
			// {
			// 	errorFlag = true;
		...
		// 验证手机号码
			...
			if (true) {
				...
				if (value == '') {
					...
					if (!mobile_reg.test(value) && area_code == 86) {
						errorFlag = true;
						errorMessage = "手机号码格式不正确";
					}
				}
