...
$(function () {
    $(".service-set-items label").click(function(){
    ...
    var uploadVideo = new UploadVideo({
        ...
        thumbnailWidth: 160,
        thumbnailHeight: 160,
        imageContainer: '#goodsVideo',
        uploadButton: '#uploadvideo',
        inputHidden: '#videoPath',
        callback: function () {
            $('#videoPath').isValid();
        }
    });
    ...
    $('#del_video').click(function(){
        $('#goodsVideo').attr('src','');
        $('#videoPath').val('');
        $('.up-progress-percent').css('width','0');
    })
    //编辑器
    ...
    $('#form').validator({
        ...
        rules: {
			...
			goods_max_price: function(element, params, field){
                ...
                if ($('input[name="goods_max_price"]').val() != '') 
				{
                    if (element.name == 'goods_max_price') 
					{
						...
						{
                        	return __('商品规格的价格，必须在供应商设置的最低零售价和最高零售价之间。');
                        }else if(Number(parent_id)>0 && Number(element.value) < Number(min_price)){
                        	return __('商品规格的价格，必须在供应商设置的最低零售价和最高零售价之间。');
                        }else{
                            return true;
            ...
            // cubageVerify: function ( element, params, field ) {
            //     var transport_template_type = $('#transport_template_type').val();
            //     var cubage  = $('input[name="cubage"]').val();
            //         if (transport_template_type == 1 && cubage == " " ) {        
            //             return '重量不能为空';
            //         }
            //         var ret = /^[1-9]\d{0,2}(.\d{0,3})?$|^(0.\d{0,3}?)$/;
            //           if(ret.test(cubage)){
            //              return '1仅支持整数、小数1';
            //           }
                    
            // },
            // glengthVerify: function ( element, params, field ) {

            //     var transport_template_type = $('#transport_template_type').val();
            //     var glength  = $('input[name="glength"]').val();
               
            //     if ( transport_template_type == 3 && (glength == " " )) {
            //         return '不能为空';
            //     }
            //     var ret = /^[0-9]+(\.[0-9]{1,3})?$/;
            //   // var ret = /^[1-9]\d{0,2}(.\d{0,3})?$|^(0.\d{0,3}?)$/;
            ...
            //   }
            // },
            // widthVerify: function ( element, params, field ) {
            //     var transport_template_type = $('#transport_template_type').val();
            //     var width  = $('input[name="width"]').val();
            //     if ( transport_template_type == 3 && width == " " ) {
            //         return '不能为空';
            //     }
            //     var ret = /^[1-9]\d{0,2}(.\d{0,3})?$|^(0.\d{0,3}?)$/;
            //   if( !ret.test(width) ){
            ...
            //   }
            // },
            // heightVerify: function ( element, params, field ) {
            //     var transport_template_type = $('#transport_template_type').val();
            //     var height  = $('input[name="height"]').val();
            //     if ( transport_template_type == 3 && height == " " ) {
            //         return '不能为空';
            //     }
            //      var ret = /^[1-9]\d{0,2}(.\d{0,3})?$|^(0.\d{0,3}?)$/;
            //   if( !ret.test(height) ){
        ...
        fields: {
            ...
            'cost_price':       'range[0.01~9999999];',
             'cubage':           'required;range[0~9999999];;',
            'imagePath':        'required;',
            'alarm':            'integer[+0];',
            'limit':            'limitVerify;',
            'code':             'codeVerify;length[~20];',
			'goods_max_price':  'goods_max_price;',
            'glength':          'required;range[0~9999999];',
            'width':            'required;range[0~9999999];;',
            'height':           'required;range[0~9999999];;',
        },
        ...
        valid: function(form){
            ...
            var c = $('.up-progress-percent').width();
            var p = $('.up-progress-percent').parent().width();
            var  arr  =document.cookie.match(new  RegExp("(^|  )"+'totalRows_o'+"=([^;]*)(;|$)"));
            var  aror  =document.cookie.match(new  RegExp("(^|  )"+'firstRow_o'+"=([^;]*)(;|$)"));
            var  firstRow_o  =  $.cookie('firstRow_o');
            var  totalRows_o  =  $.cookie('totalRows_o');

            var res = parseFloat(c)/parseFloat(p);
            if(0< res & res < 1){
                    Public.tips.warning("视频上传还未完成");
                    return false;
            }
                        ...
                        setTimeout(function () {
                            if ( data.data.action && data.data.action == 'edit' )
                            {
                            	if(data.data.dist_goods){
                                	...
                                	window.location.href  =  SITE_URL  +  "?ctl=Seller_Goods&met=online&typ=e&firstRow="+firstRow_o+"&totalRows="+totalRows_o;
                               }
                            } else {
    ...
    $('#templateButton').on('click', function () {
            ...
            if ( falg ) {
                ...
                $.dialog({
                    title: '选择运费模板',
                    content: 'url:' + SITE_URL + '?ctl=Seller_Transport&met=transportTemplateDialog&typ=e',
                    data: { callback_template: callback_template},
                    width: 400,
                    height: 400,
                    max: false,
                    min: false,
    ...
    function callback_template (data, win) {
        $('#transport_template_id').val(data.transport_template_id);
        $('#transport_template_name').html(data.transport_template_name);
        $('#transport_template_type').val(data.transport_template_type);
        if (data.transport_template_type == 1) {
            //按重量计算
             $("#goods_volume").hide();
            $("#goods_weight").show();
        } else {
