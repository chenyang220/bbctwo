...
//处理平台的提现订单的问题 type = 1
$sql = "SELECT * FROM `yf_ylfenzhangflowers` where type='2' and actdo='1' and returstatus='1' and  status in('1','3')  LIMIT 0 , 10 ";
//商家的待提现订单和提现失败的订单
...
if($order_list){
    foreach($order_list as $val){
        $id  = $val["id"] ;
        // 如果这个订单退款的了就不发起提现了
        $order_ids = explode(',', $val["order_id"]);
        //处理退款订单
        $isruning = false;
        $rs_row = [];
        foreach($order_ids as $order_id){
            ...
            $isruning = true;
            if($returnorder){
                if($returnorder["return_state"] == "3" || $returnorder["return_state"] == "6" || $returnorder["return_state"] == "5"){
                    $isruning = true;
                } else {
                    $isruning = false;
                }
