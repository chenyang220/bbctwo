...
<div class="cart-head">
	...
	<div class="wrap">
		...
		<div class="ncc-receipt-info">
            ...
            <?php } ?>
            <input type="hidden" value="<?= $_REQUEST['is_delivery'] ?>" id="is_delivery">
                        ...
                        <?php if (isset($data['address'])) {
                            ...
                            foreach ($data['address'] as $key => $value) {
                                ...
                                <li class="<?php if (!$address_id && $value['user_address_default'] == 1) { ?>add_choose<?php } ?><?php if ($address_id && $value['user_address_id'] == $address_id) { ?>add_choose<?php } ?> "
                                    ...
                                    <?php if($_REQUEST['is_delivery'] == 1){ ?>
                                        <input type="hidden" name="receiver_address" value="<?= ($value['user_address_area']) ?> <?= ($value['user_address_address']) ?>">
						...
						<tbody class="rel_good_infor">
							<tr>
								...
								<td class="goods_num goods_num_1">
                                    <p class="clearfix" style="width: 82px;margin: 0 auto;">
                                        <a class="reduce_num">-</a>
                                        <input id="buy_nums" data-id="<?=($data['goods_base']['goods_id'])?>" data-max="<?=($data['chain_goods_stock'])?>" data-min="1" value="<?=($nums)?>">
                                        <a class="<?php if($data['chain_goods_stock'] == 1){?>no_<?php }?>add_num">+</a>
                                    </p>
								</td>
								<td class="price_all">
...
<!--                            003-->
						<p class="remarks"><span class="remarks-z activity-dt"><?=__('备注')?></span><input name="remarks" id="goodsremarks" style="padding-left:8px;width: 398px;" placeholder="<?=__('限45个字（定制类商品，请将购买需求在备注中做详细说明）')?>"><?=__('提示：请勿填写有关支付、收货、发票方面的信息')?></p>
						</div>
					</div>
				</div>
			...
			<div class="frank clearfix mb10">
				...
				<p class="submit" style="text-align: center;">
					...
					<?php } ?>
                    <?php if ($_REQUEST['is_delivery'] == 1 && Web_ConfigModel::value('Plugin_Delivery') == 1){ ?>
                        <span>
                                <?= __('配送费用：') ?>
                            <strong>
                                        <i class=""><?= (Web_ConfigModel::value('monetary_unit')) ?><?= Web_ConfigModel::value('delivery') ?></i>
                                    </strong>
                                </span>
                        <span>
                    <?php } ?>
                    <span>
						...
						<strong class="bbc_color">
							...
							<?php
                                // 同城配送需要加上配送费用
                                $delivery = 0;
                                if($_REQUEST['is_delivery'] == 1 && Web_ConfigModel::value('Plugin_Delivery') == 1){
                                    $delivery = Web_ConfigModel::value('delivery');
                                }
                                $after_total = $data['goods_base']['sumprice'] - $data['bestOffer']['redPackPrice'] - $data['bestOffer']['voucherPrice'] + $delivery;
							?>
	...
	<input type="hidden" id="limit" value="<?=request_string('limit')?>">
<script>
	...
	$(function(){
        ...
        $("#is_discount").click(function(){
            var goods_num = Number($("#buy_nums").val());
        ...
        $('.add_num').click(function(){
            var goods_num = Number($("#buy_nums").val()) + 1;
            var limit = $('#limit').val();
			if (limit > 0 && goods_num > limit) {
	            return Public.tips.error('该商品每人限购'+limit+'件');
	        }
            var max_num = $("#buy_nums").data('max');
            var min_num = $("#buy_nums").data('min');
            if(goods_num > max_num)
            {
                goods_num = max_num;
            }
            if(goods_num < min_num)
            {
                goods_num = 1;
            }
            $("#buy_nums").val(goods_num);
            edit_cart_num(goods_num);
        })
        ...
        $("#buy_nums").on('blur',function(){
            if(!isNaN($(this).val())){
                var goods_num = parseInt(Number($(this).val()));
            } else{
                var goods_num = 1;
            }
            var limit = $('#limit').val();
			if (limit > 0 && goods_num > limit) {
	            Public.tips.error('改商品每人限购'+limit+'件');
	            goods_num = 1;
	        }
            var max_num = $(this).data('max');
            var min_num = $(this).data('min');
            if(goods_num > max_num)
            {
                goods_num = max_num;
            }
        ...
        //修改购物车数量
        function edit_cart_num(goods_num)
        {
        ...
        //修改订单价格
        function change_order_price(goods_num)
        {
            //最优代金券
            var best_v = Number($("input[name='best_v']").val());
            //最优红包
            var best_redp = Number($("input[name='best_redp']").val());
            //商品价格
            var now_price = "<?=($data['goods_base']['now_price'])?>";
            //会员折扣
            ...
            var pay_price = 0;
            //会员折扣
            var user_discount_price = goods_num * now_price * (1 - user_rate);;

            if($('#is_discount').is(':checked')) {
                order_price = goods_num * now_price;
                pay_price = goods_num * now_price * user_rate + delivery;
                $(".rate_total").html('减￥' + user_discount_price.toFixed(2));
            }else{
                order_price = goods_num * now_price - best_v;
                pay_price = goods_num * now_price - best_v - best_redp + delivery;
                $(".rate_total").html('未使用');
            }
            $("#use_discount").html('减￥' + user_discount_price.toFixed(2));
            $("#use_discount").attr('discount_price',user_discount_price.toFixed(2));
            $(".total").html(order_price.toFixed(2));
            $(".after_total").html(pay_price.toFixed(2));
            $("#orderAmount").val(pay_price.toFixed(2))
        }
    });
</script>
