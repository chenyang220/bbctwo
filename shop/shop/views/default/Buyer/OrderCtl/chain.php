...
<!--                <input type="text" name="" style=" margin-left:10px;" placeholder="--><?//=__('采购子账号')?><!--" value="">-->
...
<!--              002-->
                ...
                <!--E  未付款订单 -->
                    ...
                    <?php if($val['order_status'] == Order_StateModel::ORDER_FINISH && $val['order_return_num'] <= 0 && !$recycle): ?>
                            <?php if(!$val['order_buyer_evaluation_status']): ?>
                                    <?php if($val['goods_list'][0]['evaluation_count'] ==2){ ?>
                                        <p> <a target="_blank" href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Order&met=evaluation" class="to_views"><i class="iconfont icon-woyaopingjia icon_size22"></i><?=__('查看评价')?></a></p>
                                    <?php }else{ ?>
