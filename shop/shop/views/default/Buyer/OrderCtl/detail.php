                            ...
                            <?php foreach ($data['code_list'] as $codekey => $codeval): ?>
                                <p>
                                    ...
                                    <em <?php if ($codeval['virtual_code_status'] != Order_GoodsVirtualCodeModel::VIRTUAL_CODE_USED) { ?> class="cyellowl" <?php } ?>>
                                        <?php
                                            switch ($codeval['virtual_code_status']) {
                                                case Order_GoodsVirtualCodeModel::VIRTUAL_CODE_USED:
                                                    $code_status = __("已使用，使用日期：") . $codeval['virtual_code_usetime'];
                                                    break;
                                                case Order_GoodsVirtualCodeModel::VIRTUAL_CODE_FROZEN:
                                                    $code_status = __("已冻结，冻结日期：") . $codeval['virtual_code_usetime'];
                                                    break;
                                                case Order_GoodsVirtualCodeModel::VIRTUAL_CODE_INVALID:
                                                    $code_status = __("已失效，失效日期：") . $codeval['virtual_code_usetime'];
                                                    break;
                                                default:
                                                    if ($codeval['common_virtual_date'] >= date("Y-m-d")) {
                                                        $code_status = __("未使用，有效期至：") . $codeval['common_virtual_date'];
                                                    }else{
                                                        $code_status = __("已过期，有效期至：") . $codeval['common_virtual_date'];
                                                    }
                                            }

                                        ?>
                                        <time><?= $code_status; ?></time>
                                        <?php if ($data['buyer_user_id'] == Perm::$userId && $codeval['common_virtual_refund'] && date("Y-m-d") > $codeval['common_virtual_date']) { ?>
                                            <a target="_blank" style="padding: 0 3px;margin-left: 230px;" class="bbc_btns" href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Service_Return&met=index&act=add&oid=<?= ($data['goods_list'][0]['order_id']) ?>&gid=<?= ($data['goods_list'][0]['order_goods_id']) ?>"><?= __('过期退款') ?></a>
                                        <?php } ?>
                                    </em>
                                </p>
                            <?php endforeach; ?>
                        ...
                        <?php foreach ($data['goods_list'] as $ogkey => $ogval) { ?>
                            <tr class="tr_con">
                                <td class="order_goods">
                                    ...
                                    <?php if (isset($ogval['spec_text']) && $ogval['spec_text']) { ?>
                                        <!--规格-->
                                        <dd class="table-dd-set tl"
                                            title="<?= $ogval['spec_text'] ?>">
                                            <em class="one-overflow iblock order-props"><?= $ogval['spec_text'] ?></em>
                                        </dd>
                                    <?php } ?>
                                ...
                                <td class="td_color widt4">
                                    ...
                                    <?php if ($ogval['goods_return_status'] != Order_StateModel::ORDER_REFUND_NO) { ?>
                                        <a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Service_Return&met=index&act=detail&id=<?= ($ogval['order_return_id']) ?>"><?= $ogval['goods_virtual_return_status_con'] ?></a>
                                    <?php } ?>
                                        <p>
                ...
                <td class="td_rborder td_rborder_reset">
                    ...
                    <?php } ?>
                     <?php if(($data['order_status'] == Order_StateModel::ORDER_CANCEL || $data['order_status'] == Order_StateModel::ORDER_FINISH) && $data['order_buyer_hidden']== Order_BaseModel::IS_BUYER_HIDDEN) :?>
                        <p>
                            <a onclick="restoreOrder('<?= $data['order_id'] ?>')">
                  ...
                  <?php endif; ?>
                    ...
                    <?php endif; ?>
                    <!--E  未付款订单 -->
                    <?php if ($data['buyer_user_id'] == Perm::$userId && $data['order_buyer_hidden'] != Order_BaseModel::IS_BUYER_HIDDEN) { ?>
                        <?php if ($data['order_status'] == Order_StateModel::ORDER_FINISH) { ?>
