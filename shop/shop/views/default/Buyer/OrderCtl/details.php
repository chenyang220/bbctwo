        ...
        <div class="ncm-order-info">
            <div class="ncm-order-details">
                <div class="content">
                    ...
                    <dl class="line">
                        <dt><?=__('支付方式')?>：</dt>
                        <?php if($data['order_is_bargain'] == 1 && $data['order_payment_amount'] <= 0){?>
                            <dd><?= __('砍价免费拿') ?></dd>
                        <?php }else{ ?>
                    ...
                    <dl class="line line2 pt10">
                        <dt><?=__('订单编号')?>：</dt>
                        <dd><?=($data['order_id'])?><a class="more-drap"><?=__('更多')?><i class="iconfont icon-iconjiantouxia"></i>
        ...
        <table class="tboy-reset">
            ...
            <tr>
                            ...
                            <tr class="tr_con">
                                <td class="order_goods">
                                    ...
                                    <?php if(isset($ogval['order_spec_info']) && $ogval['order_spec_info']){ ?>
                                        <dd class="table-dd-set tl"><strong class="top" ><?=__('规格')?>：</strong>&nbsp;&nbsp;<em class="order-props one-overflow iblock"><?= $ogval['order_spec_info'][0].','.$ogval['order_spec_info'][1]; ?></em></dd>
                                    <?php }?>
                                ...
                                <td class="td_color widt4">
                                    ...
                                    <?php if($data['buyer_user_id'] == Perm::$userId){ ?>
                                        <p>
                                            <?php if((($data['order_status'] == Order_StateModel::ORDER_FINISH && $data['complain_status']) || $data['order_status'] != Order_StateModel::ORDER_CANCEL)&&$data['order_buyer_hidden']!= Order_BaseModel::IS_BUYER_REMOVE  && $data['complaint_status'] != 2){?>
                ...
                <td class="td_rborder td_rborder_reset">
                    ...
                    <!-- 货到付款，待发货状态显示取消订单-->
                    <?php if($data['payment_id'] == PaymentChannlModel::PAY_CONFIRM && $data['order_status'] == Order_StateModel::ORDER_WAIT_PREPARE_GOODS){?>
                    <?php if($data['order_is_bargain'] != 1){?>
                    ...
                    <?php if($data['buyer_user_id'] == Perm::$userId && $data['order_buyer_hidden'] != Order_BaseModel::IS_BUYER_HIDDEN){ ?>
                        ...
                        <?php if($data['order_status'] == Order_StateModel::ORDER_FINISH && ($data['order_nums']*1 > $data['order_refund_nums']*1)): ?>
