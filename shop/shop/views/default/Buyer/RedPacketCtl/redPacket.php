...
?>
    ...
    <div class="aright">
        ...
        <div class="div_head tabmenu clearfix">
            ...
            <a class="btn-operate" onclick="delRedpacket()" href="javascript:void(0)">清空失效红包</a>
        </div>
    ...
    <script>
        window.delRedpacket = function () {
            $.dialog({
                title: __('系统提示'),
                content: __('您确定要清除失效红包吗？'),
                icon: 'confirm.gif',
                height: 100,
                width: 210,
                lock: true,
                drag: false,
                ok: function () {
                    $.post(SITE_URL + '?ctl=Buyer_RedPacket&met=delRedpacket&typ=json', {}, function (data) {
                        if (data && 200 == data.status) {
                            Public.tips.success(__('清空成功！'));
                            setTimeout(function () {
                                window.location.reload();
                            }, 1500)
                        } else {
                            Public.tips.error(__('清除失败！暂无失效红包'));
