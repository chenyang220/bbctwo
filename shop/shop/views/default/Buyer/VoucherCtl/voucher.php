...
?>
    <a class="btn-operate" onclick="delVouchers()" href="javascript:void(0)">清空失效代金券</a>
    </div>
    ...
    <table class="ncm-default-table annoc_con">
        ...
        <?php if (!empty($data['items'])) { ?>
            <?php foreach ($data['items'] as $key => $val) { ?>
                <tr class="bd-line">
                    <td><?= $val['voucher_shop_name'] ?></td>
                    <td><?= $val['voucher_title'] ?></td>
...
<?php } ?>
    ...
    <script>
        ...
        window.delVouchers = function () {
            $.dialog({
                title: '系统提示',
                content: '您确定要清空失效代金券吗？',
                icon: 'confirm.gif',
                height: 100,
                width: 210,
                lock: true,
                drag: false,
                ok: function () {
                    $.post(SITE_URL + '?ctl=Buyer_Voucher&met=delVouchers&typ=json', {}, function (data) {
                        if (data && 200 == data.status) {
                            Public.tips.success('清除成功！');
                            setTimeout(function () {
                                window.location.reload();
                            }, 1500)
                        } else {
                            Public.tips.error('清除失败！暂无失效的代金券');
                        }
                    })
                }
            })
