                        ...
                        <div class="content" style="overflow: auto">
                            <table class="stock-table">
                                ...
                                <tr>
                                    ...
                                    ?>
                                    <th>-</th>
                                    <th class="w100 tc"><?=__('商家货号')?></th>
                                    <th class="w100 tc"><?=__('价格')?></th>
                                ...
                                ?>
                                        <tr>
                                            ...
                                            <td class="tc">-</td>
                                            <td class="tc"><?=$value['goods_code']?></td>
                                            <td class="tc"><?=format_money($value['goods_price'])?></td>
                                            <td><input type="text" class="text w40 stock-verify" name="stock[<?=$value['goods_id']?>]" id="stock" value="<?=$value['goods_stock']?>"></td>
...
<script>
    ...
    // function stock_verify(e){
    //     var stock = $(e).val();
    //     alert(stock);
    //     var reg = /^[0-9]{1,8}$/;
    //     if(!reg.test(stock)){
    //         return '库存数量最大不超过99999999';
    //     }
    // }
    ...
    $(document).ready(function(){
        ...
        $('#form').validator({
            ...
            valid:function(form){
                ...
                var stock_val = $('.stock-verify');
                console.log(stock_val);
                var reg = /^[0-9]{1,8}$/;
                for(var i=0; i<stock_val.length; i++){
                    if(!reg.test(stock_val[i].value)){
                        parent.Public.tips.error("<?=__('库存数量最大不超过99999999！')?>");
                        return false;
                    }
                }
