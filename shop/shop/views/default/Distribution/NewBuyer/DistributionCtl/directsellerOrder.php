	...
	<div class="member_infor_content">
		<div class="div_head tabmenu clearfix">
			<ul class="tab clearfix">
				<?php if($userList['distributor_type']==1){ ?>
					<li><a href="<?= Yf_Registry::get('url') ?>?ctl=Distribution_NewBuyer_Goods&met=index"><?=__('商品列表')?></a></li>
				<?php } ?>
				<li><a href="<?= Yf_Registry::get('url') ?>?ctl=Distribution_NewBuyer_Distribution&met=directsellerList"><?=__('我的推广')?></a></li>
				<li class="active"><a href="<?= Yf_Registry::get('url') ?>?ctl=Distribution_NewBuyer_Distribution&met=directsellerOrder"><?=__('我的业绩')?></a></li>
				<li><a href="<?= Yf_Registry::get('url') ?>?ctl=Distribution_NewBuyer_Distribution&met=distributionCommission"><?=__('佣金记录')?></a></li>
			</ul>
		</div>
        <div class="order_content">
			<div class="order_content_title clearfix">
				<form method="get" id="search_form" action="index.php" >
					...
					<p class="order_types">
						<a <?php if($status == ''):?>class="currect"<?php endif;?> href="<?= Yf_Registry::get('url') ?>?ctl=Distribution_NewBuyer_Distribution&met=directsellerOrder"><?=__('全部订单')?></a>
						<a <?php if($status == 'wait'):?>class="currect"<?php endif;?> href="<?= Yf_Registry::get('url') ?>?ctl=Distribution_NewBuyer_Distribution&met=directsellerOrder&status=wait"><?=__('待付款')?></a>
						<a <?php if($status == 'already'):?>class="currect"<?php endif;?> href="<?= Yf_Registry::get('url') ?>?ctl=Distribution_NewBuyer_Distribution&met=directsellerOrder&status=already"><?=__('待收货')?></a>
						<a <?php if($status == 'finish'):?>class="currect"<?php endif;?> href="<?= Yf_Registry::get('url') ?>?ctl=Distribution_NewBuyer_Distribution&met=directsellerOrder&status=finish"><?=__('已完成')?></a>
						<a <?php if($status == 'cancel'):?>class="currect"<?php endif;?> href="<?= Yf_Registry::get('url') ?>?ctl=Distribution_NewBuyer_Distribution&met=directsellerOrder&status=cancel"><?=__('已取消')?></a>
					</p>
			...
			<table>
              ...
              <?php if($data['direct']['items'] || $data['indirect']['items']){?>
              <?php foreach($data['direct']['items'] as $key => $val):?>
				<tbody class="tboy">
				<!-- 下单时间，订单号，店铺名称    -->
					<tr class="tr_title">
				...
				<?php endforeach;?>
				<?php foreach($data['indirect']['items'] as $key => $val):?>
				<tbody class="tboy">
				<!-- 下单时间，订单号，店铺名称    -->
					<tr class="tr_title">
						<th colspan="7" class="order_mess clearfix">
