        ...
        <div class="member_infor_content">
			<div class="div_head tabmenu clearfix">
				<ul class="tab clearfix">
					<li class="active"><a href="<?= Yf_Registry::get('url') ?>?ctl=Distribution_NewBuyer_Goods&met=index"><?=__('商品列表')?></a></li>
					<li><a href="<?= Yf_Registry::get('url') ?>?ctl=Distribution_NewBuyer_Distribution&met=directsellerList"><?=__('我的推广')?></a></li>
					<li><a href="<?= Yf_Registry::get('url') ?>?ctl=Distribution_NewBuyer_Distribution&met=directsellerOrder"><?=__('我的业绩')?></a></li>
				...
				<li>
					<div class="operation">
						<form id="set_name" method="post" action="">
							<div class="search-form">
								<div class="normal">
									<dt><?=__('小店名称：')?></dt>
									<dd>
										<input type="text" class="text w300" name="user_directseller_shop" id="user_directseller_shop" value="<?=$shop['distribution_name']?>">
						...
						<div class="normal" style="height:110px;margin-top:10px;">
							<dt><?=__('小店二维码：')?></dt>
							<dd><img width="110" src='<?php echo Yf_Registry::get('base_url')."/shop/api/qrcode.php?data=".urlencode(Yf_Registry::get('shop_wap_url')."/tmpl/member/distribution_shop_detail.html?sid=".$shop['distribution_shop_id'])?>'></dd>
						</div>
					</div>
				</li>
			</ul>
			...
			<div class="order_content_title clearfix">
				<div style="margin-top: 10px;" class="clearfix">
					<form id="search_form" method="get">
						<input name="ctl" value="Distribution_NewBuyer_Goods" type="hidden">
			...
			<table class="ncm-default-table annoc_con icos">
				...
				<tbody>
					...
					<tr class="bd-line">
						...
						<td>
							<?=__('普通一级比例：')?><?=$val['common_c_first'].'%'?><br />
							<?=__('普通二级比例：')?><?=$val['common_c_second'].'%'?><br />
							<?=__('高级一级比例：')?><?=$val['common_a_first'].'%'?><br />
							<?=__('高级二级比例：')?><?=$val['common_a_second'].'%'?><br />
						</td>
						<td>
							<?=__('一级分佣：')?><?=format_money($val['common_a_first']*$val['common_price']/100)?><br />
							<?=__('二级分佣：')?><?=format_money($val['common_a_second']*$val['common_price']/100)?><br />
						</td>
		...
		$(document).ready(function(){
            $('#set_name').validator({
				...
				valid: function(form)
				{
					...
					$.ajax({
						url: "index.php?ctl=Distribution_NewBuyer_Goods&met=setShopName&typ=json",
						data: $(form).serialize(),
						type: "POST",
						success:function(e){
							if(e.status == 200)
							{
								...
								location.href="index.php?ctl=Distribution_NewBuyer_Goods&met=index&typ=e";//成功后跳转
							}
							else
							{
