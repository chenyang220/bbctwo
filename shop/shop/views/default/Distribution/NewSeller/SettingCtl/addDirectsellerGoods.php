...
<div class="form-style">
    <form id="form" action="" method="post" onSubmit="return checkPrice();">
        <dl>
            <dt><i class="required">*</i><?=__('分销客一级')?>：</dt>
            <dd>
                <input id="common_c_first" name="common_c_first" class="text w150" value="" type="text">
                <span> % </span>
                <p class="hint"><?=__('设置参与分佣商品的分销客一级分佣比例')?>。</p>
            </dd>
            <dt><i class="required">*</i><?=__('分销客二级')?>：</dt>
            <dd>
                <input id="common_c_second" name="common_c_second" class="text w150" value="" type="text">
                <span> % </span>
                <p class="hint"><?=__('设置参与分佣商品的分销客二级分佣比例')?>。</p>
            </dd>
        </dl>
        <dl>
            <dt><i class="required">*</i><?=__('分销掌柜一级')?>：</dt>
            <dd>
                <input id="common_a_first" name="common_a_first" class="text w150" value="" type="text">
                <span> % </span>
                <p class="hint"><?=__('设置参与分佣商品的分销掌柜一级分佣比例')?>。</p>
            </dd>
            <dt><i class="required">*</i><?=__('分销掌柜二级')?>：</dt>
            <dd>
                <input id="common_a_second" name="common_a_second" class="text w150" value="" type="text">
                <span> % </span>
                <p class="hint"><?=__('设置参与分佣商品的分销掌柜二级分佣比例')?>。</p>
            </dd>
        </dl>
        <!--参加分佣的商品--start-->
        <dl>
...
$(function(){
    ...
    $('#form').validator({
        ...
        valid: function(form){
            ...
            $.ajax({
                url: SITE_URL  + "?ctl=Distribution_NewSeller_Setting&met=addGoods&typ=json",
                data: $(form).serialize(),
                async:'false',
                type: "POST",
                success:function(e){
                    if(e.status == 200)
                    {
                        ...
                        setTimeout(window.location.href='index.php?ctl=Distribution_NewSeller_Setting&met=directsellerGoods&typ=e',5000);
                    }
                    else
                    {
