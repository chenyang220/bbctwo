		...
		<form>
			<div class="filter-groups">
				<dl>
					<dt><?=__('下单时间：')?></dt>
					<dd>
						<input type="text" class="text w60 hasDatepicker" placeholder="<?=__('起始时间')?>" name="start_date" id="start_date" value="<?php if (request_string('start_date')) {
					...
					<input id="end_date" class="text w60 hasDatepicker" placeholder="<?=__('结束时间')?>" type="text" name="end_date" value="<?php if (request_string('end_date')) {
				...
				<dl>
					<dt><?=__('买家昵称：')?></dt>
					<dd>
						<input type="text" class="text wp100" placeholder="<?=__('买家昵称')?>" id="buyer_name" name="buyer_name" value="<?php if (request_string('buyer_name')) {
			...
			<div class="control-group">
				...
				<span class="search-filter-prev">
					<input type="checkbox" id="skip_off" value="1" <?php if (request_string('skip_off')) {
					echo 'checked';
...
<table class="ncsc-default-table order ncsc-default-table2">
	...
	<tr>
		...
		<!-- S 合并TD -->
		<?php if ( $k == 0 ) { ?>
		<td class="bdl" rowspan="<?= count($val['goods_list']); ?>">
		...
		<td class="bdl" rowspan="<?= count($val['goods_list']); ?>" style="width: 126px;">
		...
		<td class="bdl bdr" rowspan="<?= count($val['goods_list']); ?>">
...
<script>
	...
	function formSub(){
		$('.btn_search_goods').parents('form').submit();
	}
</script>