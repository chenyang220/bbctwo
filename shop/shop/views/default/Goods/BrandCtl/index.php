        ...
        <div class="floorLoadingj_BrandFloor"  style="background: none; padding-top:10px">
            <div class="brandFloor">
                <h3 class="ui-title"> <em class="ui-title-num"><?=$i ?></em>
                    <p> <a href="index.php?ctl=Goods_Goods&met=goodslist&cat_id=<?=$value['cat_id']; ?>" > <b class="ui-title-text"><?= __($value['cat_name']); ?></b> <i class=" iconfont icon-youyuan"></i> </a> </p>
                </h3>
            </div>
                    ...
                    <ul class="brandFloor-list clearfix" >
                        ...
                        <li class="bFl-item">
                            <div class="bFl-item-slide j_FloorSlide">
                                <div class="bFlis-hd"> <span style><?= __($values['cat_name']); ?></span>
                            ...
                            <ul class="bFl-item-logo clearfix">
                                ...
                                <?php foreach($values['brand'] as $k=>$v){ ?>
                                    <?php if($k%2==0){ ?>
                                <li class="brand_all_sel">
                                    <a class="bFlil-link" target="_blank" href="index.php?ctl=Goods_Goods&met=goodslist&brand_id=<?=$v['brand_id']; ?>" title="<?=__($v['brand_name']); ?>">
                                        <span class="bFlil-img">
                                            <img  width="90" alt="<?= __($v['brand_name']); ?>" src="<?=$v['brand_pic'] ?>">
                                        </span>
                                    </a>
                                </li>
                                    <?php }else{?>
                        ...
                        <ol class="bFt-list j_FloorTop">
                            <?php if(!empty($data_rank[$key])): foreach($data_rank[$key] as $ke=>$val): ?>
                                <li class="<?php if($ke == 0){ ?> bFlil-expand <?php } ?>"> <i class="bFt-list-num"><?=($ke+1) ?></i>
                                    <p class="bFt-list-name"><a href="index.php?ctl=Goods_Brand&met=brandList&brand_id=<?=$val['brand_id']; ?>" target="_blank"><?= __($val['brand_name']); ?></a></p>
