...
?>
    ...
    <div class="wrap">
        <div class="crumbs clearfix">
            <p>
                <a href="<?= Yf_Registry::get('url') ?>"><?= __('首页') ?></a> <i class="iconfont icon-iconjiantouyou"></i> <a href="<?= Yf_Registry::get('url') ?>?ctl=GroupBuy&met=index"><?= __('团购中心') ?></a> <i class="iconfont icon-iconjiantouyou"></i> <a href="<?= Yf_Registry::get('url') ?>?ctl=GroupBuy&met=groupBuyList"><?= __('线上团') ?></a>
                            ...
                            <dl>
                                ...
                                foreach (@$data['groupbuy_cat'][request_int('cat_id')]['scat'] as $skey => $scat) {
                                    ...
                                    <dd <?php if (!(request_int('s_cat_id')) && request_int('s_cat_id') == $scat['groupbuy_cat_id']){ ?>class="hova"<?php } ?>>
                                        <a onclick="cat(<?= ($scat['groupbuy_cat_id']) ?>)"><?= __($scat['groupbuy_cat_name']); ?></a>
                                    </dd>
                                    <?php
                                }
                                ?>
                    ...
                    <?php if (@$data['price_range']) { ?>
                        <dl>
                            ...
                            <?php foreach ($data['price_range'] as $key => $price) {
                                ?>
                                <dd <?php if (request_int('price') == $price['range_id']){ ?>class="hova"<?php } ?>><a href="javascript:void(0);" onclick="price(<?= $price['range_id'] ?>)"><?= __($price['range_name']); ?></a></dd>
                                <?php
                            }
                            ?>
                        </dl>
                            ...
                            foreach ($data['groupbuy_goods']['items'] as $key => $groupbuy_goods) {
                                ...
                                <li class="online">
                                    <div class="ncg-list-content">
                                        <a title="<?= $groupbuy_goods['groupbuy_name'] ?>" href="<?= Yf_Registry::get('url') ?>?ctl=Goods_Goods&met=goods&type=goods&gid=<?= $groupbuy_goods['goods_id'] ?>" class="pic-thumb" target="_blank"> <img src="<?= image_thumb($groupbuy_goods['groupbuy_image'], 260, 193) ?>" alt=""> </a>
                                        <h3 class="title">
                                            <a title="<?= $groupbuy_goods['groupbuy_name'] ?>" href="" target="_blank"><?= $groupbuy_goods['groupbuy_name'] ?></a>
                                        </h3>
                                        <div class="item-prices">
                                            <span class="price" data-price="<?= $groupbuy_goods['groupbuy_price'] ?>"><?= format_money($groupbuy_goods['groupbuy_price']) ?></span>
                                            <div class="dock">
                                                <del class="orig-price"><?= format_money($groupbuy_goods['common_price']) ?></del>
                                            </div>
                                            ...
                                            <?php } else { ?>
                                                ...
                                                <a href="<?= Yf_Registry::get('url') ?>?ctl=Goods_Goods&met=goods&type=goods&gid=<?= $groupbuy_goods['goods_id'] ?>" target="_self" class="buy-button bbc_btns">
                                                    <?php if ($groupbuy_goods['common_stock'] > 0) { ?>
                                                        <?= __('我要团') ?>
                                                    <?php } else { ?>
                                                        <?= __('已抢光') ?>
                                                    <?php } ?>
