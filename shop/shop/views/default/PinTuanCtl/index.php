            ...
            <div class="tg tg_left">
                ...
                <p> 
                    <?php if ($data['category']) { ?>
                        <?php foreach ($data['category'] as $key => $cat) { ?>
                            <a class="one-overflow" href="<?= Yf_Registry::get('url') ?>?ctl=PinTuan&met=index&cat_id=<?= $cat['cat_id'] ?>"><?= __($cat['cat_name']); ?></a>
                        <?php } ?>
                    <?php } ?>
                </p>
            </div>
                        ...
                        <div class="gr_goods_imfor">
                            <div class="gr_goods_div">
                                ...
                                <div class="gr_good_price clearfix">
                                    <span class="bbc_color"><?= format_money($recommend['detail']['price']) ?><em><?= format_money($recommend['detail']['price_ori']) ?></em></span> <a class="bbc_btns" href="javascript:void(0);"><?= __('立即拼团') ?><i class="iconfont icon-iconjiantouyou"></i></a>
                                </div>
                            ...
                            </span> <em><strong class="bbc_color"><?= $recommend['detail']['buyer_num'] ?></strong> <?= __('人已参团') ?></em>
                                </div>
                            </div>
                            <p class="hotbg"></p>
                        </div>
            ...
            <?php if ($data['goods']) { ?>
                <ul class="gr_goods_list clearfix pt_code_area">
                    <?php foreach ($data['goods'] as $value) { ?>
                        <li>
                            <a href="javascript:void(0);"><?= __($value['goods_name']); ?><em class="pt_goods_img"><b><img src="<?= $value['goods']['goods_image'] ?>"/></b></em></a>
                            <!-- 拼团 -->
                            ...
                            <h5>
                                <a href="javascript:;"><strong class="bbc_color">[<?= $value['person_num'] ?><?= __('人团') ?>] </strong><?= __($value['goods']['goods_name']); ?></a>
                            </h5>
                            <h6><?= __('单买价：￥').$value['detail']['price_one'] ?></h6>
                            
                            <p class="gr_sold_hav clearfix"><span><?= __('已拼') ?><i class="num-color ml4"><?= $value['detail']['buyer_num'] ?></i><?= __('件') ?></span> <em class="fnTimeCountDown" data-end="<?= $value['end_time'] ?>"> <span class="day">00</span><strong><?= __('天') ?></strong> <span class="hour">00</span><strong><?= __('小时') ?></strong> <span class="mini">00</span><strong><?= __('分') ?></strong> <span class="sec">00</span><strong><?= __('秒') ?></strong> </em>
                            </p>
                        </li>
                        <?php
                    }
