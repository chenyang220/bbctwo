	...
	<div class="wrap">
		...
		?>
			<li class="<?php if($value['user_address_default'] == 1){?>add_choose<?php }?>" id="addr<?=($value['user_address_id'])?>">
                ...
                <input type="hidden" name="area_code" id="area_code" value="<?=($value['area_code'])?>">
                <h5><?=($value['user_address_contact'])?></h5>
				<p><?=($value['user_address_area'])?> <?=($value['user_address_address'])?></p>
				<div><span class="phone"><i class="iconfont icon-shouji fz22 align-middle"></i><span><?=($value['user_address_phone'])?></span></span></div>
			</li>
			<?php }}?>
		</div>
		...
		<div class="cart_goods">
            ...
            <div class="goods_remark clearfix">
                <p class="remarks"><span><?=__('备注')?>：</span><input name="remarks" type="text" placeholder="<?=__('补充填写其他信息,如有快递不到也请留言备注')?>" maxlength="45"></p>
            </div>
