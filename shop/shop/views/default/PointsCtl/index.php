	...
	<div class="bbc-base-layout">
		<div class="bbc-member-left">
            ...
            <div class="bbc-member-grade">
                ...
                <?php }else{ ?>
                    <div class="progress-bar">
                        <em title="V<?= $data['user_info']['user_grade'] - 1 ?><?= __('需经验值') ?><?= $data['growth']['grade_growth_start'] ?>">V<?= $data['user_info']['user_grade'] - 1 ?></em>
                        <span title="100%"><i class="bbc_bg" style="width:100%;"></i></span>
                        <em title="V<?= $data['user_info']['user_grade'] ?><?= __('需经验值') ?><?= $data['growth']['grade_growth_end'] ?>">V<?= $data['user_info']['user_grade'] ?></em>
                    </div>
                    <div class="progress"><?= __('已达到最高会员级别，继续加油保持这份荣誉哦！') ?></div>
                <?php }?>
            </div>
            ...
            <div class="bbc-memeber-pointcart">
             <a href="<?=Yf_Registry::get('url')?>?ctl=Points&met=pointsCart" class="btn bbc_bg_col"><?=__('礼品兑换购物车')?>
             	<?php if ($data['points_cart_num'] > 0) { ?>
