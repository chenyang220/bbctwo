<style>.hidden{height:26px;line-height:26px;padding:0 10px;border:1px solid #ccc;border-radius:2px;background:#eee;}
.rm_btn_status{    vertical-align: -1px;
    position: relative;
    top: 2px;}
</style>
    ...
    <?php
     ...
     ?>
        <li>
            ...
            <div class="goods-price"><?=__('商品价格')?>：<span><?=format_money($goods['common_price'])?></span></div>
            <div class="goods-btn">
                <div data-type="btn_add_act_goods"  data-id="<?=$goods['common_id']?>"  btn-enabled="<?=$goods['common_id']?>"  class="button button_green"><i class="iconfont icon-jia"></i><?=__('选择')?></div>
                <div data-id="<?=$goods['common_id']?>" href="javascript:void(0);" btn-disabled="<?=$goods['common_id']?>" class="hidden"><i class="iconfont icon-yes"></i><?=__('已选择')?></div>
            </div>
        </li>
            <?php 	}	?>
    </ul>
...
<script>
    ...
    $('.goods-btn').on('click','[data-type="btn_add_act_goods"]',function()
	{
		...
		//$("div[btn-disabled='"+id+"']").removeAttr("class","hidden");
		//$("div[btn-disabled='"+id+"']").attr("class","button") 
		$("div[btn-disabled='"+id+"']").removeAttr("class","hidden");
		$("div[btn-disabled='"+id+"']").attr("class","button rm_btn_status"); 
