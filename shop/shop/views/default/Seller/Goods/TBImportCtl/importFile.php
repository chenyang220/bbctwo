    ...
    <dl>
        ...
        <dd>
            <div class="handle">
                ...
                <div><a href="../importGoods.xls" style="color: #0099ff;font-size: 14px;">下载空白xls文件</a></div>
            </div>
        </dd>
    </dl>
    <dl>
        <dt><i class="required">*</i><?= __('商品分类：')?></dt>
        <dd id="gcategory">
            <span nctype="gc1">
            ...
            <select name="sgcate_id[]" class="sgcategory">
                ...
                <?php if (!empty($shop_goods_cat_rows)){ ?>
                    <?php foreach ($shop_goods_cat_rows as $shop_goods_cat_id => $shop_goods_cat_data) { ?>
                        <option data-parent_id="<?= $shop_goods_cat_data['parent_id'] ?>" value="<?= $shop_goods_cat_id ?>"><?= __($shop_goods_cat_data['shop_goods_cat_name']) ?></option>
                    <?php } ?>
                <?php } else { ?>
                    <option value="-1"><?=__('暂无分类')?>...</option>
                <?php } ?>
            </select>
            <p class="hint"><?=__('商品可以从属于店铺的多个分类之下')?>，<?= __('店铺分类可以由 "商家管理中心 -> 店铺 -> 店铺分类" 中自定义')?></p>
        </dd>
    </dl>
    <dl>
        <dt><?=__('字符编码')?>：</dt>
...
<script>
    $(function() {
        ...
        $("span[nctype=gc1], span[nctype=gc2], span[nctype=gc3], span[nctype=gc4]").on('change', 'select', function (){
            ...
            var cat_name = $('#cat_name').val() + gt + cat_name; 
        ...
        function getCatList(cat_id, $select, change) {
            ...
            $.get(SITE_URL + '?ctl=Seller_Goods_Cat&met=cat&typ=json', {cat_id: cat_id}, function(data){
                ...
                if (data.status == 200) {
                    ...
                    if (change) {
                        option_rows.push("<option value='0'><?= __('请选择')?></option>");
                    }
        ...
        function submit(file_path) {
            ...
            /*$.post(SITE_URL + "?ctl=Seller_Goods_TBImport&met=addGoods&typ=json", param, function( data ) {
                 if (data.status == 200) {
                    Public.tips.success("<?=__('上传成功')?>！");
                 }else{
                     Public.tips.error(data.msg);
                 }
            })*/
			$(".bbc_seller_btns").hide();
			$("#import_txt").html("<span style='color:blue;font-size:22px;'>正在执行导入，请耐心等候......</span>");
            $.post(SITE_URL + "?ctl=Seller_Goods_TBImport&met=addGoods&typ=json", param, function( data ) {
                if (data.status == 200) {
                    Public.tips.success("<?=__('商品导入成功')?>！处理商品总耗时："+data.data.total_time+"秒！");
					$("#import_txt").html("<span style='color:green;font-size:22px;'>商品导入成功,处理商品总耗时："+data.data.total_time+"秒！</span>");
				}else{
                    Public.tips.error(data.msg);
					$("#import_txt").html("<span style='color:red;font-size:22px;'>商品导入失败!</span>");
                }
				setTimeout(function(){ 
