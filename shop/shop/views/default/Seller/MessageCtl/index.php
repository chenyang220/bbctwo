...
<div class="form-style cs">
    ...
    <form action="" method="post" id="form">
        ...
        <!-- YF_IM 设置客服入口 END -->
        <!-- 老版本/定制项目 请自行删除以下设置项中的IM选项 -->
        <dl data-type="pre">
            <dt>售前客服：</dt>
            <dd>
                <div class="mt">
                    <span class="name">客服名称</span> <span class="tool">客服工具</span> <span class="number">客服账号</span>
                </div>
                <?php if (!empty($data['pre'])) { ?>
                    <?php foreach ($data['pre'] as $key => $val) { ?>
                        <div class="mc clearfix">
                            <span class="name"><input type="text" maxlength="10" name="prename[<?= $key ?>]" value="<?= $val['name'] ?>" class="text w60"></span> <span class="tool">
                ...
                </span> <span class="number"><input type="text" name="prenum[<?= $key ?>]" class="text" value="<?= $val['number'] ?>"></span> <span class="op"><a data-param="{'id':'<?= $val['id'] ?>'}" href="javascript:void(0);" data-type="del"><i class="iconfont icon-lajitong"></i>删除</a></span>
                        </div>
                    <?php } ?>
                <?php } else { ?>
                    <div class="mc clearfix">
                ...
                </span> <span class="op"><a href="javascript:void(0);" data-type="del"><i class="iconfont icon-lajitong"></i>删除</a></span>
                    </div>
                <?php } ?>
            ...
            <dd>
                ...
                <?php if (!empty($data['after'])) { ?>
                    <?php foreach ($data['after'] as $key => $val) { ?>
                        <div class="mc clearfix">
                <span class="name">
                <input type="text" maxlength="10" name="aftername[<?= $key ?>]" value="<?= $val['name'] ?>" class="text w60"></span> <span class="tool">
                ...
                </span> <span class="op"><a data-param="{'id':'<?= $val['id'] ?>'}" href="javascript:void(0);" data-type="del"><i class="iconfont icon-lajitong"></i>删除</a></span>
                        </div>
                    <?php } ?>
                <?php } else { ?>
                    <div class="mc clearfix">
