...
<div class="form-style">
    <form method="post" id="form" action="">
        ...
        <dl>
            <dt><i>*</i><?=__('资讯标题')?>：</dt>
            <dd>
                <input type="text" name="news_title" id="news_title" class="text w450" value="<?=$data['title']?>" maxlength="20"/>
        ...
        <dl>
            <dt><?=__('资讯副标题')?>：</dt>
            <dd>
                <input type="text" name="news_subtitle" id="news_subtitle" class="text w450" value="<?=$data['subtitle']?>" maxlength="30"/>
        ...
        <dl>
            ...
            <dd>
                <select name="newsclass_type" id="newsclass_type" style="margin-right: 5px;">
                    ...
                    <option value="<?= $v['id'] ?>" <?= $v['id'] == $data['type'] ? 'selected':'' ?>><?= $v['newsclass_name']?></option>
                    <?php }?>
                </select>
            </dd>
        </dl>
        <dl class="row">
            ...
            <dd class="opt">
                <!-- 加载编辑器的容器 -->
                <textarea id="article_desc" style="width:700px;height:300px;" name="content" type="text/plain"><?= $data['content']?></textarea>
            </dd>
        </dl>
    ...
    $(document).ready(function () {
        $('#form').validator({
            ...
            valid: function (form) {
                ...
                $.ajax({
                    url: "index.php?ctl=Seller_Promotion_InformationNews&met=editInformationNews&typ=json",
                    data: params,
                    type: "POST",
                    success: function (e) {
                        console.log(e);
                        if (e.status == 200) {
                            ...
                            var dest_url = "index.php?ctl=Seller_Promotion_InformationNews&met=index&op=edit&typ=e";//成功后跳转
