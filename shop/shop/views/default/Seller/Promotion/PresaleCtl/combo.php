    ...
    <div class="form-style">
        <dl>
            ...
            <dd>
                ...
                <p class="hint"><?=__('购买单位为月(30天)，您可以在所购买的周期内发布预售活动')?>。</p>
                <p class="hint"><?=__('每月您需要支付')?><?=format_money(Web_ConfigModel::value('promotion_presale_price'))?>。</p>
    ...
    $(function(){
        $('#form').validator({
            ...
            valid: function(form){
                ...
                $.ajax({
                    url: "index.php?ctl=Seller_Promotion_Presale&met=addCombo&typ=json",
                    data: $(form).serialize(),
                    type: "POST",
                    success:function(e){
                        if(e.status == 200)
                        {
                            ...
                            setTimeout(window.location.href='index.php?ctl=Seller_Promotion_Presale&met=index&typ=e',5000);
                        }
                        else
                        {
