...
<div class="form-style">
    <form method="post" id="form" action="<?=Yf_Registry::get('url')?>?ctl=Seller_Seller_SellerUser&met=addLabels">
        <dl>
            ...
            <dd>
                <input type="text" name="label_name"  value="" class="text w200"/>
            </dd>
        </dl>
        <dl>
            <dt class="sale_width_reset"><i>*</i><?=__('标签排序')?>：</dt>
            <dd>
                <input type="text" name="label_sort"  value="" class="text w200"/>
            </dd>
        </dl>
        <dl>
            <dt class="sale_width_reset"><i>*</i><?=__('标签描述')?>：</dt>
            <dd>
                <textarea name="label_desc" class="sale_width_reset"></textarea>
            </dd>
        </dl>
        <dl>
            <dt><i>*</i><?=__('标签图片')?>：</dt>
            <dd>
                ...
                <p class="upload-button">
                    <input type="hidden" id="label_img" name="label_img" value="" />
    ...
    $(document).ready(function(){
        ...
        $('#form').validator({
            ...
            valid: function(form){
                ...
                {
                    $.ajax({
                        url: "index.php?ctl=Seller_Seller_SellerUser&met=addLabels&typ=json",
                        data: $(form).serialize(),
                        type: "POST",
                        success:function(e){
                            if(e.status == 200)
