...
<div class="eject_con">
    <div id="warning" class="alert alert-error"></div>
            <form id="form" action="#" method="post">
                ...
                <div class="bottom">
                    <label class="submit-border"><input type="submit" id="handle_submit" class="submit bbc_seller_btns w200 mt20" value="<?=__('回复')?>"/></label>
                </div>
            </form>
</div>
...
<script>
    $(document).ready(function ()
    {
        ...
        {
            var consult_answer = $('.textarea_text').val();
            if(consult_answer == '')
            {
                parent.Public.tips.error("<?=__('请填写内容')?>");
                return false;
            }
            ...
            $.ajax({
                ...
                success: function (a)
                {
                    ...
                    {
                        $.dialog().close();
                        parent.Public.tips.success("<?=__('回复成功')?>");
