...
?>
	...
	<ul>
    	<li class="active bbc_seller_bg"><a href="./index.php?ctl=Seller_Shop_Info&met=info&typ=e&act=category"><?=__('经营类目')?></a></li>
            ...
            <?php if(Yf_Registry::get('yunshanstatus') == 1) {?>
             <li><a href="./index.php?ctl=Seller_Shop_Info&met=info&typ=e&act=setpayconfig"><?=__('支付入网')?></a></li>
            <?php } ?>
    </ul>
    ...
    <?php } ?>
      ...
      <?php if($data['items']){
        ...
        <tr class="row_line">
            <td class="tlf" >
                 <?php foreach($value as $cat_keys =>$cat_val){ ?>
                    <div>
                    <?php  $i=0; foreach($cat_val as $keys =>$val){ if(!empty($val['cat_name'])){ ?>
                         <?php if($i >= 1){ ?>  
                            <i class="iconfont icon-iconjiantouyou" style="font-size: 12px;"></i>
                        <?php } ?>
                    <?=$val['cat_name']?><?php } $i++;}?>
                    </div>
                <?php } ?>
            ...
            <td>
                <?php foreach($data['items']['commission_rate'][$key] as $commission_rate_keys =>$commission_rate_val){ ?>
                   <div> 
                    <?=$commission_rate_val?>%
                    </div>
                <?php } ?>
            ...
            <?php if($data['items']['shop_class_bind_enable'][$key] == "0") { ?>
                    <td title="拒绝原因：<?= $data['items']['shop_class_bind_desc'][$key] ?>"><?= $data['items']['shop_class_bind_enablecha'][$key]?></td>
            <?php } else { ?>
