            ...
            <div id="apply_company_info" class="apply-company-info">
                ...
                <form id="form_company_info"  method="post">
                    <table class="all" border="0" cellpadding="0" cellspacing="0">
                        ...
                        <tr>
                            <th><i>*</i><?=__('手机号码')?>：</th>
                            <td><input name="contacts_phone" class="w200 bor-inp" type="tel" maxlength="20" id="re_user_mobile" value="<?php echo isset($shop_company['contacts_phone']) ? $shop_company['contacts_phone'] : '';?>" />
                                <input name="area_code"  type="hidden"  id="area_code" value="<?php echo isset($shop_company['area_code']) ? $shop_company['area_code'] : '';?>" />
                                <span></span></td>
                        </tr>
                        <tr>
                            <th><i>*</i><?=__('图形验证码')?>：</th>
                    ...
                    <table class="all" border="0" cellpadding="0" cellspacing="0">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <?php if($user_info['user_identity_end_time'] == 1){ ?>
                                    <input readonly="readonly" id="end_time" name="business_licence_end"  class="w90 hasDatepicker" type="text" value="长期"/>
                                <?php }else{?>
                ...
                <div class="bottom"><?php if(!isset($shop_company['shop_id'])){?><a href="<?= Yf_Registry::get('base_url')?>/index.php?ctl=Seller_Shop_Settled&met=index&op=step1&rp=step1&apply=1" class="btn bbc_btns"><?=__('上一步')?></a>&nbsp;&nbsp;&nbsp;<?php } ?><a id="btn_apply_company_next" href="javascript:;" class="btn button bbc_btns"><?=__('下一步，提交财务资质信息')?></a></div>
            </div>
...
//                    utilsScript: "<?//= $this->view->js ?>///utils.js"
                ...
                function getyzm(type){
                    ...
                    if(type === 'phone'){
                        ...
                        var val = obj.val();
                        var patrn = /^1\d{10}$/;
                        pars = 'mobile='+val+'&yzm='+img_yzm+'&area_code='+area_code;
                    }
                    if(type === 'email'){
                        var obj = $("#contacts_email");
                        var val = obj.val();
                        var patrn = /^[0-9a-zA-Z_.-]+@(([0-9a-zA-Z]+)[.])+[a-z]{2,4}$/;
                    ...
                    if(!val){
                        return false;
                    }else if(!patrn.test(val) && area_code == 86 && type === 'phone'){
                        alert("<?=__('手机号码有误')?>");
                        return false;
                    }else if(!patrn.test(val) && type === 'email'){
                        alert("<?=__('邮箱格式不正确！')?>");
                        return false;
                    }else{
