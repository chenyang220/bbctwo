...
foreach ($shop_company['shop_class'] as $keyss=>$vals){?>
    <input type="hidden" id="shop_class_deposit" value="<?php echo $vals['shop_class_deposit']?>">

<?php } ?>

<script type="text/javascript">
    var shop_class_deposit=  $("#shop_class_deposit").val();

    if(shop_class_deposit==0)
    {
        $.ajax({
            url: "./index.php?ctl=Seller_Shop_Settled&met=shopPayadd&typ=json&apply=<?=$apply?>&shop_id=<?=$shop_company['shop_id']?>",
            data:$("#form").serialize(),
            type: 'POST',
            success:function(a){
                if(a.status == 200)
...
<div class="main">
  ...
  <div class="right-layout">
    <div class="w fn-clear">
     ...
     <?php }else{ ?>
        <div class="joinin-concrete content-step5"style="padding:19px">
           ...
           <br/>
           <span style="color:red;font-size: 13px;"><?php echo isset($shop_company['shop_verify_reason']) && $shop_company['shop_status'] == 5 ? "拒绝原因：" . $shop_company['shop_verify_reason'] : '';?></span>
          </div>
    ...
    <table cellpadding="0" cellspacing="1" width="100%">
        ...
        <tr>
            <td><?=__('应付金额')?>：</td>
            <td colspan="3"><?=number_format($val['shop_grade_fee']*$shop_company['joinin_year']+$vals['shop_class_deposit'],2,'.','') ?><?=__('元')?></td>
        </tr>
            <?php }}}?>
    </tbody></table>

    <?php if(Web_ConfigModel::value('company_name') && Web_ConfigModel::value('bank_name') && Web_ConfigModel::value('bank_account')){ ?>
      <div>
          <h5>收款账户信息</h5>
          <table cellpadding="0" cellspacing="1" width="100%">
              <tbody>
                  <tr>
                      <td width="70">单位名称：</td>
                      <td><?= Web_ConfigModel::value('company_name') ?></td>
                  </tr>
                  <tr>
                      <td>开户行：</td>
                      <td><?= Web_ConfigModel::value('bank_name') ?></td>
                  </tr>
                  <tr>
                      <td>银行账号：</td>
                      <td><?= Web_ConfigModel::value('bank_account') ?></td>
                  </tr>
              </tbody>
          </table>
      </div>
    ...
    <div class="next"><a href="<?= Yf_Registry::get('base_url')?>/index.php?ctl=Seller_Shop_Settled&met=index&op=step4&rp=step4&apply=<?=$apply?>" style='display: none;' class="btn bbc_btns "><?=__('上一步')?></a>&nbsp;&nbsp;&nbsp;<a id="btn_apply_company_next" class="btn bbc_btns button" href="javascript:void(0);"><?=__('提交')?></a></div>
    </form>
</div>
</div>
     <?php } ?>
