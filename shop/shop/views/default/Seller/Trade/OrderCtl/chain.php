		...
		<form>
			<div class="filter-groups">
				...
				<dl>
					<dt><?=__('门店名称：')?></dt>
					<dd>
						<input type="text" class="text wp100" placeholder="<?= __('门店名称') ?>" id="chain_name" name="chain_name"
	...
	<tr>
		...
		<td class="bdl bdr" rowspan="<?= $val['goods_cat_num']; ?>">
            ...
            <?php if($val['payment_id']==1) {?>
                <p class="goods-pay bbc_color" title="<?=__('支付方式')?>：<?=__('在线支付')?>"><?=__('在线支付')?></p>
            <?php }else{?>
                <p class="goods-pay bbc_color" title="<?=__('支付方式')?>：<?=__('门店付款')?>"><?=__('门店付款')?></p>
            <?php }?>
			<!-- 订单查看 -->
