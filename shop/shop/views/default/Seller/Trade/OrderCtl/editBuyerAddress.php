        ...
        <div class="adds">
            ...
            <form id="address_form">
                ...
                <dl>
                    ...
                    <dd>
                        <input type="text" class="text wp100" name="order_receiver_contact" id="re_user_mobile" value="">
...
<script>
    ...
    $( function () {
        ...
        $('#re_user_mobile').val(address_data.order_receiver_contact);
        $('#area_code').val(address_data.area_code);
        ...
        $('#address_form').validator({
            ...
            rules: {
                tel:function(){
                    ...
                    var order_receiver_contact = $('#re_user_mobile').val();
                    order_receiver_contact = order_receiver_contact.replace(/\s+/g,"");
                    var reg = /^1[3-9]\d{9}$/;
                    if(area_code == 86 && !reg.test(order_receiver_contact)){
            ...
            fields: {
                ...
                'order_receiver_contact': 'required;tel',
                'select_1':  'required;',
                'select_2':  'required;',
                'select_3':  'required;',
            ...
            valid: function(form) {
                var data = {
                    ...
                    order_receiver_contact: $("#re_user_mobile").val(),
                ...
                $.post(parent.SITE_URL + '?ctl=Seller_Trade_Order&met=editBuyerAddress&typ=json',data, function (data) {
                    if ( data.status == 200 ) {
                        parent.Public.tips({content:"<?=__('操作成功！')?>", type:3});
                        callback(data.data);
                        api.close();
                    } else {
                        parent.Public.tips({content:"<?=__('操作失败！')?>", type:1});
                    }
                })
            }
        });
