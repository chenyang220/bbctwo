    ...
    <div class="">
        ...
        <form name="deliver_form" method="POST" id="deliver_form">
            ...
            <table class="ncsc-default-table order deliver">
                <tbody>
                <tr>
                    <td>全选<input type="checkbox" name="selectAll"></td>
                    <th colspan="20">
                ...
                <?php if (!empty($data['goods_list'])) { ?>
                    <?php foreach ($data['goods_list'] as $key => $val) { ?>
                        <tr>
                            <td>
                                <?php if($val['order_goods_is_receiving']==0 || $style == 'edit'){?>
                                    <input type="checkbox" name="selectOne" value="<?php echo $val['order_goods_id']?>">
                                <?php } ?>
                            </td>
                            <td class="bdl w10"></td>
                            <td class="w50">
                            ...
                            <td class="tl">
                                <dl class="goods-name">
                                    ...
                                    <?php if (isset($val['order_spec_info']) && $val['order_spec_info']) { ?>
                                        <dd><strong><?= __('规格') ?>：</strong>&nbsp;&nbsp;<em class="seller-props-limit one-overflow"><?= $val['order_spec_info']; ?></em></dd>
                                    <?php } ?>
                                    <?= $val['return_txt'] ?>
                                </dl>
                            </td>
                            <?php if ($key == 0) { ?>
                                <td class="bdl bdr order-info w500 sender-td" rowspan="<?= $data['goods_cat_num']; ?>">
                                    <dl>
                                        <dt><?= __('运费') ?>：</dt>
                        ...
                        <td colspan="20" class="tl bdl bdr" style="padding:0px">
                            <div class="fapiao-cons clearfix">
                                ...
                                <div class="right">
                                    ...
                                    <?php
                                        if($data['invoice']['invoice_title'] != '个人'){
                ...
                <tr>
                    <td colspan="20" class="tl bdl bdr" style="padding:8px" id="address">
                        ...
                        <input type="hidden" id="area_code" name="area_code" value="<?= $data['area_code']; ?>">
                        ...
                        <a href="javascript:void(0)" dialog_id="edit_buyer_address" data-order_receiver_name="<?= $data['order_receiver_name']; ?>" data-order_receiver_address="<?= $data['order_receiver_address']; ?>" data-order_receiver_contact="<?= $data['order_receiver_contact']; ?>" data-order_id="<?= $data['order_id']; ?>" data-area_code="<?= $data['area_code']; ?>" class="ncbtn-mini fr bbc_seller_btns"><i class="icon-edit"></i><?= __('编辑') ?></a>
                    </td>
                </tr>
                </tbody>
            </table>
            ...
            发货方式：<input type="radio" class="radio" checked value="0" name="send_type">物流方式
            <input type="radio" value="1" class="radio" style="margin-left: 3rem;" name="send_type">自主配送
            <div class="wuliu_send">
            ...
            <div class="radio_zizhu" style="display: none;">
                <table class="ncsc-default-table order" style="width: 40%!important;" id="texpress2">
                    <tbody>
                        <tr>
                            <td colspan="2"></td>
                        </tr>
                        <tr>
                            <td class="bdl" style="width: 151px;">联系人</td>
                            <td class="bdl bdl_input" style="width: 249px;"><input name="user_name" type="text" class="text w200 tip-r"></td>
                        </tr>
                        <tr>
                            <td class="bdl" style="width: 151px;">联系方式</td>
                            <td class="bdl bdl_input" style="width: 249px;"><input name="user_mobile" type="text" class="text w200 tip-r"></td>
                        </tr>
                        <tr>
                            <td class="bdl" style="width: 151px;">备注</td>
                            <td class="bdl bdl_input" style="width: 249px;">
                                <textarea name="remarks" class="text w200 tip-r" maxlength="50"></textarea>
                            </td>
                        </tr>
                        <tr>
                            <td style="border-bottom: 0px solid #e1e1e1;"></td>
                            <td style="border-bottom: 0px solid #e1e1e1;"><a nc_value="0" href="javascript:void(0);" class="ncbtn bbc_seller_btns btn-width"><?= __('确认') ?></a></td>
                        </tr>
                    </tbody>
                </table>
            </div>
...
<script>
    ...
    $(function () {
            ...
            $.dialog({
                ...
                data: {
                    ...
                    callback: function (data) {
                        ...
                        _this.data('area_code', data.area_code);
                    }
                }
            })
        });
        ...
        $('[name=selectAll]').on('change', function(){
            $('[name=selectOne]').prop('checked', $('[name=selectAll]').prop('checked'));
        });
        //提交表单
        $(document).on('click', 'a[nc_value]', function () {
            ...
            var workIds = [];  
            $('[name=selectOne]:checked').each(function(index, item){  
                workIds.push(item.getAttribute('value'));
            });
            var order_send_type = $('input:radio[name="send_type"]:checked').val();
            if(order_send_type){
                var $this = $(this),
                send_data = {
                    order_id: $('#order_id').val(),
                    ...
                    order_send_type: order_send_type ,
                    user_name: $('input[name="user_name"]').val(),
                    user_mobile: $('input[name="user_mobile"]').val(),
                    remarks: $('textarea[name="remarks"]').val(),
                    selectOne: workIds.join(",")
                };
            }else{
                var $this = $(this),
                send_data = {
