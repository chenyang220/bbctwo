		...
		<div class="filter-groups">
			<dl>
				...
				<dd style="width: 250px;">
					<input type="text" class="text w85 hasDatepicker heigh" placeholder="<?=__('起始时间')?>" name="query_start_date" id="query_start_date" value="<?php if (!empty($condition['order_create_time:>='])) {
					...
					<input id="query_end_date" class="text w85 hasDatepicker heigh" placeholder="<?=__('结束时间')?>" type="text" name="query_end_date" value="<?php if (!empty($condition['order_create_time:<='])) {
			...
			<dl>
				<dt><?=__('买家昵称：')?></dt>
				<dd><input type="text" class="text wp100" placeholder="<?=__('买家昵称')?>" id="buyer_name" name="buyer_name" value="<?php if (!empty($condition['buyer_user_name:LIKE'])) {
...
<table class="table-list-style order-list-style ncsc-default-table2 ncsc-default-table order" width="100%" cellpadding="0" cellspacing="0">
	...
	<tr>
		<td class="bdl">
			<dl class="fn-clear">
				...
				<dd>
                    ...
                    <?php if($goods_list['order_goods_benefit']){?><em class="td_sale bbc_btns small_details"><?=($goods_list['order_goods_benefit'])?></em><?php }?>
                    <!--规格-->
                    <?php if (isset($goods_list['spec_text']) && $goods_list['spec_text']) { ?>
                        <dd title="<?= $goods_list['spec_text'] ?>">
                            <em class="one-overflow"><?= $goods_list['spec_text'] ?></em>
                        </dd>
                    <?php } ?>
				</dd>
			</dl>
		...
		<td class="count br" rowspan="1">
            ...
            <?php if($val['order_shop_benefit']){ $str = $val['order_shop_benefit'];$str = str_replace('店铺满送活动','满即送', $str);?>
                <span class="td_sale bbc_btns"><p><?= $str ?></p></span>
            <?php } ?>
		</td>
		...
		<td class="operate bdr" rowspan="1">
            ...
            <?php if(($val['order_status'] == Order_StateModel::ORDER_CANCEL || $val['order_status'] == Order_StateModel::ORDER_FINISH) && $val['order_shop_hidden'] == 1):?>
                        ...
                        <p><a onclick="restoreOrder('<?=$val['order_id']?>')"><i class="iconfont icon-huanyuan fz20 align-middle"></i><span class="align-middle"><?=__('还原订单')?></span></a></p>
                      
            <?php endif; ?>
            <?php if(($val['order_status'] == Order_StateModel::ORDER_CANCEL || $val['order_status'] == Order_StateModel::ORDER_FINISH) && $val['order_shop_hidden'] == 1):?>
                      
                        <p><a onclick="delOrder('<?=$val['order_id']?>')"><i class="iconfont icon-lajitong fz20 align-middle"></i><span class="align-middle"><?=__('彻底删除')?></span></a></p>
...
<script>
	$(function () {
		...
		//虚拟兑换码
		$('.tabmenu').append('<a href="'+ SITE_URL +'?ctl=Seller_Trade_Order&met=virtualExchange&typ=e" class="button"><i class="icon-edit"></i><?=__('校验兑换码')?></a>');
