...
<div class="ncsc-oredr-show">
    ...
    <div class="ncsc-order-step" style="text-align: center;">
        ...
        <dl class="<?php if($order_data['payment_id'] != PaymentChannlModel::PAY_CONFIRM && $order_data['order_status'] >= Order_StateModel::ORDER_PAYED){echo 'current';}?>">
            <dt><?=__('完成付款')?></dt>
            <dd class="bg"></dd>
            <dd class="date" title="<?=__('付款时间')?>"><?php echo $order_data['payment_time']; ?></dd>
        </dl>
        <dl class="<?php if($order_data['order_status'] >= Order_StateModel::ORDER_WAIT_CONFIRM_GOODS){echo 'current';}?>">
            <dt><?=__('发放兑换码')?></dt>
            <dd class="bg"></dd>
            <dd class="date" title="<?=__('发放兑换码时间')?>"><?php echo $order_data['payment_time']; ?></dd>
        </dl>
        <dl class=" <?php if($order_data['order_status'] >= Order_StateModel::ORDER_FINISH){echo 'current';}?>">
            <dt><?=__('订单完成')?></dt>
            <dd class="bg"></dd>
            <dd class="date" title="<?=__('订单完成')?>"><?php echo $order_data['order_finished_time']; ?></dd>
        </dl>
        <dl class="<?php if($order_data['order_buyer_evaluation_status'] == 1){echo 'current';}?>">
            <dt><?= __('评价') ?></dt>
            <dd class="bg"></dd>
    ...
    <div class="ncsc-order-contnet">
        <table class="ncsc-default-table order">
            ...
            <tr>
                ...
                <td class="tl">
                    <dl class="goods-name ml10">
                        <dt>
                            <a target="_blank" href="<?php echo Yf_Registry::get('url') . '?ctl=Goods_Goods&met=goods&type=goods&gid=' . $goods_list['goods_id']; ?>" target="_blank" title="<?php echo $goods_list['goods_name']; ?>"><?php echo $goods_list['goods_name']; ?></a>
                        ...
                        <?php if (isset($goods_list['spec_text']) && $goods_list['spec_text']) { ?>
                            <dd class="one-overflow" title="<?= $goods_list['spec_text'] ?>">
                                <?= $goods_list['spec_text'] ?>
                            </dd>
                        <?php } ?>
                        <!-- S消费者保障服务 -->
            ...
            <tr>
                <td colspan="20" class="transportation tl">
                    ...
                    <?php if ($order_data['order_shop_benefit']) { ?>
                        <dl class="">
                            <dd><em class="bbc_seller_color"><?php echo $order_data['order_shop_benefit']; ?></em></dd>
                        </dl>
                    <?php } ?>
                </td>
            </tr>
