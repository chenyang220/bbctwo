...
<div class="wrap clearfix">
        ...
        <div class="basic">
            <div class="displayed">
                ...
                <span class="all-rate">
                    ...
                    <em><?= $shop_scores_count ?></em>
                    <em><?= __('分') ?></em>
                </span>
            </div>
            <div class="sub">
                <div class="store-logo">
                    <img class="wp100" src="<?php if (empty($shop_base['shop_logo'])) {
                        echo $this->web['shop_head_logo'];
                    } else {
                        echo $shop_base['shop_logo'];
                    } ?>" alt="<?= $shop_base['shop_name'] ?>" title="<?= $shop_base['shop_name'] ?>">
                </div>
                <!--店铺基本信息 S-->
                <div class="bbc-info_reset">
                    ...
                    <div class="content_reset">
                        ...
                        <div class="btns">
                            <a href="index.php?ctl=Shop&met=goodsList&id=<?= $shop_id ?>" class="goto"><?= __('进店逛逛') ?></a>
                            <a href="javascript:;" onclick="collectShop(<?= $shop_id ?>)"><?= __('收藏店铺') ?></a>
                        </div>
                        ...
                        <dl>
                            <dt><?= __('企业资质') ?>：</dt>
                            <dd class="enterprise">企业资质</dd>
                        </dl>
                        <?php if ($shop_base['shop_qq'] || $shop_base['shop_ww']) { ?>
                        <dl class="messenger">
                            <dt><?= __('联系方式') ?>：</dt>
                            <dd>
                                <span member_id="9"></span>
                                <?php if ($shop_base['shop_qq']) { ?>
                                    <a target="_blank" href='http://wpa.qq.com/msgrd?v=3&uin=<?= $shop_base['shop_qq'] ?>&site=qq&menu=yes'>
                                        <img border="0" src="http://wpa.qq.com/pa?p=2:<?= $shop_base['shop_qq'] ?>:52&amp;r=0.22914223582483828" style=" vertical-align: middle;">
                                    </a>
                                <?php } ?>
                                <?php if ($shop_base['shop_ww']) { ?>
                                    <a target="_blank" href='http://www.taobao.com/webww/ww.php?ver=3&touid=<?= $shop_base['shop_ww'] ?>&siteid=cntaobao&status=2&charset=utf-8'>
                                        <img border="0" src='http://amos.alicdn.com/realonline.aw?v=2&uid=<?= $shop_base['shop_ww'] ?>&site=cntaobao&s=2&charset=utf-8' alt="<?= __('点击这里给我发消息') ?>" style=" vertical-align: middle;">
                                    </a>
                                <?php } ?>
                            </dd>
                        </dl>
    ...
    <div id="store_decoration_content" class="background" style="<?= $decoration_detail['decoration_background_style']; ?>">
        <div>
            ...
            <?php if (isset($decoration_detail['decoration_banner'])) { ?>
                ...
                <?php if (@$decoration_detail['decoration_banner']['display'] == 'true') { ?>
                    <div id="decoration_banner" class="ncsl-nav-banner" style="text-align: center;">
                        <img src="<?= $decoration_detail['decoration_banner']['image_url']; ?>" alt="">
                    </div>
                <?php } ?>
            <?php } else { ?>
                <!-- 不启用店铺装修 -->
...
<div class="wrap clearfix">
    <?php if (empty($shop_base['is_renovation'])) { ?>
        <div class="bbc-store-info">
            <div class="basic">
                <div class="displayed"><a href=""><?= $shop_base['shop_name'] ?></a> <span class="all-rate">
                     <div class="rating"><span style="width: <?= $shop_scores_percentage ?>%"></span></div>
                       <em><?= $shop_scores_count ?></em><em><?= __('分') ?></em></span>
                </div>
                <div class="sub">
                    <div class="store-logo"><img src="<?php if (empty($shop_base['shop_logo'])) {
                            echo $this->web['shop_head_logo'];
                        } else {
                            echo $shop_base['shop_logo'];
                        } ?>" alt="<?= $shop_base['shop_name'] ?>" title="<?= $shop_base['shop_name'] ?>"></div>
                    <!--店铺基本信息 S-->
                    <div class="bbc-info_reset">
                        <div class="title">
                            <h4><?= $shop_base['shop_name'] ?></h4>
                        ...
                        <div class="content_reset">
                            ...
                            <dl>
                                <dt><?= __('企业资质') ?>：</dt>
                                <dd class="enterprise"><img class="zizhi cursor" src="<?= $this->view->img ?>/intel.gif" alt=""></dd>
                            </dl>
                            <?php if($shop_base['shop_qq'] || $shop_base['shop_ww']){ ?>
                            <dl class="messenger">
                                <dt><?= __('联系方式') ?>：</dt>
        ...
        <div id="nav" class="bbc-nav">
            <ul>
                ...
                if ($shop_nav['items']) {
                    foreach ($shop_nav['items'] as $key => $value) {
                        ?>
                        <li><a href="<?php if (!empty($value['url'])) {
                                echo $value['url'];
                            } else {
                                echo 'index.php?ctl=Shop&met=info&id=' . $shop_id . '&nav_id=' . $value['id'];
                            } ?>" <?php if ($value['target']){ ?>target="_blank" <?php } ?>><span><?= $value['title'] ?><i></i></span></a></li>
                    <?php }
                } ?>
            </ul>
        </div>
        ...
        <div class="t_goods_bot_left">
            <div class="goods_classify">
                ...
                <p class="classify_ser">
                    <input name="searchGoodsList" type="text" value="<?= __('搜索店内商品') ?>" onclick="this.value = '';"> <a id="searchGoodsList"><?= __('搜索') ?></a>
                </p>
                <ul class="ser_lists">
            ...
            <div class="goods_ranking">
                ...
                <ul class="ncs-top-tab pngFix">
                    <li id="hot_sales_tab" class="current overflow"><a><?= __('热销商品排行') ?></a></li>
                    <li id="hot_collect_tab" class="overflow"><a><?= __('热门收藏排行') ?></a></li>
                </ul>
                <div id="hot_sales_list" class="ncs-top-panel">
                    <ol>
                        <?php if (!empty($goods_selling_list['items'])&&$shop_base['shop_type'] == 2) {
                ...
                <div id="hot_collect_list" class="ncs-top-panel hide">
                    <ol>
                       <?php if (!empty($goods_collec_list['items'])&&$shop_base['shop_type'] == 2) {
                ...
                <ul>
                    ...
                    if (isset($hot_groupbuy_data)):
                        foreach ($hot_groupbuy_data as $key_hot => $value_hot):
                            ?>
                            <li>
                                <a href="./index.php?ctl=GroupBuy&met=detail&id=<?= $value_hot['groupbuy_id'] ?>"> <img src="<?= $value_hot['groupbuy_image_rec'] ?>"> </a>
        ...
        <div class="t_goods_bot_right-1">
            <div class="bbc-main-container">
                ...
                <div class="content_s bbc-goods-list">
                   <ul>
                        <?php if (!empty($goods_recom_list['items'])&& $shop_base['shop_type'] == 2) {
                            foreach ($goods_recom_list['items'] as $key => $value) {
                                ?>
                                <li>
                                    <dl>
                                        <dt><a href="index.php?ctl=Supplier_Goods&met=goods&gid=<?= $value['goods_id'] ?>" class="goods-thumb" target="_blank"><img src="<?= image_thumb($value['common_image'], 220, 220) ?>" alt="<?= $value['common_name'] ?>"></a>
                                            <!--<ul class="goods-thumb-scroll-show">
                      <li class="selectedSS"><a href="javascript:void(0);"><img width="60" src="<?= $value['common_image'] ?>"></a></li>
                  </ul>-->
                                        </dt>
                                        <dd class="goods-name"><a href="index.php?ctl=Supplier_Goods&met=goods&gid=<?= $value['goods_id'] ?>" title="<?= $value['common_name'] ?>" target="_blank"><?= $value['common_name'] ?></a></dd>
            ...
            <div class="bbc-main-container">
                ...
                <div class="content_s bbc-goods-list">
                     <ul>
                        <?php if (!empty($goods_new_list['items'])&& $shop_base['shop_type'] == 2) {
...
<script>
    ...
    $(".enterprise").click(function () {
        $.dialog({
            title: "网店经营者营业执照信息",
            content: 'url: ' + SITE_URL + '?ctl=Shop&met=getCompanyInfo&typ=e&id=' + shop_id,
            data: {shop_id: shop_id, callback: recallback},
            width: 520,
            height: 260,
            max: !1,
            min: !1,
    ...
    function recallback(t, e, i, msg) {
        if (200 == e) {
            ...
            i && i.api.close()
            window.location.href = SITE_URL + "?ctl=Shop&met=getCompany&id=" + shop_id;
        } else {
            parent.Public.tips({content: msg});
            i && i.api.close()
        }
...
<div class="bbuilder_code">
    <span class="bbc_codeArea"><img src="<?= Yf_Registry::get('base_url') ?>/shop/api/qrcode.php?data=<?= urlencode(Yf_Registry::get('shop_wap_url') . "/tmpl/store.html?shop_id=" . $shop_base['shop_id']) ?>"></span> <span class="bbc_arrow"></span>
