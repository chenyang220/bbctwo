...
?>
	...
	<link rel="stylesheet" type="text/css" href="<?= $this->view->css ?>/web.css"/>
...
<!--                                      <div id="stock_area_item" data-widget="tab-content" data-area="2" style="display: none;">
						...
						<p class="crhead2_ways">
							...
							<input type="checkbox" class="checkbox rel_top-1" <?php if($op2 == 'active'){?>checked<?php }?> name="op2" /> <label><?=__('仅显示折扣商品')?></label><label><input type="checkbox"  class="checkally" /><?= __('全选') ?></label> <label class="up" id="batch"><i class="iconfont icon-shangjia1"></i>上架</label>
				...
				<ul class="cr_xie clearfix" id="searadll">
					<?php if($data['items']):
							foreach($data['items'] as $key => $val):?>
					<li class="h424">
                        <?php if($val['product_is_behalf_delivery'] && isset($suppliers) && in_array($val['shop_id'],$suppliers)){?>
                            <?php if(isset($common_ids) && in_array($val['common_id'],$common_ids)){?>
                            <?php }else{?>
                                <input type="checkbox"class="checkitemt"  value="<?php echo $val['common_id']?>" style="margin_left: 10px; position: absolute;z-index: 10;right: 0px;">
                            <?php }?>
                        <?php }?>
						...
						<div class="list_padd">
							...
							<?php if(isset($suppliers) && !in_array($val['shop_id'],$suppliers)){?>
                            	<a class="apply_dist"  data-id = <?=$val['shop_id']?> style="padding: 3.5px 80px;background-color: #e45050;color: white;"><?=__('申请经销商')?></a>
                            ...
                            <?php }elseif($val['product_is_behalf_delivery'] && isset($suppliers) && in_array($val['shop_id'],$suppliers)){?>
                            	<?php if(isset($common_ids) && in_array($val['common_id'],$common_ids)){?>
                            	<a  style="padding: 3.5px 50px;background-color: #e45050;color: white;" > <?=__('已上架')?> </a>
                            	<?php }else{?>
...
//批量上架
    ...
    $('.checkally').click(function(){
        var _self = this;
        $('.checkitemt').each(function(index,elements) {
    ...
    $('#batch').click(function(){
        var length = $(".checkitemt:checked").length;
        if (length > 0) {
            $.dialog.confirm("<?=__('您确定要批量上架吗')?>?", function () {
                var chk_value = [];
                $('#searadll input:checkbox:checked').each(function() {
                    chk_value.push($(this).val());
                });
                $.post(SITE_URL + "?ctl=Supplier_Goods&met=addto_distributorrow&typ=json",{chk: chk_value}, function (data) {
