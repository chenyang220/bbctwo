	...
	<div class="wrap clearfix bbc_bg">
		<div class="bbuyer_head_fl">
			...
			<div class="bbuyer_others">
				...
				<p class="back_mall_index"><a href="<?= Yf_Registry::get('url') ?>"><?=__('返回店铺首页')?></a></p>
				<?php }else{ ?>
			...
			<div class="bbuyer_head_nav">
				<ul>
					...
					<li><a href="<?=Yf_Registry::get('paycenter_api_url')?>" target="_blank"><?= __(Yf_Registry::get('paycenter_api_name'))?></a></li>
					<li><a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Message&met=message"><?=__('消息')?></a>
						<?php if($this->countMessage['countMessage'] > 0 && $this->countMessage['countMessage'] < 99){?>
							<i class="bbuyer_news"><?=$this->countMessage['countMessage']?></i>
						<?php }else if($this->countMessage['countMessage'] >99){?>
							<i class="bbuyer_news">99+</i>
						<?php } ?>
					</li>
					<?php if(Web_ConfigModel::value('Plugin_Directseller')){ ?>
                        <li><a href="<?= Yf_Registry::get('url') ?>?ctl=Distribution_NewBuyer_Distribution&met=directsellerList&typ=e"><?=__('分销中心')?></a></li>
		...
		<div class="bbuyer_cart">
			<div class="bbc_buyer_icon">
				...
				<i class="ci-count bbc_bg" id="cart_num"></i>
			</div>
		</div>
     ...
     <div class="wrapper ">
		 <?php if ($this->ctl == 'Buyer_Index'):?>
			 <div class="Div_2 clearfix">
				 <div class="left  ">
					 <div class="_img">
						 <img src="<?=Yf_Registry::get('ucenter_api_url')?>?ctl=Index&met=img&user_id=<?=Perm::$userId?>" width="108" height="108"/>
					 </div>
					 <div class="font">
						 <table>
						 <tbody>
                             ...
                             <?php if($user_is_plus){ ?>
                                 <tr><td class="fontColor" colspan="2"><b class="plus-login-logo mr0"></b><?=$tag?> </td></tr>
                             <?php  } ?>
						 ...
						 <div class="b_rol">
							 ...
							 <div class="same">
								 <div class="same_1">
									 ...
									 <a href="<?= Yf_Registry::get('url') ?>?ctl=Buyer_Voucher&met=voucher"><?= __('代金券') ?></a>
								 </div>
	                         ...
	                         <div class="same">
	                             ...
	                             <a href="<?= Yf_Registry::get('paycenter_api_url') ?>?ctl=Info&met=btinfo" target="_blank"><div class="same_2" id="bt_status_div"><?php if($bt_info['user_bt_status'] == 0){echo __('未激活');}else if($bt_info['user_bt_status'] == 1){echo  __('审核中');}else if($bt_info['user_bt_status'] == 2){
	                             		if ($bt_info['bt_type']==1) {
	                             	        echo  format_money($bt_info['user_credit_availability']);
	                             		}else{
	                             			echo __('不限额度');
	                             		}
	                                  }else{echo __('激活失败');}?></div></a>
							 </div>
	                         <?php } ?>
	...
	?>
      <div class="left_1   visible-lg">
        ...
        <ul class="jiaoyizhognxin">
           ...
           ?>
            <li id="li_<?=$menus_row['menu_id']?>"><a href="<?=sprintf('%s?ctl=%s&met=%s', Yf_Registry::get('url'), $menus_row['menu_url_ctl'], $menus_row['menu_url_met'], $menus_row['menu_url_parem']);?>" class="_Color <?=($menus_row['menu_id'] == $level_row[1]) ? 'sel_color' : ''?>"><?php if($_COOKIE['SHOP_ID']&&$menus_row['menu_name']=='平台客服'){echo"店铺客服";}else{echo $menus_row['menu_name'];}?></a></li>
	...
	<div class="aright">
        <div class="member_infor_content">
		   ...
		   <?php
			...
			{
                $buyer_menu_li_id = 'li_"'.$menus_row['menu_id'].'"';
                $buyer_menu_li_class = $menu_row['menu_id'] == $level_row[4] ? 'active' : 'normal';
                $buyer_menu_li_href = sprintf('%s?ctl=%s&met=%s', Yf_Registry::get('url'), $menu_row['menu_url_ctl'], $menu_row['menu_url_met'], $menu_row['menu_url_parem']);
                if($_GET['ctl'] == 'Buyer_Order' && in_array($_GET['met'], ['physical','chain','virtual']) && in_array($_GET['act'], ['details','detail'])){
                    if($menu_k > 0){
                        continue;
                    }
                    echo '<li id="'.$buyer_menu_li_id.'" class="active"><a>'.__('订单详情').'</a></li>';
                } else {
                    echo '<li id="'.$buyer_menu_li_id.'" class="'.$buyer_menu_li_class.'"><a  href="'.$buyer_menu_li_href.'">'.$menu_row['menu_name'].'</a></li>';
                }
                $menu_k ++;
			}
			?>
