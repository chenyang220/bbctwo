...
class Rights_GroupCtl extends AdminController
{
	...
	public function manage()
	{
        ...
        $rights_group_name=$rows[$rights_group_id]['rights_group_name'];
        $rights_group_id=$rows[$rights_group_id]['rights_group_id'];
		$baseRightsModel = new Rights_Base();
		$baseRightsModel->sql->setLimit(0, 1000);
		$rights_rows = $baseRightsModel->getRightsNew('*');
		...
		{
			if (in_array($rights_row['rights_id'], $group_rights_id_row))
			{
		...
		$rights_rows_new = array();
		
		
		//一级
		foreach ($rights_rows as $key => $val) {
			if($val['rights_parent_id']==0){
				$rights_rows_new[] = $val;
			}
		}

		//二级
		foreach ($rights_rows_new as $k=> $v) {
			foreach ($rights_rows as $kk => $vv) {
				if($vv['rights_parent_id']==$v['rights_id']){
					$rights_rows_new[$k]['child'][] = $vv;
				}
			}
		}
		...
		foreach ($rights_rows_new as $k => $v) {
			foreach ($v['child'] as $kk=> $vv) {
				foreach ($rights_rows as $kkk => $vvv) {
					if($vvv['rights_parent_id']==$vv['rights_id']){
						$rights_rows_new[$k]['child'][$kk]['children'][] = $vvv;
					}
				}
			}
		}
