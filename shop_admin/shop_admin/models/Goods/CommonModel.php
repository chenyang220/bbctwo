        ...
        public function getGoodsList($cond_row = array(), $order_row = array(), $page = 1, $rows = 10, $property_value_row = array(),$del_flg = true)
        {
            ...
            if ($property_value_row) {
                $sql = "
                    ...
                    FROM
                        " . yf_GeneralOperator::getInstance()->shopTablePerfix() . "goods_common c LEFT OUTER JOIN " . yf_GeneralOperator::getInstance()->shopTablePerfix() . "goods_property_index gp ON c.common_id = gp.common_id AND c.common_verify=1 AND c.common_state=1 AND c.shop_status=3
                    ";
                //需要分页如何高效，易扩展
                $offset = $rows * ($page - 1);
            ...
            if ('SKU' == $type) {
                ...
                if ($common_ids) {
                    ...
                    $sql_gbase = 'SELECT * FROM '.yf_GeneralOperator::getInstance()->shopTablePerfix().'goods_base where common_id in ('.implode(',', $common_ids).') and goods_is_shelves=1 and is_del =1';
...
//            : ['common_id'=> $common_id, 'open_limit'=> true, 'valid_status'=> false, 'valid_num'=> $valid_num];
        ...
        public function getGoodsListByShopIds($shop_goods_cat_ids, $cond_row, $order_row, $page, $rows)
        {
            ...
            $order = $this -> sql -> getOrder();
            $sql = "SELECT * from " . yf_GeneralOperator::getInstance()->shopTablePerfix() . "goods_common";
            $sql = $sql . $where . $order . $limit;
            $common_rows = $this->sql->getAll($sql);
