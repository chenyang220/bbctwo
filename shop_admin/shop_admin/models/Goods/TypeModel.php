...
//		$Goods_TypeModel      = new Goods_TypeModel();        //商品类型表
		...
		if (!empty($type_rows))
		{
			...
			if($property_is_search == 1){
				$goods_property_rows = $Goods_PropertyModel->getByWhere(array('type_id' => $type_id, 'property_is_search'=>1),array('property_displayorder'=>'asc'));
			}else{
			...
			if (!empty($goods_property_rows))
			{
				foreach ($goods_property_rows as $key_property => $value_property)
				{
					...
					{
						$property_values                   = $Goods_PropertyValueModel->getByWhere(array('property_id' => $value_property['property_id']),array('property_value_displayorder'=>'asc'));
	...
	public function getTypeInfoByPublishGoods($cat_id = 0,$shop_id=0)
	{
		...
		$condi_format['shop_id'] = $shop_id?$shop_id:Perm::$shopId;
	...
	public function getType($type_id = 0)
    {
        if($type_id){
            $sql = "SELECT type_id,type_name FROM " . yf_GeneralOperator::getInstance()->shopTablePerfix() . "goods_type where type_id = '".$type_id."' LIMIT 1000";
        }else{
            $sql = "SELECT type_id,type_name FROM " . yf_GeneralOperator::getInstance()->shopTablePerfix() . "goods_type LIMIT 1000";
        }
