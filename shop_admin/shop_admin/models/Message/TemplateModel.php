...
class Message_TemplateModel extends Message_Template
{
	...
	//$messageWechartPublic
    public static $messageWechartPublic = array(
        "0" => '关闭',
        "1" => '开启'
    );
	...
	public function getTemplateList($cond_row = array(), $order_row = array(), $page = 1, $rows = 100)
	{
		...
		foreach ($data["items"] as $key => $value)
		{
            ...
            $data["items"][$key]["is_wechart_pulic"] = __(Message_TemplateModel::$messageWechartPublic[$value["is_wechart_pulic"]]);
		}
		return $data;
	}
    ...
    public function getTemplateInfo($cond_row,$content_data = array()){
        ...
        if($content_data){
            foreach ($content_data as $key => $value){
                ...
                $data['content_wechart_pulic'] = str_replace($key, $value, $data['content_wechart_pulic']);
            }
        }
        return $data;
    }
     ...
     * @return type
     */
    public function sendMessage($receiver,$send_type, $tpl_code,$content_data = array(),$area_code){
        ...
        if($send_type === 'phone'){
            ...
            $result = Sms::send($receiver, $area_code,$tpl_info[$content], $tpl_info['baidu_tpl_id'],$content_data);
        }
        if($send_type === 'email'){
