...
class Order_GoodsModel extends Order_Goods
{
     ...
     * @return mixed
     */
    public function getGoodsFromOrder($cond_row)
    {
        $sql = "SELECT order_goods.*,order_base.buyer_user_id,goods_common.brand_id,goods_common.brand_name FROM ";
        $sql .= Yf_GeneralOperator::getInstance()->shopTablePerfix() . "order_goods AS order_goods JOIN ";
        $sql .= Yf_GeneralOperator::getInstance()->shopTablePerfix() . "order_base AS order_base ON ";
        $sql .= "order_goods.order_id = order_base.order_id JOIN ";
        $sql .= Yf_GeneralOperator::getInstance()->shopTablePerfix() . "goods_common AS goods_common ON ";
        $sql .= "goods_common.common_id = order_goods.common_id ";
        $sql .= "where order_base.buyer_user_id = " . $cond_row['user_id'];
        $sql .= " AND goods_common.is_del = 1 AND goods_common.common_state = 1 AND goods_common.common_verify = 1";
        if ($cond_row['goods_name']) {
            $sql .= " AND order_goods.goods_name LIKE '%" . $cond_row['goods_name'] . "%' ";
        }
        $sql .= " ORDER BY order_goods.order_goods_id DESC LIMIT 0,20";
        $order_goods_list = $this->sql->getAll($sql);
        foreach($order_goods_list as $k=>$v){
            $order_time = explode(' ', $v['order_goods_time']);
            $order_goods_list[$k]['order_time'] = $order_time[0];
        }
        return $order_goods_list;
    }

    /**
     * 根据给定单号返回数组商品名和商品购买数量
     * 
     * @author fzh
     */
    public function getOrderGoodsByIds($orderIds){
    	...
    	$sql = "SELECT order_id,goods_name,order_goods_num FROM ";
        $sql .= Yf_GeneralOperator::getInstance()->shopTablePerfix() . "order_goods WHERE order_id IN (";
        $sql .= $condition.")";
