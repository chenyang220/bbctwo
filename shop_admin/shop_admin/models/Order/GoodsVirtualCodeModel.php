<?php if (!defined('ROOT_PATH')) {
    ...
    class Order_GoodsVirtualCodeModel extends Order_GoodsVirtualCode
    {
         ...
         * @return type
         */
        public function virtualExchange($virtual_code_id = 0, $user_id)
        {
            $rs_row = array();
            if (!$virtual_code_id) {
            ...
            if ($shop_id != $order_goods['shop_id']) {
                return array('status' => false, 'msg' => '该兑换码不属于本店商品', 'data' => array());
            }
            ...
            if ($virtual_base['virtual_code_status'] == Order_GoodsVirtualCodeModel::VIRTUAL_CODE_NEW) {
                ...
                // 如果兑换日期 已超过 兑换码有效期，则返回  或者  商家开启了自动退款功能。
                $Goods_CommonModel = new Goods_CommonModel();
                $goods_common = $Goods_CommonModel->getOne($order_goods['commond_id']);
                $virtual_code_time = strtotime($goods_common['common_virtual_date'] . ' 23:59:59');
                if (time()> $virtual_code_time) {
                    return ['status' => false, 'msg' => '该兑换码已过期，请联系商家~谢谢', 'data' => []];
                }
                ...
                $data['order_code'] = $virtual_code_id;
