...
class Order_InvoiceModel extends Order_Invoice
{
     ...
     * @return int
     */
    public function getOrderInvoiceId($invoice_id,$invoice_title,$invoice_content,$order_id, $flag = false){
        if(!$invoice_id || !$order_id){
            return 0;
        }

        if($flag){
            $ShopInvoiceModel = new ShopInvoiceModel();
            $invoice = $ShopInvoiceModel->getOne($invoice_id);
            if(!$invoice){
                return 0;
            }
        } else {
