		file_put_contents(dirname(__FILE__).DIRECTORY_SEPARATOR.'abs.php',print_r($data,true),FILE_APPEND);<?php if (!defined('ROOT_PATH'))
{
	exit('No Permission');
}
...
class Shop_BaseModel extends Shop_Base
{
	...
	public function getShopDetail($shop_id)
	{
		...
		else
		{
			...
			if($data['shop_desc_scores'] > $avg_desc_scores) {
				if(($max_evaluation_desccredit - $avg_desc_scores) !=0)
				{
					$data['com_desc_scores'] = round(($data['shop_desc_scores'] - $avg_desc_scores)/($max_evaluation_desccredit - $avg_desc_scores)*100, 2);
				}
				else
				{
					$data['com_desc_scores'] = 0;
				}
               
				$data['desc_status'] = 1;
			} else {
				if(($avg_desc_scores- $min_evaluation_desccredit) !=0)
				{
			...
			if($data['shop_service_scores'] > $avg_service_scores) {
				if(($max_evaluation_servicecredit - $avg_service_scores) !=0)
				{
				...
				{
					$data['com_service_scores'] = 0;
				}

				$data['service_status'] = 1;
			} else {

				if(($avg_service_scores- $min_evaluation_servicecredit) !=0)
				{
			...
			if($data['shop_send_scores'] > $avg_send_scores) {
				...
				if(($avg_send_scores- $min_evaluation_deliverycredit) !=0)
				{
	...
	public function getBaseOneList($cond_row = array(), $order_row = array(), $page = 1, $rows = 100)
	{
		...
		if ($data)
		{
			...
			if ($data['shop_grade_id'])
			{
                ...
                $data["shop_grade"]     = $data["shop_grade_row"]['shop_grade_name'];
			}
			else
			{
				$data["shop_grade"] = __(self::$shop_grade_name);
	...
	public function getCategorylist($cond_row = array(), $order_row = array(), $page = 1, $rows = 100)
	{
		...
		foreach ($data['items'] as $key => $value)
		{
			...
			$product_class_id_arr = array_values(explode(",",$value['product_class_id']));
			$commission_rate_arr = array_values(explode(",",$value['commission_rate']));
			unset($data['items'][$key]['commission_rate']);
			foreach (($product_class_id_arr) as $cat_key => $product_class_id) {
				$cat_num                            = $this->catNameNum($product_class_id);
				$data['items'][$key]['cat_namenum'][$cat_key] = implode(" --> ", $cat_num);
				$data['items'][$key]['commission_rate'][$cat_key] = $commission_rate_arr[$cat_key];
			}
		}
		return $data;
	}
    ...
    public function getNearShop($lat, $lng, $distance=20000, $page = 1, $rows=10)
    {
        ...
        $shop_entity = Yf_GeneralOperator::getInstance()->shopTablePerfix() . 'shop_entity';
