...
class Sub_SiteModel extends Sub_Site
{
     ...
     * @nsy 2019-02-20
     */
    public function getIPLoc_sina_new($IP=null){
        $data = array('province'=>'上海','city'=>'徐汇区');
        $ip = $_COOKIE['client_ip_address'];
        if(!$ip){
            return $data;
        }
        //取第一个ip获取准确地理位置
        ...
        list($prov,$city) =  $addr;
        if($prov && $city){
            $data['province'] = mb_substr($prov,0,-1,'utf-8');
            $data['city'] = $city;
        }
        return $data;
    }
     ...
     * @return type
     */
    public function getIPLoc_sina_new_old($queryIP){
        return false;
