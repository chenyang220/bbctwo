...
class User_FootprintModel extends User_Footprint
{
    ...
    public function userFavorite()
    {
        ...
        if($user_id) {
            $sql = "select a.common_id from " . TABEL_PREFIX . "user_footprint a left join " . TABEL_PREFIX . "goods_common b on a.common_id = b.common_id where 1 and b.common_state=1 AND b.is_del=1 AND b.common_verify=1 AND b.shop_status=3 AND b.supply_shop_id<>0 AND a.user_id = ".$user_id." order by a.footprint_time desc limit 10";
        ...
        $sql1 .= " order by rand() limit 4";

        $data = $this->sql->getAll($sql1);
        $goods_ids = array_unique(array_column($data,'goods_id'));
        if(!is_array($data)){$data = array();}
        if(count($data) < 4) {
            $limit = 4 - count($data);
            $sql2 = "select a.* from " . TABEL_PREFIX . "goods_base a left join " . TABEL_PREFIX . "shop_base c on a.shop_id = c.shop_id where a.goods_is_shelves = 1 and a.is_del = 1 and c.shop_status=3 and c.shop_type=1";
            ...
            $goods_list = $this->sql->getAll($sql2);
