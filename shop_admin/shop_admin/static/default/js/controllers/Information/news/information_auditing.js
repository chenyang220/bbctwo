function initEvent() {
    ...
    $('.wrapper').on('click', '#audit', function (a) {
        ...
        var b = $('#grid').jqGrid('getGridParam', 'selarrrow'),
            c = b.join();
        return c ? void Public.ajaxPost('./index.php?ctl=Information_News&met=check&typ=json', {
            id: c
        }, function (a) {
    ...
    $('.wrapper').on('click', '#notaudit', function (a) {
        ...
        var b = $('#grid').jqGrid('getGridParam', 'selarrrow'),
            c = b.join();
        return c ? $.dialog.confirm("确定要拒绝吗？", function () {void Public.ajaxPost('./index.php?ctl=Information_News&met=notcheck&typ=json', {
            id: c
        }, function (a) {
            200 === a.status ? parent.Public.tips({
                content: '审核不通过！'
...
function initGrid() {
    ...
    $("#grid").jqGrid({
        url: SITE_URL + "?ctl=Information_News&met=informationNewsList&typ=json&status=1&auditing=3",
        datatype: "json",
