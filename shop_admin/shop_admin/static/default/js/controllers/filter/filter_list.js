$(function() {
    ...
    var handle = {
                ...
                Public.ajaxPost(SITE_URL + '?ctl=Base_FilterKeyword&met=remove&typ=json', {
                    ...
                    if (data && data.status == 200) {
                        ...
                        if (row_ids.split(',').length === id_arr.length) {
                            parent.Public.tips({
                                content: sprintf(__('成功删除 %d 个！'), id_arr.length)
                            });
                        } else {
                            parent.Public.tips({
                                type: 2,
        ...
        operFormatter: function(val, opt, row) {
            var html_con = '<div class="operating" data-id="' + row.id + '"><span class="ui-icon ui-icon-pencil" title="'+__("修改")+'"></span><span class="ui-icon ui-icon-trash" title="'+__("删除")+'"></span></div>';
            return html_con;
        },

        stausFormatter: function(val, opt, row) {
            var text = val == 1 ? __('禁止') : __('替换');
            ...
            loadComplete: function(response) {
                ...
                if (re_records == 0 || re_records == null) {
                    if ($("#grid").parent().find(".norecords").length < 1) {
                        $("#grid").parent().append(_('<div class="norecords">'+ __("没有符合数据")+'</div>'));
                    }
    ...
    function initEvent() {
        ...
        $('#search').on('click', function(e) {
            e.preventDefault();
            var skey = match_con.val() === __('输入客户编号/ 名称/ 联系人/ 电话查询') ? '' : $.trim(match_con.val());
