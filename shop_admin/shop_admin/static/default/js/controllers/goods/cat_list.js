var handle = {
    ...
    operate: function (oper, row_id)
    {
        ...
        else if(oper == 'add'&&row_id)
        {
            ...
            var met = 'manage';
            Public.ajaxPost(SITE_URL + '?ctl=Goods_Cat&met=getCatInfo&typ=json', {
                cat_id: row_id
            }, function (res) {
                if (res.status == 200) {
                    $.dialog.confirm('当前分类已关联品牌，若添加下级分类，则原绑定品牌前台不显示', function () {
                        $.dialog({
                            title: title,
                            content: 'url:' + SITE_URL + '?ctl=Goods_Cat&met=' + met + '&typ=e',
                            data: data,
        ...
        {
            var title = sprintf('编辑分类导航 [%s]', row_id);
            var data = {
                oper: oper,
                rowId: row_id,
                rowData: $("#grid").jqGrid("getRowData",row_id),
                callback: this.callback
            };
        ...
        {
            var title = sprintf('修改分类 [%s]', $("#grid").jqGrid("getRowData",row_id)['cat_name']);
            var data = {
                oper: oper,
                rowId: row_id,
                rowData: $("#grid").jqGrid("getRowData",row_id),
    ...
    {
        $.dialog.confirm(__('删除的将不能恢复，请确认是否删除？'), function ()
        {
        ...
        $.dialog.confirm(__('删除的将不能恢复，请确认是否删除？'), function ()
        {
            ...
            }, function (data)
            {
                if (data && data.status == 200)
                {
                    ...
                    {
                        parent.Public.tips({
                            content:__('成功删除') + id_arr.length + '个！'
                        });
                    }
                    else
                    {
    ...
    imageFmatter: function (val, opt, row)
    {
        if (row.cat_pic)
        {
            val = '<img src="' + row.cat_pic + '" style="width:100px;">';
        }
        else
        {
            val = '&#160;';
    ...
    rglFmatter:function (val, opt, row) {
        var rgl = row.return_goods_limit;
        var slct_val= '未设置';
        switch (rgl) {
            case "0":
                slct_val = '不支持退货退款';
...
var colModel = [{
    ...
    "name": "return_goods_limit",
    "index": "return_goods_limit",
    "label":__("分类商品退货期"),
    "classes": "ui-ellipsis",
    "align": "center",
    "title": false,
