...
function postData(oper) {
    ...
    for (var k = 0; k < ($("input[name='goods_class_recommend']:checked").length); k++) {
        goods_class_recommend[k] = $("input[name='goods_class_recommend']:checked").eq(k).val();
    }
    ...
    params = {
        ...
        goods_class_recommend: goods_class_recommend
    };

    if ($('input[name=goods_class_recommend]').length>=1 && goods_class_recommend.length<1) {
        Public.tips({type: 1, content: '首页分类导航展示下至少选择一个下级分类！'});
        return false;
    }
...
function initField() {
    ...
    $.get('./index.php?ctl=Goods_Cat&met=getNav&typ=json&id=' + id, function (a) {
        if (a.status == 200) {
            ...
            if (b.goods_cat_nav_recommend_up) {
                $.each(b.goods_cat_nav_recommend_up, function (name, value) {
                    var name_re = 'recommend_class_' + value;
                    $("#" + name_re).attr('checked', 'checked');
                   // var p_id = $('#' + name_re).data('p_id');
                   // all_up_check(p_id);
                });
