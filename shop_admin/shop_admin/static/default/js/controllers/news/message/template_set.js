...
$(function ()
{
    console.log($('#wechart_public_form').serialize());
    $('#wechart_public_form').validator({
        ignore: ':hidden',
        theme: 'yellow_bottom',
        timely: 1,
        stopOnError: true,
        ...
        valid: function (form)
        {
            ...
            parent.$.dialog.confirm('确认更改此消息模板？', function ()
            {
                Public.ajaxPost(SITE_URL + '?ctl=News_Message&met=editTemplateWxPublicAcct&typ=json', $("#wechart_public_form").serialize(), function (data)
                {
                    if (data.status == 200)
                    {
                        parent.Public.tips({content: '编辑成功！'});
