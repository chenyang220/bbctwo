...
var THISPAGE = {
    ...
    loadGrid: function(){
        ...
        var colModel = [
            ...
            {name: "information_title", label: "标题", align:"left",width:380,sortable:false},
            {name: "issue_user_type_name", label: "发布方", align: "center",width: 100,sortable:false},
            {name: "`information_add_time` ", label: "发布时间", align: "center",width: 150,sortable:true},
            {name: "check_num", label: "浏览量", align: "center",width:100,sortable:false}
        ];
        this.mod_PageConfig.gridReg('grid', colModel);
        colModel = this.mod_PageConfig.conf.grids['grid'].colModel;
        $("#grid").jqGrid({
            url:SITE_URL + "?ctl=Operation_Information&met=getInformationList&typ=json",
    ...
    }, del: function (t)
    {
        $.dialog.confirm("删除的资讯将不能恢复，请确认是否删除？", function ()
        {
            Public.ajaxPost(SITE_URL+"?ctl=Operation_Custom&met=delService&typ=json", {id: t}, function (e)
            {
                if (e && 200 == e.status)
                {
                    parent.Public.tips({content: "资讯类型删除成功！"});
