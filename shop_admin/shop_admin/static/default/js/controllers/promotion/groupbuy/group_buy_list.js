...
var THISPAGE = {
    ...
    loadGrid: function(){
        ...
        function operFmatter (val, opt, row) {
            ...
            var endtimes = new Date(row.groupbuy_endtime);
            endtimes = endtimes.getTime();
            var html_con =  '<div class="operating" data-id="' + row.groupbuy_id + '"><span class="ui-icon ui-icon-trash" title="删除"></span><a href="'+SHOP_URL+'?ctl=Goods_Goods&met=goods&type=goods&gid=' + row.goods_id+'" target="_blank"><span class="ui-icon ui-icon-search" title="查看活动"></span>';

            if(endtimes > timestamp){
                html_con = html_con+'</a><span class="ui-icon ui-icon-pencil" title="设置"></span>';
            }
            html_con = html_con +'</div>';
