...
var THISPAGE = {
    ...
    loadGrid: function(){
        ...
        var colModel = [
            ...
            {name:'commission_rate', label:'分佣比例（%）',  width:100, align:"center", formatter:commissionRate},
            {name:'shop_class_bind_enablecha', label:'状态',  width:100, align:"center"},
            {name:'cat_namenum', label:'经营类目',  width:300, align:"left", formatter:catNamenum},
    ...
    function commissionRate(val, opt, row) {
        var html = '';
        for (var i=0;i<val.length;i++)
        { 
            html += '<div><span>'+val[i] +'%</span></div>';
        }
         return html;

    }
    function catNamenum(val, opt, row) {
        var html = '';
        for (var i=0;i<val.length;i++)
        { 
            html += '<div><span>'+val[i] +'</span></div>';
