...
var THISPAGE = {
    ...
    loadGrid: function(){
        ...
        var colModel = [
            ...
            {name: 'shop_class_bind_enablecha', label: '经营类目状态', width: 100, align: "center"},
            {name: 'delivery_status', label: '同城配送状态', width: 100, align: "center", formatter: handle.delivery_state, hidden: is_show},
            {name: 'delivery_price', label: '每单配送费（元）', width: 100, align: "center", hidden: is_show},
            {name:'cat_namenum', label:'经营类目',  width:200, align:"center"},
        ];
        this.mod_PageConfig.gridReg('grid', colModel);
        colModel = this.mod_PageConfig.conf.grids['grid'].colModel;
        $("#grid").jqGrid({
            url:SITE_URL +  "?ctl=Shop_Manage&met=getEditCategory&typ=json",
...
var handle = {
    ...
    delivery_state: function (val, opt, row) {
        if (val == 1){
            return '<span>开启</span>';
        } else{
            return '<span>关闭</span>';
