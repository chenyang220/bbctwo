	...
	var THISPAGE = {
		...
		loadGrid: function(){
			...
			var colModel = [
				...
				{name:'payment_other_number', label:'支付单号',  width:200, align:"center"},
				...
				{name:'order_shipping_code', label:'发货物流单号',  width:200, align:"right"},
		...
		addEvent: function()
		{
			...
			$('.grid-wrap').on('click', '.ui-icon-trash', function(e){
				...
				$.dialog.confirm('您确定要取消该订单吗？', function(){
					Public.ajaxPost(SITE_URL + '?ctl=Trade_Order&met=cancelOrder&typ=json', {order_id: order_id}, function ( data ) {
						if ( data.status == 200 ) {
							parent.Public.tips({type: 3, content : '取消成功'}), THISPAGE.reloadData();
						} else {
							parent.Public.tips({type: 1, content : '取消失败'});
						}
					})
				});
			});
			...
			$('#search').click(function(){
				...
				queryConditions.payment_other_number = $('#payment_other_number').val();
            ...
            $("#btn-excel").click(function ()
            {
                ...
                $.dialog({
                    title: '订单导出',
                    content: "url:"+SITE_URL + '?ctl=Trade_Order&met=exportOrder',
