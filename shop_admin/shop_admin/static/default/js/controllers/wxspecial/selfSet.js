...
function init() {
    Public.ajaxPost(SITE_URL + "?ctl=WxSpecial_Column&typ=json&met=getColumnInfo&special=1", {}, function (rs) {
...
var handle = {
    operate: function (t) {
        ...
        $.dialog({
            title: i,
            content: "url:./index.php?ctl=WxSpecial_Column&met=columnGoods",
            data: a,
            width: 700,
            height: 500,
            max: !1,
    ...
    image: function (t) {
        $.dialog({
            title: '上传图片',
            content: "url:./index.php?ctl=WxSpecial_Column&met=columnImage",
...
$(document).on('click','.icon-cancel',function () {
    ...
    var that = $(this);
    if (special_set_id) {
        Public.ajaxPost(SITE_URL + "?ctl=WxSpecial_Column&typ=json&met=removeColumn", {special_set_id: special_set_id}, function (data) {
            if (data.status == 200) {
    ...
    if (item_info) {
        ...
        for (var i = 0; i < item_info.length; i++) {
            html += '<li data-goods_id="' + item_info[i]['goods_id'] + '" class="goods_info" >' +
                '<div><em class="img-box"><img class="wp100" src="' + item_info[i]['goods_image'] + '" alt=""></em>' +
                '<span class="goods_name">' + item_info[i]['goods_name'] + '</span>' +
...
function postData() {
    ...
    Public.ajaxPost(SITE_URL + "?ctl=WxSpecial_Column&typ=json&met=AddOrEditColumn", params, function (data) {
        if (data.status == 200) {
