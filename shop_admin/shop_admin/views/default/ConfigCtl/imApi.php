...
?>
    ...
    <div class="wrapper page">
        ...
        <form method="post" id="im-shop_api-setting-form" name="imSettingForm">
            ...
            <div class="ncap-form-default">
                <dl class="row">
                    <dt class="tit"><?= __('IM状态'); ?></dt>
                    <dd class="opt">
                        <div class="onoff">
                            <input id="im_statu1" name="im_api[im_statu]" value="1" type="radio" <?= (@Yf_Registry::get('im_statu') == 1 ? 'checked':'') ?>> <label <?php if ($im_s != 1){ ?>onclick="return false;"<?php } ?> title="<?= __('开启'); ?>" class="cb-enable <?= (@(Yf_Registry::get('im_statu') == 1 && $im_s == 1) ? 'selected':'') ?> " for="im_statu1" id="im_statu01"><?= __('开启'); ?></label> <input id="im_statu0" name="im_api[im_statu]" value="0" type="radio"
                                <?= (@(Yf_Registry::get('im_statu') == 0 || $im_s != 1) ? 'checked':'') ?>> <label <?php if ($im_s != 1){ ?>onclick="return false;"<?php } ?> title="<?= __('关闭'); ?>" class="cb-disable <?= (@(Yf_Registry::get('im_statu') == 0 || $im_s != 1) ? 'selected':'') ?>" for="im_statu0" id="im_statu00"><?= __('关闭'); ?></label>
                        </div>
                        <p class="notic"></p>
                    </dd>
                </dl>
                ...
                <dl class="row">
                    ...
                    <dd class="opt">
                        ...
                        <p class="notic"><?= __('请填写商城系统与IM通讯的Key值,此外的值要与IM后台应用的值保持一致'); ?></p>
                    </dd>
                </dl>
                ...
                <div class="bot">
                    <a href="javascript:void(0);" <?php if ($im_s != 1){ ?>disabled style="background-color: #96a6a6;border-color: #808b8d;color: #fff;"<?php } ?> class="ui-btn ui-btn-sp im-submit-btn"><?= __('确认提交'); ?></a>
                </div>
            </div>
        </form>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#explanation ul li").html(__('您未购买IM权限，请联系远丰商务部'));
            
            $("a.im-submit-btn,label").on("click", function () {
                Public.tips({type: 1, content: __('请联系客服开通IM服务')});
