...
<div class="wrapper page">
                ...
                <?php
                    foreach ($menus['brother_menu'] as $key => $val) {
                        if (in_array($val['rights_id'], $admin_rights) || $val['rights_id'] == 0) {
                            ?>
                            <li>
                                <a <?php if (!array_diff($menus['this_menu'], $val)) { ?> class="current"<?php } ?> href="<?= Yf_Registry::get('url') ?>?ctl=<?= $val['menu_url_ctl'] ?>&met=<?= $val['menu_url_met'] ?><?php if ($val['menu_url_parem']) { ?>&<?= $val['menu_url_parem'] ?><?php } ?>"> <span><?= __($val['menu_name']); ?></span> </a>
                            </li>
                        <?php }
                    } ?>
            </ul>
    ...
    <form method="post" id="logistics-setting-form" name="settingForm">
        ...
        <div class="ncap-form-default">
            <dl class="row">
                <dt class="tit">
                    <label for="site_name"><?= __('选择采用的快递公司'); ?></label>
                </dt>
                <dd class="opt" style="width:706px;">
                    
                    <table id="checkbox_table">
                        <tr>
                            <td><input type="checkbox" id="selectall" onclick="selectAll()"/><?= __('全选'); ?></td>
                            <td colspan="5"></td>
                        </tr>
                        <?php
                            ...
                            foreach ($logistics_config as $k => $expres) {
                                ...
                                ?>
                                <td><label title="<?= __('开启'); ?>" class="titlelabel" for="logistics_express_<?= $k ?>" style="padding-right: 10px;"><input id="express_<?= $k ?>" name="logistics[logistics_express][]" value="<?= $k ?>" type="checkbox" <?= in_array($k, $data['logistics_express']['config_value']) ? "checked" : "" ?> /> <?= $expres ?></label></td>
