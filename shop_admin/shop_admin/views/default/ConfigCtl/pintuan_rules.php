    ...
    <div class="wrapper">
        <form method="post" enctype="multipart/form-data" id="pintuan_rules_form" name="form1">
            ...
            <div class="ncap-form-default">
                <dl class="row">
                    <dt class="tit">
                        <label for="help_info">*<?= __('拼团规则描述'); ?></label>
                    </dt>
                    <dd class="opt">
...
<script>
    ...
    setTimeout("show()",300);    //设置页面加载后'); ?>1<?= __('秒显示内容层'); ?>
                ...
                parent.$.dialog.confirm(__('<?= __('修改立马生效'); ?>,<?= __('是否继续？'); ?>'), function ()
                    {
                        Public.ajaxPost(SITE_URL + '?ctl=Config&met=edit&typ=json', $('#pintuan_rules_form').serialize(), function (data)
                        {
                            if (data.status == 200)
                            {
                                Public.tips({content: '<?= __('修改操作成功！'); ?>'});
                            }
                            else
                            {
                                Public.tips({type: 1, content: data.msg || '<?= __('操作无法成功，请稍后重试！'); ?>'});
                            }
                        });
                    }
                )
