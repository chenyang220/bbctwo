        ...
        <body class="<?=$skin?>">
                ...
                <div class="fixed-bar">
                    <div class="item-title">
                        ...
                        <ul class="tab-base nc-row">
                            ...
                            ?>
                            <!-- <li><a href="<?= Yf_Registry::get('url') ?>?ctl=Config&met=onlineTheme&config_type%5B%5D=site"><span><?= __('线上模板'); ?></span></a></li> -->
                                        ...
                                        <dl class="current-style">
                                            ...
                                            foreach ($data['theme_row'] as $id=>$theme_row)
                                            {
                                                ...
                                                ?>
                                                    ...
                                                    <dd><?= __('当前在用模板名称：'); ?><strong id="current_style"><?=$theme_row['name']?></strong></dd>
                                                    <dd><?= __('当前在用风格名称：'); ?><strong id="current_style"><?=$theme_row['config']['theme_label']?></strong></dd>
                                                    <?php if (isset($config['index_tpl']) && $config['index_tpl']):?>
                                                    <p class="links"><a href="<?= Yf_Registry::get('url') ?>?ctl=Floor_Adpage&met=adpage" onclick="parent.tab.addTabItem({text:'<?= __('首页模板'); ?>', url: '<?= Yf_Registry::get('url') ?>?ctl=Floor_Adpage&met=adpage'}); return false;"><span><?= __('首页模板设置'); ?></span></a></p>
                                                <?php endif;?>
                                                    <?php if (isset($config['index_slider']) && $config['index_slider']):?>
                                                    <p class="links"><a href="<?= Yf_Registry::get('url') ?>?ctl=Config&met=index_slider&config_type%5B%5D=index_slider" onclick="parent.tab.addTabItem({text:'<?= __('首页幻灯片'); ?>', url: '<?= Yf_Registry::get('url') ?>?ctl=Config&met=index_slider&config_type%5B%5D=index_slider'}); return false;"><span><?= __('首页幻灯片设置'); ?></span></a></p>
                                                <?php endif;?>
                                                    <?php if (isset($config['index_slider_img']) && $config['index_slider_img']):?>
                                                    <p class="links"><a href="<?= Yf_Registry::get('url') ?>?ctl=Config&met=index_liandong&config_type%5B%5D=index_liandong" onclick="parent.tab.addTabItem({text:'<?= __('首页联动小图'); ?>', url: '<?= Yf_Registry::get('url') ?>?ctl=Config&met=index_liandong&config_type%5B%5D=index_liandong'}); return false;"><span><?= __('首页联动小图设置'); ?></span></a></p>
                                                <?php endif;?>
                                                <p class="links"><a href="<?= Yf_Registry::get('url') ?>?ctl=Forum&met=front_forum" onclick="parent.tab.addTabItem({text:'<?= __('首页版块'); ?>', url: '<?= Yf_Registry::get('url') ?>?ctl=Forum&met=front_forum'}); return false;"><span><?= __('首页版块'); ?></span></a></p>
                                            <?php
                                                }
                                 ...
                                 ?>
                                    <li>
                                        <dl>
                                            ...
                                            <dd><?= __('模板名称：'); ?><?=$theme_row['name']?></dd>
                                            <dd><?= __('风格名称：'); ?><?=$theme_row['config']['theme_label']?></dd>
                                            <dd class="btn"> <a href="javascript:useTheme('<?=$theme_row['name']?>');" class="yfbtn bbc_seller_btns"><i class="icon-cogs"></i><?= __('使用'); ?></a></dd>
                                        </dl>
                                    </li>
                                <?php
                                }
            ...
            <script type="text/javascript">
                ...
                function useTheme(theme_id)
                {
                    ...
                    parent.$.dialog.confirm('<?= __('修改风格后，有可能需要修改对应的首页模板、首页幻灯片、首页联动小图'); ?>, <?= __('是否继续？'); ?>', function ()
                        {
