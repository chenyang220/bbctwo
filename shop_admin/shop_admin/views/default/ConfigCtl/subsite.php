  ...
  <div class="fixed-bar">
    ...
    <form method="post" id="subsite-setting-form" name="settingForm">
        ...
        <div class="ncap-form-default">
            <dl class="row">
                <dt class="tit"><?= __('是否开启城市分站'); ?></dt>
                <dd class="opt">
                    <div class="onoff">
                        <input id="subsite_is_open1" name="subsite[subsite_is_open]" value="1" type="radio" <?=($data['subsite_is_open']['config_value']==1 ? 'checked' : '')?> >
						<label title="<?= __('开启'); ?>" class="cb-enable <?=($data['subsite_is_open']['config_value']==1 ? 'selected' : '')?> " for="subsite_is_open1"><?= __('开启'); ?></label>

                        <input id="subsite_is_open0" name="subsite[subsite_is_open]" value="0" type="radio"  <?=($data['subsite_is_open']['config_value']==0 ? 'checked' : '')?> >
						<label title="<?= __('关闭'); ?>" class="cb-disable <?=($data['subsite_is_open']['config_value']==0 ? 'selected' : '')?>" for="subsite_is_open0"><?= __('关闭'); ?></label>
                    </div>
