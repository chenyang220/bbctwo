	...
	<div class="wrapper page">
		<div class="fixed-bar">
			<div class="item-title">
				<div class="subject">
					<h3><?= __('供应商入驻'); ?></h3>
					<h5><?= __('供应商入驻将在首页展示'); ?></h5>
				</div>
				<ul class="tab-base nc-row">
					<li><a class="current"><span><?= __('幻灯片管理'); ?></span></a></li>
		...
		<div class="explanation" id="explanation">
			...
			<ul>
				<li><?= __('该组幻灯片滚动图片应用于供应商入驻使用，最多可上传'); ?>2<?= __('张图片。'); ?></li>
				<li><?= __('图片要求使用宽度为'); ?>1900<?= __('像素，高度为'); ?>350<?= __('像素'); ?>jpg/gif/png<?= __('格式的图片。'); ?></li>
		...
		<form method="post" enctype="multipart/form-data" id="supplier_slider-setting-form" name="form1">
			...
			<div class="ncap-form-default">
				<dl class="row">
					...
					<dd class="opt">
                        <div class="div-image">
                            ...
                            <a href="javascript:void(0)" class="del" onclick="delImage('supplier_slider1_review')" title="<?= __('移除'); ?>"><i class="iconfont icon-cancel"></i></a>
                        </div>
                        <br>
						...
						<div  id='supplier_slider1_upload' class="image-line upload-image" ><?= __('图片上传'); ?></div>
						<label title="<?= __('请输入图片要跳转的链接地址'); ?>" >
							<i class="fa fa-link"></i>
							<input class="ui-input w400" style="margin:8px 0" type="text" name="supplier_slider[supplier_slider_link1]" value="<?=@($data['supplier_slider_link1']['config_value'])?>" placeholder="<?= __('请输入图片要跳转的链接地址'); ?>">
						</label>
					    <span class="err"><label for="supplier_slider_link1" class="error valid"></label></span>
						<p class="notic"><?= __('请使用宽度'); ?>1900<?= __('像素，高度'); ?>350<?= __('像素的'); ?>jpg/gif/png<?= __('格式图片作为幻灯片'); ?>banner<?= __('上传，'); ?><br>
				...
				<dl class="row">
					...
					<dd class="opt">
                        ...
                        <div class="div-image">
                            ...
                            <a href="javascript:void(0)" class="del" onclick="delImage('supplier_slider2_review')" title="<?= __('移除'); ?>"><i class="iconfont icon-cancel"></i></a>
                        </div>
                        <br>
						...
						<div  id='supplier_slider2_upload' class="image-line upload-image" ><?= __('图片上传'); ?></div>							 
						<label title="<?= __('请输入图片要跳转的链接地址'); ?>" class="">
							<i class="fa fa-link"></i>
							<input class="ui-input  w400" type="text" name="supplier_slider[supplier_slider_link2]" value="<?=@($data['supplier_slider_link2']['config_value'])?>" placeholder="<?= __('请输入图片要跳转的链接地址'); ?>">
						</label>
				...
				<dl class="row">
					...
					<dd class="opt">
					<textarea style="width: 500px;height: 200px;" maxlength="100" id="content" name="supplier_slider[supplier_slider_tip]"><?=@($data['supplier_slider_tip']['config_value'])?></textarea>
