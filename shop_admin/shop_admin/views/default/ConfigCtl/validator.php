...
<div class="wrapper page">
	...
	<p class="warn_xiaoma"><span></span><em></em></p><div class="explanation" id="explanation">
		...
		{
			?>
			<ul><li><span  class="red"><?= __('程序有异常！'); ?></li>
				<li>&nbsp;</li>
			<?php
			if (@$data['file']['modify'])
			{
				...
				{
					echo sprintf('<li><span  class="red">'.__('被修改文件: %s ').'</li>', $item);
				}
			}
			...
			if (@$data['name']['increase'])
			{
				...
				{
					echo sprintf('<li><span  class="red">'.__("新增文件: %s").'</li>', $item);
				}
			}
			...
			if (@$data['name']['decrease'])
			{
				...
				{
					echo sprintf('<li><span  class="red">'.__("缺失文件: %s ").'</li>', $item);
				}
			}
			?>
			</ul>
		...
		{
			?>
			<ul><li><span  class="green"><?= __('很好,程序无异常!'); ?></li></ul>
			<?php
		}
		?>
	<form method="post" enctype="multipart/form-data" id="validator-form" name="form1">
		...
		<div class="ncap-form-default">
			<div class="bot"><a href="javascript:void(0);" class="ui-btn ui-btn-sp submit-btn"><?= __('创建基准程序'); ?></a></div>
		</div>
	</form>
</div>
		...
		parent.$.dialog.confirm(__('修改立马生效,是否继续？'), function ()
			{
				...
				Public.ajaxPost(SITE_URL + "?ctl=Config&typ=json&met=setStandard", {}, function (e)
				{
					if (200 == e.status)
					{
						parent.Public.tips({content: __("创建基准成功！")});
					}
					else
					{
						parent.Public.tips({type: 1, content: __("创建基准成功！")+ e.msg})
					}
