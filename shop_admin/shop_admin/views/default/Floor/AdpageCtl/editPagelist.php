...
<div style="   overflow: hidden;
   ...
   <input type="hidden" name="page_id"  id="page_id" value="<?=$data['items']['page_id']?>" />
    <table class="form-table-style" style="margin-top:-8px;">
       	<tr>
        	<th colspan="2"><em>*</em><?= __('板块名称：'); ?></th>
        </tr>
        <tr>
            <td width="300"><input type="text" class="text w250" name="page_name" id="page_name" value="<?=$data['items']['page_name']?>" maxlength="4"/></td>
            <td><p class="hits"><?= __('最多4个字'); ?></p></td>
        </tr>
       	<tr>
        	<th colspan="2"><?= __('色彩风格：'); ?></th>
        </tr>
        <tr>
            <td>
                <input type="hidden" name="page_color" id="page_color" value="<?=$data['items']['page_color']?>" />
            <ul class="color_list fn-clear" style="background-color:#e7d9d9;">
            <?php
                if(!empty($data['color_floor'])){
                foreach($data['color_floor'] as $key => $value){
            ...
            <td><p class="hits"><?= __('选择板块色彩风格将影响商城首页模板该区域的边框、背景色、字体色彩，但不会影响板块的内容布局。'); ?></p></td>
        </tr>

  
       	<tr>
        	<th colspan="2"><?= __('排序：'); ?></th>
        </tr>
        <tr>
            <td><input type="text" class="text w250" name="page_order" id="page_order" value="<?=$data['items']['page_order']?>" /></td>
            <td><p class="hits"><?= __('数字范围为'); ?>0~255<?= __('，数字越小越靠前'); ?></p></td>
        </tr>
       	<tr>
        	<th colspan="2"><?= __('排序：'); ?></th>
        </tr>
        <tr>
            <td>
                <label><input type="radio" name="page_status"  id="page_status" <?php if($data['items']['page_status'] == "1"){ ?> checked="checked" <?php }?>value="1" /><?= __('开启'); ?></label>&nbsp;&nbsp;
            	<label><input type="radio" name="page_status"  id="page_status" <?php if($data['items']['page_status'] == "0"){ ?> checked="checked" <?php }?> value="0" /><?= __('关闭'); ?></label>
            </td>
            <td></td>
        </tr>     
