    ...
    <div class="items-box">
        <ul class="sale-limit-items">
            <li v-for="(item,index) in contentList" :key="index" :id="item.goods_id">
                ...
                <button class="del-contentGood" v-on:click="delSale(item.goods_id,index)"><?= __('删除'); ?></button>
            </li>
        </ul>
    </div>
        ...
        mounted:function(){
            ...
            for(var a=0; a<api.data.item.length; a++) {
                this.contentList.push({
                    goods_image:api.data.item[a].goods_image,
                    goods_name:api.data.item[a].goods_name,
                    discount_price:api.data.item[a].discount_price,
                    goods_id:api.data.item[a].goods_id,
                })
                this.idList.push(api.data.item[a].discount_goods_id);
            }
            ...
            adddsGoods:function(idx){
                ...
                if(this.contentList.length<this.max){
                    this.contentList.push({
                        ...
                        goods_id:this.discountList[idx].goods_id
                    })
