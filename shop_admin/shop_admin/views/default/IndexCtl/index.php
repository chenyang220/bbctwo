...
<div id="container" class="cf">
    ...
    <div class="col-hd cf">
        ...
        <div class="right cf">
            ...
            ?>
            <ol>
                <li><a href="<?= $ucenter_admin_url ?>" target="_blank"><i class="nav_href">用户中心</i></a></li>
                <li><a href="<?= $paycenter_admin_api_url ?>" target="_blank"><i class="nav_href">支付中心</i></a></li>
                <?php
                $im_statu = Yf_Registry::get('im_statu');
                if ($im_statu == 1) {
                    ?>
                <?php } ?>
                <li id="analytics" style="display:none"><a href="<?= Yf_Registry::get('analytics_api_url') ?>" target="_blank"><i class="nav_href"><?php echo __('数据统计') ?></i></a></li>
                <li><img src="<?= Yf_Registry::get('ucenter_api_url') ?>?ctl=Index&met=img&user_id=<?= Perm::$userId ?>">
                    <div class="exchange-drap"><span><?= Perm::$row['user_account'] ?></span>
                </li>
...
<script>
    ...
    $(function(){
        $.post(SITE_URL + '?ctl=Config&met=getAnalyticsConfig&typ=json', {}, function (data) {
            if (data.status == 200) {
                $('#analytics').show();
            } else {
                $('#analytics').hide();
    ...
    // 账号下拉
    $(".exchange-drap").click(function(){
        $(".exchange-account").toggle(100);
    })
    // 编辑账号
    $(".js-account-edit").click(function(){
        $.dialog({
            lock: !0,
            width: 420,
            height: 280,
            title: "编辑账号",
            content: 'url:' + SITE_URL + '?ctl=Index&met=accountEdit&typ=e',
            ok: function() {
