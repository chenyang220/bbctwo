                    ...
                    <dl class="row">
                        ...
                        <dd class="opt">
                            <div class="input-file-show"><span class="type-file-box">
                                ...
                                <div name="button" id="button" class=""><?= __('选择上传'); ?></div>
                            </span></div>
...
<script>
    $(function() {
        var api = frameElement.api,
            ...
            img_data = {};
        $('#dialog_image_desc').text('<?= __('推荐图片尺寸'); ?>'+api.data.image_spec);
        image_spec = api.data.image_spec.split('*');
        image_spec_width = image_spec[0];
        image_spec_height = image_spec[1];
        if (dialog_type) {
            if (layout_data.rectangle1) {
                $('#dialog_item_image').show().prop('src', layout_data.rectangle1.image);
                $('#dialog_item_image_data').val(layout_data.rectangle1.image_data);
                $('#dialog_item_image_type').val(layout_data.rectangle1.image_type);
                $('#textfield').val(layout_data.rectangle1.image);
            }
            else {
        ...
        function change_image_type_desc(type) {
            ...
            if (type != '') {
                //desc_array['keyword'] = '<?//= __('关键字类型会根据搜索关键字跳转到商品搜索页面，输入框填写搜索关键字。'); ?>//';
        ...
        new UploadImage({
            ...
            callback: function(res) {
                $('#dialog_item_image').prop('src',res.url);
                $("#textfield").val(res.url);
                if (!image_name) {
