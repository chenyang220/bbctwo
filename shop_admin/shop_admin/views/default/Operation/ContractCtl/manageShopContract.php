...
<body class="<?=$skin?>">
    <form method="post" id="manage-form" name="settingForm">
	    ...
	    <dl class="row">
                <dt class="tit"><?= __('关闭状态'); ?></dt>
                <dd class="opt">
                    <ul class="nofloat">
                        <li>
						<div class="onoff" id="contract_type_state">
								<label title="<?= __('允许使用'); ?>" class="cb-enable <?=($data['contract_state_etext']=='inuse' ? 'selected' : '')?> " for="contract_state_enable"><?= __('允许使用'); ?></label>
								<label title="<?= __('永久禁止使用'); ?>" class="cb-disable <?=($data['contract_state_etext']=='unuse' ? 'selected' : '')?>" for="contract_state_disabled"><?= __('永久禁止使用'); ?></label>
