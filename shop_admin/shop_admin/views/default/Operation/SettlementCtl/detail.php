    ...
    <div class="wrapper page">
        <div class="fixed-bar">
            <div class="item-title">
                <div class="subject">
                    <h3><?= __('结算管理'); ?> - <?= __('账单明细'); ?></h3>
                    <h5><?= __('实物商品订单结算索引及商家账单表'); ?></h5>
                </div>
            </div>
        </div>
        <!-- <?= __('操作说明'); ?> -->
        <p class="warn_xiaoma"><span></span><em></em></p><div class="explanation" id="explanation">
            ...
            <ul>
                <li><?= __('账单计算公式：订单金额'); ?>(<?= __('含运费'); ?>) + <?= __('红包金额'); ?> - <?= __('佣金金额'); ?> - <?= __('退还红包金额'); ?> - <?= __('退单金额'); ?> + <?= __('退还佣金'); ?> - <?= __('店铺消费'); ?> - <?= __('分销佣金总额'); ?></li>
                <li><?= __('账单处理流程为：系统出账'); ?> > <?= __('商家确认'); ?> > <?= __('平台审核'); ?> > <?= __('财务支付'); ?>(<?= __('完成结算'); ?>) 4<?= __('个环节，其中平台审核和财务支付需要平台介入，请予以关注'); ?></li>
            </ul>
        </div>

        <div class="ncap-form-default">
            <dl class="row">
                <dt class="tit"><?= __('结算单号'); ?></dt>
                <dd class="opt">
                    <ul class="nofloat">
                        <li>
            ...
            <dl class="row">
                <dt class="tit"><?= __('起止日期'); ?></dt>
                <dd class="opt">
                    <ul class="nofloat">
                        <li>
                            <span><?= $data['items']['os_start_date'] ?> <?= __('至'); ?> <?= $data['items']['os_end_date'] ?></span>
                        </li>
                    </ul>
                </dd>
            </dl>
            <dl class="row">
                <dt class="tit"><?= __('出账日期'); ?></dt>
                <dd class="opt">
                    <ul class="nofloat">
                        <li>
            ...
            <dl class="row">
                <dt class="tit"><?= __('平台应付金额'); ?></dt>
                <dd class="opt">
                    <ul class="nofloat">
                        <li>
                        <span><?= $data['items']['os_amount'] ?> = <?= $data['items']['os_order_amount'] ?>
                            (<?= __('订单金额'); ?>) + <?= $data['items']['os_redpacket_amount'] ?>
                            (<?= __('红包金额'); ?>) - <?= $data['items']['os_commis_amount'] ?>
                            (<?= __('佣金金额'); ?>) - <?=  $data['items']['os_redpacket_return_amount']?>
                            (<?= __('退还红包金额'); ?>) - <?=  $data['items']['os_order_return_amount']?>
                            (<?= __('退单金额'); ?>) + <?= $data['items']['os_commis_return_amount'] ?>
                            (<?= __('退还佣金'); ?>) - <?= $data['items']['os_shop_cost_amount'] ?>
                            (<?= __('店铺促销费用'); ?>) - <?= $data['items']['os_directseller_amount']?>
                            (<?= __('分销佣金总额'); ?>)</span>
                        </li>
                    </ul>
                </dd>
                            ...
                            <ul class="nofloat">
                                <li>
                                    <a id="btn_handle_submit" class="ui-btn ui-btn-sp submit-btn"
                                       href="javascript:void(0)"><?= __('审核'); ?></a>
                                </li>
                            </ul>
                        </dd>
                    </dl>
                    ...
                    <dl class="row">
                        ...
                        <dd class="opt">
                            <ul class="nofloat">
                                <li>
                                    <a id="btn_handle_submit" class="ui-btn ui-btn-sp submit-btn"
                                       href="javascript:void(0)"><?= __('完成付款'); ?></a>
                                </li>
                            </ul>
                        </dd>
                    </dl>
                        ...
                        <li><a <?php if ($data['tab'] == "order")
                            {
                                ...
                                href="<?= Yf_Registry::get('url') ?>?ctl=Operation_Settlement&met=detail&id=<?= $data['id'] ?>&tab=order"><span><?= __('订单列表'); ?></span></a>
                        </li>
                        <li><a <?php if ($data['tab'] == "return")
                            {
                                echo "class='current'";
                            } ?>
                                href="<?= Yf_Registry::get('url') ?>?ctl=Operation_Settlement&met=detail&id=<?= $data['id'] ?>&tab=return"><span><?= __('退单列表'); ?></span></a>
                        </li>
                        <li><a <?php if ($data['tab'] == "shop")
                            {
                                echo "class='current'";
                            } ?>
                                href="<?= Yf_Registry::get('url') ?>?ctl=Operation_Settlement&met=detail&id=<?= $data['id'] ?>&tab=shop"><span><?= __('店铺费用'); ?></span></a>
                        </li>
                    <?php }
                        ...
                        <li><a <?php if ($data['tab'] == "used")
                            {
                                ...
                                href="<?= Yf_Registry::get('url') ?>?ctl=Operation_Settlement&met=detail&id=<?= $data['id'] ?>&tab=used"><span><?= __('已使用'); ?></span></a>
                        </li>
                        <li><a <?php if ($data['tab'] == "unused")
                            {
                                echo "class='current'";
                            } ?>
                                href="<?= Yf_Registry::get('url') ?>?ctl=Operation_Settlement&met=detail&id=<?= $data['id'] ?>&tab=unused"><span><?= __('已过期'); ?></span></a>
                        </li>
                    <?php } ?>
                </ul>
            </div>
        ...
        <div class="wrapper">
            <div class="mod-toolbar-top cf">
                ...
                <div class="fr">
                <a class="ui-btn" id="btn-excel"><?= __('导出'); ?><i class="iconfont icon-btn04"></i></a>
