...
<body class="<?=$skin?>">
	<div id="manage-wrap" class="manage-wrap">
		<div class="ncap-form-default">
                    ...
                    <div class="rule-goods-list">
                        <ul class="promotion-ms">
                            ...
                            <li title="<?=$goods['goods_name']?>"> <img alt="" src="<?=$goods['goods_image']?>" style="width:30px;">
                                ...
                                <span><?= __('商品价：'); ?><em> <?= __('￥'); ?><?=$goods['goods_price']?></em></span>
                            </li>
                            <?php } ?>
                        </ul>
                    </div>
            ...
            <dl class="row">
                <dt class="tit"><?= __('活动换购规则'); ?></dt>
                <dd class="opt">
                    <div class="rule-goods-list">
                        <?php if(@$data['rule']){ foreach($data['rule'] as $key=>$rule){ ?>
                        <div class="cou-rule">
                            <span><?= __('规则'); ?><?=$key+1?><?= __('：消费满'); ?><strong><?=$rule['rule_price']?></strong><?= __('元可换购最多'); ?><strong><?=$rule['rule_goods_limit']?></strong><?= __('种优惠商品'); ?></span>
                            ...
                            <ul class="promotion-ms">
                                <?php foreach($rule['redemption_goods'] as $kk=>$rede_goods){ ?>
                                    <li title="">
                                        ...
                                        <span><?= __('换购价：'); ?><em> <?= __('￥'); ?><?=$rede_goods['redemp_price']?></em></span>
                                    </li>
                                <?php } ?>
                            </ul>
                            <?php }  ?>
