...
<div id="saleBox" class='sale-box' v-show="show">
    ...
    <div class="table-box">
        <table class="sale-table sale2 table-presale" cellspacing="0" cellpadding="0" >
            <thead>
            <tr>
            ...
            <tbody>
            <tr v-for="(item,index) in presaleList" :key="index" :data-id="item.presale_goods_id">
                <td>{{item.presale_name}}</td>
                <td>{{item.shop_name}}</td>
                <td><img :src="item.goods_image"></td>
                <td>{{item.goods_price}}</td>
                <td>{{item.presale_price}}</td>
    ...
    var vm=new Vue({
        ...
        mounted:function(){
            ...
            if(api.data.data.length !="0"){
                if(api.data.data[0].type=="presale" ){
                    that.contentList=api.data.data[0].content_info;
                    console.log(that.contentList);
                    for(var q=0;q<that.contentList.length;q++){
                        that.idList.push(that.contentList[q].presale_goods_id);
                    }
                }
            }

            axios.get(BASE_URL+"/index.php?ctl=Promotion_Presale&met=getPresaleGoodsList&typ=json",{
                params:{
                    presale_state:1,
                ...
                if (res.data.data.length == 0) {
                    ...
                    that.show = true;
                    that.presaleList = res.data.data.items;
                    that.totalnum = res.data.data.total;
                }
                console.log( that.presaleList);
            }).then(function(){
                console.log("suceess");
            }).catch(function(){
                console.log("fail");
        ...
        methods:{
            ...
            adddsGoods:function(idx){
                if(this.contentList.length<this.max){
                    this.contentList.push({
                        goods_image:this.presaleList[idx].goods_image,
                        goods_name:this.presaleList[idx].goods_name,
                        goods_price:this.presaleList[idx].goods_price,
                        presale_price:this.presaleList[idx].presale_price,
                        goods_id:this.presaleList[idx].presale_goods_id
                    });
                    this.idList.push(this.presaleList[idx].presale_goods_id);
            ...
            btndsSearch:function(){
                ...
                axios.get(BASE_URL+"/index.php?ctl=Promotion_Presale&met=getPresaleGoodsList&typ=json",{
                    params:{
                        presale_state:1,
                        presale_name:that.searchInput1,
