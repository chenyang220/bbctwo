	...
	<form id="manage-form" action="#">
		<div class="ncap-form-default">
            <dl class="row diqu_row">
                <dt class="tit">
                    <label for="sub_site_district_ids"><?= __('地区选择'); ?></label>
                    <a href="javascript:add_district()" class="add_row"><?= __('添加地区'); ?></a>
                </dt>
                <dd class="opt">
...
// 重新载入的时候
    ...
    function postData(t, e){   
        ...
        Public.ajaxPost(SITE_URL +"?ctl=Subsite_Config&typ=json&met=addSubsiteDistrict", params, function (e)
        {
            if (200 == e.status)
            {
                parent.parent.Public.tips({content: "<?= __('设置分站地区成功！'); ?>"});
                callback && "function" == typeof callback && callback(e.data, t, window)
            }
            else
            {
                parent.parent.Public.tips({type: 1, content: "<?= __('设置分站地区失败！'); ?>" + e.msg})
            }
        })
    }
