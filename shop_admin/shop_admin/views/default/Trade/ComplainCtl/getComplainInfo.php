...
<div class="wrapper page">
    ...
    <div class="ncap-order-style">
            ...
            <div class="tabs-panels">
                <div class="misc-info">
                    <dl>
                        ...
                        <dt><?= __('订单状态：'); ?></dt><dd><?=($data['order_detail']['status'])?></dd>
                        ...
                        <dt><?= __('下单时间：'); ?></dt><dd><?=($data['order']['order_create_time'])?> </dd>
                        <dt><?= __('订单总额：'); ?></dt><dd><?= __('￥'); ?><?=($data['order']['order_payment_amount'])?> </dd>
                    </dl>
                </div>
                    ...
                    <table>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td><?= __('￥'); ?><?=($data['complain_goods']['goods_price'])?></td>
                            </tr>
                        </tbody>
                        <tfoot>
                            <tr>
        ...
        <div class="ncap-form-default" <?=( ('talk' != $data['complain']['state']) && ('handle' != $data['complain']['state']) && ('finish' != $data['complain']['state']) ? 'id="hidden"' : '')?> >
            ...
            <dl cass="row" <?=( ('talk' != $data['complain']['state']) && ('handle' != $data['complain']['state']) ? 'id="hidden"' : '')?>>
                ...
                <dd class="opt">
                    ...
                    <div>
                        <a id="btn_refresh" class="ncap-btn" href="JavaScript:void(0);"><?= __('刷新对话'); ?></a>
                        <a id="btn_publish" class="ncap-btn" href="JavaScript:void(0);"><?= __('发布对话'); ?></a>
                    </div>
                </dd>
            </dl>
        </div>

        <!-- <?= __('投诉处理'); ?>  <?= __('（新投诉，待申诉，对话中，待仲裁）'); ?> -->
        <div class="ncap-form-default" <?=( 'finish' == $data['complain']['state'] ? 'id="hidden"' : '')?>>
            ...
            <form id="close_form" action="" method="post">
                ...
                <div class="bot complain_dialog" style="display: none;">
                    <a id="btn_handle_submit" class="ncap-btn-big ncap-btn-blue" href="javascript:void(0)"><?= __('确认提交'); ?></a>
                    <a id="btn_close_cancel" class="ncap-btn-big ncap-btn-blue" href="javascript:void(0)"><?= __('取消'); ?></a>
                </div>
            </form>
        </div>
        ...
        <div class="ncap-form-default" <?=( 'finish' != $data['complain']['state'] ? 'id="hidden"' : '')?>>
            ...
            <dl class="row">
                <dt class="tit"><?= __('处理意见：'); ?></dt>
                <dd class="opt"><?=($data['complain']['final_handle_message'])?></dd>
            </dl>
            <dl>
                <dt class="tit"><?= __('处理时间：'); ?></dt>
                <dd class="opt"><?=($data['complain']['final_handle_datetime'])?></dd>
            </dl>
            <div class="bot">
                <a id="finish_return_button" class="ncap-btn-big ncap-btn-blue"  href="JavaScript:void(0);"><?= __('返回'); ?></a>
            </div>
        </div>
    </div>
</div>
    ...
    function forbit_talk(talk_id) {
        $.ajax({
            type:'POST',
                ...
                dataType:'json',
                error:function(){
                    parent.Public.tips({type:1,content: '<?= __('对话屏蔽失败！'); ?>'});
                    //alert("<?= __('对话屏蔽失败'); ?>");
                },
                    success:function(data){
                        if(data.msg == 'success') {
                            get_complain_talk();
                            parent.Public.tips({content: '<?= __('对话屏蔽成功！'); ?>'});
                            //alert("<?= __('对话屏蔽成功'); ?>");
                            get_complain_talk();
                        }
                        else {
            ...
            success:function(data){
                ...
                if(Object.getOwnPropertyNames(data.data).length >= 1) {
                    for(var i = 0; i < Object.getOwnPropertyNames(data.data).length-1; i++)
                    {
                        ...
                        {
                            var mask_state = "<a onclick='forbit_talk("+data.data[i].talk_id+")' href='#'><?= __('屏蔽'); ?></a>"
                        }else
                        {
                            var mask_state = '';
                        }

                        var link = "<p class='"+data.data[i].acc_type+"'>"+data.data[i].talk_datetime+data.data[i].member_type+'('+data.data[i].user_name+')<?= __('说：'); ?>'+data.data[i].talk_content+mask_state+"</p>";
