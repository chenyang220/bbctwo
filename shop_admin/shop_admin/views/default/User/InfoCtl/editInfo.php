        ...
        <div class="ncap-form-default">
			...
			<!--<dl class="row">
                ...
                <dd class="opt">
                    ...
                    <p class="notic"> <?= __('留空表示不修改密码。'); ?></p>
                </dd>
            </dl>-->
			<dl class="row">
                <dt class="tit">
                    <label><em>*</em><?= __('电子邮箱'); ?></label>
                </dt>
                <dd class="opt">
					<input id="user_email" name="user_email" readonly value="<?=($data['user_email'])?>" class="ui-input w400" type="text"/>
                    <p class="notic"> <?= __('请输入常用的邮箱，将用来找回密码、接受订单通知等。'); ?></p>
                </dd>
            </dl>
			<dl class="row">
                <dt class="tit">
            ...
            <dl class="row">
                ...
                <dd class="opt">
                    <img id="user_logo_image" name="user_logo" alt="<?= __('选择图片'); ?>" src="<?=($data['user_logo'])?>" width="120px" height="120px" />

                    <div class="image-line upload-image"  id="user_logo_upload"><?= __('上传图片'); ?><i class="iconfont icon-tupianshangchuan"></i></div>

                    <input id="user_logo"  name="user_logo" value="<?=($data['user_logo'])?>" class="ui-input w400" type="hidden"/>
                    <div class="notic"><?= __('默认会员头像图片请使用'); ?>120*120<?= __('像素'); ?>jpg/gif/png<?= __('格式的图片'); ?></div>
                </dd>
            </dl>
			<!--<dl class="row">
				<dt class="tit">
				...
				<dd class="opt">
				  <div class="onoff">
					<label title="<?= __('允许'); ?>" class="cb-enable <?=($data['user_report']==1 ? 'selected' : '')?> " for="user_report_enable"><?= __('允许'); ?></label>
					<label title="<?= __('禁止'); ?>" class="cb-disable <?=($data['user_report']==0 ? 'selected' : '')?>" for="user_report_disabled"><?= __('禁止'); ?></label>
				  ...
				  <p class="notic"><?= __('如果禁止该项则会员不能在商品详情页面进行举报。'); ?></p>
				</dd>
			</dl>
			<dl class="row">
				<dt class="tit">
				<label><?= __('允许购买商品'); ?></label>
				</dt>
				<dd class="opt">
				  <div class="onoff">
					<label title="<?= __('允许'); ?>" class="cb-enable <?=($data['user_buy']==1 ? 'selected' : '')?> " for="user_buy_enable"><?= __('允许'); ?></label>
					<label title="<?= __('禁止'); ?>" class="cb-disable <?=($data['user_buy']==0 ? 'selected' : '')?>" for="user_buy_disabled"><?= __('禁止'); ?></label>
				  ...
				  <p class="notic"><?= __('如果禁止该项则会员不能在前台进行下单操作。'); ?></p>
				</dd>
			</dl>
			<dl class="row">
				<dt class="tit">
				<label><?= __('允许发表言论'); ?></label>
				</dt>
				<dd class="opt">
				  <div class="onoff">
					<label title="<?= __('允许'); ?>" class="cb-enable <?=($data['user_talk']==1 ? 'selected' : '')?> " for="user_talk_enable"><?= __('允许'); ?></label>
					<label title="<?= __('禁止'); ?>" class="cb-disable <?=($data['user_talk']==0 ? 'selected' : '')?>" for="user_talk_disabled"><?= __('禁止'); ?></label>
			...
			<dl class="row">
				<dt class="tit">
				<label><?= __('允许会员登录'); ?></label>
				</dt>
				<dd class="opt">
				  <div class="onoff">
					<label title="<?= __('允许'); ?>" class="cb-enable <?=($data['user_delete']==0 ? 'selected' : '')?> " for="user_delete_enable"><?= __('允许'); ?></label>
					<label title="<?= __('禁止'); ?>" class="cb-disable <?=($data['user_delete']==1 ? 'selected' : '')?>" for="user_delete_disabled"><?= __('禁止'); ?></label>
				  ...
				  <p class="notic"><?= __('如果禁止该项则会员不能在前台登录。'); ?></p>
				</dd>
			</dl>
			<dl class="row">
				<dt class="tit">
				...
				<dd class="opt"><strong class="red"><?=$data['user_points'];?></strong>&nbsp;<?= __('积分'); ?> </dd>
			</dl>
			<dl class="row">
				<dt class="tit">
				  <label><?= __('经验值'); ?></label>
				</dt>
				<dd class="opt"><strong class="red"><?=$data['user_growth'];?></strong>&nbsp;<?= __('经验点'); ?> </dd>
			</dl>
			<dl class="row">
				<dt class="tit">
				  <label><?= __('可用预存款'); ?></label>
				</dt>
				<dd class="opt"><strong class="red"><?=$data['user_cash'];?></strong>&nbsp;<?= __('元'); ?> </dd>
			</dl>
			<dl class="row">
				<dt class="tit">
				  <label><?= __('冻结预存款'); ?></label>
				</dt>
				<dd class="opt"><strong class="red"><?=$data['user_freeze_cash'];?></strong>&nbsp;<?= __('元'); ?> </dd>
			</dl>
        </div>
    </form>
</div>
		...
		if ( agent.indexOf("msie") > -1 && (version = agent.match(/msie [\d]/), ( version == "msie 8" || version == "msie 9" )) ) {
			...
			$('#user_logo_upload').on('click', function () {
				$.dialog({
					...
					data: { SHOP_URL: SHOP_URL, width: 120, height: 120, callback: callback },    // <?= __('需要截取图片的宽高比例'); ?>
					width: '800px',
