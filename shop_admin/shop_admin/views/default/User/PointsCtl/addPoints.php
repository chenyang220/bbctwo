    ...
    <form method="post" enctype="multipart/form-data" id="points-add-form" name="form">		
        <div class="ncap-form-default">
			...
			<dl style="display: none;" class="row" id="tr_memberinfo">
				<dt class="tit"><?= __('符合条件的会员'); ?></dt>
				<dd class="opt" id="td_memberinfo"></dd>
			</dl>
			<dl class="row">
                <dt class="tit">
                    <label><em>*</em><?= __('增减类型'); ?></label>
                </dt>
                <dd class="opt">
					 <select name="points_log_type" id="points_log_type" class="valid">
						<option value="1"><?= __('增加'); ?></option>
						<option value="2"><?= __('减少'); ?></option>
					  </select>
                    <p class="notic"></p>
                </dd>
            </dl>
...
<script>
  function checkuser(){
	...
	if(username == ''){
		$("#user_id").val('0');
		$.dialog.confirm(<?= __('请输入会员名'); ?>, function() {});
		return false;
	}
	...
	$.getJSON(url, {'user_name':username}, function(a){
	        if (a.status == 200)
	        {
		        ...
		        $("#tr_memberinfo").show();
				var msg= " "+ a.data.user_name + ", <?= __('当前积分数为'); ?>" + a.data.user_points;
	        ...
	        else
	        {
	        	$("#user_name").val('');
	        	$("#user_id").val('0');
		        $.dialog.confirm(<?= __('会员信息错误'); ?>, function() {});
	        }
	});
}
</script>
