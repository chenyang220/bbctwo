...
<body class="<?=$skin?>">
    <form method="post" id="manage-form" name="settingForm">
	    ...
	    <dl class="row">
                <dt class="tit"><?= __('是否开启接口'); ?></dt>
                <dd class="opt">
                    <ul class="nofloat">
                        <li>
						<div class="onoff" id="shared_bindings_statu">
								<label title="<?= __('开启'); ?>" class="cb-enable <?=($data['shared_bindings_statu']=='1' ? 'selected' : '')?> " for="shared_bindings_statu_enable"><?= __('开启'); ?></label>
								<label title="<?= __('关闭'); ?>" class="cb-disable <?=($data['shared_bindings_statu']=='0' ? 'selected' : '')?>" for="shared_bindings_statu_disabled"><?= __('关闭'); ?></label>
