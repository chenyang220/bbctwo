...
<script>
    var vm=new Vue({
        ...
        mounted:function(){
            var that=this;
            axios.get(BASE_URL+"?ctl=Wx_TplLayout&met=tplLayoutList&typ=json",{
                params:{
                    wx_tpl_layout_id:frameElement.api.data.item_data.wx_tpl_layout_id
                }
            }).then(function(res){
                that.type=res.data.data.items[0].wx_tpl_layout_data[0].type;
                that.itemList=res.data.data.items[0].wx_tpl_layout_data[0].content_info;
                if(res.data.data.items[0].wx_tpl_layout_data[0].type){
                    that.btnEdit1=true;
                    switch(res.data.data.items[0].wx_tpl_layout_data[0].type){
                        case "groupbuy":
                        that.titleSel="团购风暴";
                        break;
                        case "discount":
        ...
        methods:{
            edit:function(){
                ...
                $.dialog({
                    ...
                    data:{
                        ...
                        forumType:forumType,
                        item_id:api.data.item_data.wx_tpl_layout_id,
            ...
            change:function(){
                ...
                if (actType && this.type != actType.type) {
                    this.type = '';
                }
            }
        }
