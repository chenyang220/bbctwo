$(function () {
    ...
    // 所有全选
    $('.all_checkbox').live('change',function () {
        if ($(this).is(":checked")){
          $('.nctouch-cart-container input[type=checkbox]').prop('checked',true);
        } else {
          $('.nctouch-cart-container input[type=checkbox]').prop('checked',false);
        }
        calculateTotalPrice();
    })
...
function calculateTotalPrice() {
    ...
    $('.cart-litemw-cnt').each(function () {
        if ($(this).find('input[name="cart_id"]').prop('checked')) {
            ...
            nums+=parseInt($(this).find('.value-box').find('input').val());
            console.log( parseInt($(this).find('.value-box').find('input').val()),nums);
        }
    });
...
$(function () {
    ...
    function computeProductNumber() {
        var $productNumber = $('#productNumber').find("em"),
