$(function () {
    ...
    //图片上传
       var i = 0;
        ...
        url: ApiUrl + "/index.php?ctl=Upload&action=snsFile",
        data: {key: k},
        start: function (e) {
             var _uploadFile = e.prop('files')[0];
             var file_type = _uploadFile.type.split("/")[0];
            if (i >= 1 && file_type=='video') {
              $.sDialog({skin: "red", content: '图片和视频不能混合上传', okBtn: false, cancelBtn: false});
              return 'error';
            }
        ...
        success: function (e, res) {
            $(".loading-box").addClass('hide');
                if (res.state == 'SUCCESS') {
                    ...
                    addExploreImages(res.url,i,res.type,res.poster_image, res.ts_url);
                } else {
                    $('.heart-img-input').removeClass('hide');
                    $(".heart-upimg-swiper").addClass('hide');
                    $("#edit_image").addClass('hide');
                    $("#explore").removeClass('hide');
                    $.sDialog({skin: "red", content: res.state, okBtn: false, cancelBtn: false});
                    return false
                }
                if(res.type == ".mp4" || res.type == ".mov"){
                    //视频只可以传一个
                    $(".swiper-add-image").addClass('hide');
                }else{
                    //图片可以添加9张
    ...
    $(document).on('click','.icon-forbid',function (event) {
        var _this = $(this);
        var images_id = _this.prev().find('img').data('images_id');
        $.ajax({
            type: "post",
                ...
                if (data.status == 200) {
                    ...
                    $.ajax({
                        ...
                        success: function (rel) {
                            if(rel.type == ".mp4" || rel.type == ".mov"){
                                var img_str = "<video controls style='width: 100%;height: 17rem;' autoplay src='" + image_url + "'' alt='video' data-images_id='" + images_id + "'>";
                                //var img_str = "<img src='" + poster + "'' alt='img' data-images_id='" + images_id + "'>";
    ...
    function addExploreImages(images_url, i,type,poster_image,ts_url) {
        $.ajax({
            type: "post",
            url: ApiUrl + "/index.php?ctl=Explore_Explore&met=addExploreImages&typ=json",
            data: {images_url: images_url,type:type,poster_image:poster_image,ts_url:ts_url, k: k, u: u},
            dataType: "json",
            success: function (data) {
                if (data.status == 200) {
                    $("#edit_image").removeClass('hide');
                    ...
                    if(type == ".mp4" || type == ".mov"){
                        var img_str = "<video autoplay style='width: 100%;height: 17rem;' controls  src='" + images_url + "'' alt='video' data-images_id='" + data.data.id + "' >";
                        //var img_str = "<img src='" + poster_image + "'' alt='img' data-images_id='" + data.data.id + "'>";
                        images_url = poster_image;
                        $(".file_type").html('编辑视频');
                    }else{
                        var img_str = "<img src='" + images_url + "'' alt='img' data-images_id='" + data.data.id + "'>";
                        var image_num = Number(i);
                       $(".file_type").html('编辑照片（<span>'+image_num+'</span>/9）');
                    }
                    var str = "<li class='swiper-slide'>\n" +
                        "  <div>\n" +
                        "   <em class='img-box edit-images-goods' style='background:url("+images_url+") no-repeat center;background-size:cover;' data-images_id='"+ data.data.id+"'><img class='hide' src='"+images_url+"' data-images_id='"+data.data.id+"'></em>\n" +
   ...
   //商品、品牌搜索
    ...
    $("#addExplore").click(function () {
        ...
        if (images_id.length <= 0){
            $.sDialog({skin: "red", content: "请上传图片/视频", okBtn: false, cancelBtn: false});
            return false;
        }
        if (explore_title == '') {
