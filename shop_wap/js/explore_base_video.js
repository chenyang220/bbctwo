...
$(function () {
    // $('#explore_images').css({'width':availWidth,'height':availHeight});
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result.status == 200) {
                    ...
                    // } else {
                    //     jsmpegPlay(canvas,'out.ts',data.explore_images[0].poster_image, startCallBack, playingCallBack, endCallBack)
                    // }

                    // function jsmpegPlay(Vcanvas, vVideo, poster,startFun, playingFun, endFun) {
                    ...
                    $("#video-operate").html(info_html);

                    //内容 标签
                    var info_html = template.render("content_info", data);
                    $("#video-bottom-operate").html(info_html);

                    //详情内容 标签
                    var info_html = template.render("content_info_s", data);
                    $("#video-see-html").html(info_html);
    ...
    $.ajax({
        ...
        data: {k: getCookie("key"), u: getCookie("id"), type: 2, search_status: "", search_content: ""},
        type: "POST",
        dataType: "json",
        success: function (result) {
            $.each(result.data.explore_base, function (i, t) {
                if (t.type == ".mov" || t.type == ".mp4") {
                    s++;
                    row[s] = t.explore_id;
                }

                if (t.explore_id == explore_id) {
                    exid = s;
                }
    ...
    $(document).on("click", "#explore_images", function (e) {
        chicks++;
        if (chicks == 2) {
            if (!key) {
...
// 分享
    ...
    $(document).on("click", ".weixin", function () {
        //分享获得积分
        $.ajax({
            url: ApiUrl + "/index.php?ctl=Explore_Explore&met=addjifeng&typ=json",
            type: "get",
            data: {explore_id: explore_id, k: key, u: getCookie("id"), user_account: getCookie("user_account")},
            dataType: "json",
            success: function (e) {
...
//用户关注与取消关注按钮
    $(document).on("click", ".follow-btn", function () {
        ...
        var guanzhu = $(".follow-btn").find('em').html();
...
//
//显示所有评论
    $(document).on("click", ".js-video-comments", function () {
        getCommentAll();
        // $(document.body).css({"position":"fixed","width":"100%","height":"100%","overflow":"hidden"});
        // document.body.addEventListener('touchmove',handler,false);
        // document.body.addEventListener('wheel',handler,false);
...
// })
    ...
    $("body").on("click", ".js-video-save", function (e) {

        if ($(".js-video-save i").hasClass('active')) {
            //取消收藏
            $.ajax({
...
//点击发布看是否登录
    $(document).on("click", ".icon-xiangji1", function () {
        if (getCookie("key")) {
...
//点击发布看是否登录
    $(document).on("click", ".icon-user", function () {
        if (getCookie("key")) {
            window.location.href = ShopWapUrl + "/tmpl/explore_center.html?user_id=" + id;
        } else {
            $(".social-login-dialog").show();
        }
    });
...
//点击发布看是否登录
    $(document).on("click", ".comment-close", function () {
        ...
        //alert(234);
        $("#heart-comments-more").removeClass('up');
        $("#heart-comments-more").addClass('down');
    });
//点击发布看是否登录
    $(document).on("click", ".xiangqing", function () {

        //alert(234);
        $("#video-see-html").removeClass('up');
        $("#video-see-html").addClass('down');
    });

    //心得点赞与取消点赞按钮
    $("body").on("click", ".js-video-zan", function (e) {
        //未登录显示登录框
        if (!key) {
            $(".social-login-dialog").show();
            return false;
...
function keyup_submit(e) {
    ...
    // if (evt.keyCode == 13) {
        ...
        var comment_content = $("#pinglun").val();
