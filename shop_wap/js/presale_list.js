...
var hasmore = true;
var footer = false;
var discount_title = getQueryString("discount_title")?getQueryString("discount_title"):'预售';
...
function get_list() {
    ...
    param.presale_goods_id = getQueryString("presale_goods_id");
//定位到上次浏览的商品
    var goods_pos = getCookie('goods_pos');
    param.pos = goods_pos
    $.getJSON(ApiUrl + "/index.php?ctl=Goods_Goods&met=getPresaleGoodsList&typ=json&ua=wap&sub_site_id=" + sub_site_id, param, function (e) {
        if (e.status == 200){
            if (!e) {
                e = [];
                e.datas = [];
