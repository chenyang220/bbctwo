...
setCookie("goods_id_cart",goods_id)
var map_list = [];
var map_index_id = "";
var shop_id;
$(function () {
            ...
            success: function (result) {
                ...
                if (result.status == 200) {
                    ...
                    if (data.goods_info) {
                        ...
                        //渲染模板
						var imgarr=data.goods_image;
						var newarr=[];
						imgarr.map(function(value,index){
							var newImage=value.split("!");
							newarr.push(newImage[0]);
						})
						data.goods_image=newarr;
                        var html = template.render("product_detail", data);
						
                        $("#product_detail_html").html(html);
						// 服务保障
						$.animationUp({
						    valve: ".js-services-det",            // 动作触发
						    wrapper: "#services-det-con",                   // 动作块
						});
                        ...
                        //礼包商品隐藏加减icon
                        if (goodsInformation.goods_info.cat_id == 9002) {
                            $(".minus").hide();
                            $(".add").hide();
                            ...
                            $("#buy-now").click(function () {
                                ...
                                if (!key) {
                                    ...
                                    if (data.goods_info.goods_parent_id > 0 && data.goods_info.product_is_behalf_delivery == 1) {
                                        ...
                                        $.ajax({
                                            url: ApiUrl + "/index.php?ctl=Buyer_Cart&met=addCart&typ=json",
                                            data: {k: key, u: getCookie("id"), goods_id: getCookie("goods_id_cart"), goods_num: buynum, buy_now:1,seckill_goods_id:seckill_goods_id},
                                            type: "post",
                                            success: function (result) {
                                                if (checkLogin(result.login)) {
                                                    if (result.status == 200) {
                ...
                $.animationUp({
                    ...
                    start: function () {                                    // 开始动作触发事件
                        $(".goods-detail-foot").addClass("hide").removeClass("flex");
                    ...
                    close: function () {                                    // 关闭动作触发事件
                        ...
                        $(".goods-detail-foot").removeClass("hide").addClass("flex");
                    }
                });
    ...
    $("body").on("click", "#ziti", function () {
        var key = getCookie("key");//登录标记
        var is_delivery = $(this).data('is_delivery');
        if (!key) {
            //window.location.href = WapSiteUrl+'/tmpl/member/login.html';
            callback = window.location.href;
            login_url = UCenterApiUrl + "?ctl=Login&met=index&typ=e";
        ...
        $.getJSON(ApiUrl + "/index.php?ctl=Goods_Goods&met=chain&typ=json", {
            ...
            if (result.status == 200) {
                window.location.href = WapSiteUrl + "/tmpl/ziti.html?goods_id=" + goods_id + "&shop_id=" + shop_id + "&goods_num=1&is_delivery=" + is_delivery;
            } else {
                $.sDialog({
                    skin: "red",
                    content: result.msg,
...
$(function(){
    $.ajax({
        ...
        success: function (data) {
            ...
            {
                html += '<p class="attr-p"><span>品牌</span><em class="one-overflow">'+ data.data.brand_name +'</em></p>';
            }
            if(data.data.common_property_row)
            {
                for(var i in data.data.common_property_row)
                {
                    if(data.data.common_property_row[i]){
                        html += '<p class="attr-p"><span class="one-overflow">'+ i +'</span><em class="one-overflow">'+ data.data.common_property_row[i] +'</em></p>';
                    }
                }
            }
