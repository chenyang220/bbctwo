...
function get_list(goods_time_slot) {
    if(!goods_time_slot){
       var goods_time_slot= $('#now_time').attr('index');
    }
    // $(".loading").remove();
    if (!hasmore) {
        return false
    ...
    param.goods_time_slot = goods_time_slot;
//定位到上次浏览的商品
    ...
    $.getJSON(ApiUrl + "/index.php?ctl=Goods_Goods&met=getSeckillGoodsList&typ=json&ua=wap&sub_site_id=" + sub_site_id, param, function (e) {
        if (e.status == 200){
            ...
            var r = template.render("home_cat", e);
            $("#modal #cat-list").append(r);
            //hasmore = e.hasmore
...
$(document).on("click", ".qie", function () {

    $('.qie').css('color','rgba(74,74,74,1)');
    $(this).css('color','red');
    hasmore = true;
    $('.goods-secrch-list').html('');
    $('#cat-list').html('');
    var goods_time_slot = $(this).attr('index');
    get_list(goods_time_slot);
