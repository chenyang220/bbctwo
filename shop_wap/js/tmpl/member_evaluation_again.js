$(function () {
    ...
    $.getJSON(ApiUrl + "/index.php?ctl=Buyer_Order&met=getEvaluationByOrderId&typ=json", {k: e,u:u, order_id: a}, function (r) {
        ...
        $(".btn-l").click(function () {
            ...
            if(from == 'chain')
            {
                window.location.href = WapSiteUrl + "/tmpl/member/member_evaluation_again_add.html?from=chain&oge_id="+a; //门店自提订单
            }
            else if(from == 'vr'){
                window.location.href = WapSiteUrl + "/tmpl/member/member_evaluation_again_add.html?from=vr&oge_id="+a; //虚拟订单
            }else{
