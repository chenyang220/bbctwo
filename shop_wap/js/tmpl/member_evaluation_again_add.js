        ...
        $(".btn-l").click(function () {
            ...
            $.ajax({
                ...
                success: function (e) {
                    ...
                    checkLogin(e.login);
                    if (e.status == 250) {
                        $.sDialog({skin: "red", content: '追加评价失败!'+e.msg, okBtn: false, cancelBtn: false});
                        return false
                    }
                    if(from == 'chain')
                    {
                        window.location.href = WapSiteUrl + "/tmpl/member/chain_order_list.html"; //门店自提订单列表页
                    }
                    else if(from == 'vr')
                    {
                        window.location.href = WapSiteUrl + "/tmpl/member/vr_order_list.html"; //虚拟订单列表页
                    }else{
