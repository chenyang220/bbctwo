        ...
        $(".btn-l").click(function () {
            ...
            $.ajax({
                ...
                success: function (e) {
                    checkLogin(e.login);
                    if (e.status == 250) {
                        $.sDialog({skin: "red", content: '加评价失败!'+e.msg, okBtn: false, cancelBtn: false});
                        return false
                    }
                    // window.location.href = WapSiteUrl + "/tmpl/member/vr_order_list.html"
                    window.location.href = WapSiteUrl + "/tmpl/member/member_evaluation_again.html?from=vr&order_id=" + t;
                }
            })
        })
    })
