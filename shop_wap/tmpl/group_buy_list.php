   ...
   <!--  <div id="Group-head" class="clearfix">
    ...
    <!-- <?= __('新增团购搜索'); ?> -->
      <header id="header" class="nctouch-product-header fixed">
        <div class="header-wrap">
            ...
            <div class="header-inp clearfix" style="display:none"><a href="../tmpl/search.html?f=groupbuy&groupbuy_type=1" id="group_buy_search"> <i class="icon"></i> <span class="search-input col2" id="keyword"><?= __('线上团'); ?></span></a></div>
            <div class="header-title"><h1 class=" group_buy"><?= __('线上团'); ?></h1></div>
    ...
    <div class="goods-search-list-nav">
        <ul id="nav_ul">
            ...
            <li><a href="javascript:void(0);" class="" id="grade_default"><?= __('团购状态'); ?><i></i></a></li>
    ...
    <div id="sort_inner" class="goods-sort-inner hide">
        <span><a href="javascript:void(0);" onclick="init_get_list('order', '')"><?= __('默认排序'); ?><i></i></a></span>
        <span><a href="javascript:void(0);" onclick="init_get_list('order', 'pricedesc')"><?= __('价格从高到低'); ?><i></i></a></span>
        <span><a href="javascript:void(0);" onclick="init_get_list('order', 'priceasc')"><?= __('价格从低到高'); ?><i></i></a></span>
        <span><a href="javascript:void(0);" onclick="init_get_list('order', 'saledesc')"><?= __('销量优先'); ?><i></i></a></span>
    </div>
    <div id="grade_inner" class="goods-sort-inner hide">
        <span><a href="javascript:void(0);" onclick="init_get_list('state', '')"><?= __('正在进行'); ?><i></i></a></span>
        <span><a href="javascript:void(0);" onclick="init_get_list('state', 'underway')"><?= __('即将开始'); ?><i></i></a></span>
    </div>
...
<script type="text/html" id="swiper-container-nav">
    <div class="swiper-wrapper">
        ...
        <div class="swiper-slide" onclick="location.href='group_buy_list.html?groupbuy_type=<%= data.cat.type %>'"><?= __('全部'); ?></div>
        <% }else{ %>
        <div class="swiper-slide bor-2" onclick="location.href='group_buy_list.html?groupbuy_type=<%= data.cat.type %>'"><?= __('全部'); ?></div>
        <% }%>
...
<script type="text/html" id="maskG">
    <div class="title">
        <a href="group_buy_list.html?groupbuy_type=1"><?= __('线上团'); ?></a>
    </div>
    ...
    <div class="title">
        <a href="group_buy_list.html?groupbuy_type=2"><?= __('虚拟团'); ?></a>
    </div>
    <ul class="list clearfix">
        <% if(data.cat.virtual){ %>
...
<script type="text/html" id="goods-secrch-list">
    ...
    <li class="goods-item" goods_id="<%= items[i].goods_id %>" onclick="location.href='product_detail.html?goods_id=<%= items[i].goods_id %>'">
        ...
        <dl class="goods-info">
            ...
            <dd class="goods-sale">
                <a href="product_detail.html?goods_id=<%= items[i].goods_id %>">
                    <span class="goods-price"><?= __('￥'); ?><em><%= items[i].groupbuy_price %></em></span>
                </a>
            </dd>
            </dt>
            <dd class="goods-assist goods-tuangou">
                <a href="product_detail.html?goods_id=<%= items[i].goods_id %>">
                                <span class="goods-sold"><?= __('已售'); ?>
...
<script type="text/html" id="search_items">
    <div style="transition-timing-function: cubic-bezier(0.1, 0.57, 0.1, 1); transition-duration: 0ms; transform: translate(0px, 0px) translateZ(0px);">
        <dl>
            <dt><?= __('价格'); ?></dt>
            <dd>
                <a href="javascript:;" nctype="items" onclick="init_rows('price','',this)"><?= __('不限'); ?></a>
                <% if(data.price_range){ %>
        ...
        <dl>
            <dt><?= __('区域'); ?></dt>
            <dd>
                <a href="javascript:void(0);" nctype="items" onclick="init_rows('area_id','',this)"><?= __('不限'); ?></a>
                <% var items = data.area %>
                <% for( var i in items ){ %>
        ...
        <% } %>
        <div class="bottom">
            <a href="javascript:void(0);" class="btn-l" id="search_submit" onclick="search_adv()"><?= __('筛选商品'); ?></a>
        </div>
    </div>
</script>
