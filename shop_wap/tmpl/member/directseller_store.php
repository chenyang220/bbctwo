...
<body>
        ...
        <div class="header-wrap">
            ...
            <span class="header-tab" style="margin:0px;width:75%;">
				<div class="nctouch-order-search">
					<form>
						<span>
							<input type="text" autocomplete="on" maxlength="50" placeholder="<?= __('输入商品名称进行搜索'); ?>" name="goodskey" id="goodskey" oninput="writeClear($(this));" >
    ...
    <script type="text/html" id="store_banner_tpl">
        ...
        <div class="store-favorate">
			<span class="num"><em id="store_favornum"><%=data.totalsize%></em><p><?= __('商品'); ?></p></span>
        </div>
    </script>	
	...
	<div class="goods-search-list-nav posr" style="top:0px;">
		<ul id="nav_ul">
			...
			<li><a href="javascript:void(0);" class="" onclick="init_get_list('uptime', 'DESC')"><?= __('最新'); ?></a></li>
			<li><a href="javascript:void(0);" class="" onclick="init_get_list('sales','DESC');"><?= __('最热'); ?></a></li>
			<!-- <li><a href="javascript:void(0);" class="current" id="sort_default"><?= __('综合排序'); ?><i></i></a></li>
			<li><a href="javascript:void(0);" id="sales"><?= __('销量优先'); ?></a></li>
	...
	<div id="sort_inner" class="goods-sort-inner hide" style="position:static">
		...
		<span><a href="javascript:void(0);" onclick="init_get_list('price', 'DESC')"><?= __('价格从高到低'); ?><i></i></a></span> 
		<span><a href="javascript:void(0);" onclick="init_get_list('price', 'ASC')"><?= __('价格从低到高'); ?><i></i></a></span>
	</div>
...
<script type="text/html" id="home_body">
	...
	<li class="goods-item" goods_id="<%=goods_id%>">
        ...
        <dl class="goods-info" style="height:4.62rem;">
			...
			<p><span class="goods-price"><?= __('￥'); ?><em><%=goods_info.common_price%></em></span> </p>
        </dl>
    </li>
    <%}%>
    <% if (hasmore) {%>
