...
<body>
	...
	<script type="text/html" id="directseller-userapply-list-tmpl">
		...
		<li class="distribution">
			<div class="mt0"><?= __('申请条件：'); ?><span><%if(info.expenditure > 0){%><?= __('消费满'); ?><%=info.expenditure%><?= __('可申请'); ?><%}else{%><?= __('成功消费任意金额'); ?><%}%></span></div>

			<div class="distribution-z1">
				<%if(info.status == 0){%><?= __('待审核'); ?><%}else if(info.status == 1){%><?= __('审核通过'); ?><%}else{%><?= __('未申请'); ?><%}%>
			</div>

			<div><?= __('消费金额：'); ?><span><?= __('￥'); ?><%=info.expends%></span></div>
			<div><?= __('通过时间：'); ?><span><%=info.directseller.directseller_create_time%></span></div>
			<%if(info.status){%>
			<div class="distribution-z2"><?= __('已申请'); ?></div>
			<%}else if(info.expends > info.expenditure){%>
			<div class="distribution-z2" id="apply" data-shop_id="<%=info.shop_id%>"><?= __('分销申请'); ?></div>
			<%}%>
		</li>
		<%}%>
		<% if (hasmore) {%>
