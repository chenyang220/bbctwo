        ...
        <ul class="nctouch-evaluation-goods">
            <li>
                ...
                <div class="evaluation-inp-block  hg4 pl-30 pr-30">
                    <textarea type="text" class="textarea wp100 chain js-content" name="goods[<%=order_goods[0].order_goods_id%>][comment]" placeholder="<?= __('亲，写点什么吧，您的意见对其他买家有很大帮助！'); ?>" style="position: relative;"><% if(order_base.order_buyer_evaluation_status == 1){ %><%=evaluation.content%><% } %></textarea>
                    ...
                    <% } %>
                    <!--<label>
                        <input type="checkbox" class="checkbox" name="goods[<%=order_goods[0].order_goods_id%>][anony]" value="1" /><p><?= __('匿'); ?>&nbsp;<?= __('名'); ?></p>
                    </label>-->
                </div>
            </li>
        </ul>
