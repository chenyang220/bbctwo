...
<!--             <div class="header-l">
            ...
            <div class="header-inp clearfix"> <i class="icon"></i> <span class="search-input" id="keyword"><?= __('请输入关键词'); ?></span> </div>
    ...
    <div class="goods-search-list-nav">
        <ul id="nav_ul">
            ...
            <li><a href="javascript:void(0);" class="" onclick="init_get_list('2', '1')"><?= __('销量优先'); ?></a></li>
    ...
    <div id="sort_inner" class="goods-sort-inner hide"> <span><a href="javascript:void(0);" class="cur"  onclick="init_get_list('', '')"><?= __('综合排序'); ?><i></i></a></span> <span><a href="javascript:void(0);" onclick="init_get_list('2', '3')"><?= __('价格从高到低'); ?><i></i></a></span> <span><a href="javascript:void(0);" onclick="init_get_list('1', '3')"><?= __('价格从低到高'); ?><i></i></a></span> <span><a href="javascript:void(0);" onclick="init_get_list('2', '2')"><?= __('人气排序'); ?><i></i></a></span> </div>
    ...
    <script type="text/html" id="search_items">
        <div>
            <dl>
                <dt><?= __('价格区间'); ?></dt>
                <dd>
                    <span class="inp-balck"><input type="text" id="price_from" nctype="price" pattern="[0-9]*" class="inp" placeholder="<?= __('最低价'); ?>"/></span>
                    <span class="line"></span>
                    <span class="inp-balck"><input nctype="price" type="text" id="price_to" pattern="[0-9]*" class="inp" placeholder="<?= __('最高价'); ?>"/></span>
                </dd>
            </dl>
            <dl>
                <dt><?= __('商品所在地'); ?></dt>
            ...
            <dl>
                <dt><?= __('商品类型'); ?></dt>
                <dd>
                    <a href="javascript:void(0);" nctype="items" id="gift" class=""><?= __('赠品'); ?></a>
                    <a href="javascript:void(0);" nctype="items" id="groupbuy"><?= __('团购'); ?></a>
                    <a href="javascript:void(0);" nctype="items" id="xianshi"><?= __('限时折扣'); ?></a>
        ...
        <% if(goods_list.length >0){%>
            <%for(i=0;i<goods_list.length;i++){%>
                ...
                <%}%>
                    <% if (hasmore) {%>
                        <li class="loading">
                            <div class="spinner"><i></i></div><?= __('商品数据读取中'); ?>...</li>
                        <% } %>
                            <%
	   }else {
	%>
