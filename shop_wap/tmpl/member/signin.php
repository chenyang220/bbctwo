...
?>
    ...
    <div class="member-top member-top2 mb-20">
        <div class="my-pointnum"><em><i class="icon"></i><b><?= __('我的积分'); ?></b></em><span id="pointnum"></span> </div>
        <div class="sign-box" id="signdiv">
            <div id="signinbtn" class="sign-btn" style="display:none;">
                <h2><?= __('登录'); ?></h2>
                <h6>+<span class="points_signin"></span> <?= __('积分'); ?></h6>
            </div>
        ...
        <div id="description_link" class="signin-help"><i class="icon"></i><?= __('活动说明'); ?></div>
        <div id="description_info" style="display: none;">
            <h4><?= __('活动说明'); ?></h4>
            <ul>
                <li>1<?= __('、每人每天登录只可获取一次积分。'); ?></li>
        ...
        <h3 class="int-daily"><a href="pointslog_list.html"><?= __('积分明细'); ?><i class="iconfont icon-arrow-right fz-24"></i></a></h3>
    </div>
    <div class="bgf mine-definite pad20">
        <h6><img src="../../images/new-how2x.png"><?= __('如何获取积分'); ?></h6>
        <ul>
            <li><em><?= __('会员注册'); ?></em><span id="points_reg">+1<?= __('分'); ?></span></li>
            <li><em><?= __('会员登录'); ?></em><span id="points_login">+1<?= __('分'); ?></span></li>
            <li><em><?= __('商品评价'); ?></em><span id="points_evaluate">+1<?= __('分'); ?></span></li>
            <li><em><?= __('每单最多赠送'); ?></em><span id="points_order">+11<?= __('分'); ?></span></li>
            <li><em><?= __('消费与赠送比例'); ?></em><span id="points_recharge">+1<?= __('分'); ?></span></li>
        </ul>
    </div>
    <footer id="footer" class="bottom"></footer>
    <!-- <?= __('新增积分底部导航栏'); ?> -->
    <ul class="integral-foot-tab">
        <li><a href="../integral.html"><i class="icon"></i><span><?= __('积分兑换'); ?></span></a></li>
        <li class="active"><a href="signin.html"><i class="icon"></i><span><?= __('我的积分'); ?></span></a></li>
    </ul>
    ...
    <script>
        $(function ()
        {
            ...
            $.getJSON(ApiUrl + "/index.php?ctl=Api_User_Points&met=getPoint&typ=json", function (t)
            {
                ...
                $("#points_login").html('+'+points_login+'<?= __('分'); ?>');
                $("#points_reg").html('+'+points_reg+'<?= __('分'); ?>');
                $("#points_evaluate").html('+'+points_evaluate+'<?= __('分'); ?>');
                $("#points_recharge").html('+'+points_recharge+'<?= __('分'); ?>');
                $("#points_order").html('+'+points_order+'<?= __('分'); ?>');
                if(key)
                {
