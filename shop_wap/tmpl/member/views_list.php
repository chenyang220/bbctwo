...
?>
    ...
    if (u_id) {
       window.location.href = UCenterApiUrl + '/?ctl=Login&met=oauth&typ=e&u_id=' + u_id + "&return_url=" + WapSiteUrl + "/tmpl/member/views_list.html";
    }
</script>
    <body>
    <header id="header" class="fixed">
        <div class="header-wrap">
            ...
            <div class="header-r">
                <span class="history-edit js-history-edit fz-24 col8 mr-20"><?= __('编辑'); ?></span>
    ...
    <div class="nctouch-main-layout">
        ...
        <div class="history-bottom js-history-bottom clearfix">
            <label class="history-selall fl pl-20 fz-30"><input type="checkbox" class="mr-20"><em class="iblock align-middle"><?= __('全选'); ?></em></label>
            <button class="fr fz-30" id="clearbtn"><?= __('删除'); ?></button>
        </div>
    </div>
            ...
            <% for (var i=0; i < arr.items.length; i++) {%>
                <li class="goods-item flex wp100">
                    <div class="goods-check fl ml-20">
                        <input type="checkbox" value="<%=arr.items[i].common_id%>" >
                    </div>
                    <span class="goods-pic fl mr-20 ml-20">
                        <a class="posr" href="../product_detail.html?goods_id=<%=arr.items[i].goods_id%>">
                        	...
                        	<p class="old-Failed  old-Failed-wh"><?= __('此商品已失效'); ?></p>
                            <% } %>
                            <img src="<%=arr.items[i].common_image%>" />
                        </a>
                    </span>
                    <dl class="goods-info flex1">
                        <dt class="goods-name">
                            <a href="../product_detail.html?goods_id=<%=arr.items[i].goods_id%>">
                                <h4 class="more-overflow pr-20"><%=arr.items[i].common_name%></h4>
                                <h6></h6>
                            </a>
                        </dt>
                        <dd class="goods-sale">
                            <a href="../product_detail.html?goods_id=<%=arr.items[i].goods_id%>">
                                <span class="goods-price"><?= __('￥'); ?><em><%=arr.items[i].common_price%></em></span>
                            </a>
                        </dd>
                    </dl>
                </li>
