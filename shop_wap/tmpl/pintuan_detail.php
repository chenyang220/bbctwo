...
<div class="nctouch-main-layout nctouch-main-layout-bottom">
    <ul class="pt_goods_previews">
        <li class="borb1 mt0">
            ...
            <div class="pt_goods_title">
                ...
                <strong class="common-red">[<label id="pt_person_num">0</label><?= __('人团'); ?>]</strong>
                
                <p class="pt_goods_name"><?= __('拼团活动'); ?></p>
            </div>
            <dl class="clearfix">
                <dt class="fl">
                    <p class="part1"><strong class="common-red"><?= __('￥'); ?><label id="pt_price">0</label></strong><em class="common-red free-ship" id="pt_transport_fee"><?= __('包邮'); ?></em><b><?= __('单买价：￥'); ?><label id="pt_price_one">0</label></b></p>
                    <p class="part2"><b class="through"><?= __('原价：￥'); ?><label id="pt_price_ori">0</label></b></p>
                    <!-- <p class="part3"><em class="common-red free-ship" id="pt_transport_fee"><?= __('包邮'); ?></em></p> -->
                </dt>
                <dd class="fr"><em class="pt_surplus"><?= __('仅剩'); ?><label id="pt_goods_stock">0</label><?= __('件'); ?></em><em class="pt_surplus"><?= __('已拼'); ?><label id="pt_buyer_num">0</label><?= __('件'); ?></em></dd>
            </dl>
        </li>
    </ul>
    <h4 class="pt_tit borb1 bort1 pt_tit_mrt"><a href="javascript:" id="pt_mark_url"><span><?= __('以下小伙伴发起的团购，你可直接参与'); ?></span><span id="more" style="float:right"><?= __('更多'); ?><i class="icon fr" id="pt_mark_icon"></i></span></a></h4>
    <ul class="pt_det_lists" id='pt_mark_list'>
    
    </ul>
    <h4 class="pt_tit borb1 pt_tit_mrb"><a href="/tmpl/pintuan_rule.html"><span><?= __('支付成功即算参团，时间截止人数不足自动退款'); ?></span><i class="icon fr"></i></a></h4>
    <h4 class="pt_tit borb1 bort1" onclick="show_mask('goods_spec',1)" id="spec"><a href="javascript:"><span><?= __('已选择：'); ?><span id="pt_goods_spec"></span></span><i class="icon fr"></i></a></h4>
    ...
    <div class="goods-detail-store">
        <a href="javascript:" id='pt_store_url'>
            ...
            <!--  <div class="item-more"><b><?= __('进店逛逛'); ?></b></div>-->
        </a>
    </div>
    <div class="pt_bottom_btn">
        <a href="javascript:" class="btn_style1 js-buy" data-type="alone"><span id='bt_price_one'>0.00</span><em><?= __('单独购买'); ?></em></a>
        <a href="javascript:" class="btn_style2 js-buy" data-type="pintuan"><span id='bt_price'>0.00</span><em><?= __('一键拼单'); ?></em></a>
    </div>
</div>
<!-- <?= __('上拉商品规格内容'); ?> -->
<div class="nctouch-bottom-mask down" id="goods_spec">
    <!-- <?= __('显示隐藏切换：'); ?>class<?= __('为'); ?> up down -->
    ...
    <div class="nctouch-bottom-mask-block">
        <div class="nctouch-bottom-mask-top goods-options-info">
            ...
            <dl>
                <dt class="pt_good_status pt_goods_name"></dt>
                <dd class="goods-price">
                    <span class="goods-price price-color fz-28"><?= __('价格：'); ?><em class="goods-price price-color fz-28" id="price"></em></span>
                </dd>
                 <span class="goods-storage"><?= __('库存：'); ?><em id="goods_stock" style="color:#111"></em></span>
            </dl>
            <a href="javascript:void(0);" class="nctouch-bottom-mask-close" onclick="show_mask('goods_spec',0)"><i></i></a>
        </div>
        <div class="nctouch-bottom-mask-rolling mb20 overflow-auto">
            <div class="goods-options-stock bort1" id="goods_spec_html">
                <!--                    <dl class="spec JS-goods-specs ">
                                        <dt><?= __('颜色：'); ?></dt>
                                        <dd>
                                            <a href="javascript:void(0);" class="current"><?= __('白色'); ?></a>
                                            <a href="javascript:void(0);"><?= __('黑色'); ?> </a><a href="javascript:void(0);"><?= __('灰色'); ?> </a>
                                        </dd>
                                    </dl>
                                    <dl class="spec JS-goods-specs">
                                        <dt><?= __('尺码：'); ?></dt>
...
<div class="nctouch-bottom-mask down" id="share_mask">
    ...
    <div class="nctouch-bottom-mask-block height-auto">
        ...
        <div class="goods-option-foot">
            <div class="">
                <a href="javascript:void(0);" class="pt_common cancel" onclick="show_mask('share_mask',0)" style="background:#ecdfdf"><?= __('取消'); ?></a>
...
<script type="text/html" id="goodsReview">
    <p class="evals bgf mt5 bort1 borb1"><i class="icon"></i><span><?= __('商品评价'); ?>
                    <% if (num > 0) { %>
                    (<%= num %>)
                    <% } %></span></p>
    <div id="mui-tagscloud-i" class="mui-tagscloud bort1">
        ...
        <div class="mui-tagscloud-more">
            <% if (goods_review_rows.length > 0) { %>
            <% if (num > 2) { %>
            <button id="reviewLink" onclick="more_review()"><?= __('查看全部评价'); ?></button>
            <% } %>
            <% } else { %>
            <?= __('暂无评价'); ?>
            <% } %>
