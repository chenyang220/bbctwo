...
<div class="nctouch-main-layout">
    <div class="pt_det bgf">
        ...
        <div>
            <h4><strong class="common-red" id="person_num">[5<?= __('人团'); ?>]</strong><span id="goods_name"><?= __('韩亚，韩国进口牛奶'); ?>(<?= __('延世牧场'); ?>)<?= __('香蕉味、草莓味牛奶饮料'); ?></span></h4>
            <b id="buyer_sum" class="block"><?= __('已拼'); ?>256<?= __('件'); ?></b><br />
            <p><strong class="common-red" id="price"><?= __('￥'); ?>125.00</strong><em class="common-red"><?= __('包邮'); ?></em><b class="through" id="price_ori"><?= __('原价：￥'); ?>259.00</b></p>
        </div>
    </div>
    <div class="pt_status bgf">
        ...
        <p class="pt_tips tc"><?= __('还差'); ?>1<?= __('人，一起拼成功'); ?></p>
        <div class="tc">
            <a href="javascript:" class="pt_btn_big"><?= __('立即拼团'); ?></a>
        </div>
    </div>
    <h4 class="pt_tit borb1"><a href="pintuan_rule.html"><span><?= __('拼单须知'); ?></span><i class="icon fr"></i></a></h4>
    <div id='pinTuanGoods'>
    ...
    <ul class="pt_lists clearfix">
        ...
        <li>
            <a href="pintuan_detail.html?&goods_id=<%= pinTuanGoods[i].goods.goods_id %>">
                ...
                <h5><strong class="common-red">[<%= pinTuanGoods[i].person_num %><?= __('人团'); ?>]</strong><span><%= pinTuanGoods[i].goods.goods_name %></span></h5>
                <p><strong class="common-red"><?= __('￥'); ?><%= pinTuanGoods[i].detail.price %></strong><b><?= __('单买价：￥'); ?><%= pinTuanGoods[i].detail.price_one %></b></p>
                <p class="clearfix"><b class="through fl"><?= __('原价：￥'); ?><%= pinTuanGoods[i].detail.price_ori %></b>
                    <b class="fr"><?= __('已拼'); ?><% if (pinTuanGoods[i].detail.buyer_num == 0) { %>0<% }else{ %> <%= pinTuanGoods[i].detail.buyer_num<?= __('件'); ?> %><% } %><?= __('件'); ?></b></p>
            </a>
        </li>
        <% } %>
    </ul>
        ...
        $.ajax({
            ...
            success: function (e) {
                if (e.status == 200) {
                    ...
                    $('#person_num').html('[' + e.data.person_num + '<?= __('人团'); ?>]');
                    $('#price').html('<?= __('￥'); ?>' + e.data.detail.price);
                    $('#price_ori').html('<?= __('原价：'); ?>' + e.data.detail.price_ori);
                    $('#buyer_sum').html('<?= __('已拼'); ?>' + e.data.buyer_sum + '<?= __('件'); ?>');
                    $('.pt_tips').html('<?= __('还差'); ?>' + e.data.lack + '<?= __('人，一起拼成功'); ?>');
                    
                    if ($.inArray(getCookie('id'), e.data.buyer_id) >= 0) {
                        $('.pt_btn_big').html('<?= __('不可重复参团'); ?>');
