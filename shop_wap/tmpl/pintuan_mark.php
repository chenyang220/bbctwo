			...
			<% for (var i = 0; i < groupList.length; i++) { %>
				<li class="clearfix">
					...
					<input type="hidden" class="shop_user_id" value="<%= groupList[i].shop_user_id %>">
						...
						<div class="fr">
							<p class="fl"><strong><?= __('还差'); ?><%= groupList[i].lack %><?= __('人成团'); ?></strong><em><%= groupList[i].hour %>:<%= groupList[i].min %>:<%= groupList[i].second %><?= __('后结束'); ?></em></p>
							<p class="fr"><a class="gobuy" href="pintuan_info.html?mark_id=<%= groupList[i].id %>&detail_id=<%= groupList[i].detail_id %>"><?= __('去拼团'); ?></a></p>
						</div>
					</div>
				</li>
			<% } %>
...
<script type="text/javascript">
	$(function(){
        ...
        $('#groupList').on('click','.gobuy',function(){
        	var user_id = getCookie('id');//登录'); ?>user_id
		    var pintuan_uid = $('.shop_user_id').val();//店铺'); ?>user_id
		    if(user_id == pintuan_uid)
		    {
		        $.sDialog({
		            content: '<?= __('商家不可以购买自己店铺的商品！'); ?>',
		            okBtn:false,
		            cancelBtnText:'<?= __('返回'); ?>',
		        });
		        return false;
