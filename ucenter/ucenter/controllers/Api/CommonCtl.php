...
class Api_CommonCtl extends Api_Controller
{
    ...
    public function getVerifyCode()
	{
        ...
        if($cache_type == 1){
            ...
            $result = $Cache_Lite->save($code,$val,$group);
            $code = !$result ? '' : $code;
        }
        
        if(!$code){
            return $this->data->addBody(-140, array(), __('验证码创建验证码'), 250);
        }
        
        //获取模板
        $message_info = $this->getMsgContent($temp_code, $code);
        if(!$message_info){
            return $this->data->addBody(-140, array(), __('信息内容创建失败'), 250);
        }
        
        //发送
		if ($type == 'mobile') {
            
			$str = Sms::send($val,$area_code, $message_info['content_phone'], $message_info['baidu_tpl_id'],['weburl_name'=>Web_ConfigModel::value("site_name"),'yzm'=>$temp_code]);
		} else {
