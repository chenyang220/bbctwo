...
class Api_UserCtl extends Api_Controller
{
    ...
    public function listUser()
    {
        ...
        if($skey)
        {
            ...
            unset($cond_row['user_name:LIKE']);
            $cond_row['user_mobile'] = trim($skey);
            $data_list = $userInfoModel->getInfoDetailList($cond_row, $order_row, $page, $rows);
            $data['items'] = array_merge($data['items'],$data_list['items']);
        } else {
            $data = $userInfoModel->getInfoDetailList($cond_row, $order_row, $page, $rows);
        }
    ...
    //获取列表信息
    public function getUserExcelMsg()
    {

        $data = array();
        $is_limit = $_REQUEST['is_limit'];
        ...
        $data = $userInfoModel->getUserDetailExcel($cond_row,$is_page);

        foreach ($data as $key => &$value) {
            $value['user_reg_time'] = date("Y-m-d H:i:s",$value['user_reg_time']);
            $value['user_lastlogin_time'] = date("Y-m-d H:i:s",$value['user_lastlogin_time']);
    ...
    function add()
    {
        ...
        if($user_name_info_1){
            $user_name_info_2 = $User_InfoModel->getByWhere($cond_row);
            ...
            //查询user_id_seq表生成最新user_id
            $Db = Yf_Db::get('ucenter');
            $seq_name = 'user_id';
            $user_id = $Db->nextId($seq_name);
            $cond_row['user_id'] = $user_id;
            
            $last_id = $User_InfoModel->addInfo($cond_row);
    ...
    public function editUserPassword()
    {
        ...
        if ($reg_row['reg_uppercase']['config_value'] == 1) {
            if (!preg_match("/^(?=.*[0-9])(?=.*[a-zA-Z])([a-zA-Z0-9]{0,20})$/", $user_password)) {
                $this->data->setError('密码必须数字英文字母组合');
        ...
        if ($reg_row['reg_symbols']['config_value'] == 1) {
            if (!preg_match("/^(?=.*\d)(?=.*[a-zA-Z])(?=.*[`~!@#$%^&*()_\-+=<>?:\"{}|,.\/;'\\[\]·~！@#￥%……&*（）——\-+={}|《》？：“”【】、；‘’，。、])[\da-zA-Z`~!@#$%^&*()_\-+=<>?:\"{}|,.\/;'\\[\]·~！@#￥%……&*（）——\-+={}|《》？：“”【】、；‘’，。、]{0,20}$/", $user_password)) {
                $this->data->setError('密码必须数字英文字母及符号组合');

                return false;
            }
        }
        if (strlen($user_password) < $reg_row['reg_pwdlength']['config_value'] ) {
            if(strlen($user_password)  >= 20) {
                $this->data->setError('密码最大长度为20位');
                return false;
            }
            $this->data->setError('密码最小长度为'.$reg_row['reg_pwdlength']['config_value'].'位');
            return false;
 ...
 * */
    ...
    public function getUserBind()
    {
        ...
        $bind_info = $User_BindConnectModel -> getByWhere(array('user_id'=>$user_id,'bind_type'=>3));

        $bind_info = current($bind_info);

        $this->data->addBody(-1, $bind_info);
    }
