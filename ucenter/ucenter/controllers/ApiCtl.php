...
class ApiCtl extends  Api_Controller
{
    ...
    public function addUserAndBindAppServer()
    {
        ...
        $user_mobile = $_REQUEST['company_phone'];
        $area_code = $_REQUEST['area_code']?:86;
        ...
        else
        {
            ...
            else
            {
                ...
                if($user_mobile)
                {
                    $checkmobile = $User_InfoDetail->checkUserName($user_mobile, Perm::$userId);
                    $data = [];
                    if (count($checkmobile)) {
                        $this->data->setError('该手机号已被注册');
                        return false;
                    }
                }
            ...
            if (is_ok($rs_row) && $Base_App->sql->commit())
            {
                $arr_body = array("user_name"=>$user_name, "server_id"=>$server_id, "user_id"=>$user_id,'user_mobile'=>$user_mobile,'area_code'=>$area_code);
                $this->data->addBody(100, $arr_body);
            }
            else
            {
...
//        }
    ...
    */
    ...
    public function edit()
    {
        ...
        $data['app_showname'] = $_REQUEST['app_showname'];
     ...
     * 用户微信绑定标识
     */
    public function getUserWxBindFlag(){
        $user_id = request_string('user_id');
        $type = User_BindConnectModel::WEIXIN;//微信标识
        $mdl = new User_BindConnectModel();
        $result = $mdl->getBindConnectByUseridType($user_id, $type);
        if($result)
        {
            $msg    = 'success';
            $status = 200;
