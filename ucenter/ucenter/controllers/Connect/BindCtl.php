<?php if (!defined('ROOT_PATH')) {
    ...
    class Connect_BindCtl extends Yf_AppController implements Connect_Interface
    {
        ...
        public function __construct(&$ctl, $met, $typ)
        {
            ...
            if ($this -> type == 'weixin') {
                if (strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') !== false) {
                    $this -> redirect_url = sprintf('%s?ctl=WeiXin&met=callback&from=%s&callback=%s&type=%s&op=%s', Yf_Registry::get('paycenter_api_url'), request_string('from'), urlencode(request_string('callback')), 'weixin', $this -> op);
                } else {
                    $this -> redirect_url = sprintf('%s?ctl=Connect_Bind&met=callback&from=%s&callback=%s&type=%s&op=%s', Yf_Registry::get('url'), request_string('from'), urlencode(request_string('callback')), 'weixin', $this -> op);
                }
                ...
                if ($data['status'] == 200) {
                    ...
                    if (isset($connect_row['user_id']) && $connect_row['user_id']) {
                        ...
                        if ($bind_rows && $bind_row = array_pop($bind_rows)) {
                            ...
                            //插入绑定表
                            $bind_array = array();
                            //互联登录中的用户名为用户昵称，若重复昵称加4位随机（数字，大小写字母）的字符
                            $user_infos = $User_InfoModel -> getInfoByName($data['user_info']['bind_nickname']);
                            if ($user_infos) {
                                $bind_nickname = $data['user_info']['bind_nickname'].rand(0000, 9999);
                            } else {
                                $bind_nickname = $data['user_info']['bind_nickname'];
                            }
                        ...
                        if ($connect_flag) {
                            ...
                            if (!Perm::checkUserPerm()) {
                                if ($this -> op) {
                                    ...
                                    $data_regist_row['token'] = $data['access_token'];
                                    $data_regist_row['password'] = $User_InfoModel -> random_str(8);
                                    //$res = $this->bindRegist($data_regist_row);
                                    $backUrl = request_string('callback') ? : $_GET['callbak'];
                                    $url = Yf_Registry::get('url').'?ctl=Login&met=bindphone&callback_url='.urlencode($backUrl).'&token='.$data_regist_row['token'].'&password='.$data_regist_row['password'];
                                    //绑定手机号
        ...
        public function getWBuser($code)
        {
            ...
            if ($error) {
                ...
                $data['status'] = 200;
                $data['bind_id'] = sprintf('%s_%s', 'wb', $user_info->id);
                $data['access_token'] = $user_info->id;
                $data['openid'] = $user_info->id;
                $data['user_info']['bind_avator'] = $user_info -> avatar_large;
                $data['user_info']['bind_nickname'] = $user_info->screen_name;//$user_info -> figureurl_qq_2;
                $data['user_info']['bind_gender'] = $user_gender;
        ...
        public function getWechatInfo()
        {
            ...
            return $this->data->addBody(-140, $connect_config['wechat']);
        }
