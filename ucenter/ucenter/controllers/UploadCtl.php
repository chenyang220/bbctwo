...
class UploadCtl extends Yf_AppController
{
	...
	public function wxUploadImage()
	{
		...
		$unuploaded=0;
		//上传文件路径
		$img_url= "http://lab.ucenter.com/ucenter/data/upload/images/";
		//如果当前图片不为空
		 if(!empty($uplad_name))
		 {
		   //判断上传的图片的类型是不是jpg,gif,png,bmp中的一种，同时判断是否上传成功
		    $uptype = explode(".",$uplad_name);
			$type = current(array_slice($uptype, -1, 1, true));
			if (!in_array($type, $uptypes)) {
				return $this->data->addBody(-140, array(),'图片不符合上传格式',250);
			}
		    //如果上传的文件没有在服务器上存在
		    if(!file_exists($img_url.$uplad_name))
		    {
		        //把图片文件从临时文件夹中转移到我们指定上传的目录中
		    	if (move_uploaded_file($uplad_tmp_name,"./ucenter/data/upload/images/".$uplad_name)) {
		    		$data['data'] = $img_url . $uplad_name;
					$data['status'] = 200;
					$data['msg'] = "上传成功";
		    		return $this->data->addBody(-140, $data, "上传成功", 200);
		    	}
		    }		 
		  } else {
		  	 return $this->data->addBody(-140, array(), "上传失败,图片不能为空", 250);
		  }
	}
