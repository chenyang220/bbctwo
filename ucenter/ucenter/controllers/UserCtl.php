...
class UserCtl extends Yf_AppController
{
    ...
    public function getUserInfo()
    {
       ...
       if ($user_grade_id) {
            ...
            $this->user['grade']['user_grade_name'] = __('普通会员');
        }
        ...
        //获取plus会员信息 @nsy 2019-02-13
        $is_plus_user = 0;//plus标识
        $formvars['ctl'] = 'Api_User_Info';
        $formvars['met'] = 'getUserPlusFlag';
        $formvars['user_id'] = $user_id;
        $formvars['typ'] = 'json';
        $result = get_url_with_encrypt($key, $url, $formvars);

        if ($result['status'] == 200) {
            $cret = current($result['data']);
            $user_status = $cret['user_status'];
            $end_date = $cret['end_date'];
            ($user_status!=3 && $end_date>time()) && $is_plus_user = $user_status;
        }
        $data['is_plus'] = $is_plus_user;;
     ...
     * @access public
     */
    public function editWxappUserInfo()
    {
        $u = request_int('u');
        $date['user_name'] = request_string('user_name');
        $date['nickname'] = request_string('user_nickname');
        $date['user_gender'] = request_string('user_gender');
        $date['user_avatar'] = request_string('image');
        $User_InfoModel = new User_InfoModel();
        $User_InfoDetailModel = new User_InfoDetailModel();
        $User_Info = $User_InfoModel->getOne(array("user_id"=>$u));
        $User_InfoModel->sql->startTransactionDb();
        $rs_row = array();
        if ($User_Info) {
            $editInfo['user_name'] = request_string('user_name');
            $edit_User_Info = $User_InfoModel->editInfo($u,$editInfo);
            check_rs($edit_User_Info, $rs_row);
            $edit_User_InfoDetail = $User_InfoDetailModel->editInfoDetail($User_Info['user_name'],$date);
            check_rs($edit_User_InfoDetail, $rs_row);
        }
        ...
        $formvars['user_gender'] = request_string('user_gender');
        $formvars['app_id'] = $shop_app_id;
        $formvars['user_id'] = $u;
        $rs = get_url_with_encrypt($key, sprintf('%s?ctl=Api_User_Info&met=wxEditUserInfo&typ=json', $url), $formvars);
        if ($rs['status'] == 200) {
            $edit_User_Info = true;
        } else {
...
//                $this->view->setMet('passwd');
    ...
    public function editMobileInfo()
    {
        ...
        $data = array();
        if (!VerifyCode::checkCode($user_mobile, $yzm) || !$yzm) {
            $data['code'] = 1;
        ...
        if (isset($bind_info['user_id']) && $bind_info['user_id'] != $user_id) {
            $user_info = $userModel->getOne(['user_id'=>$bind_info['user_id']]);
            $data['code'] = 4;
        ...
        if ($check_user_name) {
            $user_info = $userModel->getOne(['user_id'=>$user_id]);
            $data['code'] = 2;
            $data['name'] = $user_info['user_name'];
            return $this->data->addBody(-140, $data, __('该手机已经被使用'), 250);
        }
        ...
        if ($user_name) {
            ...
            $edit_user_row['area_code'] = $area_code;
    ...
    public function unbind()
    {
        ...
        if ($flag) {
            ...
            $area_code = $user_info_datail['area_code']?:86;
            ...
            Sms::send($mobile,$area_code, $me, $de['baidu_tpl_id'],['weburl_name'=>$this->web['web_name']]);
        }
