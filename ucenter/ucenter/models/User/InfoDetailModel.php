...
class User_InfoDetailModel extends User_InfoDetail
{
	...
	public function sync($user_id)
	{
        ...
        $edit_user_row['area_code']     = $user_info_row['area_code']?:86;
    ...
    public function getUserDetailExcel ($cond_row = array(),$noLimit = false) {

        $sql = "SELECT a.user_name,a.user_truename,a.user_gender,a.user_mobile,a.user_email,a.user_area,a.user_birth,a.user_reg_time,
        a.user_lastlogin_time,a.user_count_login,b.user_state FROM " .  $this->_tableName  . " as a  LEFT JOIN ucenter_user_info as b ON a.user_name = b.user_name  WHERE 1=1";
        
        if($cond_row['user_name']) {
            $sql .= " AND a.user_name LIKE '%".$cond_row['user_name']."%'";
            $sql .= " OR a.user_mobile LIKE '%".$cond_row['user_name']."%'";
        }
        $sql .= " ORDER BY a.user_reg_time DESC";
        //需要分页
        if($noLimit){
