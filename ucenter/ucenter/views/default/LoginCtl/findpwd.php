...
<div class="login-layout" style="background:url('<?=$this->view->img?>/53.jpg') no-repeat;">
	...
	<div class="fr login-area" style="height:400px;">
		...
		<div class="box">
							<span>
								<label for="user_name"><?= __('帐号');?></label>
							...
							<span>
								<label for="user_name"><?= __('手机');?></label>
								<input type="text" name="mobile"  autocomplete="off" class="input-text text mobile" tabindex="3" value="">
							</span>

							<span>
								<input type="text" name="user_code" class="input-code text3 user_code" autocomplete="off" title="<?= __('验证码为');?>4<?= __('个字符');?>"
									   maxlength="4" placeholder="<?= __('输入验证码');?>" id="captcha-input" tabindex="4">
								<div class="code" style="display: block;">
									<div id="captcha" class="code-img">
										<a onClick="get_randfunc(this);" style="cursor:pointer;line-height:40px;color:#333333;" class="randfuc"/><?= __('获取手机验证码');?></a>
									</div>
								</div>
							</span>
							...
							<span>
								<input type="submit" value="<?= __('确认');?>" class="input-button submit" name="">
							</span>
			<!-- </form> -->
			<span>
				<a class="ml15 shadow" class="ml15"><?= __('用户注册？');?></a>
				<a class="ml5 shadow" class="ml5"><?= __('用户登录');?></a>
			</span>
		</div>
	</div>
</div>
<script>
	...
	{
		delayTime--;
		$('.randfuc').html(delayTime + "<?= __('秒后重新获取'); ?>");
		if (delayTime == 0) {
			delayTime = 60;
			$('.randfuc').html(msg);
