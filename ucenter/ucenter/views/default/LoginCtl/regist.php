...
<div id="form-header" class="header">
    <div class="logo-con w clearfix">
        ...
        <div class="logo-title"><?= __('欢 迎 注 册')?></div>
...
<div class="agreement hidden">
    ...
    <div class="ui-alert">
        <div class="ui-title">
            <span><?= __('商城用户注册协议')?></span>
            <a href="javascript:" class="btn-close iconfont icon-cuowu"></a>
        </div>
        <div class="ui-content">
            <div class="ui-content-text">
                <?= __($reg_row['reg_protocol']['config_value']) ?>
            </div>
            <div class="button-area">
                <a href="javascript:" class="btn protocol_close"><?= __('同意并继续')?></a>
            </div>
        </div>
    </div>
    <!-- 遮罩层 -->
        ...
        <div class="reg-form fl">
            <form action="" id="register-form" method="post" novalidate="novalidate" onsubmit="return false;" autocomplete="off">
                ...
                <div class="form-item form-item-account form-item-wap" id="form-item-account">
                    <label><em class="must">*</em><?= __('用  户  名：');?></label>
                    <span class="clear-btn js_clear_btn clear-icon"></span>
                    <input type="text" id="re_user_account" class="field re_user_account" maxlength="20" placeholder="<?= __('您的账户和登录名')?>" default="<i class=&quot;i-def&quot;></i><?= __('用户名非手机号不能注册！')?>" onfocus="checkAcount()" onblur="userCallback()">
                    <i class="i-status"></i>
                </div>
                ...
                <div class="pas">
                    <div id="form-item-password" class="form-item" style="z-index: 12;">
                        <label><em class="must">*</em><?= __('设 置 密 码：')?></label>
                        <span class="close-btn js_clear_btn clear-icon"></span>
                        <input type="password" id="re_user_password" class="field re_user_password" maxlength="20" placeholder="<?=$pwd_icon?__($pwd_icon):__('建议至少使用两种字符组合密码')?>" default="<i class=i-def></i><?= $pwd_str ?>" onfocus="checkPwd()" onblur="pwdCallback()">
                        <!--                        <input type="password" id="re_user_password" class="field re_user_password" maxlength="20" placeholder="--><?//=$pwd_icon?__($pwd_icon):__('建议至少使用两种字符组合密码')?><!--" default="<i class=i-def></i>--><?//= $pwd_str ?><!--">-->
                ...
                <div id="form-item-rpassword" class="form-item form-item-wap">
                    <label><em class="must">*</em><?= __('确 认 密 码：')?></label>
                    <span class="close-btn js_clear_btn clear-icon"></span>
                    ...
                    <div id="Mobile" style="display: <?= $mobile_display ?>">
                        <div class="item-phone-wrap">
                            <div class="form-item form-item-mobile" id="form-item-mobile">
                                <label class="select-country" id="select-country"><em class="must">*</em><?= __('手 机 号 码：')?></label>
                                <span class="clear-btn js_clear_btn clear-icon"></span>
<!--                                <input type="number" oninput="if(value.length>11)value=value.slice(0,11)" id="re_user_mobile" class="field re_user_mobile" placeholder="--><?//= __('建议使用常用手机')?><!--" default="<i class=&quot;i-def&quot;></i>--><?//= __('完成验证后，可以使用该手机登录和找回密码')?><!--" onblur="checkMobile()" onfocus="showTip(this)">-->
                                <input type="text"  id="re_user_mobile" class="field re_user_mobile" placeholder="请输入手机号码" default="<i class=&quot;i-def&quot;></i><?= __('完成验证后，可以使用该手机登录和找回密码')?>" onblur="checkMobile()" onfocus="showTip(this)">
                        ...
                        <div class="form-item" id="form-item-authcode">
                            <label><em class="must">*</em><?=__('验　证　码：')?></label>
                            <input type="text" name="authcode" id="form-authcode" maxlength="4" class="field form-authcode" placeholder="<?= __('请输入验证码')?>" default="<i class=&quot;i-def&quot;></i><?= __('看不清？点击图片更换验证码')?>" onfocus="showTip(this)" onblur="checkCode()">
                        ...
                        <div class="form-item form-item-phonecode">
                            <label><em class="must">*</em><?= __('手机验证码：')?></label>
                            <input type="text" name="mobileCode" maxlength="6" id="phoneCode" class="field phonecode  re_mobile" placeholder="<?= __('请输入手机验证码')?>">
                ...
                <div id="Email" style="display: <?= $email_display ?>">
                    <div class="item-phone-wrap">
                        <div class="form-item form-item-email" id="form-item-email">
                            <label class="select-country" id="select-country"><em class="must">*</em><?= __('邮 箱 地 址：')?></label>
                            <input type="text" id="re_user_email" class="field re_user_email" placeholder="<?= __('建议使用常用邮箱'); ?>" default="<i class=&quot;i-def&quot;></i><?= __('完成验证后，可以使用该邮箱登录和找回密码')?>" onblur="checkEmail()" onfocus="showTip(this)">
                            <i class="i-status"></i>
                        </div>
                        <div class="input-tip">
                            <span></span>
                        ...
                        <div class="form-item" id="form-item-authcode">
                            <label><em class="must">*</em><?= __('验　证　码：'); ?></label>
                            <input type="text" name="authcode" id="form-authcode1" maxlength="6" class="field form-authcode" placeholder="<?= __('请输入验证码'); ?>" default="<i class=&quot;i-def&quot;></i><?= __('看不清？点击图片更换验证码'); ?>" onfocus="showTip(this)" onblur="checkCode()">
                    ...
                    <div class="form-item form-item-phonecode">
                        <label><em class="must">*</em><?= __('邮箱验证码：')?></label>
                        <input type="text" name="emailCode" maxlength="6" id="emailCode" class="field emailcode  re_email" placeholder="<?= __('请输入邮箱验证码'); ?>">
                    ...
                    <div class="form-item  clearfix form-item-<?= $opt_row['reg_option_id'] ?>" id="form-item-<?= $opt_row['reg_option_id'] ?>">
                        <label>
                            <?php if ($opt_row['reg_option_required'] == 1) { ?>
                                <em class="must">*</em>
                            <?php } ?>
                            <?= __($opt_row['reg_option_name']) ?>：
                        </label>
                        <?php if ($opt_row['option_id'] == 1): ?>
                            <select id="re_user_<?= $opt_row['reg_option_id'] ?>" name="option[<?php echo $opt_row['reg_option_id']; ?>]" class="field select-drap field-reset re_user_<?= $opt_row['reg_option_id'] ?>" maxlength="20" placeholder="<?= __($opt_row['reg_option_placeholder']) ?>" default="<?= $opt_row['reg_option_placeholder'] ?>" data-datatype="<?= $opt_row['reg_option_datatype'] ?>">
                                <?php $reg_option_value_row = explode(',', $opt_row['reg_option_value']); ?>
                                <?php foreach ($reg_option_value_row as $k => $option_value) { ?>
                                    <option value="<?= $k; ?>"><?= __($option_value); ?></option>
                                <?php } ?>
                            </select>
                        ...
                        <?php elseif ($opt_row['option_id'] == 4): ?>
                            <input type="text" id="re_user_<?= $opt_row['reg_option_id'] ?>" name="option[<?= $opt_row['reg_option_id']; ?>]" class="field field-reset re_user_<?= $opt_row['reg_option_id'] ?>" <?php if ($opt_row['reg_option_datatype'] == 1){ ?>maxlength="11"<?php } else { ?> minlength="15" maxlength="18"<?php } ?> placeholder="<?= __('请输入') . __($opt_row['reg_option_name']) ?>" default="<?= $opt_row['reg_option_placeholder'] ?>" data-datatype="<?= $opt_row['reg_option_datatype'] ?>" onblur="checkFormatter(this)" onfocus="showTip(this)">
                        <?php elseif ($opt_row['option_id'] == 5): ?>
                            <textarea type="text" id="re_user_<?= $opt_row['reg_option_id'] ?>" name="option[<?= $opt_row['reg_option_id']; ?>]" class="field field-reset re_user_<?= $opt_row['reg_option_id'] ?>" placeholder="<?= __('请输入') . __($opt_row['reg_option_name']); ?>" default="<?= $opt_row['reg_option_placeholder'] ?>" data-datatype="<?= $opt_row['reg_option_datatype'] ?>" onblur="checkFormatter(this)" onfocus="showTip(this)"></textarea>
                ...
                <div class="form-agreen">
                    <div>
                        <input id="agree_button" type="checkbox" name="agreen" checked=""><?= __('我已阅读并同意'); ?>
                        <a href="javascript:" id="protocol" onclick="registalert()"><?= __('《用户注册协议》'); ?></a>与<a href="<?=Yf_Registry::get('url')?>?ctl=Login&met=privacy"><?= __('《用户隐私协议》'); ?></a>
                    </div>
                    <div class="input-tip"><span></span></div>
                </div>                
                <div>
                    <button type="submit" class="btn-register" onclick="registclick()"><?= __('立即注册'); ?></button>
                </div>
            </form>
        </div>
...
<script>
    ...
    function checkAcount() {
        ...
        var reg = /^1[3-9]\d{9}$/;
        var errormsg = icons.error + "<?= __('用户名非手机号不能注册！');?>";
            ...
            if (user_account != '' && !reg.test(user_account)) {
                ...
                $.post("./index.php?ctl=Login&met=checkUserName&typ=json", {"user_name": user_account}, function (data) {
                    if (data.status == 200) {
                        ...
                        user_check = false;
                        var errormsg = icons.error + "<?= __('该用户名已被使用，请重新输入')?>";
                        onKeyupHandler(form_account, errormsg);
                    } else {
                ...
                $.ajax({
                    ...
                    success: function (respone) {
                        if (respone.status == 250) {
                            var errormsg = icons.error +"<?= __( '该手机号已被注册')?>";
                            //onKeyupHandler(form_mobile, errormsg);
                            onKeyupHandler(form_mobile, errormsg);
                            mobile_check = false;
                        } else {
        ...
        if (email) {
            ...
            if (reg.test(email)) {
                ...
                $.ajax({
                    ...
                    success: function (respone) {
                        if (respone.status == 250) {
                            var errormsg = icons.error + "<?= __( '该邮箱已被注册')?>";
            ...
            if (val) {
                ...
                if (1 == datatype) {
                    ...
                    if (reg.test(val)) {
                        ...
                        var errormsg = icons.error + "<?= __('请输入正确的手机号码')?>";
                    ...
                    if (reg.test(val)) {
                        ...
                        var msg = checkObj.attr('default');
                        var errormsg = icons.error +"<?= __( '请输入合法的身份证号码')?>";
                        onKeyupHandler(checkObj, errormsg);
                        option_check = false;
                    }
                } else if (3 == datatype) {
                    ...
                    if (reg.test(val)) {
                        ...
                        var msg = checkObj.attr('default');
                        var errormsg = icons.error +"<?= __( '请输入纯数字')?>";
                        onKeyupHandler(checkObj, errormsg);
                        option_check = false;
                    }
                } else if (4 == datatype) {
                    ...
                    if (true) {
                        ...
                        var msg = checkObj.attr('default');
                        var errormsg = icons.error +"<?= __( '请输入字符串')?>";
                        onKeyupHandler(checkObj, errormsg);
                        option_check = false;
                    }
                } else if (5 == datatype) {
                    ...
                    if (reg.test(val)) {
                        ...
                        var msg = checkObj.attr('default');
                        var errormsg = icons.error + "<?= __('请输入正确的邮箱地址')?>";
                        onKeyupHandler(checkObj, errormsg);
                        option_check = false;
                    }
                } else if (6 == datatype) {
                    ...
                    if (reg.test(val) && val.length <= 20) {
                        ...
                        var msg = checkObj.attr('default');
                        var errormsg = icons.error + "<?= __('真实姓名格式有误')?>";
                        onKeyupHandler(checkObj, errormsg);
                        option_check = false;
                    }
                }
    ...
    function get_randcode_email() {
        ...
        if (email) {
            ...
            var errormsg = icons.error +"<?= __( '请填写邮箱地址')?>";
            onKeyupHandler(form_email, errormsg);
            return;
        }
        //验证码
        if (!$('#form-authcode').val()) {
            var errormsg = icons.error + "<?= __('请输入正确的验证码')?>";
        ...
        $.ajax({
            ...
            success: function (respone) {
                if (respone.status == 250) {
                    ...
                    window.countDown();
                    Public.tips.alert("<?= __('请查看邮箱获取验证码!')?>");
                    return false;
                }
            }
        });
    ...
    function get_randcode_phone() {
        ...
        if (!user_check) {
            Public.tips.alert("<?= __('请确认用户名是否符合要求')?>");
            return;
        }
        /*if (!pwd_check) {
    ...
    function registclick() {
        ...
        if (!$('#agree_button').is(':checked')) {
            Public.tips.alert("<?= __('请确认是否同意用户注册协议')?>");
            return;
        }

        if (!user_account) {
            Public.tips.alert("<?= __('请输入用户名')?>");
            return false;
        }
        ...
        var area_code = $('#area_code').val();
        var email = $("#re_user_email").val();
        var shop_id_wap = "<?=request_int('shop_id_wap');?>";
