...
<div class="ncm-default-form">
    <form method="post" id="form" name="form" action="" class="formcs">
        <input type="hidden" name="<?php echo CSRF::name(); ?>" value="<?php echo CSRF::create(); ?>"> <input value="edit" name="submit" type="hidden"> <input type="hidden" name="user_name" value="<?= $data['user_name'] ?>"/>
        <dl>
            <dt><?= __('用户名称：') ?></dt>
            <dd>
                    <span class="w400"><?= __($data['user_name']); ?>&nbsp;&nbsp;
                        <div class="nc-grade-mini bbc_bg" style="cursor:pointer;">
                            <?php if ($this->user['grade']['user_grade_name']) { ?>
                                <?= __($this->user['grade']['user_grade_name']); ?>
                            <?php } else { ?>
                                <?= __('普通会员') ?>
                            <?php } ?>
                        </div>
                        <?php
                        //PLUS 标识 @nsy 2019-02-13
                        if($data['is_plus']){
                            echo '<b class="plus-login-logo"></b>';
                            if($data['is_plus']==1)echo '<b class="plus-login-logo-try">试用</b>';
                        }?>
                    </span>
        ...
        <dl>
            ...
            <dd>
                <span class="w400">
                        <input type="text" class="text" maxlength="20" name="nickname" value="<?= __($data['nickname']); ?>">
                        </span>
            </dd>
        </dl>
        <?php if ($data['user_email']) { ?>
            <dl>
                ...
                <dd><span class="w400"><?= $data['user_email']; ?>&nbsp;&nbsp;
                    <a class="f12" href="<?= Yf_Registry::get('url') ?>?ctl=User&met=security&op=email">
                     <?= __('修改邮箱') ?></a>
                     </span>
        ...
        <?php if ($data['user_mobile']) { ?>
            <dl>
                ...
                <dd><span class="w400"><?= $data['user_mobile']; ?>&nbsp;&nbsp;
                    <a class="f12" href="<?= Yf_Registry::get('url') ?>?ctl=User&met=security&op=mobile">
                      <?= __('修改手机') ?></a>
                     </span>
                    <!--<span class="midisplay">
        ...
        <dl>
            ...
            <dd><span class="w400">
                        <label>
                          <input class="align-middle" type="radio" name="user_gender"
        ...
        <dl class="bottom">
            <dt></dt>
            <dd>
                <label class="submit-border"> <input type="submit" class="submit bbc_btns" value="<?= __('保存修改') ?>"> </label>
            </dd>
        </dl>
    </form>
</div>
    ...
    $(document).ready(function () {
        ...
        $("#form").validator({
            ...
            rules: {
                ...
                nickname: [/^[a-zA-Z0-9_\u4e00-\u9fa5]+$/, "<?=__('昵称只能使用中文、数字、字母、下划线')?>"]
