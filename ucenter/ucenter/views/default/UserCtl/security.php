...
?>
    ...
    <div class="ncm-security-container">
        ...
        <div class="current low clearfix <?=$low_class?>"><!--对此div添加不同等级的class:低（lower）,中（medium）,高（high） -->
            ...
            <div class="fl progress-text"><?=__('安全等级：')?><strong><?php if($data['user_level_id']<1){?><?=__('低')?><?php }elseif($data['user_level_id']==1){?><?=__('中')?><?php }else{?><?=__('高')?><?php }?></strong></div>
            <div class="divleft">
                
                <div class="divright">
                    <span><?=__('(建议您开启全部安全设置，以保障账户及资金安全)')?></span>
                </div>
            ...
            <dl>
                <dt>
                    <span class="iconfont icon-icon_geren" style="top:4px;"></span><span class="itemss">
                        <h4><?=__('实名认证')?></h4>
                        <?php if($user_info['user_identity_statu'] == 2){ $str=__('查看实名信息'); ?><h6 class="active"><?=__('已认证')?></h6><?php }else{  $str=_('去实名认证');?><h6><?=__('未认证')?></h6><?php }?></h6>
                      </span>
                </dt>
                <dd>
                        <span class="handle">
                        <?php if($user_info['user_identity_statu'] == 2){ ?>
                            <?php if($duff_time <= 30){?>
                                <a class="red" href="<?=Yf_Registry::get('paycenter_api_url')?>?ctl=Info&met=account&typ=e"><?=__('证件快到期，前去修改')?></a>
                            <?php }?>
                        <?php }?>
                            <?php if($user_info['user_identity_statu'] !== 2){?>
        ...
        <dl id="email" class="<?php if($data['user_email_verify']){ ?> yes<?php }else{?>no<?php }?>">
            <dt><span class="iconfont icon-email" style="top:4px;"><?php if($data['user_email_verify']){ ?><?php }?></span><span class="itemss">
              <h4><?=__('邮箱绑定')?></h4>
              <h6 <?=$emailh6_class?>><?php if($data['user_email_verify']){ ?><?=__('已绑定')?><?php }else{?><?=__('未绑定')?><?php }?></h6><!-- 已绑定给h6添加class为active -->
              </span></dt>
            <dd>
                <span class="explain"><?=__('进行邮箱验证后，可用于接收敏感操作的身份验证信息，以及订阅更优惠商品的促销邮件。')?></span>
                <span class="handle">
                <?php if($data['user_email_verify']){?>
                    <a href="<?= Yf_Registry::get('url') ?>?ctl=User&met=security&op=email" class="red ml30"><?=__('修改邮箱')?></a>
                <?php }else{?>
                    <a href="<?= Yf_Registry::get('url') ?>?ctl=User&met=security&op=email&is_bind=1" class="ncbtn ncbtn-aqua bd  bbc_btns"><?=__('绑定邮箱')?></a>
                <?php }?>
                </span>
            </dd>
        </dl>
        ...
        <dl id="mobile" class="<?php if($data['user_mobile_verify']){ ?> yes<?php }else{?>no<?php }?>">
            <dt><span class="iconfont icon-phone" style="top:4px;"><?php if($data['user_mobile_verify']){ ?><?php }?></span><span class="itemss">
              <h4><?=__('手机绑定')?></h4>
              <h6 <?=$mobile_h6_class?>><?php if($data['user_mobile_verify']=='1'){ ?><?=__('已绑定')?><?php }else{?><?=__('未绑定')?><?php }?></h6>
              </span></dt>
            <dd><span class="explain"><?=__('进行手机验证后，可用于接收敏感操作的身份验证信息，以及进行积分消费的验证确认，非常有助于保护您的账号和账户财产安全。')?></span><span class="handle">
            <?php if($data['user_mobile_verify']=='1'){ ?>
                    <a href="<?= Yf_Registry::get('url') ?>?ctl=User&met=security&op=mobile" class="red ml30"><?=__('修改手机')?></a></span></dd>
            <?php }else{?>
                <a href="<?= Yf_Registry::get('url') ?>?ctl=User&met=security&op=mobile&is_bind=1" class="ncbtn ncbtn-aqua bd  bbc_btns"><?=__('绑定手机')?></a>
            <?php }?>
        </dl>
                            ...
                            <div class="table-cell">
                                <p><i class="icon-unbind"></i><!-- 绑定成功的话，i标签的class改为icon-success-->
                                    <?php if($identify_status != 2){ ?>
                                        <span class="text"><?=__('您还未实名认证，为了保证账户安全，请进行修改！')?></span>
                                    <?php }else if($duff_time<=0){ ?>
                                        <span class="text"><?=__('您的证件已到期，为了保证账户安全，请进行修改！')?></span>
                                    <?php } ?>
                                </p>
                                <div><a href="<?=Yf_Registry::get('paycenter_api_url')?>?ctl=Info&met=account&typ=e" class="btn btn-sure lh36"><?=__('立即修改')?></a></div>
                            </div>
                        </div>
                    </div>
                </div>
