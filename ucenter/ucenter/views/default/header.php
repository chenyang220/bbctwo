...
$app_rows = $Base_AppModel->getByWhere(array('app_showname:!='=>''));
...
?>
	...
	<head>
        ...
        <script type="text/javascript" src="<?= $this->view->js ?>/plugins/jquery.cookie.js"></script>
		<!--[if lt IE 9]>
	...
	<body>
			...
			<div class="wrap clearfix bbc_bg">
				<div class="bbuyer_head_fl">
					...
					<div class="bbuyer_others">
						<p class="mine_shopmall"><?=__('我的UCenter')?></p>
						<p class="back_mall_index"><a href="<?= Yf_Registry::get('url') ?>"><?=__('返回UCenter首页')?></a></p>
					</div>
     ...
     <div  class="shangcheng  " >
      ...
      <ul class="nav-menu clearfix">
        <li class="hov">
          <a href="#" class="current" style="color:#333;"><?= __('我的商城');?></a>

        </li>
        <li class="set"><a href="<?= Yf_Registry::get('url') ?>?ctl=User&met=getUserInfo" class="current"><?= __('用户设置');?><i class="iconfont">&#xe632;</i></a>
          <div class="sub-menu">
            <dl>
              <dt><a href="<?= Yf_Registry::get('url') ?>?ctl=User&met=security" style="color: #3AAC8A;"><?= __('安全设置');?></a></dt>
              <dd><a href="<?= Yf_Registry::get('url') ?>?ctl=User&met=security&op=mobile"><?= __('手机绑定');?></a></dd>
              <dd><a href="<?= Yf_Registry::get('url') ?>?ctl=User&met=security&op=email"><?= __('邮件绑定');?></a></dd>
            </dl>
            <dl>
              <dt><a href="<?= Yf_Registry::get('url') ?>?ctl=User&met=getUserInfo" style="color: #EA746B"><?= __('个人资料');?></a></dt>
              <dd><a href="<?= Yf_Registry::get('url') ?>?ctl=User&met=tag"><?= __('兴趣标签');?></a></dd>
              <dd><a href="<?= Yf_Registry::get('url') ?>?ctl=User&met=getUserImg"><?= __('修改头像');?></a></dd>
              <dd><a href="<?= Yf_Registry::get('url') ?>?ctl=Message&met=message&op=manage"><?= __('消息接受设置');?></a></dd>

            </dl>
            <dl>
              <dt><a href="<?= Yf_Registry::get('url') ?>?ctl=Property&met=cash" style="color: #FF7F00"><?= __('账户财产');?></a></dt>
              <dd><a href="<?= Yf_Registry::get('url') ?>?ctl=Voucher&met=voucher"><?= __('我的代金券');?></a></dd>
			  <dd><a href="<?= Yf_Registry::get('url') ?>?ctl=Points&met=Points"><?= __('我的积分');?></a></dd>

            </dl>
            <dl>
              <dt><a href="<?= Yf_Registry::get('url') ?>?ctl=Favorites&met=favoritesGoods"  style="color: #398EE8"><?= __('我的收藏');?></a></dt>
              <dd><a href="<?= Yf_Registry::get('url') ?>?ctl=Favorites&met=favoritesShop"><?= __('店铺收藏');?></a></dd>
            </dl>
          </div>
        </li>
      </ul>
