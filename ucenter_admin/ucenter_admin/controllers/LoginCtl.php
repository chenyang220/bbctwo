...
//			location_go_back('验证码错误');
		...
		if($err_ct>=6){
			return  $this->data->addBody(-140, $data, '登录失败次数已达6次，今天已被禁止登录！', 250);
		}
		//本地读取远程信息
...
//			location_go_back(isset($init_rs['msg']) ? '登录失败,请重新登录!' . $init_rs['msg'] : '登录失败,请重新登录!');
	...
	//				return location_go_back('该账户禁用，请启用后登录！');
					return $this->data->addBody(-140, $data, '该账户禁用，请启用后登录！1', 250);
				}
			} else {
				$user_row = $init_rs['data'];
	//			return location_go_back('该账户未启用，请启用后登录！');
				return $this->data->addBody(-140, $data, '该账户未启用，请启用后登录！2', 250);
			}
        } else {
			//shop_admin设置ucenter账号
            $sql = "select * from yf_admin_user_account where 1=1 and user_name='{$user_account}' and user_for='UCenter'";
            $user_row = $userBaseModel->sql->getAll($sql)[0];
            if($user_row){
				//判断是否启用
				if($user_row['enable'] == 2){
                    return $this->data->addBody(-140, $data, '该账户禁用，请启用后登录！', 250);
				}
			}else{
                $user_row = $init_rs['data'];
                return $this->data->addBody(-140, $data, '该账户不存在，请启添加后登录！', 250);
			}
        }
