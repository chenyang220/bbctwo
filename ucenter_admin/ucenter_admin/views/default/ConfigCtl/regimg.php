...
<div class="wrapper page">
    ...
    <div class="fixed-bar">
        <div class="item-title">
            ...
            <ul class="tab-base nc-row">
                ...
                <li><a class="current"><span><?= __('注册图片设置');?></span></a></li>
    ...
    <form style="" method="post" name="form_index" id="log-setting-form">
        ...
        <div class="ncap-form-default">
            ...
            <dl class="row">
                <dt class="tit">
                    <label><?= __('登录页广告图片');?></label>

                </dt>
                <dd class="opt">

                    <img id="login_logo_image" name="register_img[login_logo]" alt="<?= __('选择图片');?>" src="<?=($data['login_logo']['config_value'])?>"/>

                    <div class="image-line upload-image" id="login_logo_upload"><?= __('上传图片');?><i class="iconfont icon-tupianshangchuan"></i></div>

                    <input id="login_logo" name="register_img[login_logo]" value="<?=($data['login_logo']['config_value'])?>" class="ui-input w400" type="hidden"/>
                    <p class="notic"><?= __('登录页显示，最佳显示尺寸为');?>500*500<?= __('像素');?>,<?= __('请根据背景色选择使用图片色彩');?></p>
                    <input type="text" placeholder="<?= __('链接地址');?>" name="register_img[login_logo_url]" value="<?=($data['login_logo_url']['config_value'])?>">
                    <input type="text" placeholder="<?= __('背景色，例如：');?>#ffffff" name="register_img[login_backcolor]" value="<?=($data['login_backcolor']['config_value'])?>">
                </dd>
            </dl>
            ...
            <dl class="row">
                <dt class="tit">
                    <label><?= __('注册页广告图片');?></label>

                </dt>
                <dd class="opt" style="width: 30%;">

                    <img id="register_logo_image" name="register_img[register_logo]" alt="<?= __('选择图片');?>" src="<?=($data['register_logo']['config_value'])?>"/>

                    <div class="image-line upload-image"  id="register_logo_upload"><?= __('上传图片');?><i class="iconfont icon-tupianshangchuan"></i></div>
                    ...
                    <p class="notic"><?= __('注册页广告图片，最佳显示尺寸为');?>210*270<?= __('像素');?></p>
                    <input type="text" placeholder="<?= __('链接地址');?>" name="register_img[register_logo_url]" value="<?=($data['register_logo_url']['config_value'])?>">
                </dd>
            </dl>
    ...
    $(function(){
        ...
        $('#register_logo_upload, #login_logo_upload').on('click', function () {
            ...
            $.dialog({
                ...
                data: { UCENTER_URL: UCENTER_URL, width: imageWidth, height: imageHeight, callback: callback },    // <?= __('需要截取图片的宽高比例');?>
                width: '800px',
                lock: true
            })
        });
