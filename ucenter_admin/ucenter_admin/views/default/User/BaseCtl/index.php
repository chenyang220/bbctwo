    ...
    $.ajax({
      ...
      success: function(data){
        if(data.status === 200){
        	...
        	{
        		parent.Public.tips({type:2, content : '<?= __('共享用户已经达到上限值：');?>'+json.totalUserNum});
        		return false;
        	}
        	else
        	{
...
<div class="wrapper page">
    ...
    <div class="fixed-bar">
        <div class="item-title">
            <div class="subject">
                <h3><?= __('权限管理');?></h3>
                <h5><?= __('管理商城用户的权限，以及权限组');?></h5>
            </div>
            <ul class="tab-base nc-row">
                <li><a class="current"  href="<?= Yf_Registry::get('url') ?>?ctl=User_Base&met=index"><span><?= __('管理员');?></span></a></li>
                <li><a href="<?= Yf_Registry::get('url') ?>?ctl=Rights_Group&met=index"><span><?= __('权限组');?></span></a></li>
                <li><a  href="<?= Yf_Registry::get('url') ?>?ctl=Log_Action&met=index"><span><?= __('操作日志');?></span></a></li>
            </ul>
        </div>
    </div>
    <!-- <?= __('操作说明');?> -->
    <p class="warn_xiaoma"><span></span><em></em></p><div class="explanation" id="explanation">
        <div class="title" id="checkZoom"><i class="iconfont icon-lamp"></i>
            ...
            <span id="explanationZoom" title="<?= __('收起提示');?>"></span><em class="close_warn iconfont icon-guanbifuzhi"></em><em class="close_warn iconfont icon-guanbifuzhi"></em></div>
        <ul>
            <li><?= __('实现不同的用户拥有不同的角色，不同的角色拥有不同的权限');?></li>
        </ul>
    </div>

    <div class="mod-toolbar-top" >
       <div class="fr"><a href="./index.php?ctl=Rights_Group&met=index" class="ui-btn ui-btn-sp mrb" id="btn-add"><?= __('权限组管理');?></a></div>

       <a href="javascript:void(0)" id="btn-add_user" class="ui-btn ui-btn-sp mrb"><?= __('新增同事');?></a>
<!--       <span class="tit" id="shareInfo" style="display:none;"><?= __('该账套主服务最多支持');?><strong id="totalUser"></strong><?= __('用户共同管理，已共享');?><strong id="usedTotal"></strong><?= __('人，剩余');?><strong id="leftTotal"></strong><?= __('。');?></span>-->
    </div>
    <div class="grid-wrap">
      <table id="grid">
      </table>
  ...
  (function($){
    ...
    $('.grid-wrap').on('click', '.authorize', function(e){
      ...
      $.ajax({
        ...
        success: function(data){
          if (data.status == 200) {
            parent.Public.tips({content: '<?= __('授权成功！');?>'});
            usedTotal--;
            leftTotal++;
            showShareCount();
            if (rowData.isCom) {
            ...
            parent.Public.tips({type: 1, content: '<?= __('取消用户授权失败！');?>' + data.msg});
          }
        },
        error: function(){
           parent.Public.tips({content:'<?= __('取消用户授权失败！请重试。');?>', type: 1});
        }
      });
    });
    ...
    $('.grid-wrap').on('click', '.delete', function(e){
      ...
      e.preventDefault();
       $.ajax({
        ...
        success: function(data){
          if (data.status == 200) {
            parent.Public.tips({content : '<?= __('取消用户授权成功！');?>'});
        ...
        error: function(){
          parent.Public.tips({type:1, content : '<?= __('用户授权失败！请重试。');?>'});
        }
      });
    });

function operDataFormatter(val, opt, row) {
	var html_con = '<div class="operating" data-id="' + row.user_id + '"><span class="ui-icon ui-icon-pencil" title="<?= __('修改');?>"></span></div>';
	return html_con;
    ...
    operate: function (t, e)
    {
        if ("add" == t)
        {
            var i = "<?= __('新增职员');?>", a = {oper: t, callback: this.callback};
        }
        else
        {
            var i = "<?= __('修改职员');?>", a = {oper: t, rowData: $("#grid").data("gridData")[e], callback: this.callback};
        }
        $.dialog({
            title: i,
    ...
    function initGrid(){
      $('#grid').jqGrid({
        ...
        colNames:["<?= __('操作');?>", "<?= __('用户');?>id",  "<?= __('用户');?>",  "<?= __('公司');?>","<?= __('权限组');?>","<?= __('启用授权');?>"],
        colModel:[
            {
                name: 'operate',
                width: 40,
...
//		  {name:'setting_data', index:'setting_data', width:100, align:"center", editable: !1, title:false, formatter: settingDataFormatter, hidden:(parent.SYSTEM.siType == 1)},
	...
	{
		return a = "boolean" == typeof a ? a ? "1" : "0" : a, "1" === a ? "<?= __('是');?>" : "&#160;"
	}
        ...
        if (Number(val) || '1' == row.user_admin) {
          if ('1' == row.user_admin) {
              return '<?= __('管理员');?>';
               ...
               return '<div class="operating" data-id="' + row.user_id + '"><span class="authorize ui-label ui-label-default"><?= __('已停用');?></span></div>';
          }
        } else {
          //return '<p class="operate-wrap"><span class="authorize ui-label ui-label-default"><?= __('已停用');?></span></p>';
          return '<div class="operating" data-id="' + row.user_id + '"><span class="delete ui-label ui-label-success"><?= __('已启用');?></span></p>';
        }
    };
    function settingFormatter(val, opt, row) {
		if ('1' == row.user_admin || row.user_delete) {
			...
			return '<div class="operating" data-id="' + row.user_id + '"><a class="ui-icon ui-icon-pencil" title="<?= __('详细设置授权信息');?>" href="./erp.php?ctl=Rights_Base&met=index&user_account=' + row.user_account + '&user_id=' + row.user_id + '"></a></div>';
		}
    };
    function settingDataFormatter(val, opt, row) {
		if ('1' == row.user_admin || row.user_delete) {
			...
			return '<div class="operating" data-id="' + row.user_id + '"><a class="ui-icon ui-icon-pencil" title="<?= __('详细设置授权信息');?>" href="./erp.php?ctl=Rights_Base&met=indexData&user_account=' + row.user_account + '&user_id=' + row.user_id + '"></a></div>';
		}
    };
  })(jQuery)
